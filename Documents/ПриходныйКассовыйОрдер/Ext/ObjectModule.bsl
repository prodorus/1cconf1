...
Функция ПечатьПКО()
	...
	|	ПриходныйКассовыйОрдер.Касса.ПодразделениеОрганизации.ПрефиксКассовыхДокументов КАК ПрефиксКассовыхДокументов
	|ИЗ
...
Процедура ДвиженияПоРегистрамРегл(СтруктураШапкиДокумента, Режим, Отказ, Заголовок)
	...
	СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям = БухгалтерскийУчетДополнительный.ПолучитьПараметрыФормированияОтдельныхКассовыхКнигДляДокумента(ЭтотОбъект,СчетДт,Отказ);
		...
		Если СтруктураШапкиДокумента.ВыручкаСНТТ Тогда
			...
			Если СуммаОстаткаВыручки > 0 Тогда
				...
				Если СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям.УстанавливатьПодразделение Тогда
					...
					Проводка.Содержание  = "";
					Проводка.Сумма       = МожноРаспределить;
					
					Проводка.СчетДт      = СчетДт;
					Проводка.СубконтоДт.СтатьиДвиженияДенежныхСредств = РасшифровкаПлатежа[0].СтатьяДвиженияДенежныхСредств;
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Подразделения",
												СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям.ПодразделениеОрганизации);
			...
			Если СуммаПослеРаспределения > 0 Тогда
				...
				СуммаНДС = УчетНДС.РассчитатьСуммуНДС(СуммаПослеРаспределения, Истина, Истина,УчетНДС.ПолучитьСтавкуНДС(РасшифровкаПлатежа[0].СтавкаНДС));
				
				// По регистру "Розничная выручка организаций"
				СтрокаДвижений = ТаблицаДвижений.Добавить();
				СтрокаДвижений.Организация   = СтруктураШапкиДокумента.Организация;
				СтрокаДвижений.КассаККМ      = СтруктураШапкиДокумента.Контрагент;
				СтрокаДвижений.СтавкаНДС	 = РасшифровкаПлатежа[0].СтавкаНДС;
				...
				Иначе
					...
					Проводка.СубконтоКт.СтавкиНДС = РасшифровкаПлатежа[0].СтавкаНДС;
				КонецЕсли;
				...
				Если НЕ (СуммаНДС = 0) Тогда
					...
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, РасшифровкаПлатежа[0].СтавкаНДС);
