...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ОбновитьДокументИБПослеЗаполнения" Тогда
		ЭтаФорма.Прочитать();
	ИначеЕсли ИмяСобытия = "ВыполненоСопоставлениеНоменклатуры" И Параметр = Объект.Ссылка Тогда
...
Процедура ТекстСостоянияЭДНажатие(Элемент, СтандартнаяОбработка)
	...
	ЭлектронныеДокументыКлиент.ОткрытьСписокЭД(Объект.Ссылка, ПараметрыОткрытия);
