...
Функция ЭлектронноеПредставление(УникальныйИдентификатор) Экспорт
	...
	Если НалоговыйПериод >= '2017-10-01' Тогда
		Возврат УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПокупок_503(ЭтотОбъект, УникальныйИдентификатор);
		
	ИначеЕсли НалоговыйПериод >= '2014-10-01' Тогда
		Возврат УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПокупок_502(ЭтотОбъект, УникальныйИдентификатор);
...
Процедура ЗаполнитьДокумент() Экспорт
	...
	Если НЕ СтруктураПараметров.ФорматПоПостановлению735 Тогда
		...
		|ВсегоПокупок,СуммаБезНДС10,НДС10,СуммаБезНДС18,НДС18,НДС0,СуммаСовсемБезНДС,НДС,СуммаБезНДС20,НДС20");
	...
	Если СтруктураПараметров.ФорматПоПостановлению735 Тогда
		...
		Если СтруктураДанных.СписокСформированныхЛистов.Количество() = 0 Тогда 
			НалоговыйПериодПредставление = ПредставлениеПериода(НачалоКвартала(НалоговыйПериод), КонецКвартала(НалоговыйПериод), "ФП = Истина" );
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Нет данных для заполнения дополнительного листа книги покупок за период %1",НалоговыйПериодПредставление);
			ОбщегоНазначения.СообщитьОбОшибке(Сообщение);
			Возврат;
		КонецЕсли;
