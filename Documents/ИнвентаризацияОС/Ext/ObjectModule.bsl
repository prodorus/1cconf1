...
Функция ПечатьИНВ1() Экспорт
	...
	ВыбраннаяОрганизация = ?(ЗначениеЗаполнено(Док.Организация.ГоловнаяОрганизация), Док.Организация.ГоловнаяОрганизация, Док.Организация);
	ДанныеМОЛ = ОбщегоНазначения.ДанныеФизЛиц(ВыбраннаяОрганизация, СписокМОЛ, Док.Дата);
	...
	Если НЕ (ПредседательКомиссии = Неопределено) Тогда
		...
		ВыбраннаяОрганизация = ?(ЗначениеЗаполнено(Док.Организация.ГоловнаяОрганизация), Док.Организация.ГоловнаяОрганизация, Док.Организация);
		ДанныеПредседателя = ОбщегоНазначения.ДанныеФизЛица(ВыбраннаяОрганизация, ПредседательКомиссии.ФизЛицо, Док.Дата);
	...
	Для Каждого ЧленКомиссии Из ТаблицаИнвентаризационнаяКомиссия Цикл
		...
		ВыбраннаяОрганизация = ?(ЗначениеЗаполнено(Док.Организация.ГоловнаяОрганизация), Док.Организация.ГоловнаяОрганизация, Док.Организация);
		ДанныеЧленаКомиссии = ОбщегоНазначения.ДанныеФизЛица(ВыбраннаяОрганизация, ЧленКомиссии.ФизЛицо, Док.Дата);
...
Функция ПечатьИНВ18() Экспорт
	...
	// Выведем подписи МОЛов
	ВыбраннаяОрганизация = ?(ЗначениеЗаполнено(Организация.ГоловнаяОрганизация), Организация.ГоловнаяОрганизация, Организация);
	ДанныеМОЛ = ОбщегоНазначения.ДанныеФизЛиц(ВыбраннаяОрганизация , СписокМОЛ, Дата);
...
Функция ПечатьИНВ22() Экспорт
	...
	ВыбраннаяОрганизация = ?(ЗначениеЗаполнено(Док.Организация.ГоловнаяОрганизация), Док.Организация.ГоловнаяОрганизация, Док.Организация);
	ДанныеЧленовКомиссии = ОбщегоНазначения.ДанныеФизЛиц(ВыбраннаяОрганизация, СписокЧленовКомиссии, Док.Дата);
