...
Процедура ЗаполнитьПоРасчетномуДокументуУпр() Экспорт
	...
	Иначе
		...
		Иначе
			...
			СчетУчетаРасчетовСКонтрагентомНУ = РасчетныйДокумент.СчетУчетаРасчетовСКонтрагентомНУ;
			...
			СубконтоНУДт1 = РасчетныйДокумент.СубконтоНУДт1;
			СубконтоНУДт2 = РасчетныйДокумент.СубконтоНУДт2;
			СубконтоНУДт3 = РасчетныйДокумент.СубконтоНУДт3;
...
Функция СформироватьЗапросПоДокПлатежныйОрдерСписание()
	...
	|	Перечислено.СтатьяДвиженияДенежныхСредств
	...
	|	ЕСТЬNULL(СуммыПоВедомости.КомпенсацияЗаЗадержкуЗарплаты, 0) КАК КомпенсацияЗаЗадержкуЗарплаты,
	|	Перечисления.СтатьяДвиженияДенежныхСредств
	|ИЗ
	|	ВТПеречисления КАК Перечисления
...
Процедура ДвиженияПоРегистрамЗарплРегл(РежимПроведения, Отказ, 
	Заголовок, РегистрироватьТолькоНДФЛ = Ложь, СтруктураШапкиДокумента = Неопределено) Экспорт
	...
	КонецЕсли;
	Если Не (ВидОперации = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ПеречислениеЗП 
		И Не мУпрощенныйУчетВзаиморасчетов) Тогда
		Возврат;
	КонецЕсли;
	...
	СчетКт = УправлениеДенежнымиСредствами.ОпределитьСчетУчетаДенежныхСредств(СтруктураШапкиДокумента.СчетОрганизации, мВалютаРегламентированногоУчета, Ложь);
	...
	Выборка = СформироватьЗапросПоДокПлатежныйОрдерСписание().Выбрать(ОбходРезультатаЗапроса.Прямой);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.СуммаВзаиморасчетов <> 0 Тогда
			...
			ДокументВыплаты = Выборка.Ведомость;
			...
			Если ВестиРеглУчетРасчетовСПерсоналомПодробно Тогда
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, Выборка.ФизЛицо);
			КонецЕсли;
			
			Проводка.СчетКт      = СчетКт;
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1, СчетОрганизации);
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 2, Выборка.СтатьяДвиженияДенежныхСредств);
			...
			Если Выборка.КомпенсацияЗаЗадержкуЗарплаты > 0 Тогда
				...
				Проводка.СчетКт			= СчетКт;
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 1, СчетОрганизации);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 2, РасчетныйДокумент.РасшифровкаПлатежа[0].СтатьяДвиженияДенежныхСредств);
	...
	|	И ПеречислениеЗП.Ведомость.ХарактерВыплаты В (ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.Отпускные), ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.ПоБольничнымЛистам))
	|
	|СГРУППИРОВАТЬ ПО
	...
	|ГДЕ
	|	ПеречислениеЗП.Ссылка = &ДокументСсылка
	|	И НЕ ПеречислениеЗП.Ведомость.ХарактерВыплаты В (ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.Отпускные), ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.ПоБольничнымЛистам))
	|
	|СГРУППИРОВАТЬ ПО
	|	ПеречислениеЗП.Физлицо
	|
