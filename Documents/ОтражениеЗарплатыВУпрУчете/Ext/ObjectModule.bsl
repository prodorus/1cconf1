...
Процедура РаспределитьПоКассовомуМетоду() Экспорт
	...
	|				ТОГДА 1
	|			ИНАЧЕ -1
	|		КОНЕЦ * НДФЛРасчетыСБюджетом.Налог / ЕСТЬNULL(КурсыВалютДляРасчетовСПерсоналом.Курс, 1) * ЕСТЬNULL(КурсыВалютДляРасчетовСПерсоналом.Кратность, 1)
	|	ИЗ
...
Функция ПодготовитьВременныеТаблицы(АктуальныйПериодРегистрации)
	...
	Если мВалютаРегламентированногоУчета <> мВалютаУправленческогоУчета Тогда
		...
		Если Не ЗначениеЗаполнено(СрезКурсаВалюты.Курс) ИЛИ Не ЗначениеЗаполнено(СрезКурсаВалюты.Кратность) Тогда
			ОбщегоНазначенияЗК.СообщитьОбОшибке("Не задан курс " + мВалютаУправленческогоУчета + " для расчетов с персоналом на " + ПредставлениеПериода(НачалоМесяца(ПериодРегистрации), КонецМесяца(ПериодРегистрации))+ "!");
	...
	Иначе
		...
		|	ВзносыПоРаботникам.ПФРСтраховаяОборот + ВзносыПоРаботникам.ПФРПоСуммарномуТарифуОборот + ВзносыПоРаботникам.ПФРДоПредельнойВеличиныОборот + ВзносыПоРаботникам.ПФРСПревышенияОборот КАК ПФРСтраховая_НеЕНВД,
		|	ВзносыПоРаботникам.ПФРНакопительнаяЕНВДОборот КАК ПФРНакопительная_ЕНВД,
		|	ВзносыПоРаботникам.ПФРСтраховаяЕНВДОборот + ВзносыПоРаботникам.ПФРПоСуммарномуТарифуЕНВДОборот + ВзносыПоРаботникам.ПФРДоПредельнойВеличиныЕНВДОборот + ВзносыПоРаботникам.ПФРСПревышенияЕНВДОборот КАК ПФРСтраховая_ЕНВД,
...
Процедура ПроверитьЗаполнениеШапки(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ПериодРегистрации) Тогда
		ОбщегоНазначенияЗК.СообщитьОбОшибке("Не задан период за который выполняется отражение начислений учете!", Отказ);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиДохода(ВыборкаПоДоходам, Отказ)
	...
	Если ВыборкаПоДоходам.Подразделение.Пустая() Тогда
		ОбщегоНазначенияЗК.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не указано подразделение!", Отказ);
	КонецЕсли;
	
	Если ВыборкаПоДоходам.СтатьяЗатрат = Справочники.СтатьиЗатрат.ПустаяСсылка() Тогда
		ОбщегоНазначенияЗК.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не указана статья затрат!", Отказ);
	КонецЕсли;
	...
	Если ВыборкаПоДоходам.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ВложенияВоВнеоборотныеАктивы и 
		...
		ОбщегоНазначенияЗК.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не задан объект строительства!", Отказ);
	КонецЕсли;
