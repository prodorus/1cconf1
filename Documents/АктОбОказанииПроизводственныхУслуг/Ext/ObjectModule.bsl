...
Функция ПечатьУниверсальногоПередаточногоДокумента()
	...
	РеквизитыДокумента = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Ссылка, "Ссылка, Дата, Номер, Организация, 
		|Контрагент, ДоговорКонтрагента, Подразделение");
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Если ВалютаДокумента = мВалютаРегламентированногоУчета Тогда
		СтруктураПечатныхФорм.Вставить( "Акт",    "Акт об оказании услуг");
	Иначе
		СтруктураПечатныхФорм.Вставить( "Акт",    "Акт об оказании услуг (вал.)");
		СтруктураПечатныхФорм.Вставить( "АктРуб", "Акт об оказании услуг (руб.)");
	КонецЕсли;
...
Функция ОпределитьСчетФактураТребуется() Экспорт 
	...
	ЕстьНДС = (Услуги.Итог("СуммаНДС") > 0);
	...
	Если НЕ ЗначениеЗаполнено(УчетнаяПолитика) Тогда
		Возврат Ложь;
	ИначеЕсли УчетнаяПолитика.УСН ИЛИ УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС Тогда
		Возврат ЕстьНДС;
