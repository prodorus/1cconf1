...
Процедура ДвиженияПоРегистрам(Отказ, Заголовок)
	...
	// Сторнируем контролируемые значения по бюджетам
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗакрытиеЗаявокНаРасходованиеСредствЗаявкиНаРасходованиеСредств.Ссылка
	|ПОМЕСТИТЬ ЗаявкиИЗакрытияЗаявок
	|ИЗ
	|	Документ.ЗакрытиеЗаявокНаРасходованиеСредств.ЗаявкиНаРасходованиеСредств КАК ЗакрытиеЗаявокНаРасходованиеСредствЗаявкиНаРасходованиеСредств
	|ГДЕ
	|	ЗакрытиеЗаявокНаРасходованиеСредствЗаявкиНаРасходованиеСредств.Заявка В(&ЗаявкиКЗакрытию)
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗакрытиеЗаявокНаРасходованиеСредствЗаявкиНаРасходованиеСредств.Ссылка
	|
	...
	Запрос.УстановитьПараметр("ЗаявкиКЗакрытию", ЗаявкиНаРасходованиеСредств.ВыгрузитьКолонку("Заявка"));
	Запрос.Выполнить();
	...
	Если глЗначениеПеременной("ИспользоватьБлокировкуДанных") Тогда
		...
		СтруктураПараметровБлокировки = Новый Структура(
			"ИмяТаблицы, ИсточникДанных, ИмяВременнойТаблицы", 
			"КонтролируемыеЗначенияБюджетов", МенеджерВремТаблиц, "ЗаявкиИЗакрытияЗаявок");
		СтруктураИсточникаДанных = Новый Структура(
			"Регистратор", "Ссылка");
		ОбщегоНазначения.УстановитьУправляемуюБлокировку(СтруктураПараметровБлокировки, , СтруктураИсточникаДанных, Отказ, Заголовок);
	...
	|	КонтролируемыеЗначенияБюджетовОбороты.КонтролирующийСценарий,
	|	КонтролируемыеЗначенияБюджетовОбороты.ИспользованиеКонтролируемогоЗначения,
	...
	|	КонтролируемыеЗначенияБюджетовОбороты.Контрагент,
	|	КонтролируемыеЗначенияБюджетовОбороты.Номенклатура,
	|	СУММА(-КонтролируемыеЗначенияБюджетовОбороты.СуммаСценарияКонтрольОборот) КАК СуммаСценарияКонтроль,
	|	СУММА(-КонтролируемыеЗначенияБюджетовОбороты.СуммаСценарияИсполнениеОборот) КАК СуммаСценарияИсполнение,
	|	ИСТИНА КАК Активность
	|ИЗ
	|	РегистрНакопления.КонтролируемыеЗначенияБюджетов.Обороты(, , Регистратор, ) КАК КонтролируемыеЗначенияБюджетовОбороты
	|ГДЕ
	|	КонтролируемыеЗначенияБюджетовОбороты.Регистратор В(ВЫБРАТЬ РАЗЛИЧНЫЕ Ссылка ИЗ ЗаявкиИЗакрытияЗаявок)
	|
	|СГРУППИРОВАТЬ ПО
	|	КонтролируемыеЗначенияБюджетовОбороты.Период,
	|	КонтролируемыеЗначенияБюджетовОбороты.Сценарий,
	...
	|
	|ИМЕЮЩИЕ
	|	(СУММА(-КонтролируемыеЗначенияБюджетовОбороты.СуммаСценарияКонтрольОборот) <> 0
	|		ИЛИ СУММА(-КонтролируемыеЗначенияБюджетовОбороты.СуммаСценарияИсполнениеОборот) <> 0)";
	
	ТаблицаКонтролируемыеЗначенияБюджетов = Движения.КонтролируемыеЗначенияБюджетов.Выгрузить();
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		СтрокаДвиженийБюджетов = ТаблицаКонтролируемыеЗначенияБюджетов.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаДвиженийБюджетов, Выборка);
		
	КонецЦикла;
	
	Движения.КонтролируемыеЗначенияБюджетов.мТаблицаДвижений = ТаблицаКонтролируемыеЗначенияБюджетов;
