			...
			Если СтрокаДереваПачкаВходящихДокументов <> Неопределено Тогда
				Для Каждого СтрокаДереваЗаявление Из СтрокаДереваПачкаВходящихДокументов.Строки Цикл
					Если СтрокаДереваЗаявление.Имя = "ЗАЯВЛЕНИЕ_О_ДУБЛИКАТЕ" Или СтрокаДереваЗаявление.Имя = "ЗАЯВЛЕНИЕ_ОБ_ОБМЕНЕ" Тогда
						...
						Если СтрокаДереваИзменившиесяДанные <> Неопределено Тогда
							...
							Если СтрокаДереваЗаявление.Имя = "ЗАЯВЛЕНИЕ_ОБ_ОБМЕНЕ" Тогда
								...
								Если СтрокаДереваМестоРождения <> Неопределено Тогда
									...
									Иначе	
										ОбластьЗаявление.Параметры.МестоРожденияТип = "";
									КонецЕсли;
						...
						//Подвал
						ОбластьЗаявление.Параметры.ДатаЗаполнения = Символы.ПС + ПроцедурыПерсонифицированногоУчета.ДатаXMLВОтчет(ПроцедурыПерсонифицированногоУчета.ИзвлечьДанное(СтрокаДереваЗаявление, "ДатаЗаполнения"));
						...
						Если СтрокаДереваЗаявление.Имя = "ЗАЯВЛЕНИЕ_О_ДУБЛИКАТЕ" Тогда 
							...
							Если СтрокаДереваМестоРождения <> Неопределено Тогда
								ТипМестаРождения = ПроцедурыПерсонифицированногоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "ТипМестаРождения");
								Если ТипМестаРождения = "ОСОБОЕ" Тогда
									ОбластьЗаявление.Параметры.МестоРожденияТипВСвидетельстве = "особое";
								Иначе	
									ОбластьЗаявление.Параметры.МестоРожденияТипВСвидетельстве = "";
								КонецЕсли;
...
Функция СформироватьВыходнойФайл(Отказ) Экспорт
	...
	Если ВыборкаПоРаботникиОрганизации.Количество() > 200 тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("В документе должно быть не более 200 заявлений (сотрудников)!", Отказ, Заголовок);
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.ФорматФайла = Перечисления.ФорматФайлаПФР.Версия04 Тогда
		...
		Пока ВыборкаПоРаботникиОрганизации.Следующий() Цикл
			...
			Иначе	
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Строка №" + ВыборкаПоРаботникиОрганизации.НомерСтроки + ": Сотрудник " + ВыборкаПоРаботникиОрганизации.ФизЛицоНаименование + " указан(а) в документе дважды!", Отказ, Заголовок);
			КонецЕсли;
	...
	ИначеЕсли ВыборкаПоШапкеДокумента.ФорматФайла = Перечисления.ФорматФайлаПФР.Версия07 Тогда
		...
		Пока ВыборкаПоРаботникиОрганизации.Следующий() Цикл
			...
			Если ЗаменятьБуквуЕ И ВыборкаПоШапкеДокумента.ВидАДВ =  Перечисления.ВидыАДВ.АДВ3 Тогда
			...
			Если ПустаяСтрока(НаборЗаписейФИОИзСтрахового.Отчество) Тогда
				НаборЗаписейФИОИзСтрахового.Удалить("Отчество");
				СтрокаПоля = НаборЗаписейАДВ23.ФИОИзСтрахового.Поля.Найти("Отчество");
				Если СтрокаПоля <> Неопределено Тогда
					НаборЗаписейАДВ23.ФИОИзСтрахового.Поля.Удалить(СтрокаПоля);
				КонецЕсли;
			КонецЕсли;
			
			//Поля, присутствующие только в АДВ-3
			Если ВыборкаПоШапкеДокумента.ВидАДВ = Перечисления.ВидыАДВ.АДВ3 Тогда
				//Отметка о предоставлении сведений
				Если СокрЛП(ВыборкаПоРаботникиОрганизации.ОтметкаОПредставленииСведений)= "Представлялись" тогда
				...
				ЗаполнитьМестоРожденияВФорматеПФР07(НаборЗаписейАДВ23, "МестоРожденияИзСтрахового", ВыборкаПоРаботникиОрганизации.МестоРожденияВСвидетельстве);
			КонецЕсли;
			
			// ИзменившиесяДанные
			НаборЗаписейИзменившиесяДанные = НаборЗаписейАДВ23.ИзменившиесяДанные.Значение;
			
			//Поля, присутствующие только в АДВ-2
			Если ВыборкаПоШапкеДокумента.ВидАДВ = Перечисления.ВидыАДВ.АДВ2 Тогда
				
				// ФИО
				НаборЗаписейИзмФИО = НаборЗаписейИзменившиесяДанные.ФИО.Значение;
				Если ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.Фамилия) Тогда
					...
					КонецЕсли;
				Иначе
					НаборЗаписейИзмФИО.Удалить("Фамилия");
					СтрокаПоля = НаборЗаписейИзменившиесяДанные.ФИО.Поля.Найти("Фамилия");
					Если СтрокаПоля <> Неопределено Тогда
						НаборЗаписейИзменившиесяДанные.ФИО.Поля.Удалить(СтрокаПоля);
					КонецЕсли;
				КонецЕсли;
				Если ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.Имя) Тогда
					...
					КонецЕсли;
				Иначе
					НаборЗаписейИзмФИО.Удалить("Имя");
					СтрокаПоля = НаборЗаписейИзменившиесяДанные.ФИО.Поля.Найти("Имя");
					Если СтрокаПоля <> Неопределено Тогда
						НаборЗаписейИзменившиесяДанные.ФИО.Поля.Удалить(СтрокаПоля);
					КонецЕсли;
				КонецЕсли;
				...
				Если ВыборкаПоРаботникиОрганизации.ПризнакОтменыОтчества Тогда
					...
					НаборЗаписейИзмФИО.Удалить("Отчество");
					СтрокаПоля = НаборЗаписейИзменившиесяДанные.ФИО.Поля.Найти("Отчество");
					Если СтрокаПоля <> Неопределено Тогда
						НаборЗаписейИзменившиесяДанные.ФИО.Поля.Удалить(СтрокаПоля);
					КонецЕсли;
				ИначеЕсли Не ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.Отчество) Тогда
				...
				Если НаборЗаписейИзмФИО.Количество() = 0 Тогда
					НаборЗаписейИзменившиесяДанные.Удалить("ФИО");
				КонецЕсли;
				...
				Иначе
					НаборЗаписейИзменившиесяДанные.Удалить("Пол");
				КонецЕсли;
				...
				Иначе
					НаборЗаписейИзменившиесяДанные.Удалить("ДатаРождения");
				КонецЕсли;
				...
				КонецЕсли;
				ЗаполнитьМестоРожденияВФорматеПФР07(НаборЗаписейИзменившиесяДанные, "МестоРождения", ВыборкаПоРаботникиОрганизации.МестоРождения);
			...
			// Гражданство
			Если ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.Гражданство) Тогда
				НаборЗаписейИзменившиесяДанные.Гражданство.Значение = ВРег(СокрЛП(ВыборкаПоРаботникиОрганизации.Гражданство));
				Если ЗаменятьБуквуЕ Тогда
					ПроцедурыПерсонифицированногоУчета.ЗаменитьБуквуЁ(НаборЗаписейИзменившиесяДанные.Гражданство.Значение, "Гражданство");
				КонецЕсли;
			Иначе
				НаборЗаписейИзменившиесяДанные.Удалить("Гражданство");
			КонецЕсли;
			...
			Иначе
				...
				Если Не ПустаяСтрока(ТекстОшибки) Тогда
					ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Предупреждение: Строка №"+ВыборкаПоРаботникиОрганизации.НомерСтроки+": Сотрудник "+ВыборкаПоРаботникиОрганизации.ФизЛицо+ " Адрес регистрации" + ТекстОшибки, Ложь);
				КонецЕсли;
			КонецЕсли;
			...
			Иначе
				...
				Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
					ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Предупреждение: Строка №"+ВыборкаПоРаботникиОрганизации.НомерСтроки+": Сотрудник "+ВыборкаПоРаботникиОрганизации.ФизЛицо+ " Адрес фактический" + ТекстОшибки, Ложь);
				КонецЕсли;
			КонецЕсли;
			
			// Телефон
			Если ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.Телефоны) Тогда
				НаборЗаписейИзменившиесяДанные.Телефон.Значение = ВыборкаПоРаботникиОрганизации.Телефоны;
			Иначе
				НаборЗаписейИзменившиесяДанные.Удалить("Телефон");
			КонецЕсли;
			
			Если НаборЗаписейИзменившиесяДанные.Количество() = 0 Тогда
				НаборЗаписейАДВ23.Удалить("ИзменившиесяДанные");
			КонецЕсли;
			...
			ТипДокументаУЛ = ?(ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.ДокументВидКодПФР), СокрЛП(ВыборкаПоРаботникиОрганизации.ДокументВидКодПФР), "ПРОЧЕЕ");
			НаборЗаписейУдостоверяющийДокумент.ТипУдостоверяющего.Значение = ТипДокументаУЛ;
			НаборЗаписейДокумент.НаименованиеУдостоверяющего = ?(ТипДокументаУЛ = "ПРОЧЕЕ", ВРег(СокрЛП(ВыборкаПоРаботникиОрганизации.ДокументВидНаименование)), ТипДокументаУЛ);
			...
			Если Не ЗначениеЗаполнено(НаборЗаписейДокумент.СерияРусскиеБуквы) Тогда
				НаборЗаписейДокумент.Удалить("СерияРусскиеБуквы");
				СтрокаКУдалению = НаборЗаписейУдостоверяющийДокумент.Документ.Поля.Найти("СерияРусскиеБуквы");
				Если СтрокаКУдалению <> Неопределено Тогда
					НаборЗаписейУдостоверяющийДокумент.Документ.Поля.Удалить(СтрокаКУдалению);
				КонецЕсли;
			КонецЕсли;
			Если ЗаменятьБуквуЕ Тогда
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ФамилияВСвидетельстве + ВыборкаПоСтрокамДокумента.ИмяВСвидетельстве) Тогда
			СтрокаСообщения = СтрокаСообщения + Символы.ПС+Символы.Таб + "- не указаны фамилия и имя!";
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.АдресРегистрации) Тогда 
			...
			Если ЗаПределамиРФ <> Истина Тогда
				Если  РегламентированнаяОтчетность.АдресСоответствуетТребованиям(АдресРегистрацииСписком) Тогда
					...
					Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
						ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("На закладке ""Изменившиеся данные"" в строке № "+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки)+": Адрес регистрации не соответствует адресному классификатору: в классификаторе не найден" + ТекстОшибки + "!", , Заголовок);
					КонецЕсли;
				Иначе
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.АдресФактический) Тогда
			...
			Если ЗаПределамиРФ <> Истина Тогда
				Если РегламентированнаяОтчетность.АдресСоответствуетТребованиям(АдресФактическийСписком) Тогда
					...
					Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
						ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("На закладке ""Изменившиеся данные"" в строке № "+ СокрЛП(ВыборкаПоСтрокамДокумента.НомерСтроки)+": Фактический адрес проживания не соответствует адресному классификатору: в классификаторе не найден" + ТекстОшибки + "!", , Заголовок);
					КонецЕсли;
				Иначе
...
Процедура ЗаполнитьМестоРожденияВФорматеПФР07(НаборЗаписейАДВ, ИмяУзла, МестоРождения)
	
	Если Не ЗначениеЗаполнено(МестоРождения) Тогда 
		НаборЗаписейАДВ.Удалить(ИмяУзла);
		Возврат;
	КонецЕсли;
	
	НаборЗаписейМестоРождения = НаборЗаписейАДВ[ИмяУзла].Значение;
	ЧастиМестаРождения = РегламентированнаяОтчетность.РазложитьМестоРождения(МестоРождения);
	Если НЕ (ЗначениеЗаполнено(ЧастиМестаРождения.НаселенныйПункт) ИЛИ ЗначениеЗаполнено(ЧастиМестаРождения.Район) 
		ИЛИ ЗначениеЗаполнено(ЧастиМестаРождения.Область) ИЛИ ЗначениеЗаполнено(ЧастиМестаРождения.Страна)) Тогда
		НаборЗаписейАДВ.Удалить(ИмяУзла);
	Иначе
		НаборЗаписейМестоРождения.ТипМестаРождения = ?(ЧастиМестаРождения.Особое = 1, "ОСОБОЕ", "СТАНДАРТНОЕ");
		
		Сокращение = "";
		НаборЗаписейМестоРождения.ГородРождения = УправлениеКонтактнойИнформацией.ПолучитьИмяИАдресноеСокращение(ЧастиМестаРождения.НаселенныйПункт,Сокращение);
		НаборЗаписейМестоРождения.РайонРождения = УправлениеКонтактнойИнформацией.ПолучитьИмяИАдресноеСокращение(ЧастиМестаРождения.Район,Сокращение);
		НаборЗаписейМестоРождения.РегионРождения = ЧастиМестаРождения.Область;
		НаборЗаписейМестоРождения.СтранаРождения = ЧастиМестаРождения.Страна;
		...
		ОписаниеПолей = НаборЗаписейАДВ[ИмяУзла].Поля;
		Если ПустаяСтрока(НаборЗаписейМестоРождения.ГородРождения) Тогда
			НаборЗаписейМестоРождения.Удалить("ГородРождения");
			СтрокаКУдалению = ОписаниеПолей.Найти("ГородРождения", "ИмяПоля");
			ОписаниеПолей.Удалить(СтрокаКУдалению);
		КонецЕсли;
		Если ПустаяСтрока(НаборЗаписейМестоРождения.РайонРождения) Тогда
			НаборЗаписейМестоРождения.Удалить("РайонРождения");
			СтрокаКУдалению = ОписаниеПолей.Найти("РайонРождения", "ИмяПоля");
			ОписаниеПолей.Удалить(СтрокаКУдалению);
		КонецЕсли;
		Если ПустаяСтрока(НаборЗаписейМестоРождения.РегионРождения) Тогда
			НаборЗаписейМестоРождения.Удалить("РегионРождения");
			СтрокаКУдалению = ОписаниеПолей.Найти("РегионРождения", "ИмяПоля");
			ОписаниеПолей.Удалить(СтрокаКУдалению);
		КонецЕсли;
		Если ПустаяСтрока(НаборЗаписейМестоРождения.СтранаРождения) Тогда
			НаборЗаписейМестоРождения.Удалить("СтранаРождения");
			СтрокаКУдалению = ОписаниеПолей.Найти("СтранаРождения", "ИмяПоля");
			ОписаниеПолей.Удалить(СтрокаКУдалению);
		КонецЕсли;
