...
Функция ПечатьЗаявления(ИмяМакета)
	...
	ПредшествующийГод = НачалоГода(ДатаНачала) - 1;
	...
	|	СУММА(ОтпускРасчетСреднего2011.ДнейБолезниУходаЗаДетьми) КАК ДнейБолезниУходаЗаДетьми
	...
	|	ВЫБОР
	|		КОГДА ОтпускПоУходуЗаРебенком.НаличиеКартыМИР
	|			ТОГДА ОтпускПоУходуЗаРебенком.НомерКартыМИР
	|		ИНАЧЕ """"
	...
	|	ПервыйГодРасчета2011.Год - 2000 КАК ПервыйГодРасчета,
	|	ВторойГодРасчета2011.Год - 2000 КАК ВторойГодРасчета,
	|	ВЫБОР
	|		КОГДА ОтпускПоУходуЗаРебенком.РасчетПоПравилам2010Года
	...
	|	ВЫБОР
	|		КОГДА ОтпускПоУходуЗаРебенком.Дата >= &ДатаФормы2017года
	...
	|	ВЫБОР
	|		КОГДА ОтпускПоУходуЗаРебенком.ИсправляемыйДокумент = ЗНАЧЕНИЕ(Документ.ОтпускПоУходуЗаРебенком.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|	ДанныеДокумента.ПервыйГодРасчета,
	|	ДанныеДокумента.ВторойГодРасчета,
...
Функция ИмяМакетаЗаявленияВФСС()
	
	Если ЗначениеЗаполнено(ИсправляемыйДокумент) Тогда
		ИмяМакета = ?(Дата < ОбменСведениямиОПособияхСФСС.ДатаВступленияВСилуФорм2017года(), "ЗаявлениеВФССОДоплатеПособия", "ЗаявлениеВФССОВыплатеПособия_2017");
	Иначе
		ИмяМакета = "ЗаявлениеВФССОВыплатеПособия_" + ?(Дата < ОбменСведениямиОПособияхСФСС.ДатаВступленияВСилуФорм2012года(), "2011", ?(Дата < ОбменСведениямиОПособияхСФСС.ДатаВступленияВСилуФорм2017года(), "2012", "2017"))	
	КонецЕсли;
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "ЗаявлениеЧистыйБланк" Тогда
		ТабДокумент = ОбменСведениямиОПособияхСФСС.ПечатьЗаявлениеВФССОВыплатеПособия(ОбменСведениямиОПособияхСФСС.СтруктураДанныхДляПечатиЗаявленийВФССОВыплатеПособия(), ИмяМакетаЗаявленияВФСС());
	...
	ИначеЕсли ИмяМакета = "ЗаявлениеВФССОВыплатеПособия" Тогда
		ТабДокумент = ПечатьЗаявления(ИмяМакетаЗаявленияВФСС());
