...
Процедура ДобавитьСтрокуВТехОперации(ТаблицаТехнологическиеОперации, СтрокаВыпуска, ТехОпер)
	...
	СтруктПоиска.Вставить( "Заказ",   	   СтрокаВыпуска.Заказ);
...
Функция ПодготовитьТаблицуСписанияНаЗатраты()
	...
	|		) КАК НаправленияСписанияВсего
	|	ПО
	|		ОтчетПроизводстваЗаСмену.КлючСвязи = НаправленияСписанияВсего.КлючСвязи
	|			
	|ГДЕ
	|	ОтчетПроизводстваЗаСмену.Ссылка = &ТекущийДокумент
	|	И ОтчетПроизводстваЗаСмену.Ссылка.ИспользоватьНаправленияВыпуска
		...
		Пока ВыборкаПоРаботники.Следующий() Цикл 
			...
			Если ВыборкаПоРаботники.СуммаКНачислению <> 0 Тогда
				...
				Для каждого СтрокаТЗ Из ТаблицаРабот Цикл
					...
					СтрокаНабора.ДокументОснование			= Ссылка;
					...
					Если Дата >= ПроведениеРасчетов.ДатаВводаДополнительногоТарифаЗаРаботыСДосрочнойПенсией() Тогда
					...
					КонецЕсли;
					Если Дата >= РасчетСтраховыхВзносов.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() Тогда
...
Функция СформироватьЗапросПоИсполнителиРегл()
	...
	// Описание текста запроса:
	// 
	// 1. Выборка "ОтчетПроизводстваЗаСменуИсполнители": 
	//		Выбираются строки документа.  
	...
	// 4. Выборка "КлассыУсловийТрудаПоДолжностямСрезПоследних": 
	//		Из регистра (срез последних) КлассыУсловийТрудаПоДолжностям выбираются строки, 
	//		соответствующие должностям работников документа.  
	...
	|				И РаботникиОрганизацииСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА РаботникиОрганизацииСрезПоследних.ДолжностьЗавершения.ЯвляетсяФармацевтическойДолжностью
	|		ИНАЧЕ РаботникиОрганизацииСрезПоследних.ПодразделениеОрганизации.ПрименяемыйЛьготныйТерриториальныйТариф
