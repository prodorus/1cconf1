...
Функция ПечатьРКО()
	...
	|	РасходныйКассовыйОрдер.Касса.ПодразделениеОрганизации.ПрефиксКассовыхДокументов КАК ПрефиксКассовыхДокументов
	|ИЗ
	...
	Если ЗначениеЗаполнено(Шапка.ПрефиксКассовыхДокументов) Тогда
		МассивПрефиксов = ОбщегоНазначения.СформироватьМассивПрефиксовДляРИБИОрганизации(Шапка.Организация);
		МассивПрефиксов.Добавить(Шапка.ПрефиксКассовыхДокументов);
		ОбластьМакета.Параметры.НомерДокумента    = ОбщегоНазначения.ПолучитьНомерНаПечать(Шапка,МассивПрефиксов);
	Иначе	
...
Процедура ДвиженияПоВзаиморасчетыСРаботникамиОрганизации(Отказ, Заголовок, РегистрироватьТолькоНДФЛ)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
		...
		Если Не РегистрироватьТолькоНДФЛ Тогда
			...
			ПроведениеРасчетов.ПроверитьХронологиюУчетаУдержанногоНДФЛ(Организация, Движения.ВзаиморасчетыСРаботникамиОрганизаций.ВыгрузитьКолонку("Физлицо"), Дата, Отказ, Заголовок);
			Если Отказ Тогда
				Возврат
			КонецЕсли;
		...
		|	И РанееВыплаченныеСуммы.Физлицо ЕСТЬ NULL 
		|	И ЗарплатаКВыплате.Ссылка.ХарактерВыплаты В (ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.Отпускные), ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.ПоБольничнымЛистам))
		|
		|СГРУППИРОВАТЬ ПО
		...
		СтруктураДокумента.Вставить("ХарактерВыплаты", Перечисления.ХарактерВыплатыЗарплаты.Отпускные);
		ПроведениеРасчетов.СформироватьРасчетыПоНДФЛПриВыплате(СтруктураДокумента, ТекстТаблицыВыплаты, Движения.НДФЛРасчетыСБюджетом);
		...
		|	И ЗарплатаКВыплате.ВыплаченностьЗарплаты = ЗНАЧЕНИЕ(Перечисление.ВыплаченностьЗарплаты.Выплачено)
		|	И РанееВыплаченныеСуммы.Физлицо ЕСТЬ NULL 
		|	И НЕ ЗарплатаКВыплате.Ссылка.ХарактерВыплаты В (ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.Отпускные), ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.ПоБольничнымЛистам))
		|
		|СГРУППИРОВАТЬ ПО
		|	ЗарплатаКВыплате.Физлицо
		|
		...
		ПроведениеРасчетов.СформироватьРасчетыПоНДФЛПриВыплате(СтруктураДокумента, ТекстТаблицыВыплаты, Движения.НДФЛРасчетыСБюджетом, , , Истина);
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику Тогда
		...
		СтруктураДокумента.Вставить("ХарактерВыплаты", ОбщегоНазначенияЗК.ПолучитьЗначениеРеквизита(РасчетныйДокумент, "ХарактерВыплаты"));
...
Процедура ДвиженияПоЗарплатеРегл(Отказ, Заголовок, РегистрироватьТолькоНДФЛ = Ложь) Экспорт
	...
	// Дополнительная проверка в случае внешнего вызова из обработки "ИзменениеПорядкаУчетаДокументов"
	Если РегистрироватьТолькоНДФЛ Тогда
	...
	Иначе
		мУпрощенныйУчетВзаиморасчетов = Ложь;
	КонецЕсли;
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям ИЛИ
		...
		Если НЕ мУпрощенныйУчетВзаиморасчетов Тогда
			ДвиженияПоВзаиморасчетыСРаботникамиОрганизации(Отказ, Заголовок, РегистрироватьТолькоНДФЛ);
		КонецЕсли;
...
Процедура ДвиженияПоРегистрам(Режим, Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
		...
		Если НЕ мУпрощенныйУчетВзаиморасчетов Тогда
			...
			ДвиженияПоЗарплатеРегл(Отказ, Заголовок);
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, Отказ, Заголовок,СтруктураШапкиДокумента)
	...
	СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям = БухгалтерскийУчетДополнительный.ПолучитьПараметрыФормированияОтдельныхКассовыхКнигДляДокумента(ЭтотОбъект,СчетКт,Отказ);
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Сумма <> 0 Тогда
				...
				Если СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям.УстанавливатьПодразделение Тогда
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Подразделения",
													СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям.ПодразделениеОрганизации);
				КонецЕсли;
			КонецЕсли;
	...
	Иначе
		...
		Если ЕстьРасчетыСКонтрагентами Тогда
			...
			Если Не (СтруктураПараметровДДС = Ложь) тогда
				Если СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям.УстанавливатьПодразделение Тогда
					БухгалтерскийУчетДополнительный.ДобавитьСубконтоПодразделенияВТаблицуПлатежей(СтруктураПараметровДДС,
																СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям.ПодразделениеОрганизации);
				КонецЕсли;
			...
			Иначе
				Если СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям.УстанавливатьПодразделение Тогда
					СтруктураШапкиДокумента.Вставить("ОбособленноеПодразделение",СтруктураПараметровФормированияОтдельныхКассовыхКнигПоПодразделениям.ПодразделениеОрганизации);
				КонецЕсли;
...
Процедура ПриУстановкеНовогоНомера(СтандартнаяОбработка, Префикс)
	...
	БухгалтерскийУчетДополнительный.ПриУстановкеНовогоНомераКассовыхДокументов(ЭтотОбъект,СтандартнаяОбработка,Префикс);
