...
Процедура 	РасчетНалогаНаПрибыль(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	СуммаНалога = 0;
	СформироватьПроводкиПоНалогуНаПрибыльРасчетыСБюджетом(СтруктураШапкиДокумента, СуммаНалога);

    Если Дата >= Дата(2014,01,01) Тогда
		Остаток68 = СуммаНалога - (СуммаУсловногоНалога - Сумма68);
	КонецЕсли;
...
Процедура 	РасчетНалогаНаПрибыльБезПоддержкиПБУ18(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	СуммаНалога = Окр(СуммаДоходаРасхода * ВсеСтавки, ТочностьРасчета) - СуммаНалогаНаПрибыльРанееНачисленного;
	...
	Если СуммаНалога <> 0 И ВсеСтавки <> 0 Тогда
		...
		СуммаФБ 	 = Окр(СуммаНалога * СтавкаФБ / ВсеСтавки, ТочностьРасчета);
		СуммаМестный = Окр(СуммаНалога * СтавкаМестный / ВсеСтавки, ТочностьРасчета);
		СуммаНалога  = Окр(СуммаНалога, ТочностьРасчета);
		СуммаРФ 	 = СуммаНалога - СуммаФБ - СуммаМестный;

		Если СтавкаФБ <> 0 Тогда
			СформироватьПроводкиПоНалогуНаПрибыль(СуммаФБ, Сч99_01_1, Перечисления.ПрибылиИУбытки.НалогНаПрибыль, "Д", "Налог на прибыль", ,Перечисления.УровниБюджетов.ФедеральныйБюджет);
		КонецЕсли;
		Если СтавкаРФ <> 0 Тогда
			СформироватьПроводкиПоНалогуНаПрибыль(СуммаРФ, Сч99_01_1, Перечисления.ПрибылиИУбытки.НалогНаПрибыль, "Д", "Налог на прибыль", ,Перечисления.УровниБюджетов.РегиональныйБюджет);
		КонецЕсли;
		Если СтавкаМестный <> 0 Тогда
			СформироватьПроводкиПоНалогуНаПрибыль(СуммаМестный, Сч99_01_1, Перечисления.ПрибылиИУбытки.НалогНаПрибыль, "Д", "Налог на прибыль", ,Перечисления.УровниБюджетов.МестныйБюджет);
		КонецЕсли;
	КонецЕсли;
...
//
Процедура СформироватьПроводкиПоНалогуНаПрибыльРасчетыСБюджетом(СтруктураШапкиДокумента, СуммаНалога)
