...
Процедура ЗаполнитьЗначенияПеременныхВеденияУчета(СтруктураШапкиДокумента) Экспорт
...
//
Функция СформироватьЗапросПоИсполнители(ВыборкаПоШапкеДокумента)
...
//	проверяемых параметров из связанного с
//
Функция СформироватьЗапросПоИсполнителиРегл(ВыборкаПоШапкеДокумента, ВыборкаДанныхПоГрафику = Неопределено) Экспорт 
	...
	|	ВЫБОР
	|		КОГДА &Месяц < ДАТАВРЕМЯ(2018, 1, 1)
	|			ТОГДА ВЫБОР
	|					КОГДА Работники.ПериодЗавершения ЕСТЬ NULL 
	|						ТОГДА БудущиеДанные.ПодразделениеОрганизации.УдалитьСоответствуетСудамПодФлагомРФ
	|					КОГДА Работники.ПериодЗавершения <= ДатыРегистра.ДатаДня
	|							И Работники.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|						ТОГДА Работники.ПодразделениеОрганизацииЗавершения.УдалитьСоответствуетСудамПодФлагомРФ
	|					ИНАЧЕ Работники.ПодразделениеОрганизации.УдалитьСоответствуетСудамПодФлагомРФ
	|				КОНЕЦ
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ПодразделениеЧленаЭкипажаСуднаПодФлагомРФ,
	|	ВЫБОР
	|		КОГДА &Месяц < ДАТАВРЕМЯ(2018, 1, 1)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТарифыСтраховыхВзносов.ПустаяСсылка)
	|		ИНАЧЕ ВЫБОР
	|				КОГДА Работники.ПериодЗавершения ЕСТЬ NULL 
	|					ТОГДА БудущиеДанные.ПодразделениеОрганизации.ПрименяемыйЛьготныйТерриториальныйТариф
	|				КОГДА Работники.ПериодЗавершения <= ДатыРегистра.ДатаДня
	|						И Работники.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|					ТОГДА Работники.ПодразделениеОрганизацииЗавершения.ПрименяемыйЛьготныйТерриториальныйТариф
	|				ИНАЧЕ Работники.ПодразделениеОрганизации.ПрименяемыйЛьготныйТерриториальныйТариф
	|			КОНЕЦ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КлассыУсловийТрудаПоДолжностям.СрезПоследних(&ДатаАктуальности, ) КАК КлассыУсловийТрудаПоДолжностямСрезПоследних
	|		ПО (ВЫБОР
	|				КОГДА Работники.ПериодЗавершения ЕСТЬ NULL 
	|					ТОГДА БудущиеДанные.Должность
	|				КОГДА Работники.ПериодЗавершения <= ДатыРегистра.ДатаДня
	|						И Работники.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	...
	|	Работники.ЛьготныйТерриториальныйТариф КАК ЛьготныйТерриториальныйТариф,
	|	ЕСТЬNULL(ВЫБОР
...
//  проверяемых параметров из связанного с
//
Функция СформироватьЗапросПоВыработкаРегл() Экспорт 
...
//
Процедура ДвиженияРегистров(Движения, ВыборкаПоШапкеДокумента, ВыборкаПоРаботникиУпр, ВыборкаПоРаботникиРегл, ТаблицаРаботРегл)
	...
	ДвиженияРегистровБУиНУРегл(Движения, ВыборкаПоШапкеДокумента, ВыборкаПоРаботникиРегл, ТаблицаРаботРегл);
...
Процедура ДвиженияРегистровБУиНУРегл(Движения, ВыборкаПоШапкеДокумента, ВыборкаПоРаботники, ТаблицаРабот) Экспорт 
	...
	Пока ВыборкаПоРаботники.СледующийПоЗначениюПоля("НомерСтроки") Цикл
		Пока ВыборкаПоРаботники.СледующийПоЗначениюПоля("Сотрудник") Цикл
			...
			Для Каждого СтрокаТЗ Из ТаблицаРабот Цикл
				...
				Если ВыборкаПоШапкеДокумента.ПериодРегистрации >= ПроведениеРасчетов.ДатаВводаДополнительногоТарифаЗаРаботыСДосрочнойПенсией() Тогда
				...
				Если ВыборкаПоШапкеДокумента.ПериодРегистрации >= РасчетСтраховыхВзносов.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() Тогда
					СтрокаНабора.КлассУсловийТруда = ВыборкаПоРаботники.КлассУсловийТруда;
				КонецЕсли;
				...
				Если РегистрироватьСтраховыеВзносы Тогда
					...
					СтрокаНабора.ОблагаетсяПоДополнительномуТарифу 			 = ВыборкаПоРаботники.ОблагаетсяПоДополнительномуТарифу;
					СтрокаНабора.ОблагаетсяВзносамиНаДоплатуКПенсииШахтерам  = ВыборкаПоРаботники.ОблагаетсяВзносамиНаДоплатуКПенсииШахтерам;
					СтрокаНабора.ЛьготныйТерриториальныйТариф = ВыборкаПоРаботники.ЛьготныйТерриториальныйТариф;
					Если ВыборкаПоШапкеДокумента.ПериодРегистрации >= ПроведениеРасчетов.ДатаВводаДополнительногоТарифаЗаРаботыСДосрочнойПенсией() Тогда
						СтрокаНабора.ОблагаетсяВзносамиЗаЗанятыхНаРаботахСДосрочнойПенсией = ВыборкаПоРаботники.ОблагаетсяВзносамиЗаЗанятыхНаРаботахСДосрочнойПенсией;
					КонецЕсли;
					Если ВыборкаПоШапкеДокумента.ПериодРегистрации >= РасчетСтраховыхВзносов.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() Тогда
				...
				Если ТипЗнч(Движения) = Тип("КоллекцияДвижений") ИЛИ 
					(ТипЗнч(Движения) = Тип("Структура") И Движения.Свойство("Хозрасчетный")) Тогда
					////////////////////////////////////
				...
				Если ТипЗнч(Движения) = Тип("КоллекцияДвижений") ИЛИ 
					(ТипЗнч(Движения) = Тип("Структура") И Движения.Свойство("Налоговый")) Тогда
					/////////////////////////////////
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ВыборкаПоРаботники = СформироватьЗапросПоИсполнители(СтруктураШапкиДокумента).Выбрать();
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		...
		РезультатЗапросаПоРаботыРегл = СформироватьЗапросПоВыработкаРегл();
	...
	// Движения стоит добавлять, если в проведении еще не отказано (отказ =ложь)
	Если НЕ Отказ Тогда
		ДвиженияРегистров(Движения, СтруктураШапкиДокумента, ВыборкаПоРаботники, ВыборкаПоРаботникиРегл, ТаблицаРаботРегл)
	КонецЕсли; 
