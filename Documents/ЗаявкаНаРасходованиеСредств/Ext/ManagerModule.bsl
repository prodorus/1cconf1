...
//
// Устанавливает значения реквизитов "КонтрактИсполнителя", "КонтрактСЗаказчиком"
Процедура ЗаполнитьКонтракты() Экспорт
	...
	|	И ЗаявкаНаРасходованиеСредств.КонтрактИсполнителя = ЗНАЧЕНИЕ(Справочник.КонтрактыИсполнителей.ПустаяСсылка)
	|	И ЗаявкаНаРасходованиеСредств.КонтрактСЗаказчиком = ЗНАЧЕНИЕ(Справочник.КонтрактыСЗаказчиками.ПустаяСсылка)";
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Заявка.КонтрактИсполнителя = Заявка.КонтрактИсполнителя();
			Заявка.КонтрактСЗаказчиком = Заявка.КонтрактСЗаказчиком();
...
// Обработчик обновления
//
// Устанавливает значения реквизита "ИдентификаторПлатежа"
Процедура ЗаполнитьИдентификаторПлатежа() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗаявкаНаРасходованиеСредств.Ссылка,
	|	ЗаявкаНаРасходованиеСредств.КонтрактИсполнителя.ГосударственныйКонтракт.Код КАК ИсполнительГосКонтрактКод,
	|	ЗаявкаНаРасходованиеСредств.КонтрактСЗаказчиком.ГосударственныйКонтракт.Код КАК ЗаказчикГосКонтрактКод
	|ИЗ
	|	Документ.ЗаявкаНаРасходованиеСредств КАК ЗаявкаНаРасходованиеСредств
	|ГДЕ
	|	ЗаявкаНаРасходованиеСредств.ПлатежПоФЗ275";
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Записывать = Ложь;
			Если ЗначениеЗаполнено(Выборка.ИсполнительГосКонтрактКод) Тогда
				Заявка.ИдентификаторПлатежа = Выборка.ИсполнительГосКонтрактКод;
				Записывать = Истина;
			ИначеЕсли ЗначениеЗаполнено(Выборка.ЗаказчикГосКонтрактКод) Тогда
				Заявка.ИдентификаторПлатежа = Выборка.ЗаказчикГосКонтрактКод;
				Записывать = Истина;
			КонецЕсли;
			...
			Попытка
				ОбновлениеИнформационнойБазы.ЗаписатьДанные(Заявка);
			Исключение
				ТекстСообщения = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление информационной базы'"), УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);
			КонецПопытки;
