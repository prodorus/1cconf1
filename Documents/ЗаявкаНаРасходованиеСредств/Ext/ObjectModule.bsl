...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("СправочникСсылка.КонтрактыИсполнителей") Тогда
		ЗаполнитьПоКонтрактуСИсполнителем(Основание);
	ИначеЕсли ТипЗнч(Основание) = Тип("СправочникСсылка.КонтрактыСЗаказчиками") Тогда
		ЗаполнитьПоКонтрактуСЗаказчиком(Основание);
	ИначеЕсли ТипЗнч(Основание) = Тип("СправочникСсылка.ЭтапыКонтрактов") Тогда
		ЭтапКонтракта = Основание;
		ЗаполнитьПоКонтрактуСИсполнителем(Основание.Владелец);
		ДатаРасхода = Основание.Дата;
		СуммаДокумента = Основание.СуммаОплаты;
		СтатьяКалькуляции = Основание.СтатьяКалькуляции;
		РасшифровкаПлатежа[0].СуммаПлатежа = Основание.СуммаОплаты;
		РасшифровкаПлатежа[0].СуммаВзаиморасчетов = Основание.СуммаОплаты;
	КонецЕсли;
...
Процедура ЗаполнитьПоКонтрактуСИсполнителем(ДанныеЗаполнения)
	Запрос = Новый Запрос;
	...
	|	ДанныеЗаполнения.Ссылка КАК КонтрактИсполнителя,
	|	ДанныеЗаполнения.КонтрактСУчастникомГОЗ
	|		ИЛИ ДанныеЗаполнения.ПлатежиПо275ФЗ КАК ПлатежПоФЗ275,
	|	ДанныеЗаполнения.ГосударственныйКонтракт КАК ГосударственныйКонтракт,
	|	ДанныеЗаполнения.ТипПлатежаФЗ275 КАК ТипПлатежаФЗ275,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДенежныхСредств.Безналичные) КАК ФормаОплаты,
	|	ДанныеЗаполнения.Договор КАК ДоговорКонтрагента,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаявкиНаРасходование.ОплатаПоставщику) КАК ВидОперации,
	|	ДанныеЗаполнения.ГосударственныйКонтракт.Код КАК ИдентификаторПлатежа
	|ИЗ
	|	Справочник.КонтрактыИсполнителей КАК ДанныеЗаполнения
	|ГДЕ
	...
	РасшифровкаПлатежа[0].ДоговорКонтрагента = Выборка.ДоговорКонтрагента;
	ЗаполнитьПодтверждающиеДокументы(Ложь);
	БанковскийСчетКасса=Справочники.БанковскиеСчета.ПустаяСсылка();
	СтавкаНДС = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(
		глЗначениеПеременной("глТекущийПользователь"), "ОсновнаяСтавкаНДС");
	
КонецПроцедуры

Процедура ЗаполнитьПоКонтрактуСЗаказчиком(ДанныеЗаполнения)
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДанныеЗаполнения", ДанныеЗаполнения);
	...
	|	ДанныеЗаполнения.Контрагент КАК Контрагент,
	|	ДанныеЗаполнения.Ссылка КАК КонтрактСЗаказчиком,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаявкиНаРасходование.ВозвратДенежныхСредствПокупателю) КАК ВидОперации,
	|	ДанныеЗаполнения.КонтрактСУчастникомГОЗ КАК ПлатежПоФЗ275,
	|	ДанныеЗаполнения.ГосударственныйКонтракт КАК ГосударственныйКонтракт,
	|	ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.СписаниеНаОтдельныйСчет) КАК ТипПлатежаФЗ275,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДенежныхСредств.Безналичные) КАК ФормаОплаты,
	|	ДанныеЗаполнения.Договор КАК ДоговорКонтрагента,
	|	ДанныеЗаполнения.ГосударственныйКонтракт.Код КАК ИдентификаторПлатежа
	|ИЗ
...
Процедура ЗаполнитьПодтверждающиеДокументы(ВыводитьСообщение = Истина) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |
	               |ВЫБРАТЬ
	               |	КонтрактыИсполнителейПодтверждающиеДокументы.ВидДокумента,
	               |	КонтрактыИсполнителейПодтверждающиеДокументы.Файл,
	               |	КонтрактыИсполнителейПодтверждающиеДокументы.Номер,
	               |	КонтрактыИсполнителейПодтверждающиеДокументы.Дата,
	               |	КонтрактыИсполнителейПодтверждающиеДокументы.Сумма
	               |ИЗ
	               |	Справочник.КонтрактыИсполнителей.ПодтверждающиеДокументы КАК КонтрактыИсполнителейПодтверждающиеДокументы
	               |ГДЕ
	               |	КонтрактыИсполнителейПодтверждающиеДокументы.Ссылка = &Контракт
	               |	И (КонтрактыИсполнителейПодтверждающиеДокументы.Этап = &Этап
	               |			ИЛИ КонтрактыИсполнителейПодтверждающиеДокументы.Этап = ЗНАЧЕНИЕ(Справочник.ЭтапыКонтрактов.ПустаяСсылка))
	               |	И &ИзТипаПлатежа = ЛОЖЬ";
				   
	Запрос.УстановитьПараметр("ИзТипаПлатежа", Не ЗначениеЗаполнено(КонтрактИсполнителя()));
	Запрос.УстановитьПараметр("ТипПлатежа", ТипПлатежаФЗ275);
	Запрос.УстановитьПараметр("Контракт", КонтрактИсполнителя());
	Запрос.УстановитьПараметр("Этап", ЭтапКонтракта);

	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
		Если ВыводитьСообщение = Истина Тогда
			ОбщегоНазначения.СообщитьИнформациюПользователю("Подтверждающие документы не найдены");
		КонецЕсли;
	Иначе
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ПлатежПоФЗ275 Тогда
		Контракты.Очистить();
		ПодтверждающиеДокументы.Очистить();
		Для Каждого СтрокаРазмещения из РазмещениеЗаявки Цикл
			СтрокаРазмещения.КонтрактСЗаказчиком = Неопределено;
		КонецЦикла;
	Иначе
		КонтрактИсполнителя = КонтрактИсполнителя();
		КонтрактСЗаказчиком = КонтрактСЗаказчиком();
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПроверитьЗаполнениеПлатежПоФЗ275(Отказ, ПроверяемыеРеквизиты);
	
КонецПроцедуры

Процедура ПроверитьЗаполнениеПлатежПоФЗ275(Отказ, ПроверяемыеРеквизиты)
	
	Если Не глЗначениеПеременной("ВестиУчетПлатежейПоГосконтрактам") Тогда
		Возврат;
	КонецЕсли;
	
	Если СуммаОплатыЗаСчетСобственныхСредств + Контракты.Итог("Сумма") > СуммаДокумента Тогда
		ОбщегоНазначения.СообщитьОбОшибке(
			НСтр("ru = 'Сумма фиксированной оплаты по контрактам не может быть больше суммы оплаты по заявке'"), Отказ);
	КонецЕсли;
	
	Если ПлатежПоФЗ275 Тогда
		ПроверяемыеРеквизиты.Добавить("ТипПлатежаФЗ275");
		Контроли275Сервер.ОбработкаПроверкиЗаполненияПодтверждающиеДокументы(ЭтотОбъект, Отказ, ПроверяемыеРеквизиты);
	КонецЕсли;

КонецПроцедуры

Функция КонтрактИсполнителя(ДоговорКонтрагента = Неопределено) Экспорт
	Если ДоговорКонтрагента = Неопределено Тогда
		ДоговорКонтрагента = ТекущийДоговорКонтрагента();
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		Результат = Справочники.КонтрактыИсполнителей.ПустаяСсылка();
	Иначе
		Контракт = ОбщегоНазначения275КлиентСервер.НайтиПодчиненныйКонтракт(ДоговорКонтрагента);
...
Функция КонтрактСЗаказчиком(ДоговорКонтрагента = Неопределено) Экспорт
	Если ДоговорКонтрагента = Неопределено Тогда
		ДоговорКонтрагента = ТекущийДоговорКонтрагента();
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		Результат = Справочники.КонтрактыСЗаказчиками.ПустаяСсылка();
	Иначе
		Контракт = ОбщегоНазначения275КлиентСервер.НайтиПодчиненныйКонтракт(ДоговорКонтрагента);
		Если ТипЗнч(Контракт) <> Тип("СправочникСсылка.КонтрактыСЗаказчиками") Тогда
			Результат = Справочники.КонтрактыСЗаказчиками.ПустаяСсылка();
		Иначе
			Результат = Контракт;
		КонецЕсли;
...
Функция ТекущийДоговорКонтрагента() Экспорт
	
	Договоры = РасшифровкаПлатежа.Выгрузить(, "ДоговорКонтрагента");
	Договоры.Свернуть("ДоговорКонтрагента");
	
	Если Договоры.Количество() = 0 Тогда
		Результат = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
	ИначеЕсли Договоры.Количество() = 1 Тогда
		Результат = Договоры[0].ДоговорКонтрагента;
	Иначе
		Если ЗначениеЗаполнено(Договоры[0].ДоговорКонтрагента) Тогда
			Результат = Договоры[0].ДоговорКонтрагента;
		Иначе
			Результат = Договоры[1].ДоговорКонтрагента;
		КонецЕсли;
	КонецЕсли;
...
Функция КоличествоДоговоров() Экспорт
	
	Договоры = РасшифровкаПлатежа.Выгрузить(, "ДоговорКонтрагента");
	Договоры.Свернуть("ДоговорКонтрагента");

	СтрокаПустогоДоговора = Договоры.Найти(
		Справочники.ДоговорыКонтрагентов.ПустаяСсылка(), "ДоговорКонтрагента");
		
	Результат = Договоры.Количество() - Число(СтрокаПустогоДоговора <> Неопределено);
	
	Возврат Результат;
	
КонецФункции

Функция ТекстОшибки(ВидОперации) Экспорт
	
	Если ВидОперации = "ЕдинственностьДоговора" Тогда
		Результат = "При формировании заявок на платеж по 275-ФЗ в одном документе можно использовать только один договор.";
	ИначеЕсли ВидОперации = "ДоговорНеГОЗ" Тогда
		Результат = "При установленном флаге ""Платеж по 275-ФЗ"" в документе можно выбирать только договоры,
			|используемые в подсистеме учета платежей по государственным контрактам";
	ИначеЕсли ВидОперации = "НеУстановленФлагФЗ275" Тогда
		Результат = "Для проведения заявки на расходование денежных средств по договору используемому в подсистеме учета платежей
			|по государственным контрактам необходимо установить флаг ""Платеж по 275-ФЗ""";
	ИначеЕсли ВидОперации = "ИспользованиеТонкогоКлиента" Тогда
		Результат = "Проведение заявки на расходование денежных средств по договору, используемому в подсистеме учета платежей
			|по государственным контрактам, поддерживается только в обычном приложении";
	Иначе
		Результат = "";
	КонецЕсли;
