...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	СсылкаНаДоговор = ТекущийОбъект.ТекущийДоговорКонтрагента();
	Если ЗначениеЗаполнено(СсылкаНаДоговор) Тогда
		Контракт = ОбщегоНазначения275КлиентСервер.НайтиПодчиненныйКонтракт(СсылкаНаДоговор);
		Если ЗначениеЗаполнено(Контракт) Тогда
			ОбщегоНазначения.СообщитьОбОшибке(ТекущийОбъект.ТекстОшибки("ИспользованиеТонкогоКлиента"), Отказ);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
