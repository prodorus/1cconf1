...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаПосредническаяДеятельность, Отказ, Заголовок)
	...
	Для Каждого СтрокаТЧ Из ТаблицаПосредническаяДеятельность Цикл
		...
		Иначе
			...
			СтрокаЖурнала.ДатаСчетаФактурыПродавца = СтрокаТЧ.ДатаСчетаФактурыПродавца;
			...
			СтрокаЖурнала.СуммаНДС = СтрокаТЧ.СуммаНДС;
			СтрокаЖурнала.СуммаНДСРазницаУвеличение = СтрокаТЧ.СуммаНДСРазницаУвеличение;
			СтрокаЖурнала.СуммаНДСРазницаУменьшение = СтрокаТЧ.СуммаНДСРазницаУменьшение;
			СтрокаЖурнала.СуммаПоСчетуФактуре = СтрокаТЧ.СуммаПоСчетуФактуре;
			СтрокаЖурнала.СуммаПоСчетуФактуреРазницаУвеличение = СтрокаТЧ.СуммаПоСчетуФактуреРазницаУвеличение;
			СтрокаЖурнала.СуммаПоСчетуФактуреРазницаУменьшение = СтрокаТЧ.СуммаПоСчетуФактуреРазницаУменьшение;
