...
Процедура ДвиженияРегистровПодсистемыНДС(СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ) Экспорт
	...
	Если НЕ ТаблицаДляНДС.Количество() = 0 Тогда
		...
		Если ТаблицаДляНДС.Колонки.Найти("ФактурнаяСтоимостьДляУчетаНДС") <> Неопределено Тогда
			ТаблицаДляНДС.Колонки.Удалить("СуммаБезНДС");
			ТаблицаДляНДС.Колонки.ФактурнаяСтоимостьДляУчетаНДС.Имя = "СуммаБезНДС";
		КонецЕсли; 
...
Процедура СформироватьДвиженияРегистрацияВЖурналеУчетаСчетовФактур(СтруктураШапкиДокументаВалюта, ТаблицаПоТоварамРубли)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка",                            Ссылка);
	Запрос.УстановитьПараметр("КоэффициентТаможеннаяСтоимостьРуб", ?(СтруктураШапкиДокументаВалюта.КратностьДокумента=0, 1, СтруктураШапкиДокументаВалюта.КурсДокумента/СтруктураШапкиДокументаВалюта.КратностьДокумента));
	...
	ВерсияПостановленияНДС1137 = УчетНДС.ВерсияПостановленияНДС1137(СтруктураШапкиДокументаВалюта.Дата);
	Запрос.УстановитьПараметр("ОтражатьФактурнуюСтоимость", ВерсияПостановленияНДС1137 >= 4);
	
	ДатаПринятияДекларацииНаТовары = Справочники.НомераГТД.ДатаПринятияДекларацииНаТовары(СтруктураШапкиДокументаВалюта.НомерГТД);
	
	Если ЗначениеЗаполнено(ДатаПринятияДекларацииНаТовары) Тогда
		ДатаСчетаФактуры = ДатаПринятияДекларацииНаТовары;
	Иначе
		ДатаСчетаФактуры = СтруктураШапкиДокументаВалюта.Дата;
	КонецЕсли;
	...
	|	ГТДИмпортТовары.СчетУчетаБУ КАК СчетУчета
	...
	|	ГТДИмпортРазделы.ТаможеннаяСтоимостьВВалютеРеглУчета
	...
	|	СУММА(ТоварыДокумента.СуммаПошлины) КАК НДСИПошлины,
	...
	|	ТаблицаПоставщик.Поставщик КАК Контрагент,
	|	"""" КАК КППКонтрагента,
	...
	|				ТОГДА 0
	|			ИНАЧЕ ВЫБОР
	|					КОГДА РазделыДокумента.ТаможеннаяСтоимостьВВалютеРеглУчета = ЛОЖЬ
	|						ТОГДА (ВЫРАЗИТЬ(РазделыДокумента.ТаможеннаяСтоимость * &КоэффициентТаможеннаяСтоимостьРуб КАК ЧИСЛО(15, 2))) + РазделыДокумента.СуммаПошлины + РазделыДокумента.СуммаНДС
	|					ИНАЧЕ РазделыДокумента.ТаможеннаяСтоимость + РазделыДокумента.СуммаПошлины + РазделыДокумента.СуммаНДС
	|				КОНЕЦ
...
Процедура СформироватьТаблицыПоДокументу(СтруктураШапкиДокумента, ТоварыВВалюте, ТоварыВРублях)
	...
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ГТДИмпортТовары.ФактурнаяСтоимость * &КоэффициентРуб КАК ЧИСЛО(15, 2)) + ГТДИмпортТовары.СуммаПошлины
	|	КОНЕЦ КАК ФактурнаяСтоимостьДляУчетаНДС
	|ИЗ
	|	Документ.ГТДИмпорт.Разделы КАК ГТДИмпортРазделы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ГТДИмпорт.Товары КАК ГТДИмпортТовары
	|		ПО ГТДИмпортРазделы.НомерСтроки = ГТДИмпортТовары.НомерРаздела
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СформироватьДвиженияРегистрацияВЖурналеУчетаСчетовФактур(СтруктураШапкиДокументаВалюта, ТаблицаПоТоварамРубли);
