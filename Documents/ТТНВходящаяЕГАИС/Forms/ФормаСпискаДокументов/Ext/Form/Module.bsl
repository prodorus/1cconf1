...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ТипыДокументаПоступления Из Метаданные.Документы.ТТНВходящаяЕГАИС.Реквизиты.ДокументОснование.Тип.Типы() Цикл
...
// ОбработчикиСобытийЭлементовТабличнойЧастиТоварноТранспортныеНакладные
...
Процедура КомандаОформитьДокументПоступления(Команда)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.ПоступлениеТоваров) Тогда
		...
		Иначе
			ПоказатьЗначениеЕГАИС(, ТекущаяСтрока.ПоступлениеТоваров);
		КонецЕсли;
	...
	ИначеЕсли СтатусыОтказа.НайтиПоЗначению(ТекущаяСтрока.СтатусЕГАИС) = Неопределено Тогда
		...
		ПоказатьВопросЕГАИС(ОписаниеОповещенияЕГАИС("ОбработатьОтветНаВопросОСвязыванииСПрикладнымДокументом",
				ЭтаФорма,
...
Процедура ОткрытьФормуСопоставленияКлассификаторовЕГАИС(ТоварноТранспортныеНакладные, ДополнительныеПараметры = Неопределено)
	...
	СобытияФормЕГАИСКлиентПереопределяемый.ОткрытьФормуСопоставленияКлассификаторовЕГАИС(
		ЭтаФорма,
		ОписаниеОповещенияЕГАИС("ОбработатьРезультатСопоставленияКлассификаторовЕГАИС", ЭтаФорма, ДополнительныеПараметры),
...
Процедура ПоказатьПредупреждениеПоступлениеНеТребуется()
	...
	ПоказатьПредупреждениеЕГАИС(, НСтр("ru='Поступление товаров для данной ТТН ЕГАИС не требуется.'"));
...
Процедура ПоказатьПредупреждениеПоступлениеУжеОформлено()
	...
	ПоказатьПредупреждениеЕГАИС(, НСтр("ru='Поступление товаров для данной ТТН ЕГАИС уже оформлено.'"));
...
Процедура ПоказатьПредупреждениеЕстьНесопоставленнаяАлкогольнаяПродукция()
	...
	ПоказатьПредупреждениеЕГАИС(, НСтр("ru='В данной ТТН ЕГАИС имеется несопоставленная алкогольная продукция.
