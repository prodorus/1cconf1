...
Процедура ОбработкаЗаписиНового(НовыйОбъект, Источник, СтандартнаяОбработка)
	...
	СобытияФормЕГАИСКлиентПереопределяемый.ОбработкаВыбораНоменклатуры(
		ОписаниеОповещенияЕГАИС("ПриВыбореНоменклатуры", ЭтаФорма), НовыйОбъект, Источник);
...
Процедура ГрузоотправительКонтрагентНажатие(Элемент, СтандартнаяОбработка)
	...
	ПоказатьЗначениеЕГАИС(, Объект.Грузоотправитель);
...
Процедура ГрузополучательКонтрагентНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПоказатьЗначениеЕГАИС(, Объект.Грузополучатель);
...
Процедура ПоставщикКонтрагентНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПоказатьЗначениеЕГАИС(, Объект.Поставщик);
...
Процедура ТекстПоступлениеТоваровОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	НавигационнаяСсылкаФорматированнойСтроки = ИнтеграцияЕГАИСУТКлиентСервер.НавигационнаяСсылкаСтроки(Элемент, ТекстПоступлениеТоваров);
	
	Если "ОткрытьПоступление" = НавигационнаяСсылкаФорматированнойСтроки Тогда
		ПоказатьЗначениеЕГАИС(,Объект.ДокументОснование);
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ТоварыАлкогольнаяПродукция Тогда
		...
		ОткрытьФормуЕГАИС("Справочник.КлассификаторАлкогольнойПродукцииЕГАИС.ФормаОбъекта", ПараметрыФормы);
...
Процедура ТоварыНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекущиеДанные.Номенклатура = СобытияФормЕГАИСКлиентПереопределяемый.ОткрытьФормуВыбораНоменклатуры(
		ЭтаФорма,
...
Процедура УстановитьВидимостьЭлементовСерий()
	...
	Элементы.ТоварыСгенерироватьСерии.Доступность = ИспользоватьСерииНоменклатуры;
...
Процедура СгенерироватьСерии(Команда)
	...
	Если ЗначениеЗаполнено(Результат.АдресМассиваЗапросов) Тогда
		...
		ОткрытьФормуЕГАИС(
			...
			ПараметрыФормы,
			ЭтаФорма,,,,
			ОписаниеОповещенияЕГАИС("СгенерироватьСерииЗавершение",ЭтаФорма),
...
Процедура ОбновитьВидимостьОтбораСтрок()
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		Элементы.ОтборСтрок.Доступность = (ОтборСтрокКоличествоСтало <> 1);
	КонецЕсли;
...
// СканированиеАлкогольнойПродукции
...
Процедура ОткрытьФормуСканированияАлкогольнойПродукции()
	...
	ОписаниеОповещенияОкончаниеПроверки = ОписаниеОповещенияЕГАИС("ПроверкаАлкогольнойПродукцииПослеЗакрытия", ЭтаФорма);
	
	ОткрытьФормуЕГАИС("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ПроверкаИПодбор", ПараметрыОткрытия, ЭтаФорма, УникальныйИдентификатор,,
...
Процедура СоздатьДокументПоступления(СопоставлятьКлассификаторы = Истина)
	...
	Иначе
		...
		ПоказатьВопросЕГАИС(
			ОписаниеОповещенияЕГАИС("ОбработатьОтветНаВопросОбОткрытииФормыСопоставленияКлассификаторовЕГАИС",
				ЭтаФорма,
...
Процедура ВыбратьДокументПоступления(СопоставлятьКлассификаторы = Истина)
	...
	Если НесопоставленныеСтроки.Количество() = 0 Тогда
		...
		СобытияФормЕГАИСКлиентПереопределяемый.ОткрытьФормуВыбораДокументаПоступлениеТоваровУслуг(
			ЭтаФорма,
			ОписаниеОповещенияЕГАИС("ПослеВыбораДокументаПоступления", ЭтаФорма),
			СтруктураОтбора);
...
Процедура ОбновитьТекстПоступлениеТоваров()
	...
	Если Не ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Если ЕстьПравоСозданияПоступления Тогда
			ОписаниеОформитьПоступление = ФорматированнаяСтрока(НСтр("ru = 'Оформить поступление'"),
				Новый Шрифт(,,Истина), ЦветаСтиля.ЦветГиперссылкиГИСМ,, "ОформитьПоступление");
			
			Строки.Добавить(ОписаниеОформитьПоступление);
		КонецЕсли;
		
		ОписаниеСвязатьСПоступлением = ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'Связать с поступлением (%1)'"), КоличествоДокументов),
			Новый Шрифт(,,Истина), ЦветаСтиля.ЦветГиперссылкиГИСМ,, "СвязатьСПоступлением");
		
		Строки.Добавить(ОписаниеСвязатьСПоступлением);
	КонецЕсли;
	
	ТекстПоступлениеТоваров = Новый ФиксированныйМассив(Строки);
		
	ИнтеграцияЕГАИСУТ.СформироватьФорматированнуюСтроку(ЭтаФорма,
		ТекстПоступлениеТоваров,
		"ГруппаТекстПоступлениеТоваров",
		"ТекстПоступлениеТоваровОбработкаНавигационнойСсылки");
...
Процедура СформироватьТекстДокументаЕГАИС()
	...
	Если ИспользоватьРегистр2 Тогда
