...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтрокаСценарийПериодичность = ОбщегоНазначения.ИмяЗначенияПеречисления(Сценарий.Периодичность);
	СтрокаПериодичностьДетализации = ОбщегоНазначения.ИмяЗначенияПеречисления(ПериодичностьДетализации);
	
	СтруктураСложныхПолей = Новый Структура;
	СтруктураСложныхПолей.Вставить("Период",
	"ВЫБОР
	|	КОГДА РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(Док.Период, " + ?(ПериодичностьДетализации = Перечисления.Периодичность.ПустаяСсылка(), ?(Сценарий.Периодичность = Перечисления.Периодичность.ПустаяСсылка(), "ДЕНЬ", СтрокаСценарийПериодичность), СтрокаПериодичностьДетализации) + "), Док.Ссылка.ДатаПланирования, ДЕНЬ) > 0
	|		ТОГДА Док.Ссылка.ДатаПланирования
	|	ИНАЧЕ НАЧАЛОПЕРИОДА(Док.Период, " + ?(ПериодичностьДетализации = Перечисления.Периодичность.ПустаяСсылка(), ?(Сценарий.Периодичность = Перечисления.Периодичность.ПустаяСсылка(), "ДЕНЬ", СтрокаСценарийПериодичность), СтрокаПериодичностьДетализации) + ")
	|КОНЕЦ");
