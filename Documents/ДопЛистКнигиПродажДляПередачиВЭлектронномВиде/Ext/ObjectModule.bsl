...
Функция ЭлектронноеПредставление(УникальныйИдентификатор) Экспорт
	...
	Если НалоговыйПериод >= '2019-01-01' Тогда
		Возврат УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПродаж_504(ЭтотОбъект, УникальныйИдентификатор);
		
	ИначеЕсли НалоговыйПериод >= '2017-10-01' Тогда
		Возврат УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПродаж_503(ЭтотОбъект, УникальныйИдентификатор);
		
	ИначеЕсли НалоговыйПериод >= '2014-10-01' Тогда
		Возврат УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПродаж_502(ЭтотОбъект, УникальныйИдентификатор);
	...
	Иначе
		...
		ТекстВыгрузки = ВыгрузитьДеревоВXML(СтруктураВыгрузки, ОсновныеСведения);
		
		МассивВыгрузки = Новый Массив;
		
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла();
		СохраняемыйФайл = Новый ТекстовыйДокумент;
		СохраняемыйФайл.УстановитьТекст(ТекстВыгрузки);
		СохраняемыйФайл.Записать(ИмяВременногоФайла, "windows-1251");
		...
		ЭлементВыгрузки.Вставить("ИмяФайла", ОсновныеСведения.ИдФайл + ".xml");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	|	Таблица.Ссылка
	|ИЗ
	|	Документ.ДопЛистКнигиПродажДляПередачиВЭлектронномВиде КАК Таблица
	|ГДЕ
	...
	Если НалоговыйПериод < '20141001' Тогда 
		Запрос.Текст	= СтрЗаменить(Запрос.Текст, "&УсловиеПоДате", 
			"Таблица.Дата >= &ДатаНачало
	...
	Если НЕ Запрос.Выполнить().Пустой() Тогда
		...
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Уже имеется оформленый дополнительный лист книги продаж за %1 по периоду %2",ДатаПредставление,НалоговыйПериодПредставление);
...
Процедура ЗаполнитьДокумент() Экспорт
	...
	СтруктураПараметров.Вставить("ПериодПоСКНП",                ПериодПоСКНП);
	СтруктураПараметров.Вставить("НачалоПериода", ?(СтруктураПараметров.ФорматПоПостановлению735, 
		НачалоКвартала(НалоговыйПериод), НачалоДня(Дата)));
	СтруктураПараметров.Вставить("КонецПериода", ?(СтруктураПараметров.ФорматПоПостановлению735, 
		КонецКвартала(НалоговыйПериод),  КонецДня(Дата)));
	...
	СтруктураПараметров.Вставить("СоответствиеСтрокиДопИнформацииПоСчетуФактуре", Новый Соответствие);
	...
	Если НЕ СтруктураПараметров.ФорматПоПостановлению735 Тогда
		...
		|ВсегоПродаж,СуммаБезНДС10,НДС10,СуммаБезНДС18,НДС18,НДС0,СуммаСовсемБезНДС,СуммаБезНДС20,НДС20");
		
		ПараметрыДляИтогов = Новый Структура;
		
		ПараметрыДляИтогов.Вставить("Организация",              Организация);
		ПараметрыДляИтогов.Вставить("СписокОрганизаций");
		ПараметрыДляИтогов.Вставить("КонецНалоговогоПериода",   КонецКвартала(НалоговыйПериод));
		ПараметрыДляИтогов.Вставить("НалоговыйПериод",          НалоговыйПериод);
		ПараметрыДляИтогов.Вставить("ДатаФормированияДопЛиста", Дата);
		ПараметрыДляИтогов.Вставить("Ссылка",                   Ссылка);
		...
		РеквизитыДокумента.Вставить("СуммаБезНДС20До",      ИтогЗаПериод.СуммаБезНДС20);
		РеквизитыДокумента.Вставить("НДС20До",              ИтогЗаПериод.НДС20);
	...
	Отчеты.КнигаПродаж1137.СформироватьОтчет(СтруктураПараметров, АдресХранилища);
	...
	Если СтруктураПараметров.ФорматПоПостановлению735 Тогда
		...
		Если СтруктураДанных.СписокСформированныхЛистов.Количество() = 0 Тогда 
			НалоговыйПериодПредставление = ПредставлениеПериода(НачалоКвартала(НалоговыйПериод), КонецКвартала(НалоговыйПериод), "ФП = Истина" );
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Нет данных для заполнения дополнительного листа книги продаж за период %1",НалоговыйПериодПредставление);
			ОбщегоНазначения.СообщитьОбОшибке(Сообщение);
			Возврат;
		КонецЕсли;
		...
		ДанныеОтчета = Новый ХранилищеЗначения(СтруктураДанных.ДанныеОтчета);
	...
	Иначе
		...
		Если СтруктураДанных.Свойство("РеквизитыДокумента") Тогда
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, СтруктураДанных.РеквизитыДокумента);
		КонецЕсли;
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, НепосредственнаяПечать = Ложь) Экспорт
	...
	Если ИмяМакета = "ДополнительныйЛистКнигиПродаж" Тогда
		
		ТабДокумент = ПечатьДополнительногоЛистаКнигиПродаж();
...
Функция ПечатьДополнительногоЛистаКнигиПродаж()
	...
	ИтогЗаПериод.Вставить("ВсегоПродаж", ВсегоПродажДо);
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("ДополнительныйЛистКнигиПродаж","Дополнительный лист книги продаж");
...
Процедура ЗаполнитьДаннымиТабличнойЧасти(Параметры, ДеревоВыгрузки)
	...
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		...
		Если СтрокаТабличнойЧасти.СуммаСовсемБезНДС <> 0 Тогда
			НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
			ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "без НДС", СтрокаТабличнойЧасти.СуммаСовсемБезНДС);
			УдалятьОбразецУзлаВтЧисле = Истина;
		КонецЕсли;
		Если УдалятьОбразецУзлаВтЧисле Тогда
			УдалитьУзел(ОбразецУзлаВтЧисле);
