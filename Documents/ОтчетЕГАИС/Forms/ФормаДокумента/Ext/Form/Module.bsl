...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(Объект.КодФСРАР) = Тип("СправочникСсылка.КлассификаторОрганизацийЕГАИС") Тогда
		Элементы.КодФСРАР.Заголовок = "Организация ЕГАИС периода";
	Иначе
		Элементы.КодФСРАР.Заголовок = "Код ФСРАР";
	КонецЕсли;
...
Процедура КодФСРАРПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.КодФСРАР) Тогда
		...
		Если ТипЗнч(Объект.КодФСРАР) = Тип("СправочникСсылка.КлассификаторОрганизацийЕГАИС") Тогда
			Элемент.Заголовок = "Организация ЕГАИС периода";
		Иначе
			Элемент.Заголовок = "Код ФСРАР";
		КонецЕсли;
...
Процедура ПредставлениеПериодаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = ОписаниеОповещенияЕГАИС("ПредставлениеПериодаНачалоВыбораЗавершение", ЭтаФорма);
	ПараметрыФормыВыбораПериода = Новый Структура("Значение", Объект.Период);
	
	ОткрытьФормуЕГАИС("ОбщаяФорма.ВыборПериодаИС",
