...
Процедура СформироватьНачисленияРаботников(Отказ, КурсВалютыУпрУчета, ВалютаУпрУчета, Заголовок = "")
	...
	|		0,
	|		ВЫБОР
	|			КОГДА Результаты.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.НДФЛРасчетыСБюджетомВидСтроки.Начисление)
	|				ТОГДА 1
	|			ИНАЧЕ -1
	|		КОНЕЦ * Результаты.Налог
	|	ИЗ
	...
	|		И Результаты.ВидСтроки В (ЗНАЧЕНИЕ(Перечисление.НДФЛРасчетыСБюджетомВидСтроки.Начисление), ЗНАЧЕНИЕ(Перечисление.НДФЛРасчетыСБюджетомВидСтроки.ЗачетАвансовыхПлатежей))
	|		И УчетЗаработкаРаботников.УчетНачисленийПоОрганизации
	...
	Пока Выборка.СледующийПоЗначениюПоля("ФизЛицо") Цикл
		...
		Если ОтработаноДней = 0 Тогда
			...
			ОбщегоНазначенияЗК.СообщитьОбОшибке("Не заполнен рабочий график сотрудника "+Выборка.ФизЛицоНаименование+" в управленческом учете!", Отказ, Заголовок);
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если КурсВалютыУпрУчета = 0 Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Не задан курс валюты управленческого учета!", Отказ, Заголовок);
