...
Процедура ПереоценитьВалютныеСредстваРегл(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	БухгалтерскийУчет.ЗаписьВВспомогательныеРегистрыСведений(СтруктураШапкиДокумента,
		...
		"Организация,ПериодРасчета,Активность,Регистратор,Счет,Валюта,Субконто1,Субконто2,Субконто3,ВидУчета,КурсПереоценки",
		"Сумма,СуммаВВалюте,СуммаВРублях");
...
Процедура ПереоценитьРасчетыВУсловныхЕдиницахРегл(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ПереоценкаНаОтчетнуюДату Тогда
		...
		|ИЗ
		|	РегистрНакопления.РасчетыПоПриобретениюВУсловныхЕдиницахОрганизации.Остатки(&ГраницаОстатковНачалоДня, Организация = &Организация) КАК РасчетыПоПриобретениюВУсловныхЕдиницахОрганизацииОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	РасчетыПоПриобретениюВУсловныхЕдиницахОрганизацииОстатки.ДоговорКонтрагента,
		|	РасчетыПоПриобретениюВУсловныхЕдиницахОрганизацииОстатки.Контрагент,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СуммаПереоценки > 0 Тогда
			...
			Если СуммаПереоценки2015 > 0 Тогда
				...
				ПроводкаНУ["Счет" + СуфиксСчетаРасчетов]     = ПланыСчетов.Налоговый.ПоступлениеИВыбытиеИмуществаРаботУслугПрав;
				ПроводкаНУ["ВидУчета" + СуфиксСчетаРасчетов] = Перечисления.ВидыУчетаПоПБУ18.НУ;
				ПроводкаНУ["Субконто" + СуфиксСчетаРасчетов].УсловияПоступленияИВыбытия = Перечисления.УсловияПоступленияИВыбытияИмущества.ЗаПлату;
