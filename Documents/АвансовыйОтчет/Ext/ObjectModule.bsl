...
Процедура ЗаполнитьТоварыПоОснованию(ДокументОснование) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТекНоменклатура <> Выборка.Номенклатура ИЛИ 
			ТекХарактеристика <> Выборка.ХарактеристикаНоменклатуры ИЛИ 
			ТекСерия <> Выборка.СерияНоменклатуры Тогда
			// Новый остаток
			КоличествоОстатокКомпании = Выборка.КоличествоОстатокКомпании;
		Иначе
			// Ничего делать не надо, изменилась единица измерения
		КонецЕсли;
		
		// Остаток пересчитаем в единицы измерения из документа
		КоличествоОстатокКомпанииВЕдДокумента = 
			КоличествоОстатокКомпании / Выборка.ЕдиницаИзмерения.Коэффициент * СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент;
		СтрокаТабличнойЧасти.Количество                 = Мин(КоличествоОстатокКомпанииВЕдДокумента, Выборка.КоличествоПоСерии);
		КоличествоОстатокКомпании =	КоличествоОстатокКомпании - 
			СтрокаТабличнойЧасти.Количество / СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент * Выборка.ЕдиницаИзмерения.Коэффициент;
		...
		ТекХарактеристика = Выборка.ХарактеристикаНоменклатуры;
		ТекСерия          = Выборка.СерияНоменклатуры;
