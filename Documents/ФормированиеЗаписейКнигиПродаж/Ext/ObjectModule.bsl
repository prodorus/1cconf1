...
Процедура ЗаполнитьРазделАвансы(ОтменитьПроведение = Ложь, УдалятьНезаполненные = Ложь) Экспорт
	...
	// Удаление строк, в которых есть незаполненные обязательные поля
	Если УдалятьНезаполненные Тогда
		ОбработкаТабличныхЧастейСервер.УдалитьНезаполненныеСтроки(ТаблицаРезультатов, "ВидЦенности, СчетФактура, СтавкаНДС, ДоговорКонтрагента");
	КонецЕсли;
...
Процедура ЗаполнитьРазделВосстановлен(ОтменитьПроведение = Ложь, УдалятьНезаполненные = Ложь) Экспорт
	...
	// Удаление строк, в которых есть незаполненные обязательные поля
	Если УдалятьНезаполненные Тогда
		ОбработкаТабличныхЧастейСервер.УдалитьНезаполненныеСтроки(ТаблицаРезультатов, "ВидЦенности, СчетФактура, СтавкаНДС, СчетУчетаНДС");
	КонецЕсли;
...
Функция ЗаполнитьНДСНачисленныйДаннымиПоНДСНачисленномуКУплате()
	...
	|	КОНЕЦ КАК Событие,
	|	ВЫБОР
	|		КОГДА НДСНачисленныйОстатки.ДоговорКонтрагента.УчетАгентскогоНДС
	|				И НДСНачисленныйОстатки.ДоговорКонтрагента.НалоговыйАгентПоОплате
	|			ТОГДА ВЫБОР
	|					КОГДА НДСНачисленныйОстатки.СчетФактура ССЫЛКА Документ.ПлатежноеПоручениеИсходящее
	|							ИЛИ НДСНачисленныйОстатки.СчетФактура ССЫЛКА Документ.ПлатежныйОрдерСписаниеДенежныхСредств
	|							ИЛИ НДСНачисленныйОстатки.СчетФактура ССЫЛКА Документ.ПлатежноеТребованиеПолученное
	|							ИЛИ НДСНачисленныйОстатки.СчетФактура ССЫЛКА Документ.РасходныйКассовыйОрдер
	|						ТОГДА НДСНачисленныйОстатки.СчетФактура
	|					ИНАЧЕ НЕОПРЕДЕЛЕНО
	|				КОНЕЦ
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	Для каждого СтрокаСФ Из Дерево_НДСНачисленный.Строки Цикл
		...
		Для каждого СтрокаОплаты Из ТаблицаОплат Цикл
			...
			Если КРаспределениюБезНДС<>0 Тогда
				МассивРезультата_БезНДС = ОбщегоНазначения.РаспределитьПропорционально(КРаспределениюБезНДС,Массив_БезНДС);
			КонецЕсли; 
...
Функция ПодготовитьТаблицуЗаписей(СтруктураШапкиДокумента)
	...
	ВидаЦенностиБезОплаты.Добавить(Перечисления.ВидыЦенностей.ВозвратАвансовПолученных);
...
Процедура ПроверитьЗаполнениеТабличныхЧастей(СтруктураШапкиДокумента, ТаблицаПоНачислению, ТаблицаПоВосстановлениюПоАвансам, Отказ, Заголовок)
	...
	Для каждого СтрокаТаблицы из ТаблицаПоНачислению Цикл
		...
		Если (СтрокаТаблицы.ВидЦенности = Перечисления.ВидыЦенностей.АвансыПолученные
			...
			И НЕ ЗначениеЗаполнено(СтрокаТаблицы.ДоговорКонтрагента) Тогда
			СтрокаНачалаСообщенияОбОшибке = "В строке номер """ + СокрЛП(СтрокаТаблицы.НомерСтроки) + """ табличной части """ + СтрокаТаблицы.НазваниеТаблицы + """: ";
...
Процедура ДвиженияПоРегистрамНачисления(СтруктураШапкиДокумента, ТаблицаПоНачислению, Отказ, Заголовок)
	...
	Если мВестиУчетНДС Тогда
		...
		ТаблицаДвижений_НДСНачисленный.Свернуть(
			"Период,Активность,Организация,СчетФактура,ВидЦенности,СтавкаНДС,Покупатель,ДатаСобытия,Событие,ВидНачисления,ДоговорКонтрагента,СчетУчетаНДС,ИсправленныйСчетФактура",
			"СуммаБезНДС,НДС");
	...
	Если НачалоКвартала(Дата) >= '20141001' Тогда
		...
		Для Каждого СтрокаТаблицыДвижений Из ТаблицаДвижений_НДСЗаписиКнигиПродаж Цикл
			...
			Если СтрокаТаблицыДвижений.Покупатель.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо
				И ТипЗнч(СтрокаТаблицыДвижений.ДокументОплаты) <> Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
	...
	ТаблицаДвижений_НДСЗаписиКнигиПродаж.Свернуть(
			"Период,Активность,Организация,СчетФактура,ДокументОплаты,ДатаОплаты,СтавкаНДС,Покупатель,ДатаСобытия,Событие,ВидЦенности,ЗаписьДополнительногоЛиста,
			|КорректируемыйПериод,СторнирующаяЗаписьДопЛиста,ДоговорКонтрагента,ИсправленныйСчетФактура,КодВидаОперации",
			"СуммаБезНДС,НДС");
	...
	// Сформировать проводки по записям книги продаж
...
Процедура ДвиженияПоРегистрамВосстановлениеПоАвансам(СтруктураШапкиДокумента, ТаблицаПоВосстановлениюПоАвансам, Отказ, Заголовок)
	...
	ТаблицаДвижений_НДСсАвансов.Свернуть(
		"Период, Активность, Организация, СчетФактура, ВидЦенности, СтавкаНДС, Покупатель, ДоговорКонтрагента, ДатаСобытия, ИсправленныйСчетФактура",
		"СуммаБезНДС, НДС");
