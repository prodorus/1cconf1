...
Функция ПечатьЗаявления()
	...
	|	ВЫБОР
	|		КОГДА НачислениеОтпускаРаботникамОрганизаций.НаличиеКартыМИР
	|			ТОГДА НачислениеОтпускаРаботникамОрганизаций.НомерКартыМИР
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК НомерКартыМИР,
	|	ВЫБОР
	|		КОГДА НачислениеОтпускаРаботникамОрганизаций.ПерерассчитываемыйДокумент = ЗНАЧЕНИЕ(Документ.НачислениеОтпускаРаботникамОрганизаций.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	ТабДокумент = ОбменСведениямиОПособияхСФСС.ПечатьЗаявлениеВФССОВыплатеПособия(СтруктураДанныхЗаявления, ИмяМакетаЗаявленияВФСС());
	Возврат ТабДокумент
