...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоЗатратам, Отказ, Заголовок)
	...
	ТаблицаПоЗатратам.Свернуть("ПодразделениеОрганизацииПолучатель, СтатьяЗатрат, СтатьяЗатратПолучатель, ВидРасходовНУ, ВидРасходовНУПолучатель,
	...
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаПоЗатратам Цикл
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			Если (СтрокаТабличнойЧасти.ВидРасходовНУ = Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения
					ИЛИ СтрокаТабличнойЧасти.ВидРасходовНУПолучатель = Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения)
			...
			Если СтрокаТабличнойЧасти.ВидРасходовНУПолучатель = Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения Тогда
				ВидУчетаПоПБУ18Получатель = Перечисления.ВидыУчетаПоПБУ18.ПР;
			Иначе
				ВидУчетаПоПБУ18Получатель = Перечисления.ВидыУчетаПоПБУ18.НУ;
			КонецЕсли;
			
			НоваяПроводкаНУ.ВидУчетаДт = ВидУчетаПоПБУ18Получатель;
			...
			Тогда
				...
				Если СтрокаТабличнойЧасти.ВидРасходовНУПолучатель = Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения Тогда
					ВидУчетаПоПБУ18Получатель = Перечисления.ВидыУчетаПоПБУ18.ПР;
				Иначе
					ВидУчетаПоПБУ18Получатель = Перечисления.ВидыУчетаПоПБУ18.ВР;
				КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтруктураПолей.Вставить("ВидРасходовНУПолучатель",				"СтатьяЗатратПолучатель.ВидРасходовНУ");
