...
Процедура ОкруглениеСуммНДС(СтруктураШапкиДокумента)
	...
	ОкруглениеНДС(СтруктураШапкиДокумента);
	
	ОкруглениеНДСНалоговогоАгента(СтруктураШапкиДокумента);
		
КонецПроцедуры

Процедура ОкруглениеНДС(СтруктураШапкиДокумента)
	...
	Запрос.УстановитьПараметр("СписокОрганизаций", 			 СтруктураШапкиДокумента.Организация);
	...
	Если Не Результат.Пустой() Тогда 
		...
		ПараметрыОтчета.Вставить("Организация",							 СтруктураШапкиДокумента.Организация);
		ПараметрыОтчета.Вставить("НачалоПериода",						 НачалоКвартала(мНачДата));
		ПараметрыОтчета.Вставить("КонецПериода",						 КонецКвартала(мНачДата));
		ПараметрыОтчета.Вставить("СУчетомКорректировок", 				 Истина);
		ПараметрыОтчета.Вставить("ВесьОборотЗаПериод",   				 Истина);
		ПараметрыОтчета.Вставить("КонецПериодаУчитываемыхКорректировок", КонецКвартала(мНачДата));
		
		СтруктураПоказателейРаздела3 = ПолучитьСтруктуруПоказателейРаздела3Декларации();
		СтруктураПоказателейРаздела4 = ПолучитьСтруктуруПоказателейРаздела4Декларации();
		СтруктураПоказателейРаздела6 = ПолучитьСтруктуруПоказателейРаздела6Декларации();
		
		ОтчетОбъект = Отчеты.РегламентированныйОтчетНДС.Создать();
		
		ПоказателиРаздела3 = ОтчетОбъект.ПолучитьПоказателиРаздела3ФормаОтчета2009Кв4(ПараметрыОтчета);
		ПоказателиРаздела4 = ОтчетОбъект.ПолучитьПоказателиРаздела4ФормаОтчета2009Кв4(ПараметрыОтчета);
		ПоказателиРаздела6 = ОтчетОбъект.ПолучитьПоказателиРаздела6ФормаОтчета2009Кв4(ПараметрыОтчета);
		
		ЗначенияДляЗаполненияПродажи = ПоказателиРаздела3.ПоказателиПоКнигеПродаж.Выбрать();
		Если ЗначенияДляЗаполненияПродажи.Следующий() Тогда 
			ЗаполнитьЗначенияСвойств(СтруктураПоказателейРаздела3, ЗначенияДляЗаполненияПродажи);
		КонецЕсли;	
		
		ЗначенияДляЗаполненияПокупки = ПоказателиРаздела3.ПоказателиПоКнигеПокупок.Выбрать();
		Если ЗначенияДляЗаполненияПокупки.Следующий() Тогда 
			ЗаполнитьЗначенияСвойств(СтруктураПоказателейРаздела3, ЗначенияДляЗаполненияПокупки);
		КонецЕсли;	
		
		ВычетыПоРазделу4Декларации = ПоказателиРаздела4.СуммаНалога.Выбрать();
		
		Если ВычетыПоРазделу4Декларации.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(СтруктураПоказателейРаздела4, ВычетыПоРазделу4Декларации);
		КонецЕсли;
		
		ВычетыПоРазделу6Декларации = ПоказателиРаздела6.Вычеты.Выбрать();
		Если ВычетыПоРазделу6Декларации.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(СтруктураПоказателейРаздела6, ВычетыПоРазделу6Декларации);
		КонецЕсли; 
		
		НачисленияПоРазделу6Декларации = ПоказателиРаздела6.Начисления.Выбрать();
		Если НачисленияПоРазделу6Декларации.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(СтруктураПоказателейРаздела6, НачисленияПоРазделу6Декларации);
		КонецЕсли; 
		
		РасчитатьПоказателиРаздела3Декларации(СтруктураПоказателейРаздела3);
...
Процедура ОкруглениеНДСНалоговогоАгента(СтруктураШапкиДокумента)
	...
	Запрос.УстановитьПараметр("НачДата",        НачалоКвартала(мНачДата));
	Запрос.УстановитьПараметр("КонДата",        КонецКвартала(мНачДата));
	Запрос.УстановитьПараметр("Организация",	СтруктураШапкиДокумента.Организация);
	...
	|				И Субконто2.ВидАгентскогоДоговора В (ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.РеализацияИмущества), ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.Нерезидент)),
	|			,
	|			) КАК Хозрасчетный
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если НЕ Результат.Пустой() Тогда 
		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе 				
				...
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКТ,
					"СФВыданные", Выборка.ДокументРасчетовСКонтрагентами);
...
Процедура РасчитатьПоказателиРаздела3Декларации(СтруктураПоказателей)
...
Процедура СписаниеУбытковПрошлыхЛет(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СУММА(НалоговыйОбороты.СуммаОборотКт) КАК УбыткаСписано,
	|	НалоговыйОбороты.Субконто1 КАК РБП,
	|	НалоговыйОбороты.Субконто1.ДатаНачалаСписания КАК ГОД,
	|	НалоговыйОбороты.СуммаКонечныйОстаток КАК ОстатокУбытка
	|ИЗ
	...
	|ГДЕ
	|	НалоговыйОбороты.Субконто1.ДатаНачалаСписания МЕЖДУ &ДатаНачалаСписания И &ДатаНач
	|	И (НЕ &ПродолжительностьСписанияУбытковОграничена
	|	ИЛИ НалоговыйОбороты.Субконто1.ДатаОкончанияСписания >= &ДатаКон)
	...
	ИначеЕсли СтруктураШапкиДокумента.Дата < '20070101' Тогда
		Норма = 0.5;
	Иначе
		Норма = НалоговыйУчет.МаксимальнаяДоляУменьшенияПрибылиЗаСчетУбытковПрошлыхЛет(мКонДата);
	КонецЕсли;
	
	// Если получена прибыль, то считаем ограничение на сумму переносимых убытков прошлых лет
	// Если получен убыток (или прибыль = 0), то убытки прошлых лет переносить нельзя - 
	// нужно сторнировать перенесенные ранее в текущем году суммы убытков
	СписаниеУбытка = 0;
	Если ПрибыльНУ > 0 Тогда
		Если Норма < 1 Тогда
			// В декларации по прибыли суммы округляются до целых
			// Чтобы не допустить превышения лимита берем целую часть
			ЛимитСписанияУбытка = Цел(ПрибыльНУ * Норма);
		Иначе
			ЛимитСписанияУбытка = ПрибыльНУ;
		КонецЕсли;
...
Процедура РасчетРезервовПоСомнительнымДолгам(СтруктураШапкиДокумента)
	...
	Если СтруктураШапкиДокумента.Дата >= '2017-01-01' И КонецМесяца(СтруктураШапкиДокумента.Дата) < КонецГода(СтруктураШапкиДокумента.Дата) Тогда
		// Подменим дату документа в СтруктураШапкиДокумента на конец предыдущего года
		// Предварительно сохраним текущее значение, чтобы потом вернуть назад
		ПараметрДата = СтруктураШапкиДокумента.Дата;
		СтруктураШапкиДокумента.Дата = НачалоГода(СтруктураШапкиДокумента.Дата) - 1;
		НормаРасходовПоВыручкеПредыдущегоГода = НалоговыйУчет.ОпределитьНормуРасходовПоВыручке(СтруктураШапкиДокумента, 10);
		НормаРасходовПоВыручке = Макс(НормаРасходовПоВыручкеПредыдущегоГода, НормаРасходовПоВыручке);
		СтруктураШапкиДокумента.Дата = ПараметрДата;
	КонецЕсли;		
	...
	Для Каждого СтрокаТаблицыДолгов Из ТаблицаДолгов Цикл
		...
		СтрокаЗаписи.ВстречнаяЗадолженность = СтрокаТаблицыДолгов.ВстречнаяЗадолженность;
