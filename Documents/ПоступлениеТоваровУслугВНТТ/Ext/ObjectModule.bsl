...
Функция ПечатьТОРГ12ЗаПоставщика(БезУслуг = Ложь)
	...
	Если Ссылка.ВалютаДокумента <> мВалютаРегламентированногоУчета
		...
		|ИЗ
		|	Документ.ПоступлениеТоваровУслугВНТТ.ДокументыРасчетовСКонтрагентом КАК Док
		|ГДЕ
...
Функция ПечатьТОРГ4()
	...
	Пока ВыборкаСтрокТовары.Следующий() Цикл
		...
		ОбластьМакета.Параметры.Заполнить(ВыборкаСтрокТовары);
		СуммаСНДС = ?(СуммаВключаетНДС, ВыборкаСтрокТовары.Сумма, ВыборкаСтрокТовары.Сумма + ВыборкаСтрокТовары.СуммаНДС);
