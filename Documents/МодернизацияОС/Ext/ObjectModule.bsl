	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда 
		...
		Для каждого СтрокаТЧ Из ТаблицаПоОС Цикл
			...
			Иначе
				СтрокаТЗ = ТаблицаСчетов.Найти(ТекОС,"ОС_БУ");
				Если СтрокаТЗ <> Неопределено Тогда
					СчетУчетаСтоимости = БухгалтерскийУчет.ПреобразоватьСчетаБУвСчетНУ(Новый Структура("СчетБУ, ", СтрокаТЗ.СчетУчетаБУ));
				КонецЕсли;
			КонецЕсли;
...
Процедура ДвиженияРегистровПодсистемыНДС(СтруктураШапкиДокумента, ТаблицаПоОС, Отказ, Заголовок, ФормироватьПроводки = Истина) Экспорт
	...
	Если ИтоговаяСуммаМодернизацииБУХоз > 0 Тогда
		...
		СтавкаНДС = УчетНДС.ОбщаяСтавкаНДС(СтруктураШапкиДокумента.Дата);
		СуммаНДСПоХозСпособу = УчетНДС.РассчитатьСуммуНДС(ИтоговаяСуммаМодернизацииБУХоз, Истина, Ложь, УчетНДС.ПолучитьСтавкуНДС(СтавкаНДС));
