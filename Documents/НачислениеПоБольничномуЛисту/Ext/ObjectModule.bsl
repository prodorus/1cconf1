...
Функция ПолучитьСтрокуЗаполняемыхРеквизитов()
	...
	СтрокаРеквизитов = "Организация,Сотрудник,ФизЛицо,ДатаНарушенияРежима,"
			...
			+ "АдресРегистрацииКодПоФИАС,АдресПочтовыйКодПоФИАС,"
...
Функция ПечатьОборотаБольничного2011()
	...
	Если ОграничениеПособия = Перечисления.ВидыОграниченияПособия.ОграничениеМаксимальнымРазмеромЕжемесячнойСтраховойВыплаты Тогда
		РасчетПособияТекст = СтрЗаменить(РасчетПособияТекст, "НЕОПРЕДЕЛЕНО КАК ОграничениеПособия,", "ВЫРАЗИТЬ(ЕСТЬNULL(МаксимальныйРазмерЕжемесячнойСтраховойВыплаты.Размер, 0) * 4 / ДЕНЬ(КОНЕЦПЕРИОДА(ОсновныеНачисленияРаботниковОрганизаций.ПериодДействияНачало, МЕСЯЦ)) КАК ЧИСЛО(15, 2)) КАК ОграничениеПособия,")
	ИначеЕсли ОграничениеПособия = Перечисления.ВидыОграниченияПособия.ОграничениеВРазмереММОТ Тогда
		РасчетПособияТекст = СтрЗаменить(РасчетПособияТекст, "НЕОПРЕДЕЛЕНО КАК ОграничениеПособия,", ОграничениеВРазмереММОТТекст + " КАК ОграничениеПособия,")
	ИначеЕсли ЗначениеЗаполнено(ДатаНарушенияРежима) Тогда
		РасчетПособияТекст = СтрЗаменить(РасчетПособияТекст, "НЕОПРЕДЕЛЕНО КАК ОграничениеПособия,", "ВЫБОР
...
Функция СтруктураДанныхДляПечатиЗаявлений()
	...
	Если ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ПоБеременностиИРодам Тогда
		...
		Если ПоставленаНаУчетВРанниеСрокиБеременности = Перечисления.ПоставленаНаУчетВРанниеСрокиБеременности.Поставлена Тогда
	...
	ПредшествующийГод = НачалоГода(ДатаНачалаСобытия) - 1;
	ПериодыТекст = "ВЫБРАТЬ " + Формат(ПредшествующийГод,"ДФ=гггг") + " КАК Год," + Символы.ПС + Символы.Таб + ДеньГода(ПредшествующийГод) + " КАК ДнейВГоду" + Символы.ПС + Символы.Таб + "ПОМЕСТИТЬ ВТКалендарныхДнейВГоду";
	Для Сч = 1 По 20 Цикл
		ПредшествующийГод = НачалоГода(ПредшествующийГод) - 1;
		ПериодыТекст = ПериодыТекст + Символы.ПС + Символы.Таб + " ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ " + Формат(ПредшествующийГод,"ДФ=гггг") + ", " + ДеньГода(ПредшествующийГод);
	КонецЦикла;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	БольничныйРасчетСреднего2011.Ссылка,
	|	БольничныйРасчетСреднего2011.РасчетныйГод,
	|	СУММА(БольничныйРасчетСреднего2011.Заработок) КАК Заработок,
	|	СУММА(БольничныйРасчетСреднего2011.ДнейБолезниУходаЗаДетьми) КАК ДнейБолезниУходаЗаДетьми
	|ПОМЕСТИТЬ ВТРасчет2011
	|ИЗ
	|	Документ.НачислениеПоБольничномуЛисту.РасчетСреднего2011 КАК БольничныйРасчетСреднего2011
	|ГДЕ
	|	БольничныйРасчетСреднего2011.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	БольничныйРасчетСреднего2011.РасчетныйГод,
	|	БольничныйРасчетСреднего2011.Ссылка
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	БольничныйГодыРасчетаСреднего2011.НомерСтроки,
	|	БольничныйГодыРасчетаСреднего2011.Год,
	|	БольничныйГодыРасчетаСреднего2011.Ссылка
	|ПОМЕСТИТЬ ВТГоды2011
	|ИЗ
	|	Документ.НачислениеПоБольничномуЛисту.ГодыРасчетаСреднего2011 КАК БольничныйГодыРасчетаСреднего2011
	|ГДЕ
	|	БольничныйГодыРасчетаСреднего2011.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА НачислениеПоБольничномуЛисту.НаличиеКартыМИР
	|			ТОГДА НачислениеПоБольничномуЛисту.НомерКартыМИР
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК НомерКартыМИР,
	...
	|		КОГДА НачислениеПоБольничномуЛисту.ПричинаНетрудоспособности = ЗНАЧЕНИЕ(Перечисление.ПричиныНетрудоспособности.ПоБеременностиИРодам)
	|			ТОГДА ВЫБОР
	|					КОГДА НачислениеПоБольничномуЛисту.Дата >= &ДатаФормы2017года
	|						ТОГДА ЕСТЬNULL(КалендарныеДниПервогоГода.ДнейВГоду, 0) - ЕСТЬNULL(ЗаработокПервыйГод.ДнейБолезниУходаЗаДетьми, 0) + ЕСТЬNULL(КалендарныеДниВторогоГода.ДнейВГоду, 0) - ЕСТЬNULL(ЗаработокВторойГод.ДнейБолезниУходаЗаДетьми, 0)
	|					КОГДА НачислениеПоБольничномуЛисту.РасчетПоПравилам2010Года
	|							И НЕ НачислениеПоБольничномуЛисту.РасчетПособияИзТарифнойСтавки
	...
	|	ЕСТЬNULL(ЗаработокПервыйГод.Заработок, 0) КАК ЗаработокЗаПервыйГод,
	|	ЕСТЬNULL(ЗаработокВторойГод.Заработок, 0) КАК ЗаработокЗаВторойГод,
	|	ВЫБОР
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТКалендарныхДнейВГоду КАК КалендарныеДниВторогоГода
	|			ПО ВторойГодРасчета2011.Год = КалендарныеДниВторогоГода.Год
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТКалендарныхДнейВГоду КАК КалендарныеДниПервогоГода
	|			ПО ПервыйГодРасчета2011.Год = КалендарныеДниПервогоГода.Год
	...
	|	ДанныеДокумента.ЭтоПерерасчет,
	|	ДанныеДокумента.СреднийЗаработок,
	|	ДанныеДокумента.ЗаработокЗаПервыйГод,
	|	ДанныеДокумента.ЗаработокЗаВторойГод
	|ИЗ
	...
	Запрос.УстановитьПараметр("ДатаФормы2017года", ОбменСведениямиОПособияхСФСС.ДатаВступленияВСилуФорм2017года());
	Выборка = Запрос.Выполнить().Выбрать();
	Если  Выборка.Следующий() Тогда 
		ЗаполнитьЗначенияСвойств(СтруктураДанныхЗаявления, Выборка);
		СтруктураДанныхЗаявления.ЭтоЭЛН = ЗначениеЗаполнено(Хеш);
		...
		КонецЕсли;
		СтруктураДанныхЗаявления.ПервыйГодРасчета = ПервыйГодРасчета - 2000;
		СтруктураДанныхЗаявления.ВторойГодРасчета = ВторойГодРасчета - 2000;
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "СправкаКЛисткуНетрудоспособности2007" Тогда                                                                                                         
		...
		КонецЕсли;
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ЭтотОбъект, Метаданные().Синоним) + ОписаниеФормы);
	ИначеЕсли Лев(ИмяМакета,12) = "ЧистыйБланк_" Тогда                                                                                                         
		ТабДокумент = ОбменСведениямиОПособияхСФСС.ПечатьЗаявлениеВФССОВыплатеПособия(ОбменСведениямиОПособияхСФСС.СтруктураДанныхДляПечатиЗаявленийВФССОВыплатеПособия(), Сред(ИмяМакета, 13));
	...
	ИначеЕсли Лев(ИмяМакета,28) = "ЗаявлениеВФССОВыплатеПособия" Или ИмяМакета = "ЗаявлениеВФССОДоплатеПособия" Тогда                                                                                                         
		ТабДокумент = ОбменСведениямиОПособияхСФСС.ПечатьЗаявлениеВФССОВыплатеПособия(СтруктураДанныхДляПечатиЗаявлений(), ИмяМакета);
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок = "")
	...
	ИначеЕсли ПроведениеРасчетов.ПособияПоОбязательномуСтрахованиюВыплачиваетФСС(ВыборкаПоШапкеДокумента.ПериодРегистрации, ВыборкаПоШапкеДокумента.ОбособленноеПодразделение) И ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ВсегоПособия) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ОбменСведениямиОПособияхСФСС.ОписаниеОтказа(ВыборкаПоШапкеДокумента.ПериодРегистрации), Отказ, Заголовок);
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если Не Отказ Тогда
		...
		//ТаблицаДоходов = Движения.НДФЛСведенияОДоходах.Выгрузить(,"КодДохода, ДатаПолученияДохода, СуммаДохода");
		//ТаблицаНалогов = НДФЛ.Выгрузить(,"МесяцНалоговогоПериода");
		//Если Не ПроведениеРасчетов.НалогиСоответствуютДоходам(ТаблицаДоходов, ТаблицаНалогов, ДатаВыплатыДохода) Тогда
		//	ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Исчисленный НДФЛ не соответствует дате получения дохода по документу!", Отказ, Заголовок);
		//КонецЕсли;
		...
		Если Не Отказ Тогда
			...
			Если УпрощенныйУчетНДФЛ И Движения.НДФЛРасчетыСБюджетом.Количество() > 0 Тогда
				Если ДатаВыплатыДохода >= ПроведениеРасчетов.ДатаИзмененияСроковУплатыНалога() Тогда
					ПроведениеРасчетов.ВписатьДатыВыплатыСрокУплатыВУдержанныеНалоги(Движения.НДФЛРасчетыСБюджетом, ДатаВыплатыДохода, Перечисления.СрокиПеречисляемогоНалога.МежрасчетныеОтпускаИБольничные);
				КонецЕсли;
