...
Процедура ЗаполнитьДанныеБольничного(ТекущаяСтрока = Неопределено)
	...
	|	НачислениеПоБольничномуЛисту.АдресРегистрацииКодПоФИАС,
	|	НачислениеПоБольничномуЛисту.АдресПочтовыйКодПоФИАС,
	|	НачислениеПоБольничномуЛисту.Хеш
	|ИЗ
	|	Документ.НачислениеПоБольничномуЛисту КАК НачислениеПоБольничномуЛисту
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ТекущаяСтрока.ТипЛН = ЗначениеЗаполнено(Выборка.Хеш);
		КонецЕсли;
	Иначе
		...
		Для Каждого СтрокаТЗ Из ВременнаяТаблица Цикл
			...
			НоваяСтрока.ТипЛН = ЗначениеЗаполнено(СтрокаТЗ.Хеш);
			НоваяСтрока.ИнаяПолезнаяИнформация = СформироватьИнаяПолезнаяИнформация(НоваяСтрока);
		КонецЦикла;
...
Процедура ВписатьВСтрокуКодУсловийИсчисления(СтрокаУсловий, КодУсловий, НеОграничиватьКодыУсловий)

	Если ЗначениеЗаполнено(КодУсловий) И (НеОграничиватьКодыУсловий Или Найти("43,44,45,47", КодУсловий) > 0) Тогда
...
Функция ПечатьРеестрСведенийВФСС(ИмяМакета = "РеестрСведенийВФСС2011")
	...
	ВыводитьРасчетныйПериодВШапке = ОбластьШапка.Параметры.Количество() > 9;
	...
	Пока ВыборкаПоТЧРаботникиОрганизации.Следующий() Цикл
		...
		Если ВыборкаПоТЧРаботникиОрганизации.ПоставленаНаУчетВРанниеСрокиБеременности = Перечисления.ПоставленаНаУчетВРанниеСрокиБеременности.Поставлена Тогда
			Если ИмяМакета = "РеестрСведенийВФСС2011" Тогда
		...
		УсловияИсчисления = "";
		НеОграничиватьКодыУсловий = ИмяМакета = "РеестрСведенийВФСС2018";
		ВписатьВСтрокуКодУсловийИсчисления(УсловияИсчисления, ВыборкаПоТЧРаботникиОрганизации.УсловияИсчисленияКод1, НеОграничиватьКодыУсловий);
		ВписатьВСтрокуКодУсловийИсчисления(УсловияИсчисления, ВыборкаПоТЧРаботникиОрганизации.УсловияИсчисленияКод2, НеОграничиватьКодыУсловий);
		ВписатьВСтрокуКодУсловийИсчисления(УсловияИсчисления, ВыборкаПоТЧРаботникиОрганизации.УсловияИсчисленияКод3, НеОграничиватьКодыУсловий);
		...
		Если ИмяМакета = "РеестрСведенийВФСС2012"
			Или ИмяМакета = "РеестрСведенийВФСС2018" Тогда
			...
			Если ИмяМакета = "РеестрСведенийВФСС2018" Тогда
				Если Не (ЗначениеЗаполнено(ВыборкаПоТЧРаботникиОрганизации.СерияДокументаНаПроживание) 
					И ЗначениеЗаполнено(ВыборкаПоТЧРаботникиОрганизации.НомерДокументаНаПроживание)) Тогда
					ОбластьСтрока.Параметры.СерияНомерДокументаНаПроживание = "-"
				Иначе
					ОбластьСтрока.Параметры.СерияНомерДокументаНаПроживание = СокрЛП(ВыборкаПоТЧРаботникиОрганизации.СерияДокументаНаПроживание + " " + ВыборкаПоТЧРаботникиОрганизации.НомерДокументаНаПроживание)
				КонецЕсли;
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если Лев(ИмяМакета,29) = "ОписьЗаявленийИДокументовВФСС" Тогда
		...
		ПостфиксИмениМакета = ?(Дата < ОбменСведениямиОПособияхСФСС.ДатаВступленияВСилуФорм2012года(), "_2011", ?(Дата < ОбменСведениямиОПособияхСФСС.ДатаВступленияВСилуФорм2017года(), "_2012", "_2017"));
		ТабДокумент = ПечатьОписьЗаявленийИДокументовВФСС(ИмяМакета + ПостфиксИмениМакета);
...
Функция СформироватьЗапросПоТЧРаботникиОрганизации(Режим = "") Экспорт
	...
	ВременнаяТаблица = РаботникиОрганизации.Выгрузить(Новый Структура("ПоставленаНаУчетВРанниеСрокиБеременности", Перечисления.ПоставленаНаУчетВРанниеСрокиБеременности.Поставлена),"ДатаСправкиБеременной");
	...
	Если Режим <> "НеВыделятьПособиеПриПостановкеНаУчетВРанниеСрокиБеременности" Тогда
		...
		|ГДЕ
		|	Пособия.Ссылка = &ДокументСсылка
		|	И Пособия.ПоставленаНаУчетВРанниеСрокиБеременности = ЗНАЧЕНИЕ(Перечисление.ПоставленаНаУчетВРанниеСрокиБеременности.Поставлена)
...
Процедура ПроверитьЗаполнениеСтрокиТЧРаботникиОрганизации(ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ВыборкаПоСтрокамДокумента.ВидПособия = Перечисления.ВидыПособийСоциальногоСтрахования.БеременностьРоды Тогда
		...
		КонецЕсли;
		Если ВыборкаПоСтрокамДокумента.ПоставленаНаУчетВРанниеСрокиБеременности = Перечисления.ПоставленаНаУчетВРанниеСрокиБеременности.Поставлена Тогда
			Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиБеременной) Тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указана дата выдачи справки о постановке на учет в медицинских учреждениях в ранние сроки беременности!", Отказ, Заголовок);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиБеременной) Тогда
	...
	Если ВыборкаПоСтрокамДокумента.ВидПособия = Перечисления.ВидыПособийСоциальногоСтрахования.БеременностьРоды И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаИзмененияКодаПричиныНетрудоспособности) Тогда
		Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПервичногоБольничного) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДополнительныйКодПричиныНетрудоспособности) Тогда
