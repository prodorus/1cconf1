...
Функция ПолучитьДвиженияПоСпецификации(Выпуск, НомерОперацииМаршрута, КоличествоЦикловВыпуска, ЦикловКПланированию, ДатаВыпуска)
	...
	Результат.Колонки.Добавить("ВидВоспроизводства");
	...
	Если ИспользоватьДетальныеИсточники Тогда
		...
		Для каждого СтрокаВыпуска Из РезультатРазузлования.ИсходныеКомплектующие Цикл
			...
			КоличествоДляВыпускаПоСпецификации = СтрокаВыпуска.КоличествоПоСпецификации * ЦикловКПланированию.Числитель / ЦикловКПланированию.Знаменатель;
	...
	Для каждого Комплектующая из РезультатРазузлования.ИсходныеКомплектующие Цикл
		...
		Иначе
			...
			НоваяСтрока.Количество = Комплектующая.Количество * Комплектующая.Коэффициент;
...
Функция ПолучитьВыходныеИзделия(Спецификация, НомерОперацииМаршрута)
	...
	|	СУММА(ВыхИзделия.Количество * (ВЫБОР
	|			КОГДА ВыхИзделия.ЕдиницаИзмерения = ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка)
	|				ТОГДА ВыхИзделия.Номенклатура.ЕдиницаХраненияОстатков
	|			ИНАЧЕ ВыхИзделия.ЕдиницаИзмерения
	|		КОНЕЦ).Коэффициент) КАК Количество
	|ИЗ
	...
	|	СУММА(ВозврОтходы.Количество * (ВЫБОР
	|			КОГДА ВозврОтходы.ЕдиницаИзмерения = ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка)
	|				ТОГДА ВозврОтходы.Номенклатура.ЕдиницаХраненияОстатков
	|			ИНАЧЕ ВозврОтходы.ЕдиницаИзмерения
	|		КОНЕЦ).Коэффициент)
	|ИЗ
...
Функция ПолучитьПериодыПоСмене(Смена, Дата)
	...
	Если ТипЗнч(Смена.ПериодыСмены) = Тип("СправочникТабличнаяЧасть.ГрафикиРаботы.ПериодыСмены") Тогда
		
		Для каждого Период из Смена.ПериодыСмены Цикл
			
			Если ДеньНедели(Дата) = Перечисления.ДниНедели.Индекс(Период.ДеньНедели) + 1 Тогда
	...
	ИначеЕсли ТипЗнч(Смена.ПериодыСмены) = Тип("СправочникТабличнаяЧасть.Смены.ПериодыСмены") Тогда
			...
			Для каждого РабочиеЦентрИсполнитель из РабочиеЦентрыИсполнители Цикл
				...
				Если ДоступностьРЦ <> Неопределено Тогда
					...
					Для каждого Смена из ДоступностьРЦ Цикл
						...
						Если НЕ Операция.ДопускаетПеренос И (ЦикловКПланированию.Числитель <> ОставшеесяКоличествоЦиклов.Числитель ИЛИ ЦикловКПланированию.Знаменатель <> ОставшеесяКоличествоЦиклов.Знаменатель) Тогда
						...
						Если ЦикловКПланированию.Числитель > 0 Тогда
							...
							Для каждого ДвижениеПоСпецификации из ДвиженияПоСпецификации Цикл
								...
								Если ДвижениеПоСпецификации.ВидВоспроизводства = Перечисления.ВидыВоспроизводстваНоменклатуры.Производство Тогда
			...
			Для каждого СтрокаРезультат Из РезультатРазузлования.ПолноеРазузлование Цикл
				...
				Если МассивСтрок.Количество() > 0 Тогда
					...
					Для каждого СтрокаМассива Из МассивСтрок Цикл
						...
						Если Количество < СтрокаРезультат.Количество * СтрокаРезультат.Коэффициент Тогда
							
							УдалитьСтроку = Ложь;
							
							Если СтрокаМассива.Количество <= СтрокаРезультат.Количество * СтрокаРезультат.Коэффициент - Количество Тогда
							...
							Иначе
								...
								КоличествоКПланированию = СтрокаРезультат.Количество * СтрокаРезультат.Коэффициент - Количество;
								СтрокаМассива.Количество = СтрокаМассива.Количество - КоличествоКПланированию;
								
							КонецЕсли;
							
							НоваяСтрокаДерева = ТекущийРодитель.Строки.Добавить();
							СкопироватьСтрокуДерева(СтрокаМассива, НоваяСтрокаДерева);
							НоваяСтрокаДерева.КоличествоПоСпецификации = СтрокаРезультат.Количество * СтрокаРезультат.Коэффициент * РезультатРазузлования.КоличествоВыпусков.Знаменатель / РезультатРазузлования.КоличествоВыпусков.Числитель;
