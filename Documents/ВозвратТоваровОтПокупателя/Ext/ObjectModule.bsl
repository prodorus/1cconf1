...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоКомплектам, ТаблицаПоТаре, ТаблицаПоВзаиморасчетам, ТаблицаПоРасчетам, ТаблицаПоСкидкам, Отказ, Заголовок);
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		Если СтруктураШапкиДокумента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам Тогда
			...
			ТипСтаройКолонки = ТаблицаПоКомплектам.Колонки.ЗаказПокупателя.ТипЗначения;
			ТаблицаПоКомплектам.Колонки.Удалить("ЗаказПокупателя");
			ТипНовойКолонки = Новый ОписаниеТипов(ТипСтаройКолонки, "ДокументСсылка.СчетНаОплатуПокупателю");
			ТаблицаПоКомплектам.Колонки.Добавить("ЗаказПокупателя", ТипНовойКолонки);
			ТаблицаПоКомплектам.ЗаполнитьЗначения(СтруктураШапкиДокумента.Сделка, "ЗаказПокупателя");
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если (ЗначениеЗаполнено(НомерРасходногоКассовогоОрдера) И НЕ ЗначениеЗаполнено(ДатаРасходногоКассовогоОрдера))
		...
		ОбщегоНазначения.СообщитьОбОшибке("При регистрации в книге покупок даты и номера документа розничной реализации должны быть заполнены и дата, и номер расходно-кассового ордера.",
			Отказ, Заголовок);
	КонецЕсли;
	
	Если (ЗначениеЗаполнено(НомерДокументаОтгрузки) И НЕ ЗначениеЗаполнено(ДатаДокументаОтгрузки))
		ИЛИ (НЕ ЗначениеЗаполнено(НомерДокументаОтгрузки) И ЗначениеЗаполнено(ДатаДокументаОтгрузки)) Тогда
		ОбщегоНазначения.СообщитьОбОшибке("При регистрации в книге покупок даты и номера документа розничной реализации должны быть заполнены и дата и номер.",
			Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам) Экспорт
	...
	СтруктураПолей.Вставить("ДокументРеализацииДата"       , "ДокументПартии.Дата");
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
		...
		Иначе
			Если НЕ ПроцентНеЗаполнен
			   ...
			   И  СтрокаТаблицы.ЗначениеУсловияАвтоматическойСкидки = 0) ИЛИ СтрокаТаблицы.ЗначениеУсловияАвтоматическойСкидки = Неопределено) Тогда
				// Ошибок нет.
			Иначе
