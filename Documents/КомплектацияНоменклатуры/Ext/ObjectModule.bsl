...
Функция ПечатьКомплектацияНоменклатуры()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, ?(ВидКомплектации = Перечисления.ВидыКомплектации.Сборка, "Комплектация", "Разукомплектация")+" номенклатуры");
...
Процедура СформироватьДвиженияПоРегиструСписанныеТовары(
	...
	НоваяСтрока.ИзменитьСерию = НЕ ЗначениеЗаполнено(НоваяСтрока.СерияНоменклатурыНовая);
	НоваяСтрока.ИзменитьХарактеристику = НЕ ЗначениеЗаполнено(НоваяСтрока.ХарактеристикаНоменклатурыНовая);
...
Процедура ДвиженияПоСписаннымТоварам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок)
	...
	ИначеЕсли ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда
		...
		мКоэффициентов = ТаблицаПоТоварам.ВыгрузитьКолонку("ДоляСтоимости");
		мКоличествоКомплекта = ОбщегоНазначения.РаспределитьПропорционально(НеРаспределеноКоличествоКомплекта, мКоэффициентов, 3);
		...
		Для Каждого Строка Из ТаблицаПоТоварам Цикл
			...
			ИндексСтроки = ТаблицаПоТоварам.Индекс(Строка);
			Строка.Количество = мКоличествоКомплекта[ИндексСтроки];
...
Функция ПодготовитьТаблицуКомплектацииДляРасширеннойАналитики(СтруктураШапкиДокумента, ТаблицаПоКомплектующим,ТаблицаПоКомплектам)
	...
	Если СтруктураШапкиДокумента.ВидКомплектации = Перечисления.ВидыКомплектации.Сборка Тогда
		...
		РаспределениеКоличества = ОбщегоНазначения.РаспределитьПропорционально(КоличествоКомплектов, ТаблицаПоКомплектующим.ВыгрузитьКолонку("Количество"), 3); 
		...
		Для Каждого ТекущаяСтрока Из ТаблицаПоКомплектующим Цикл
			...
			НоваяСтрока.КорКоличество = РаспределениеКоличества[ТаблицаПоКомплектующим.Индекс(ТекущаяСтрока)];
	...
	Иначе
		...
		РаспределениеКоличества = ОбщегоНазначения.РаспределитьПропорционально(КоличествоКомплектов, ТаблицаПоКомплектующим.ВыгрузитьКолонку("ДоляСтоимости"), 3);
		...
		Для Каждого ТекущаяСтрока Из ТаблицаПоКомплектующим Цикл
			...
			НоваяСтрока.СчетУчетаБУ = СтруктураШапкиДокумента.СчетУчетаБУ;
			НоваяСтрока.СчетУчетаНУ = СтруктураШапкиДокумента.СчетУчетаНУ;
			НоваяСтрока.Количество = РаспределениеКоличества[ТаблицаПоКомплектующим.Индекс(ТекущаяСтрока)];
