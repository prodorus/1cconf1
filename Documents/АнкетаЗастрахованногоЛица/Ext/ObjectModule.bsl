...
Функция СформироватьВыходнойФайл(Отказ) Экспорт
	...
	ИначеЕсли ВыборкаПоШапкеДокумента.ФорматФайла = Перечисления.ФорматФайлаПФР.Версия07 Тогда 
		...
		Пока ВыборкаПоРаботникиОрганизации.Следующий() Цикл
			...
			Если ПустаяСтрока(НаборЗаписейФИО.Отчество) Тогда
				НаборЗаписейФИО.Удалить("Отчество");
				СтрокаПоля = НаборЗаписейАнкетныеДанные.ФИО.Поля.Найти("Отчество");
				Если СтрокаПоля <> Неопределено Тогда
					НаборЗаписейАнкетныеДанные.ФИО.Поля.Удалить(СтрокаПоля);
				КонецЕсли;
			КонецЕсли;
			...
			ОписаниеПолей = НаборЗаписейАнкетныеДанные.МестоРождения.Поля;
			...
			Если ПустаяСтрока(ВыборкаПоРаботникиОрганизации.Телефоны) Тогда
				НаборЗаписейАнкетныеДанные.Удалить("Телефон");
			Иначе
			...
			Если ПустаяСтрока(ВыборкаПоРаботникиОрганизации.Страна) Тогда
				НаборЗаписейАнкетныеДанные.Удалить("Гражданство");
			Иначе
			...
			ТипДокумента = ?(ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.ДокументВидКодПФР), СокрЛП(ВыборкаПоРаботникиОрганизации.ДокументВидКодПФР), "ПРОЧЕЕ");
			НаборЗаписейУдостоверяющийДокумент.ТипУдостоверяющего.Значение = ТипДокумента;
			НаборЗаписейДокумент.НаименованиеУдостоверяющего = ?(ТипДокумента = "ПРОЧЕЕ", ВРег(СокрЛП(ВыборкаПоРаботникиОрганизации.ДокументВидНаименование)), ТипДокумента);
