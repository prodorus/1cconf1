...
Процедура ЗаполнитьДанныеПриказа(ТекущаяСтрока = Неопределено)
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ДанныеРегистраСреднего.ФизЛицо.ПостоянноПроживалВКрыму18Марта2014Года
	|				И ГОД(ДанныеРегистраСреднего.ДатаНачалаСобытия) = 2015
	|			ТОГДА 289 - СУММА(ДанныеРегистраСреднего.ДнейБолезни)
	|		КОГДА ДанныеРегистраСреднего.ФизЛицо.ПостоянноПроживалВКрыму18Марта2014Года
	|				И ГОД(ДанныеРегистраСреднего.ДатаНачалаСобытия) = 2016
	|			ТОГДА 654 - СУММА(ДанныеРегистраСреднего.ДнейБолезни)
	|		КОГДА ДанныеРегистраСреднего.ДатаНачалаСобытия >= &ДатаОтдельногоРасчетаПособийПоМатеринству
	|			ТОГДА ВЫБОР
	...
	|	ДанныеРегистраСреднего.ФизЛицо.ПостоянноПроживалВКрыму18Марта2014Года
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДокументОтпуска.КоррСчетБанка,
	|	ДокументОтпуска.НомерЛицевогоСчета,
	|	ДокументОтпуска.НаличиеКартыМИР,
	|	ДокументОтпуска.НомерКартыМИР,
	|	ДокументОтпуска.АдресПочтовый,
	|	ДокументОтпуска.АдресРегистрации,
	|	ДокументОтпуска.Фамилия КАК Фамилия,
	|	ДокументОтпуска.Имя КАК Имя,
	|	ДокументОтпуска.Отчество,
	|	ВЫБОР
	|		КОГДА ДокументОтпуска.РасчетПособияИзТарифнойСтавки
	|			ТОГДА 2
	|		ИНАЧЕ ДокументОтпуска.РасчетПоПравилам2010Года
	...
	|	ВЫБОР
	|		КОГДА ДокументОтпуска.ДатаНачала >= &ДатаОтдельногоРасчетаПособийПоМатеринству
	|			ТОГДА ДанныеОСреднемЗаработке2011.КалендарныхДнейРасчетногоПериода
	|		КОГДА ДокументОтпуска.РасчетПоПравилам2010Года
	...
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК Статус,
	|	ДокументОтпуска.ДатаВыдачиДокументаНаПроживание,
	|	ДокументОтпуска.НомерДокументаНаПроживание,
	|	ДокументОтпуска.СерияДокументаНаПроживание,
	|	ДокументОтпуска.НаименованиеДокументаНаПроживание,
	|	ДокументОтпуска.ТарифнаяСтавкаОклад,
	|	ДокументОтпуска.АдресПочтовыйКодПоКЛАДР,
	|	ВЫБОР
	|		КОГДА НЕ ДокументОтпуска.ПрименятьЛьготыПриНачисленииПособия
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВыплатыПособийЗаСчетФедеральногоБюджета.ПустаяСсылка)
	|		КОГДА ДокументОтпуска.Физлицо.ЛьготаПриНачисленииПособий = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПриНачисленииБольничных.Маяк)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВыплатыПособийЗаСчетФедеральногоБюджета.Маяк)
	|		КОГДА ДокументОтпуска.Физлицо.ЛьготаПриНачисленииПособий = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПриНачисленииБольничных.ПодразделенияОсобогоРиска)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВыплатыПособийЗаСчетФедеральногоБюджета.ПодразделенияОсобогоРиска)
	|		КОГДА ДокументОтпуска.Физлицо.ЛьготаПриНачисленииПособий = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПриНачисленииБольничных.Чернобыль)
	...
	|	ДокументОтпуска.АдресРегистрацииКодПоКЛАДР,
	|	ДокументОтпуска.АдресРегистрацииКодПоФИАС,
	|	ДокументОтпуска.АдресПочтовыйКодПоФИАС
	|ИЗ
	|	Документ.ОтпускПоУходуЗаРебенком КАК ДокументОтпуска
	...
	|		ПО ДокументОтпуска.Ссылка = ДанныеОСреднемЗаработке2011.Регистратор
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(Выборка.АдресРегистрацииКодПоКЛАДР) И ЗначениеЗаполнено(Выборка.АдресРегистрации) Тогда
			ТекущаяСтрока.АдресРегистрацииКодПоКЛАДР = ОбменСведениямиОПособияхСФСС.КодАдресаПоКЛАДРдляФСС(Выборка.АдресРегистрации);
		КонецЕсли;
	...
	Иначе
		...
		Для каждого СтрокаТЧ Из РаботникиОрганизации Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаТЧ.АдресРегистрации) И ЗначениеЗаполнено(СтрокаТЧ.АдресРегистрации) Тогда
...
Функция ЗапросДляФайла()
	...
	|	ДанныеОДетях.НомерСтроки,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ДругиеДети.Фамилия ЕСТЬ НЕ NULL 
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ) КАК ОдновременныйУходЗаНесколькимиДетьми
	|ПОМЕСТИТЬ ВТМногодетныеПособия
	|ИЗ
	|	ВТДанныеОДетях КАК ДанныеОДетях
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеОДетях КАК ДругиеДети
	|		ПО ДанныеОДетях.Сотрудник = ДругиеДети.Сотрудник
	|			И ДанныеОДетях.ДатаНачала = ДругиеДети.ДатаНачала
	|			И ДанныеОДетях.НомерСтроки <> ДругиеДети.НомерСтроки
	|			И (ДругиеДети.НаРебенкаВыплачиваетсяПособие)
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеОДетях.НомерСтроки
	|;
	...
	|		КОГДА (Пособия.ПорядокРасчетаСреднегоЗаработка = 2
	|				ИЛИ Пособия.ПорядокРасчетаСреднегоЗаработка = 3)
	|				И Пособия.ДатаНачала <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА КОНЕЦПЕРИОДА(Пособия.ДатаНачала, МЕСЯЦ)
	...
	|				ИЛИ Пособия.ПорядокРасчетаСреднегоЗаработка = 3)
	|				И Пособия.ДатаНачала <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ДЕНЬ(КОНЕЦПЕРИОДА(Пособия.ДатаНачала, МЕСЯЦ))
	|		ИНАЧЕ Пособия.КалендарныхДнейРасчетногоПериода
	|	КОНЕЦ КАК КалендарныхДнейРасчетногоПериода,
	...
	|	Пособия.ПроживаниеРаботаВЛьготнойЗоне,
	...
	|	Дети.ДатаРожденияРебенка,
	|	Дети.Очередность,
	|	Дети.ВидПодтверждающегоДокумента КАК ДокументОРожденииВид,
	|	ВЫБОР
	...
	|	Пособия.Перерасчет,
	|	МногодетныеПособия.ОдновременныйУходЗаНесколькимиДетьми КАК ОдновременныйУходЗаНесколькимиДетьми,
	|	Дети.НаличиеРешенияСудаОЛишенииПрав,
	|	Пособия.АдресРегистрацииКодПоКЛАДР,
	|	Пособия.ДатаОкончанияПособия,
	|	Пособия.АдресРегистрацииКодПоФИАС,
	|	Пособия.АдресПочтовыйКодПоФИАС
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТМногодетныеПособия КАК МногодетныеПособия
	|		ПО Дети.НомерСтроки = МногодетныеПособия.НомерСтроки
	|ГДЕ
	|	Дети.НаРебенкаВыплачиваетсяПособие
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ПечатьРеестрСведенийВФСС(ИмяМакета = "РеестрСведенийВФСС2011")
	...
	Пока ВыборкаПоТЧРаботникиОрганизации.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(РасчетныеПериоды.Добавить(), ВыборкаПоТЧРаботникиОрганизации);
	КонецЦикла;
	...
	Если ВыводитьРасчетныйПериодВШапке Тогда
		ОбластьШапка.Параметры.ПериодРасчетаС = "за " + Формат(РасчетныеПериоды[0].ПервыйГодРасчета,"ЧЦ=4; ЧГ=0") + " год";
		ОбластьШапка.Параметры.ПериодРасчетаПо = "за " + Формат(РасчетныеПериоды[0].ВторойГодРасчета,"ЧЦ=4; ЧГ=0") + " год";
	КонецЕсли;
	...
	Пока ВыборкаПоТЧРаботникиОрганизации.Следующий() Цикл
		...
		Если ИмяМакета = "РеестрСведенийВФСС2011" И ЗначениеЗаполнено(ВыборкаПоТЧРаботникиОрганизации.Статус) Тогда
		...
		КонецЕсли;
		Если ИмяМакета = "РеестрСведенийВФСС2018" Тогда
			ОбластьСтрока.Параметры.НомерКартыМИР = "";
			Если ВыборкаПоТЧРаботникиОрганизации.ПеречислятьПособиеПочтовымПереводом Тогда
				ОбластьСтрока.Параметры.СпособВыплаты = "почтовым переводом";
			Иначе
				ОбластьСтрока.Параметры.СпособВыплаты = "на банковский счет";
				Если ВыборкаПоТЧРаботникиОрганизации.НаличиеКартыМИР Тогда
					ОбластьСтрока.Параметры.НомерКартыМИР = ВыборкаПоТЧРаботникиОрганизации.НомерКартыМИР;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если ВыборкаПоТЧРаботникиОрганизации.ПрименятьЛьготыПриНачисленииПособия Тогда
			...
			ИначеЕсли ВыборкаПоТЧРаботникиОрганизации.СтатусЛьготнойЗоны = Перечисления.ВидыЗонРадиационногоПоражения.ЗонаОтселения Тогда
				ОписаниеЛьгот = "3";
			ИначеЕсли ВыборкаПоТЧРаботникиОрганизации.СтатусЛьготнойЗоны = Перечисления.ВидыЗонРадиационногоПоражения.ЗонаЗагрязненияПриАварииПОМаяк Тогда
				ОписаниеЛьгот = "4";
			КонецЕсли;
		...
		Если ИмяМакета = "РеестрСведенийВФСС2018" Тогда
			...
			ОбластьСтрока.Параметры.ЗаработокПервыйГод = Формат(ВыборкаПоТЧРаботникиОрганизации.ЗаработокПервыйГодРасчета ,"ЧЦ=15; ЧДЦ=2; ЧГ=0") + ?(Не ВыводитьРасчетныйПериодВШапке, "(за " + Формат(ВыборкаПоТЧРаботникиОрганизации.ПервыйГодРасчета,"ЧЦ=4; ЧГ=0") + " год)", "");
			ОбластьСтрока.Параметры.ЗаработокВторойГод = Формат(ВыборкаПоТЧРаботникиОрганизации.ЗаработокВторойГодРасчета ,"ЧЦ=15; ЧДЦ=2; ЧГ=0") + ?(Не ВыводитьРасчетныйПериодВШапке,  "(за " + Формат(ВыборкаПоТЧРаботникиОрганизации.ВторойГодРасчета,"ЧЦ=4; ЧГ=0") + " год)", "");
		ИначеЕсли ВыборкаПоТЧРаботникиОрганизации.ПорядокРасчетаСреднегоЗаработка = 0 Тогда
			ОбластьСтрока.Параметры.ПериодРасчетаС = Формат(ВыборкаПоТЧРаботникиОрганизации.ПервыйГодРасчета,"ЧЦ=4; ЧГ=0") + ?(ЗначениеЗаполнено(ВыборкаПоТЧРаботникиОрганизации.ЗамененныйПервыйГодРасчета) И ВыборкаПоТЧРаботникиОрганизации.ПервыйГодРасчета <> ВыборкаПоТЧРаботникиОрганизации.ЗамененныйПервыйГодРасчета, " (" + Формат(ВыборкаПоТЧРаботникиОрганизации.ДатаЗаявленияОЗаменеРасчетногоГода,"ДФ=dd.MM.yyyy") + ", № " + ВыборкаПоТЧРаботникиОрганизации.НомерЗаявленияОЗаменеРасчетногоГода + ")","");
			ОбластьСтрока.Параметры.ПериодРасчетаПо = Формат(ВыборкаПоТЧРаботникиОрганизации.ВторойГодРасчета,"ЧЦ=4; ЧГ=0") + ?(ЗначениеЗаполнено(ВыборкаПоТЧРаботникиОрганизации.ЗамененныйВторойГодРасчета) И ВыборкаПоТЧРаботникиОрганизации.ВторойГодРасчета <> ВыборкаПоТЧРаботникиОрганизации.ЗамененныйВторойГодРасчета, " (" + Формат(ВыборкаПоТЧРаботникиОрганизации.ДатаЗаявленияОЗаменеРасчетногоГода,"ДФ=dd.MM.yyyy") + ", № " + ВыборкаПоТЧРаботникиОрганизации.НомерЗаявленияОЗаменеРасчетногоГода + ")","");
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли Лев(ИмяМакета, 14) = "РеестрСведений" Тогда
		...
		УточненноеИмяМакета = "РеестрСведенийВФСС" + ?(Дата < ОбменСведениямиОПособияхСФСС.ДатаВступленияВСилуФорм2012года(), "2011", ?(Дата < ОбменСведениямиОПособияхСФСС.ДатаВступленияВСилуФормРеестров2018года(), "2012", "2018"));
		ТабДокумент = ПечатьРеестрСведенийВФСС(УточненноеИмяМакета);
...
Процедура СформироватьТекстовыеПоля(ТекущаяСтрока) Экспорт
	...
	Если ТекущаяСтрока.РайонныйКоэффициент > 1 И Не ЗначениеЗаполнено(ТекущаяСтрока.Район) Тогда
		ИнаяПолезнаяИнформация = ИнаяПолезнаяИнформация + ?(ПустаяСтрока(ИнаяПолезнаяИнформация), "", ". ") + "Районный коэффициент " + Формат(ТекущаяСтрока.РайонныйКоэффициент, "ЧДЦ=2");
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("ОписьЗаявленийИДокументовВФСС, РеестрСведений", "Опись заявлений и документов", "Реестр сведений");
...
Функция СформироватьВыходнойФайл(Отказ, ПроверятьДанные = Истина, ВыборкаПоШапкеДокумента = Неопределено, ВыборкаПоРаботникиОрганизации = Неопределено) Экспорт
	...
	Возврат	ОбменСведениямиОПособияхСФСС.СформироватьФайлРеестра(ЭтотОбъект, Отказ, Ложь, ВыборкаПоШапкеДокумента, ЗапросДляФайла().Выбрать())
...
Функция СформироватьЗапросПоТЧРаботникиОрганизации()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях.Сотрудник,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях.НомерСтроки) КАК Количество
	|ПОМЕСТИТЬ ВТКоличествоДетей
	|ИЗ
	...
	|ГДЕ
	|	РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях.Ссылка = &ДокументСсылка
	|
	|СГРУППИРОВАТЬ ПО
	|	РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях.Сотрудник
	|;
	...
	|	РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях.Сотрудник,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях.НомерСтроки) КАК Количество
	|ПОМЕСТИТЬ ВТКоличествоДетейСПособиями
	|ИЗ
	|	Документ.РеестрСведенийВФССОЕжемесячныхПособияхПоУходу.ДанныеОДетях КАК РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях
	|ГДЕ
	|	РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях.Ссылка = &ДокументСсылка
	|	И РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях.НаРебенкаВыплачиваетсяПособие
	|
	|СГРУППИРОВАТЬ ПО
	|	РеестрСведенийВФССОЕжемесячныхПособияхПоУходуДанныеОДетях.Сотрудник
	|;
	...
	|	Пособия.КодПричиныПерерасчета,
	|	Пособия.Перерасчет,
	|	КоличествоДетей.Количество КАК КоличествоДетейВсего,
	|	КоличествоДетейСПособиями.Количество КАК КоличествоДетей,
	|	Пособия.АдресРегистрацииКодПоКЛАДР,
	|	Пособия.ПроживаниеРаботаВЛьготнойЗоне,
	|	Пособия.ДатаОкончанияПособия,
	|	Пособия.АдресРегистрацииКодПоФИАС,
	...
	|				ИНАЧЕ Пособия.ВторойГодРасчета
	|			КОНЕЦ = ПредельнаяВеличинаЗаВторойГодРасчета.Год)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКоличествоДетей КАК КоличествоДетей
	|		ПО Пособия.Сотрудник = КоличествоДетей.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКоличествоДетейСПособиями КАК КоличествоДетейСПособиями
	|		ПО Пособия.Сотрудник = КоличествоДетейСПособиями.Сотрудник
	|ГДЕ
...
Процедура ПроверитьЗаполнениеСтрокиТЧРаботникиОрганизации(ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаНачала) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указана дата начала отпуска по уходу за ребенком!", Отказ, Заголовок);
	ИначеЕсли ВыборкаПоСтрокамДокумента.ДатаНачала >= ПроведениеРасчетов.ДатаОтдельногоРасчетаПособийПоМатеринству() И ВыборкаПоСтрокамДокумента.ПорядокРасчетаСреднегоЗаработка <> 0 Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Для отпуска по уходу за ребенком, начинающегося 1 января 2013 года или позже указан недопустимый порядок расчета cреднего заработка: " + ?(ВыборкаПоСтрокамДокумента.ПорядокРасчетаСреднегоЗаработка = 1, "по правилам 2010 года", "из оклада или МРОТ") + "!", Отказ, Заголовок);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОкончания) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указана дата окончания отпуска по уходу за ребенком!", Отказ, Заголовок);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаПриказаНаОтпускПоУходуЗаРебенком) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указана дата приказа о предоставлении отпуска по уходу за ребенком!", Отказ, Заголовок);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПриказаНаОтпускПоУходуЗаРебенком) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указан номер приказа о предоставлении отпуска по уходу за ребенком!", Отказ, Заголовок);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаПриказаОЗаменеПослеродовогоОтпуска) Или ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПриказаОЗаменеПослеродовогоОтпуска) Тогда
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаПриказаОЗаменеПослеродовогоОтпуска) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПриказаОЗаменеПослеродовогоОтпуска) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указан номер приказа о замене послеродового отпуска на отпуск по уходу!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаПриказаОЗаменеПослеродовогоОтпуска) И ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПриказаОЗаменеПослеродовогоОтпуска) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указана дата приказа о замене послеродового отпуска на отпуск по уходу!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
		
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиОНеполученииПособияОтОтца) Или ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиОНеполученииПособияОтОтца) Тогда
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиОНеполученииПособияОтОтца) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиОНеполученииПособияОтОтца) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указан номер Справки от отца о неполучении пособия!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиОНеполученииПособияОтОтца) И ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиОНеполученииПособияОтОтца) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указана дата Справки от отца о неполучении пособия!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиОНеполученииПособияОтМатери) Или ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиОНеполученииПособияОтМатери) Тогда
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиОНеполученииПособияОтМатери) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиОНеполученииПособияОтМатери) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указан номер Справки от матери о неполучении пособия!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиОНеполученииПособияОтМатери) И ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиОНеполученииПособияОтМатери) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указана дата Справки от матери о неполучении пособия!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
	
	Если ВыборкаПоСтрокамДокумента.Совместительство Тогда
		
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиПоДругимМестамРаботы) Или ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиПоДругимМестамРаботы) Тогда
			Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиПоДругимМестамРаботы) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиПоДругимМестамРаботы) Тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указан номер Справки от матери о неполучении пособия!", Отказ, Заголовок);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаСправкиПоДругимМестамРаботы) И ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерСправкиПоДругимМестамРаботы) Тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не указана дата Справки от матери о неполучении пособия!", Отказ, Заголовок);
			КонецЕсли;
		Иначе 
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Для совместителей необходимо указать реквизиты Справки о неполучении пособия по основному месту работы!", Отказ, Заголовок);
		КонецЕсли;
		
	КонецЕсли;
	
	Если ВыборкаПоСтрокамДокумента.ПрименятьЛьготыПриНачисленииПособия И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ВыплатаЗаСчетФедеральногоБюджета) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Для лица, имеющего право на льготы для зон с льготным статусом, не указана причина радиационного воздействия!", Отказ, Заголовок);
	Иначе
		Если ВыборкаПоСтрокамДокумента.СтатусЛьготнойЗоны = Перечисления.ВидыЗонРадиационногоПоражения.ЗонаЗагрязненияПриАварииПОМаяк Тогда
			Если Не ВыборкаПоСтрокамДокумента.ПроживаниеРаботаВЛьготнойЗоне Тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Для предоставления льготы лицу, проживающему в населенном пункте, загрязненном вследствие аварии на ПО ""Маяк"", следует указать вариант ""Постоянное проживание""!", Отказ, Заголовок);
			КонецЕсли;
			Если ВыборкаПоСтрокамДокумента.ВыплатаЗаСчетФедеральногоБюджета <> Перечисления.ВыплатыПособийЗаСчетФедеральногоБюджета.Маяк Тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Для предоставления льготы лицу, проживающему в населенном пункте, загрязненном вследствие аварии на ПО ""Маяк"", следует указать причину радиационного воздействия ""ПО Маяк""!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КоличествоДетейВсего) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Не заполнены сведения о детях!", Отказ, Заголовок);
	ИначеЕсли Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КоличествоДетей) Тогда	
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "В сведениях о детях не отмечены те, на которых будет выплачиваться пособие!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеСтрокиПоДетям(СтрокаВыборкиПоПоДетям, Отказ, Заголовок)
	...
	Если Не ЗначениеЗаполнено(СтрокаВыборкиПоПоДетям.Ребенок) И Не ЗначениеЗаполнено(СтрокаВыборкиПоПоДетям.Очередность) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(НачалоТекстаОшибки + "присутствует пустая строка - без ФИО и очередности ребенка!", Отказ, Заголовок);
	Иначе 	
		...
		Если Не ЗначениеЗаполнено(СтрокаВыборкиПоПоДетям.Очередность) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(НачалоТекстаОшибки + ДопОписаниеСтроки + " не указана очередность!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаВыборкиПоПоДетям.Ребенок) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(НачалоТекстаОшибки + ДопОписаниеСтроки + " не указано ФИО ребенка!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаВыборкиПоПоДетям.ДатаРожденияРебенка) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(НачалоТекстаОшибки + ДопОписаниеСтроки + " не указана дата рождения!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаВыборкиПоПоДетям.ВидПодтверждающегоДокумента) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(НачалоТекстаОшибки + ДопОписаниеСтроки + " не указан вид документа!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаВыборкиПоПоДетям.НаименованиеПодтверждающегоДокумента) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(НачалоТекстаОшибки + ДопОписаниеСтроки + " не указано наименование документа!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаВыборкиПоПоДетям.ДатаДокумента) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(НачалоТекстаОшибки + ДопОписаниеСтроки + " не указана дата документа!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаВыборкиПоПоДетям.НомерДокумента) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(НачалоТекстаОшибки + ДопОписаниеСтроки + " не указан номер документа!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;

КонецПроцедуры // ПроверитьЗаполнениеСтрокиПоДетям()
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
    ...
    ОбменСведениямиОПособияхСФСС.ДозаполнитьДанныеСотрудниковВРеестрах(ЭтотОбъект);
