...
Функция ПодготовитьДанныеДляПечатиУниверсальногоПередаточногоДокумента()
	...
	|	ПередачаТоваров.ДоговорКонтрагента.ВедениеВзаиморасчетов КАК ВедениеВзаиморасчетов,
	...
	|		ПО ПередачаТоваров.Склад = ОтветственныеЛица.СтруктурнаяЕдиница
	|ГДЕ
	...
	|		ПередачаТоваров.Коэффициент КАК Коэффициент,
	|		ПередачаТоваров.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|		ПередачаТоваров.ЕдиницаИзмеренияМест КАК ЕдиницаИзмеренияМест,
	...
	|		ПередачаТоваров.ЕдиницаИзмеренияМест.Коэффициент
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		1,
	|		МИНИМУМ(ПередачаТоваров.НомерСтроки),
	|		ПередачаТоваров.Номенклатура,
	|		ПередачаТоваров.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент,
	|		ПередачаТоваров.Номенклатура.ЕдиницаХраненияОстатков,
	|		ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка),
	|		0,
	|		НЕОПРЕДЕЛЕНО,
	|		НЕОПРЕДЕЛЕНО,
	|		ПередачаТоваров.Цена * &Курс / &Кратность,
	|		СУММА(ПередачаТоваров.Количество),
	|		0,
	|		СУММА(ПередачаТоваров.Сумма * &Курс / &Кратность)
	|	ИЗ
	...
	|		ПередачаТоваров.Цена,
	|		ПередачаТоваров.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент,
	|		ПередачаТоваров.Номенклатура.ЕдиницаХраненияОстатков) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВложенныйЗапрос.НомерТабличнойЧасти,
	|	НомерСтроки";
	...
	ДанныеДляПечати.Вставить("Дата",             Шапка.ДатаДокумента);
	...
	ДанныеДляПечати.Вставить("Подразделение", 	 Неопределено);
	
	// необходимые данные для печати с помощью метода УчетНДС.ВывестиСчетФактуруВТабличныйДокумент()
	ДанныеДляПечати.Вставить("ДатаСчетаФактуры",	    Шапка.ДатаДокумента);
	ДанныеДляПечати.Вставить("Ссылка",   		  		Документы.СчетФактураВыданный.ПустаяСсылка());
	ДанныеДляПечати.Вставить("ВидСчетаФактуры",    		Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию);
	ДанныеДляПечати.Вставить("Исправление", 	   		Ложь);
	ДанныеДляПечати.Вставить("НомерИсправления",   		0);
	ДанныеДляПечати.Вставить("СчетФактураБезНДС",  		Ложь);
	ДанныеДляПечати.Вставить("Контрагент", 		   		Шапка.Контрагент);
	ДанныеДляПечати.Вставить("ДоговорКонтрагента", 		Шапка.ДоговорКонтрагента);
	ДанныеДляПечати.Вставить("ДокументОснование",  		Шапка.Ссылка);
	ДанныеДляПечати.Вставить("КППКонтрагента", 	   		"");
	ДанныеДляПечати.Вставить("ВидАгентскогоДоговора",   
		ОбщегоНазначения.ПолучитьЗначениеРеквизита(Шапка.ДоговорКонтрагента, "ВидАгентскогоДоговора"));

 	Руководители = РегламентированнаяОтчетность.ОтветственныеЛицаОрганизаций(Шапка.Организация, ДанныеДляПечати.Дата);
	...
	ТаблицаТовары = УчетНДС.СоздатьТаблицуДляЗаполнениеТабличнойЧастиДанныхПечати();
	
	Пока ВыборкаСтрокТовары.Следующий() = 1 Цикл

		Строчка = ТаблицаТовары.Добавить();
		...
		Строчка.ЕдиницаИзмеренияКод    = ВыборкаСтрокТовары.БазоваяЕдиницаКодПоОКЕИ;
		...
		Цена		= СуммаБезНДС / ?(Количество = 0, 1, Количество);
  	 	...
  	 	Строчка.СтавкаНДС	= Неопределено;
		Строчка.Количество  = Количество;
		Строчка.Цена        = Окр(Цена, 2);
		Строчка.СуммаНДС    = Окр(СуммаНДС, 2);
		Строчка.Сумма       = Окр(СуммаСНДС, 2);
		
	КонецЦикла;
	
	ТаблицаТовары.ЗаполнитьЗначения(Истина, "СуммаВключаетНДС");
	ДанныеДляПечати.Вставить("ТабличнаяЧасть", ТаблицаТовары);
	
	// Дата совершения факта хозяйственной жизни. Не может быть ранее даты составления УПД.
	ДанныеДляПечати.Вставить("ДатаОтгрузкиПередачи", 	Шапка.ДатаДокумента);
	ДанныеДляПечати.Вставить("СтатусУПД", 				2);
	ДанныеДляПечати.Вставить("Основание", 				Строка(Шапка.ДоговорКонтрагента));
