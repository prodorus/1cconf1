...
Процедура ДобавитьРаспределение( ТаблицаПоТоварам, ТаблицаПоОборудованию, СтруктураШапкиДокумента)
	...
	Если Не МассивСумм = Неопределено Тогда
		...
		Для К = 0 По СтрокТоваров Цикл
			...
			ТаблицаПоТоварам[К].НДСУпрРаспределения   = ТаблицаПоТоварам[К].НДСУпрРаспределения   + МассивНДСУпр[К];
		КонецЦикла;
		Для К = 0 По ТаблицаПоОборудованию.Количество() - 1 Цикл
			...
			ТаблицаПоОборудованию[К].НДСУпрРаспределения    = ТаблицаПоОборудованию[К].НДСУпрРаспределения   + МассивНДСУпр[К + СтрокТоваров + 1];
		КонецЦикла;
	КонецЕсли;
...
Процедура ПодготовитьТаблицуТоваровРегл(ТаблицаТоваров, СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		Иначе
			...
			РасчетСуммыНДСПоСтавке = УчетНДС.РасчетНДСвРубляхПоСтавкеДокумента(Дата);
			
			Если РасчетСуммыНДСПоСтавке Тогда
				СуммаКРаспределению = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту((СуммаПоСтроке + НДС), СтруктураШапкиДокумента.ВалютаДокумента,
				...
				СтрокаТаблицы.ПроводкаСуммаНДС = ?(ЗначениеСтавкиНДС = 0, 0, Окр(СуммаКРаспределению * ЗначениеСтавкиНДС /(100 + ЗначениеСтавкиНДС),2));
				СтрокаТаблицы.ПроводкаСумма    = СуммаКРаспределению - СтрокаТаблицы.ПроводкаСуммаНДС;
				СтрокаТаблицы.СуммаРегл        = СуммаКРаспределению;
			Иначе
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Иначе
		СтруктураОбязательныхПолей = Новый Структура("ВидОперации, Организация, СпособРаспределения, 
	                                 ...
	                                 |КурсВзаиморасчетов,КратностьВзаиморасчетов, СтавкаНДС");
	КонецЕсли;
	...
	Если НЕ УчитыватьНДС Тогда
		СтруктураОбязательныхПолей.Удалить("СтавкаНДС");
	КонецЕсли;
...
Процедура ДвиженияПоРегистрамУпр(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоОборудованию, ТаблицаПоВзаиморасчетам, ТаблицаПоРасчетам, Отказ, Заголовок, РежимПроведения)
	...
	Если ВидОперации <> Перечисления.ВидыОперацийПоступлениеДопРасходов.ВнутреннийРасход Тогда
		...
		// Стоимость по строке равна: доля суммы доп. расходов из шапки документа + сумма доп. расходов, указанная непосредственно в строке
		// НДС считаем аналогичным образом
		Для Каждого СтрокаЗакупки Из ТаблицаПоЗакупкам Цикл
			СтрокаЗакупки.Стоимость = СтрокаЗакупки.СуммаУпрРаспределения + СтрокаЗакупки.СуммаУпр;
			СтрокаЗакупки.НДС 		= СтрокаЗакупки.НДС + СтрокаЗакупки.НДСУпр;
		КонецЦикла;
		...
		Для Каждого СтрокаОборудование Из ТаблицаПоОборудованию Цикл
			...
			СтрокаЗакупки.НДС = СтрокаОборудование.НДСУпр + СтрокаОборудование.НДСУпрРаспределения;
		...
		Если СтруктураШапкиДокумента.ВедениеУчетаПоПроектам Тогда
			...
			УправлениеПроектами.ОтразитьДвиженияПоПроектам(ТаблицаПоЗакупкам, ТаблицаДвижений, Проект, Дата, "Закупки");
...
Процедура ОбработкаЗаполнения(Основание)
	...
	УправлениеПроектами.ЗаполнитьПроект(ЭтотОбъект, Основание);
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоОборудованию, Отказ = Ложь, Заголовок = "") Экспорт
	...
	Если ПараметрыПартионногоУчета.ВестиПартионныйУчетПоСкладам 
	 ИЛИ ПараметрыПартионногоУчета.ВестиПартионныйУчетПоСкладамРегл Тогда
