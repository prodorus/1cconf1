...
Функция ЗаполнитьСМРхозспособомПоУказаномуПериоду(НачалоПериода, КонецПериода)
	...
	|				&КонецПериода,
	|				Период,
	|				Счет В (&СчетУчетаСтроительства),
	|				&ВидыСубконто,
	|				Организация = &Организация
	|				И Субконто2 = &ВидСтроительстваХозспособом
	|				И КорСубконто2 = &ВидСтроительстваХозспособом,
	|				КорСчет В (&СчетУчетаСтроительства),
	...
	|		И НачислениеНДСпоСМРхозспособом.Ссылка <> &ТекущийДокумент) КАК СтроительствоХозспособом
	|
	|СГРУППИРОВАТЬ ПО
	...
	// Ставка НДС для начисления по хозспособу основная
	СтавкаНДС = УчетНДС.ОбщаяСтавкаНДС(Дата);
	Запрос.УстановитьПараметр("СтавкаНДС"                       , СтавкаНДС);
	Запрос.УстановитьПараметр("СтавкаНДС_Значение"              , УчетНДС.ПолучитьСтавкуНДС(СтавкаНДС));
