...
Функция ОпределитьСчетФактураТребуется() Экспорт
	...
	ИначеЕсли Дата < '20110101' Тогда
		Возврат Истина;
   	ИначеЕсли (Не (УчетнаяПолитика.НачислятьНДСПриПередачеНедвижимости ИЛИ УчетНДС.ВедетсяУчетНДСПоФЗ81(Дата)) И ПравоСобственностиПереходитПослеГосРегистрации) Тогда
		Возврат Ложь;
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, 
	...
	ЕстьСтавкаНДС = НЕ(ТаблицаПоТоварам.Колонки.Найти("СтавкаНДС") = Неопределено);
	...
	ВыборкаПоТоварамБУ.Свернуть("СчетДоходовБУ, СчетРасходовБУ,СубконтоБУ, СтавкаНДС", "Сумма,СуммаНДС");
	...
	Для каждого СтрокаТЧ Из ВыборкаПоТоварамБУ Цикл
		...
		Если СтруктураШапкиДокумента.УчитыватьНДС И СтрокаТЧ.СуммаНДС > 0 Тогда
			...
			Если РасчетСуммыНДСПоСтавке Тогда
				ЗначениеСтавкиНДС = УчетНДС.ПолучитьСтавкуНДС(СтрокаТЧ.СтавкаНДС);
				
				Если СтрокаТЧ.Сумма > 0 И ЗначениеСтавкиНДС > 0 Тогда
					Проводка.Сумма = Окр(СтрокаТЧ.Сумма * ЗначениеСтавкиНДС /(100 + ЗначениеСтавкиНДС),2);
				КонецЕсли;
			КонецЕсли;
		...
		Для Каждого СтрокаТЧ Из ТаблицаПоТоварам Цикл
			...
			Если НЕ (СуммаПроводки = 0) Тогда
				...
				Если НЕ СтруктураШапкиДокумента.ПравоСобственностиПереходитПослеГосРегистрации Или УсловияНедвижимостиС2013Года Тогда
					...
					Если Проводка.СчетДт.ВидыСубконто.Количество() > 0
						И Проводка.СчетДт.ВидыСубконто[0].ВидСубконто.ТипЗначения.Типы()[0] = ТипЗнч(СтрокаТЧ.СубконтоНУ) Тогда
...
Процедура ДвиженияРегистровПодсистемыНДС(СтруктураШапкиДокумента, ТаблицаВыручки, Отказ) Экспорт
	...
	Если Дата < '20140701' И Не СтруктураШапкиДокумента.НачислятьНДСНедвижимости Тогда
		ПодготовитьПараметрыУчетнойПолитики(Отказ, СтруктураШапкиДокумента);
	КонецЕсли;
	
	НачислятьНДССНедвижимости = ?(Дата >= '20140701', Истина, СтруктураШапкиДокумента.НачислятьНДСНедвижимости);
	
	Если СтруктураШапкиДокумента.ПравоСобственностиПереходитПослеГосРегистрации 
		И Не НачислятьНДССНедвижимости Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПодготовитьПараметрыУчетнойПолитики(Отказ, СтруктураШапкиДокумента)
	...
	Иначе
		СтруктураШапкиДокумента.Вставить("НачислятьНДСНедвижимости", УчетнаяПолитикаБух.НачислятьНДСПриПередачеНедвижимости
			ИЛИ УчетНДС.ВедетсяУчетНДСПоФЗ81(Дата));
	КонецЕсли; 
