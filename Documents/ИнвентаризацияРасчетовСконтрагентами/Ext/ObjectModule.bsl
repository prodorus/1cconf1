...
Функция ПечатьИнв17() Экспорт
	...
	Если НЕ (ПредседательКомиссии = Неопределено) Тогда
		...
		ВыбраннаяОрганизация = ?(ЗначениеЗаполнено(Выборка.Организация.ГоловнаяОрганизация), Выборка.Организация.ГоловнаяОрганизация, Выборка.Организация);
		ДанныеПредседателя = ОбщегоНазначения.ДанныеФизЛица(ВыбраннаяОрганизация, ПредседательКомиссии.ФизЛицо, Выборка.Дата);
	...
	ВыбраннаяОрганизация = ?(ЗначениеЗаполнено(Выборка.Организация.ГоловнаяОрганизация), Выборка.Организация.ГоловнаяОрганизация, Выборка.Организация);
	ДанныеЧленовКомиссии = ОбщегоНазначения.ДанныеФизЛиц(ВыбраннаяОрганизация, СписокЧленовКомиссии, Выборка.Дата);
