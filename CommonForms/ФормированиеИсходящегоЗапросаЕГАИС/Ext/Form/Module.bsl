...
Процедура Запросить(Команда)
	...
	Если ИсходящиеЗапросы.Количество() = 0 Тогда
		ПодготовитьЗапросКПередаче(
			ОписаниеОповещенияЕГАИС("ПослеПодготовкиЗапросаКПередаче", ЭтаФорма));
	ИначеЕсли МассивЗапросов.Количество() > 0 Тогда
		...
		ИнтеграцияЕГАИССлужебныйКлиент.ОбработатьОчередьПередачиДанных(
			ОписаниеОповещенияЕГАИС("ПослеПодготовкиМассиваЗапросовКПередаче", ЭтаФорма),
...
Процедура ПослеПодготовкиМассиваЗапросовКПередаче(Изменения, ДополнительныеПараметры) Экспорт
	...
	СформироватьФорматированнуюСтроку(ТекстОжидание, "ГруппаТекстОжидание", "ТекстОбработкаНавигационнойСсылки");
...
Процедура ПослеПодготовкиЗапросаКПередаче(Изменения, ДополнительныеПараметры) Экспорт
	...
	Строки = Новый Массив;
	Строки.Добавить(ФорматированнаяСтрока(НСтр("ru = 'Запрос'"),, ЦветГиперссылки,, "ОткрытьИсходящееСообщение"));
	Строки.Добавить(" ");
	...
	ТекстОжидание = Новый ФиксированныйМассив(Строки);
	СформироватьФорматированнуюСтроку(ТекстОжидание, "ГруппаТекстОжидание", "ТекстОбработкаНавигационнойСсылки");
...
Процедура ВыполнитьОбмен()
	...
	Если ИсходящиеЗапросы.Количество() > 0 Тогда
		...
		ОповещениеПриЗавершении = ОписаниеОповещенияЕГАИС("ПриЗавершенииОбменаПакетаЗапросов", ЭтаФорма);
	Иначе
		МассивОрганизацийЕГАИС.Добавить(ОрганизацияЕГАИС);
		ОповещениеПриЗавершении = ОписаниеОповещенияЕГАИС("ПриЗавершенииОбмена", ЭтаФорма);
	КонецЕсли;
...
Процедура ПриЗавершенииОбменаПакетаЗапросов(Изменения, ДополнительныеПараметры) Экспорт
	...
	Если ИсходящиеЗапросы.НайтиСтроки(ПараметрыОтбора).Количество() = 0 Тогда
		...
		Если ЕстьОшибки Тогда
			...
			СформироватьФорматированнуюСтроку(ТекстОшибка, "ГруппаТекстОшибка", "ТекстОбработкаНавигационнойСсылки");
		...
		Иначе
			...
			СформироватьФорматированнуюСтроку(ТекстРезультат, "ГруппаТекстРезультат", "ТекстОбработкаНавигационнойСсылки");
...
&НаКлиенте
Процедура ТекстОбработкаНавигационнойСсылки(Элемент, СтандартнаяОбработка, СтандартнаяОбработкаHTML = Неопределено)
	
	ВВидеHTML = ТипЗнч(СтандартнаяОбработка) = Тип("ФиксированнаяСтруктура");
	
	Если ВВидеHTML Тогда
	
		СтандартнаяОбработкаHTML = Ложь;
		
		Если Не (СтандартнаяОбработка.Свойство("Element")
			И Найти(СтандартнаяОбработка.Element.id, "ahref")) Тогда
			
			Возврат;
		КонецЕсли;
		
		Метод = СтандартнаяОбработка.Element.nameProp;
	Иначе
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
	
	Если Элемент.Родитель = Элементы.ГруппаТекстОжидание Тогда
		КоллекцияСтрок = ТекстОжидание;
	ИначеЕсли Элемент.Родитель = Элементы.ГруппаТекстРезультат Тогда
		КоллекцияСтрок = ТекстРезультат;
	ИначеЕсли Элемент.Родитель = Элементы.ГруппаТекстОшибка Тогда
		КоллекцияСтрок = ТекстОшибка;
	Иначе
		Возврат;
	КонецЕсли;
	
	Если ВВидеHTML Тогда
		НавигационнаяСсылкаФорматированнойСтроки = Метод;
	Иначе
		НавигационнаяСсылкаФорматированнойСтроки = ИнтеграцияЕГАИСУТКлиентСервер.НавигационнаяСсылкаСтроки(Элемент, КоллекцияСтрок);
	КонецЕсли;
	...
	Если СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "ОткрытьВходящееСообщение") Тогда
		...
		Если ПустаяСтрока(ИндексТаблицыЗапросов) Тогда
			Если ВходящееСообщение.Пустая() Тогда
				ПоказатьПредупреждениеЕГАИС(, НСтр("ru = 'Нет данных для отображения'"));
			Иначе
				ПоказатьЗначениеЕГАИС(, ВходящееСообщение);
			КонецЕсли;
		Иначе
			ПоказатьЗначениеЕГАИС(, ИсходящиеЗапросы[Число(ИндексТаблицыЗапросов)].ВходящееСообщение);
		КонецЕсли;
	...
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "ОткрытьИсходящееСообщение") Тогда
		...
		Если ПустаяСтрока(ИндексТаблицыЗапросов) Тогда
			Если ИсходящееСообщение.Пустая() Тогда
				ПоказатьПредупреждениеЕГАИС(, НСтр("ru = 'Нет данных для отображения'"));
			Иначе
				ПоказатьЗначениеЕГАИС(, ИсходящееСообщение);
			КонецЕсли;
		Иначе
			ПоказатьЗначениеЕГАИС(, ИсходящиеЗапросы[Число(ИндексТаблицыЗапросов)].ИсходящееСообщение);
		КонецЕсли;
	...
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "ОткрытьРезультат") Тогда
		...
		Если ТипЗнч(ПолученныйОбъект) <> Тип("Массив") Тогда
			ПоказатьЗначениеЕГАИС(, ПолученныйОбъект);
		ИначеЕсли ПолученныйОбъект.Количество() = 1 Тогда
			ПоказатьЗначениеЕГАИС(, ПолученныйОбъект[0]);
		Иначе
...
Функция ТекстПакетаЗапросов()
	...
	Если ВсегоЗапросов <> ОбработанныеЗапросы.Количество() Тогда
		Строки.Добавить(ФорматированнаяСтрока(НСтр("ru = 'Запросы'"),, ЦветГиперссылки,, "ОткрытьИсходящиеСообщения"));
		Строки.Добавить(" ");
...
&НаСервере
Процедура СформироватьФорматированнуюСтроку(ОписанияСтрок, ИмяРодителя, Действие)
	
	ИнтеграцияЕГАИСУТ.СформироватьФорматированнуюСтроку(ЭтаФорма, ОписанияСтрок, ИмяРодителя, Действие, Истина);
...
Функция ТекстОбработанногоЗапроса(ДанныеЗапроса, Объект, ТекстОшибки = "", ИндексТаблицыЗапросов = "")
	...
	ВозвращаемоеЗначение = Новый ФиксированныйМассив(Новый Массив());
	...
	Строки.Добавить(" ");
	Строки.Добавить(ФорматированнаяСтрока(НСтр("ru = 'запрос'"),, ЦветГиперссылки,, "ОткрытьИсходящееСообщение" + ИндексТаблицыЗапросов));
	Строки.Добавить(" ");
	...
	Если Объект <> Неопределено Тогда
		...
		Если ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросСправки1") Тогда
			...
			Строки.Добавить(" ");
			Строки.Добавить(ФорматированнаяСтрока(НСтр("ru = 'ответ.'"),, ЦветГиперссылки,, "ОткрытьВходящееСообщение" + ИндексТаблицыЗапросов));
			Строки.Добавить(Символы.ПС);
			...
			Строки.Добавить(" ");
			Строки.Добавить(ФорматированнаяСтрока(ПредставлениеРезультата,, ЦветГиперссылки,, "ОткрытьРезультат" + ИндексТаблицыЗапросов));
		...
		ИначеЕсли ДанныеЗапроса.Операция = ПредопределенноеЗначение("Перечисление.ВидыДокументовЕГАИС.ЗапросАлкогольнойПродукции") Тогда
			...
			Строки.Добавить(" ");
			Строки.Добавить(ФорматированнаяСтрока(НСтр("ru = 'Показать'"),, ЦветГиперссылки,, "ОткрытьРезультат" + ИндексТаблицыЗапросов));
		...
		ВозвращаемоеЗначение = Новый ФиксированныйМассив(Строки);
	...
	Иначе
		...
		Строки.Добавить(
			ФорматированнаяСтрока(
				НСтр("ru = 'Ошибка:'")));
		Строки.Добавить(" ");
		Строки.Добавить(ФорматированнаяСтрока(ТекстОшибки,, ЦветПроблема));
