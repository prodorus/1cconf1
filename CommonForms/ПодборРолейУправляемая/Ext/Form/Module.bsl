...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступКВнешнимОтчетамИОбработамБыл = Параметры.СписокРолей.Найти("ПравоЗапускаВнешнихОтчетовИОбработок") <> Неопределено;
...
Процедура ПринятьИзменения(Команда)
	...
	Если Не ПодтвердитьДоступКВнешнимОтчетамИОбработкам() Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Функция ПодтвердитьДоступКВнешнимОтчетамИОбработкам()
	
	МассивВыбранныхРолей = Новый Массив;
	КоллекцияГруппРолей = ДеревоРолейДляВыбора.ПолучитьЭлементы();
	Для каждого ГруппаРолей Из КоллекцияГруппРолей Цикл
		
		КоллекцияРолей = ГруппаРолей.ПолучитьЭлементы();
		
		Для каждого ЭлРоль Из КоллекцияРолей Цикл
			Если ЭлРоль.Пометка <> 1 Тогда
				Продолжить;
			КонецЕсли;
			МассивВыбранныхРолей.Добавить(ЭлРоль.Имя);
	...
	ДоступКВнешнимОтчетамИОбработамНазначен = МассивВыбранныхРолей.Найти("ПравоЗапускаВнешнихОтчетовИОбработок") <> Неопределено;
	
	Если Не ДоступКВнешнимОтчетамИОбработамБыл И ДоступКВнешнимОтчетамИОбработамНазначен Тогда
		Возврат РаботаСДиалогами.ПоказатьПредупреждениеБезопасности(3);
	Иначе
		Возврат Истина;
	КонецЕсли;
