...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновлениеИнформационнойБазыЕГАИСУТ.ПроверитьОбъектОбработан(Объект, ЭтаФорма);
...
Процедура ЗаполнениеСвойствПриИзмененииНоменклатуры(КэшЗначений)
	...
	СобытияФормЕГАИСПереопределяемый.УстановитьИнформациюОЕдиницеХранения(ЭтаФорма);
КонецПроцедуры
...
Процедура СгенерироватьШтрихкод(Команда)
	...
	ОписаниеОповещения = ОписаниеОповещенияЕГАИС("ОбработкаГенерацииШтрихкода", ЭтаФорма);
	
	ОткрытьФормуЕГАИС("Обработка.ГенерацияШтрихкодовУпаковок.Форма", ПередаваемыеПараметры, ЭтаФорма, УникальныйИдентификатор,,,
...
Процедура ПоискПоШтрихкодуВыполнить(Команда)
	...
	ОчиститьСообщения();
	Оповещение = ОписаниеОповещенияЕГАИС("ПоискПоШтрихкодуЗавершение", ЭтаФорма);
...
Процедура ОбработатьШтрихкодыНаСервере(ДанныеШтрихкодов, КэшированныеЗначения)
	...
	ПараметрыОбработкиШтрихкода = АкцизныеМаркиКлиентСервер.ПараметрыСканированияАкцизныхМарок(ЭтаФорма);
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если Источник = "СканерШтрихкода" И Событие = "ПолученШтрихкод" Тогда
		...
		Если ТипЗнч(Данные) = Тип("Строка") Тогда
			...
			ДанныеШтрихкодов = Новый Массив;
			ДанныеШтрихкодов.Добавить(Новый Структура("Штрихкод", Данные));
		...
		ИначеЕсли ТипЗнч(Данные) = Тип("Массив") Тогда
			...
			Для каждого Строка Из Данные Цикл
				ДанныеШтрихкодов.Добавить(Новый Структура("Штрихкод", Строка));
			КонецЦикла;
...
Процедура УстановитьУсловноеОформление()
	...
	ГосударственныеИнформационныеСистемыПереопределяемый.УстановитьУсловноеОформлениеХарактеристикНоменклатуры(ЭтаФорма, "Характеристика", "ХарактеристикиИспользуются");
...
Процедура ВложенныеШтрихкодыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элементы.ВложенныеШтрихкоды.ТолькоПросмотр Тогда
		...
		СтандартнаяОбработка = Ложь;
		ПоказатьЗначениеЕГАИС(Неопределено, ТекущиеДанные.Штрихкод);
