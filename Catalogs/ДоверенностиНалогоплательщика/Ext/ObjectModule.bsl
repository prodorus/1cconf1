...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ПредставительЯвляетсяЮЛ И НЕ ПредставительЯвляетсяСотрудником Тогда
		//должен быть заполнен ИНН физлица-представителя
		Если НЕ ЗначениеЗаполнено(ПредставительФЛ_ИНН) Тогда
			ОбщегоНазначения.СообщитьОбОшибке("У представителя, являющегося физлицом и не являющегося сотрудником организации, должен быть задан ИНН", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
