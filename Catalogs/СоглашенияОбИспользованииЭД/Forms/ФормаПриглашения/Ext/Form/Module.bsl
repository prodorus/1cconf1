...
Процедура ОтправитьПриглашение(Команда)
	...
	Если ЗначениеЗаполнено(Маркер) Тогда
		...
		Если ЗначениеЗаполнено(АдресПакетаПриглашения) Тогда
			Оповестить("СохраненПакетПриглашения", АдресПакетаПриглашения, ЭтаФорма);
		КонецЕсли;
...
Процедура ОтправитьПриглашенияСервер(ОтправленоПриглашений, Маркер)
	...
	АдресПакетаПриглашения = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяФайлаПриглашения));
	
	РезультатОтправки = ЭлектронныеДокументыВнутренний.ОтправитьЧерезОператораЭДО(
		Маркер, ПутьДляПриглашений, "SendContacts", ПрофильНастроекЭДО);
