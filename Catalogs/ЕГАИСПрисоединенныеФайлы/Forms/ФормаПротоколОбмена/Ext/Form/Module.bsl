...
Процедура ДеревоФайловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ДанныеСтроки.ПолучитьЭлементы().Количество() > 0 Тогда
		...
		ПоказатьЗначениеЕГАИС(,ДанныеСтроки.Документ);
...
&НаКлиенте
Процедура ТекстДокументОбработкаНавигационнойСсылки(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	НавигационнаяСсылкаФорматированнойСтроки = ИнтеграцияЕГАИСУТКлиентСервер.НавигационнаяСсылкаСтроки(Элемент, ТекстДокумент);
	
	ПерейтиПоНавигационнойСсылкеЕГАИС(НавигационнаяСсылкаФорматированнойСтроки);
...
Процедура ВыполнитьОбмен(Команда)
	...
	ИнтеграцияЕГАИСКлиент.ВыполнитьОбмен(,,ЭтаФорма.УникальныйИдентификатор);
...
Процедура ПоказатьСообщенияXML(Команда)
	...
	Иначе
		...
		ПоказатьПредупреждениеЕГАИС(,НСтр("ru = 'Команда не может быть выполнена для указанного объекта.'"));
...
//#КонецОбласти
	...
	СтандартныеПодсистемыСервер.УстановитьУсловноеОформлениеПоляДата(ЭтаФорма, "ДеревоФайлов.Дата", Элементы.ДеревоФайловДата.Имя);
...
Функция ПредставлениеОперации(СтрокаПоследовательности, ДокументСсылка, ВыборкаПоФайлам = Неопределено)
	...
	ИначеЕсли СтрокаПоследовательности.ТипСообщения = Перечисления.ТипыЗапросовЕГАИС.Исходящий Тогда
...
Процедура ЗаполнитьПоОснованиюДокументаАктПостановкиНаБалансЕГАИС(ДокументОснование)
	...
	Если СтатусыОформления.Свойство(МетаданныеДокумента.Имя)
		...
		Если ТекстНадписи <> Неопределено Тогда
			...
			ТекстДокументаЕГАИС = Новый ФиксированныйМассив(Строки);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоФайлам.Следующий() Цикл
			...
			Если СтрокаПоследовательности <> Неопределено 
				И СтрокаПоследовательности.ТипСообщения = Перечисления.ТипыЗапросовЕГАИС.Исходящий Тогда
...
Процедура ЗаполнитьДеревоФайлов()
	...
	Строки.Добавить(" ");
	Строки.Добавить(ФорматированнаяСтрока(Строка(Документ),,,,ПолучитьНавигационнуюСсылку(Документ)));
	ТекстДокумент = Новый ФиксированныйМассив(Строки);
	
	ИнтеграцияЕГАИСУТ.СформироватьФорматированнуюСтроку(ЭтаФорма,
		ТекстДокумент, "ГруппаТекстДокумент", "ТекстДокументОбработкаНавигационнойСсылки");
