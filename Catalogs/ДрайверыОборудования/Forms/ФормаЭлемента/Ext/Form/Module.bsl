...
Процедура УстановитьДрайверКоманда(Команда)
	...
	Если Модифицированность Тогда
		...
		Оповещение = ОписаниеОповещенияЕГАИС("УстановитьДрайверКоманда_Завершение", ЭтаФорма);
...
Процедура ВыгрузитьФайлДрайвераКоманда(Команда)
	...
	Иначе 
		...
		Если Модифицированность Тогда
			Текст = НСтр("ru='Продолжение операции возможно только после записи данных.
				|Записать данные и продолжить?'");
			Оповещение = ОписаниеОповещенияЕГАИС("ВыгрузитьФайлДрайвераКоманда_Завершение", ЭтаФорма);
			ПоказатьВопросЕГАИС(Оповещение, Текст, РежимДиалогаВопрос.ДаНет);
		Иначе
...
Процедура ЗагрузитьФайлДрайвераКоманда(Команда)
	...
	Оповещение = ОписаниеОповещенияЕГАИС("ЗагрузитьФайлДрайвераКоманда_ВыборФайлаЗавершение", ЭтаФорма);
...
Процедура ПолучитьИнформациюДрайвераПоФайлу(ПолноеИмяФайла) 
	...
	Оповещение = ОписаниеОповещенияЕГАИС("ПолучитьИнформациюДрайвераПоФайлу_ИнициализацияФайлаЗавершение", ЭтаФорма, ПараметрыВыполнения);
...
Процедура ПолучитьИнформациюДрайвераПоФайлу_ИнициализацияФайлаЗавершение(ФайлДрайвера, ПараметрыВыполнения) Экспорт 
	...
	Оповещение = ОписаниеОповещенияЕГАИС("ПолучитьИнформациюДрайвераПоФайлу_Завершение", ЭтаФорма, ПараметрыВыполнения);
	НачатьПолучениеКаталогаВременныхФайловЕГАИС(Оповещение);
...
Процедура ПолучитьИнформациюДрайвераПоФайлу_Завершение(Результат, ПараметрыВыполнения) Экспорт 
	...
	ИначеЕсли ИмяФайлаРасширение = ".ZIP" Тогда
		...
		Для Каждого ЭлементАрхива Из АрхивДрайвера.Элементы Цикл
			...
			Если ВРег(ЭлементАрхива.Имя) = "INFO.XML" Тогда
				...
				НачатьУдалениеФайловЕГАИС(, ВременныйКаталог + "INFO.XML");
			КонецЕсли;
	...
	Иначе
		ПоказатьПредупреждениеЕГАИС(, НСтр("ru='Неверное расширение файла.'"));
	КонецЕсли;
...
Процедура ЗагрузитьФайлДрайвераВБазу(ПолноеИмяФайла, ИмяФайла) Экспорт 
	...
	Оповещение = ОписаниеОповещенияЕГАИС("ЗагрузитьФайлДрайвераВБазу_Завершение", ЭтаФорма, ИмяФайла);
	НачатьПомещениеФайловЕГАИС(Оповещение, Неопределено, ПолноеИмяФайла, Ложь) 
...
Процедура ПолучениеОбъектаДрайвераЗавершение(ОбъектДрайвера, Параметры) Экспорт
	...
	Иначе
		...
		ТекущаяВерсия = "";
		Попытка
			ОповещениеМетода = ОписаниеОповещенияЕГАИС("ПолучитьНомерВерсииЗавершение", ЭтаФорма);
		...
		Попытка
			...
			URLЗагрузкиДрайвера       = "";
			ОповещениеМетода = ОписаниеОповещенияЕГАИС("ПолучитьОписаниеЗавершение", ЭтаФорма);
...
Процедура ОбновитьСтатусДрайвера();
	...
	Оповещение = ОписаниеОповещенияЕГАИС("ПолучениеОбъектаДрайвераЗавершение", ЭтаФорма);
