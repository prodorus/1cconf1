...
Процедура ПодготовитьДанныеПоРегистрационнойИнформации(РеквизитыПакета, ДвоичныеДанныеДляОператораЭДО, ДвоичныеДанныеСоглашенияНаПодключениеЭДО)
	...
	РеквизитыПакета.Вставить("Фамилия",                  Фамилия);
	РеквизитыПакета.Вставить("Имя",                      Имя);
	РеквизитыПакета.Вставить("Отчество",                 Отчество);
...
Процедура ЗаполнитьСпискиВыбораНаКлиенте()
	...
	Иначе
		...
		Для Каждого Строка ИЗ ИнформацияМенеджера.АлгоритмыПодписи Цикл
			Если Сред(Строка, 1, 4) = "GOST" ИЛИ Сред(Строка, 1, 4) = "GR" Тогда
