...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	Если ФормироватьПоПодразделению И ПодразделениеОрганизации.Пустая() Тогда
		Сообщить("Не выбрано подразделение.");
		Возврат;
	КонецЕсли;
	
	ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям = БухгалтерскийУчетДополнительный.ВестиКассовыеКнигиПоОбособленнымПодразделениям(Организация,ДатаКон);
	...
	Если ПересчитатьНомера Тогда
		...
		Если ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям Тогда
			Номера.Отбор.ПодразделениеОрганизации.Значение = ПодразделениеОрганизации;
			Номера.Отбор.ПодразделениеОрганизации.Использование = Истина;
		КонецЕсли;
	...
	Иначе
		...
		Если ДатаНач <> НачалоГода Тогда
			...
			Если ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям Тогда
				СтруктураОтбора = Новый Структура("Организация,ПодразделениеОрганизации",Организация,ПодразделениеОрганизации);
			Иначе
				СтруктураОтбора = Новый Структура("Организация",Организация);
			КонецЕсли;
				
			Номера=РегистрыСведений.НомераЛистовКассовойКниги.СрезПоследних(ДатаНач-86400,СтруктураОтбора);
			...
			Если Номера.Количество()>0 Тогда
				...
				Номера=РегистрыСведений.НомераЛистовКассовойКниги.СрезПоследних(НачалоМесяца(ДатаКон)-86400,СтруктураОтбора);
	...
	|		&НачИтоги, &ДатаКон, ДЕНЬ, ДвиженияИГраницыПериода, Счет В ИЕРАРХИИ (&Счет),"
		+ ?(ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям," &ВидыСубконто,",",")
		+ "Организация = &Организация" + ?(ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям," И Субконто1 = &ПодразделениеОрганизации","")
	...
	Если ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям Тогда
			...
			Итог.УстановитьПараметр("ВидыСубконто",ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Подразделения);
			Итог.УстановитьПараметр("ПодразделениеОрганизации",ПодразделениеОрганизации);
	...
	|		"  + ?(ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям," И ПриходныйКассовыйОрдер.Касса.ПодразделениеОрганизации = &ПодразделениеОрганизации","") + "
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				ИЛИ НЕ РасходныйКассовыйОрдер.ОтражатьВУправленческомУчете)
	|		" + ?(ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям," И РасходныйКассовыйОрдер.Касса.ПодразделениеОрганизации = &ПодразделениеОрганизации","") + ") КАК КассовыеДокументы
	...
	Если ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям Тогда
		МассивПрефиксов.Добавить(ПодразделениеОрганизации.ПрефиксКассовыхДокументов);
	Иначе
		БухгалтерскийУчетДополнительный.ДобавитьПерфиксыКассовыхДокументовОбособленныхПодразделенийОрганизации(МассивПрефиксов,Организация);
	КонецЕсли;
	...
	Пока (ПоПроводкам Или ПоДокументам) И ЕстьПроводкиВИнтервале Цикл
		...
		КонецЕсли;    
		Если ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям Тогда
			ЭтоГраницаПериода = ВсегдаВыводитьЛистыНаНачалоИКонецПериода И (Итог.Период = ДатаНач ИЛИ КонецДня(Итог.Период) = КонецДня(ДатаКон));
			Если НЕ ЭтоГраницаПериода И ПоПроводкам И (Не ПоДокументам ИЛИ Запрос.День > Итог.Период) Тогда
...
//		ЛистовЗаГод = 1 стр
		...
		Если ДатаЛиста >= ДатаНач Тогда
			...
			Если ФормироватьПоПодразделению Тогда
				РуководителиПодразделения = РегламентированнаяОтчетность.ОтветственныеЛицаОрганизаций(ПодразделениеОрганизации, КонецДня(ДатаЛиста),);
				ОбластьПодвалОтчет.Параметры.Кассир = РуководителиПодразделения.Кассир;
			Иначе
		...
		СтруктураОтбораЛистов = Новый Структура("Организация",Организация);
		Если ВестиОтдельныеКассовыеКнигиПоОбособленнымПодразделениям Тогда
			СтруктураОтбораЛистов.Вставить("ПодразделениеОрганизации",ПодразделениеОрганизации);
		КонецЕсли;
		
		ТекНомера=РегистрыСведений.НомераЛистовКассовойКниги.СрезПоследних(ДатаЛиста,СтруктураОтбораЛистов);
	...
	Если ВывестиПодвал Тогда
		Д = ДатаКон;
		Если НЕ ВсегдаВыводитьЛистыНаНачалоИКонецПериода И ((НачалоДня(КонецМесяца(ДатаЛиста)) = ДатаЛиста) Или
		...
		КонецЕсли;
		Если НЕ ВсегдаВыводитьЛистыНаНачалоИКонецПериода И ((НачалоДня(КонецГода(ДатаЛиста)) = ДатаЛиста) Или 
			...
			Если ФормироватьПоПодразделению Тогда
				ОбластьОбложка.Параметры.НазваниеПодразделения = ПодразделениеОрганизации.Наименование;
			КонецЕсли;
