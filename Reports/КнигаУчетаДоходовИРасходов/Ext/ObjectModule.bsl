...
Перем ТаблицаДанныхДоходыРасходыПоКвартально;
Перем ТаблицаДанныхУменьшенияНалогаПоКвартально;
Перем ТаблицаДанныхУплаченныйТорговыйСборПоКвартально;
...
Процедура ВывестиИтогиЗаКвартал(ТекКвартал, ПоследнийКварталПериода, Макет, ДокументРезультат)
	...
	Для Каждого ИтогиПериода Из ТаблицаДанныхДоходыРасходыПоКвартально Цикл
...
Процедура ВывестиУменьшенияНалогаЗаКвартал(ТекКвартал, ПоследнийКварталПериода, Макет, ДокументРезультат)
	...
	Секция.Параметры.ПредставлениеПериодаИтогов = "Итого за " + РегламентированнаяОтчетность.ПреобразоватьЧислоВРимскуюНотацию(РимскоеЧисло, Истина) + " квартал";
...
Процедура ВывестиУплаченныйТоровыйСборЗаКвартал(ТекКвартал, ПоследнийКварталПериода, Макет, ДокументРезультат)
	
	Секция = Макет.ПолучитьОбласть("Итог");
	РимскоеЧисло = ТекКвартал;
	Секция.Параметры.ПредставлениеПериодаИтогов = "Итого за "
		+ РегламентированнаяОтчетность.ПреобразоватьЧислоВРимскуюНотацию(РимскоеЧисло, Истина)
		+ " квартал";
		
	УплаченныйСборЗаКвартал         = 0;
	УплаченныйСборНарастающимИтогом = 0;
		
	Для Каждого ИтогКвартала Из ТаблицаДанныхУплаченныйТорговыйСборПоКвартально Цикл
		Если НомерКвартала(ИтогКвартала.Квартал) > ТекКвартал Тогда
			Прервать;
		КонецЕсли;
		
		УплаченныйСборНарастающимИтогом = УплаченныйСборНарастающимИтогом + ИтогКвартала.СуммаТорговогоСбора;
		
		Если НомерКвартала(ИтогКвартала.Квартал) = ТекКвартал Тогда
			УплаченныйСборЗаКвартал = УплаченныйСборЗаКвартал + ИтогКвартала.СуммаТорговогоСбора;
		КонецЕсли;
	КонецЦикла;
...
Функция ВыделятьНДС(УказаннаяОрганизация)
	...
	Если ЗначениеЗаполнено(УказаннаяОрганизация) Тогда
		УчетнаяПолитика = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Период, УказаннаяОрганизация);
...
Функция ПолучитьЗаписиУплатыТорговогоСбора(НачалоПериода, КонецПериода, Организация)
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаписьКУДиРУплаченныйТорговыйСбор.ДатаНомер,
	|	ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Дата КАК ПериодЗаписи,
	...
	|ГДЕ
	|	ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	И ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Организация = &Организация
	|	И ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Проведен
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПериодЗаписи,
	|	ПериодНачисления,
	|	ЗаписьКУДиРУплаченныйТорговыйСбор.НомерСтроки";
...
Функция ПолучитьУплаченныйТорговыйСборПоКвартально(НачалоПериода, КонецПериода, Организация)
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	НАЧАЛОПЕРИОДА(ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Дата, КВАРТАЛ) КАК Квартал,
	|	СУММА(ЗаписьКУДиРУплаченныйТорговыйСбор.СуммаТорговогоСбора) КАК СуммаТорговогоСбора
	|ИЗ
	|	Документ.ЗаписьКУДиР.УплаченныйТорговыйСбор КАК ЗаписьКУДиРУплаченныйТорговыйСбор
	|ГДЕ
	|	ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Дата, КВАРТАЛ)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Квартал"
	;
...
Процедура ПечатьТитульногоЛиста(ДокументРезультат)
	...
	Если Период >= Дата("20130101") Тогда
		...
		Секция.Параметры.РедакцияПриказаМинфина = РедакцияПриказаМинфина135н();
	...
	ПараметрыУчетнойПолитики = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Период, Организация);
...
Процедура ПечатьРаздела1(ДокументРезультат)
	...
	КолКварталов = НомерКвартала(Период);
	...
	Если НачДата <> НачалоГода(Период) Тогда
		...
		ТаблицаРезультата = ЗапросНПП.Выполнить().Выгрузить();
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		КварталЗаписи = НомерКвартала(ВыборкаЗаписей.Период);
	...
	Если (НомерКвартала(Период) = 4) и (Период >= Дата("20060101")) Тогда
		...
		Секция.Параметры.ПечДоходы = ТаблицаДанныхДоходыРасходыПоКвартально.Итог("Графа5");
		Секция.Параметры.ПечРасходы = ТаблицаДанныхДоходыРасходыПоКвартально.Итог("Графа7");
...
Процедура ПечатьРаздела3(ДокументРезультат)
	...
	Иначе
		...
		Для каждого Квартал из ТаблицаДанныхДоходыРасходыПоКвартально Цикл
...
Процедура ПечатьРаздела4(ДокументРезультат)
	...
	Если НачДата <> НачалоГода(Период) Тогда
		...
		|	Документ.ЗаписьКУДиР.УменьшениеНалога КАК УменьшениеНалога
		|ГДЕ
		|	УменьшениеНалога.Ссылка.Дата МЕЖДУ &НачДата И &КонДата
		|	И УменьшениеНалога.Ссылка.Организация = &Организация
		|	И УменьшениеНалога.Ссылка.Проведен";
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		КварталЗаписи = НомерКвартала(ВыборкаЗаписей.ПериодЗаписи);
...
Процедура ПечатьРаздела5(ДокументРезультат)
	
	ДокументРезультат.Очистить();
	ДокументРезультат.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	ДокументРезультат.Автомасштаб = Истина;
	ДокументРезультат.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КнигаУчетаДоходовИРасходов_Раздел5";
	
	Если Период < НалоговыйУчетУСН.ДатаНачалаФормирования5РазделаКУДиР() Тогда
		Возврат;
	КонецЕсли;
	...
	Если НачДата <> НачалоГода(Период) Тогда
		...
		|	Документ.ЗаписьКУДиР.УплаченныйТорговыйСбор КАК ЗаписьКУДиРУплаченныйТорговыйСбор
		|ГДЕ
		|	ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Дата МЕЖДУ &НачДата И &КонДата
		|	И ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Организация = &Организация
		|	И ЗаписьКУДиРУплаченныйТорговыйСбор.Ссылка.Проведен";
	...
	ВыборкаЗаписей = ПолучитьЗаписиУплатыТорговогоСбора(НачДата, КонецДня(Период), Организация);
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Пока КварталЗаписи <> ТекКвартал Цикл
			...
			ВывестиУплаченныйТоровыйСборЗаКвартал(ТекКвартал, ПоследнийКварталПериода, Макет, ДокументРезультат);
			ТекКвартал = ТекКвартал + 1;
...
Функция РедакцияПриказаМинфина135н()
...
Процедура СформироватьОтчет(ТитульныйЛист, Раздел1, Раздел2, Раздел3, Раздел4, Раздел5) Экспорт
	...
	ТаблицаДанныхДоходыРасходыПоКвартально = Запрос.Выполнить().Выгрузить();
	
	ТаблицаДанныхУменьшенияНалогаПоКвартально = ПолучитьОборотыУменьшенияНалогаПоКвартально(НачалоГода(Период), КонецДня(Период), Организация);
	ТаблицаДанныхУплаченныйТорговыйСборПоКвартально = ПолучитьУплаченныйТорговыйСборПоКвартально(НачалоГода(Период), КонецДня(Период), Организация);
