...
Процедура ДобавитьОбластьЗаголовок(Макет)
	...
	Для каждого ВыбранноеПоле из ПостроительОтчета.ВыбранныеПоля Цикл
		...
		Если НЕ ЭтоПолеПоказатель(ВыбранноеПоле.Имя) Тогда
			...
			|" + ВыбранноеПоле.Представление + " ("+ ПолучитьРазмещениеВыбранногоПоля(ВыбранноеПоле.Имя) + ", " + мПоложениеПоля[ПолучитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя)] + ");";
...
Функция ПолучитьПоложениеВыбранногоПоля(Имя) Экспорт
	...
	ПоложениеПоУмолчанию = "Послегруппировки";
...
Функция ДобавитьДополнительноеПоле(ПутьКДанным, Размещение = Неопределено, Положение = Неопределено) Экспорт
	...
	// Интерпретация числовых значений параметра "Положение" оставлена для совместимости
	Если Положение = Неопределено Тогда
		
		УстановитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя, "Послегруппировки");
	
	ИначеЕсли Положение = 1 Тогда
		
		УстановитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя, "Передгруппировкой");
		
	ИначеЕсли Положение = 2 Тогда
		
		УстановитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя, "Вместогруппировки");
	...
	Иначе
		...
		УстановитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя, Положение);
...
Функция ВосстановитьНастройки(Объект, Отчет, Форма, ИмяОбъекта = Неопределено) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ПреобразоватьФормат(Результат.СохраненнаяНастройка);
...
Процедура ПреобразоватьФормат(СохраненнаяНастройка)
	// Изменение положения поля
	Если СохраненнаяНастройка.Свойство("РазмещениеПоложениеВыбранныхПолей") Тогда
		Для Каждого Поле Из СохраненнаяНастройка.РазмещениеПоложениеВыбранныхПолей Цикл
			Поле.Значение.Положение = СтрЗаменить(Поле.Значение.Положение, " ", "");
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
мПоложениеПоля = Новый Структура;
мПоложениеПоля.Вставить("Передгруппировкой", НСтр("ru='Перед группировкой'"));
мПоложениеПоля.Вставить("Вместогруппировки", НСтр("ru='Вместо группировки'"));
мПоложениеПоля.Вставить("Послегруппировки",  НСтр("ru='После группировки'"));
