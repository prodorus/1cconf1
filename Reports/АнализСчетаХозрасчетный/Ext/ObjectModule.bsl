...
Функция СформироватьЗаголовок() Экспорт
	...
	Если ВыводитьЕдиницуИзмерения Тогда
		БухгалтерскиеОтчеты.ДобавитьТекстПроЕдиницуИзмерения(ТекстПроСписокПоказателей);
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыРасшифровки(Область, Выборка, ОтборСубконто = Неопределено)
	...
	ИначеЕсли Выборка.Группировка() = "Счет" Тогда
		...
		Если ОтборСубконто <> Неопределено Тогда
			...
			СчетГруппировки = Выборка[Выборка.Группировка()];
			ОтборСубконто.Вставить(Выборка.Группировка(), СчетГруппировки);
...
Процедура СформироватьОтчет(ДокументРезультат,  ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0, ПоказыватьПодписи = Ложь) Экспорт
	...
	Если ТипЗнч(МассивШиринКолонок) = Тип("Массив") Тогда
		...
		// При выводе отчета за период и без колонок по валюте, значение ширины 5ой, 6ой, 7ой и 8ой колонки равняется 9.
		// Из-за этого, если выводить потом отчет в валюте, 5ая, 6ая и 7ая колонки могут сужаться из-за восстановления ширины колонок 
		// А также может сужаться область печати и в нее не попадать целиком область с подписями.
		Если ШиринаТаблицы >= 5 Тогда 
			Для Сч = 5 По ШиринаТаблицы Цикл
				Если ДокументРезультат.Область(, Сч, , Сч).ШиринаКолонки < 15 Тогда
					ДокументРезультат.Область(, Сч, , Сч).ШиринаКолонки = 15;	
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
