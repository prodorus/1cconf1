...
Функция СформироватьТекстЗапроса(КоличествоОбъектовАналитики)
		...
		Текст	= "ВЫБРАТЬ
		     	  ...
		     	  |	НалоговыйОбороты.Счет.Вид КАК ВидСчета,
		     	  ...
		     	  |	НалоговыйОбороты.ВидУчетаКор КАК КорВидУчета,
		     	  |	НалоговыйОбороты.СуммаОборот КАК СуммаОборот,
		     	  |	НалоговыйОбороты.СуммаОборотДт КАК СуммаОборотДт,
		     	  ...
		     	  |////////////////////////////////////////////////////////////////////////////////
		     	  |ВЫБРАТЬ
		     	  |	ТаблицаОборотов.Субконто1 КАК Субконто1,
		     	  |	ТаблицаОборотов.Субконто2 КАК Субконто2,
		     	  |	ТаблицаОборотов.Субконто3 КАК Субконто3,
		     	  |	ВЫБОР
		     	  |		КОГДА НЕ ТаблицаОборотов.КорСчет В ИЕРАРХИИ (&СчетЕН)
		     	  |				И НЕ(ТаблицаОборотов.КорСчет В ИЕРАРХИИ (&Счет90_91)
		     	  |						И ТаблицаОборотов.КорВидУчета = &ВидУчета)
		     	  |			ТОГДА ВЫБОР
		     	  |					КОГДА ТаблицаОборотов.ВидСчета = &Пассивный
		     	  |						ТОГДА -ТаблицаОборотов.СуммаОборотКт + ВЫБОР
		     	  |								КОГДА ТаблицаОборотов.СуммаОборотДт < 0
		     	  |									ТОГДА ТаблицаОборотов.СуммаОборотДт
		     	  |								ИНАЧЕ 0
		     	  |							КОНЕЦ
		     	  |					ИНАЧЕ ТаблицаОборотов.СуммаОборотДт - ВЫБОР
		     	  |							КОГДА ТаблицаОборотов.СуммаОборотКт < 0
		     	  |								ТОГДА ТаблицаОборотов.СуммаОборотКт
		     	  |							ИНАЧЕ 0
		     	  |						КОНЕЦ
		     	  |				КОНЕЦ
		     	  |		ИНАЧЕ 0
		     	  ...
		     	  |	ВЫБОР
		     	  |		КОГДА ТаблицаОборотов.КорСчет В ИЕРАРХИИ (&СчетЕН)
		     	  |				И ТаблицаОборотов.КорВидУчета = &ВидУчета
		     	  |			ТОГДА ТаблицаОборотов.СуммаОборот
		     	  |		ИНАЧЕ 0
		     	  |	КОНЕЦ КАК СписаноНеННП,
		     	  |	ВЫБОР
		     	  |		КОГДА ТаблицаОборотов.КорСчет В ИЕРАРХИИ (&Счет90_91)
		     	  |				И ТаблицаОборотов.КорВидУчета = &ВидУчета
		     	  |			ТОГДА ТаблицаОборотов.СуммаОборот
		     	  |		ИНАЧЕ ВЫБОР
		     	  |				КОГДА ТаблицаОборотов.Счет В ИЕРАРХИИ (&Счет90_91)
		     	  |						И НЕ ТаблицаОборотов.КорВидУчета = &ВидУчета
		     	  |					ТОГДА ТаблицаОборотов.СуммаОборот
		     	  |				ИНАЧЕ  0
		     	  |			КОНЕЦ
		     	  ...
		     	  |////////////////////////////////////////////////////////////////////////////////
		     	  |ВЫБРАТЬ
		     	  |	ТаблицаРазниц.Субконто1 КАК Субконто1,
		     	  |	ТаблицаРазниц.Субконто2 КАК Субконто2,
		     	  |	ТаблицаРазниц.Субконто3 КАК Субконто3,
		     	  |	ТаблицаРазниц.Признано КАК Признано,
		     	  |	ТаблицаРазниц.Перенесено КАК Перенесено,
		     	  |	ТаблицаРазниц.СписаноНеННП КАК СписаноНеННП,
		     	  |	ТаблицаРазниц.СписаноНаННП КАК СписаноНаННП,
		     	  |	ТаблицаРазниц.ПризнаноРанее КАК ПризнаноРанее,
		     	  |	ТаблицаРазниц.Остаток КАК Остаток
		     	  |ИЗ
		...
		Если КоличествоОбъектовАналитики < 3 Тогда
			...
			Текст = СтрЗаменить(Текст, "ТаблицаРазниц.Субконто3 КАК Субконто3,", "");
			Текст = СтрЗаменить(Текст, "ТаблицаОборотов.Субконто3 КАК Субконто3,", "");
		...
		Если КоличествоОбъектовАналитики < 2 Тогда
			...
			Текст = СтрЗаменить(Текст, "ТаблицаРазниц.Субконто2 КАК Субконто2,", "");
			Текст = СтрЗаменить(Текст, "ТаблицаОборотов.Субконто2 КАК Субконто2,", "");
			Текст = СтрЗаменить(Текст, ",
			|	Субконто2", "");
		КонецЕсли;
		...
		Если КоличествоОбъектовАналитики < 1 Тогда
			...
			Текст = СтрЗаменить(Текст, "ТаблицаРазниц.Субконто1 КАК Субконто1,", "");
			Текст = СтрЗаменить(Текст, "ТаблицаОборотов.Субконто1 КАК Субконто1,", "");
			Текст = СтрЗаменить(Текст, ",
			|	Субконто1", "");
		КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь, НачалоПодписи, ПоказыватьПодписи) Экспорт
	...
	КонецЕсли;
	   ...
	   КонецЕсли; 
		...
		Запрос.УстановитьПараметр("Счет90_91", Массив);
        ...
        Для Каждого ВидАктивовИОбязательств Из ТаблицаВидовАктивовИОбязательств Цикл
			...
			СформироватьСтроку(ВидАктивовИОбязательств, ВидАктивовИОбязательств, ВыборкаСчет, ОбластьСтрока, Неопределено, 0, КоличествоОбъектовАналитики = 0);
			ЦиклБезПодразделения(ВыборкаСчет,ВидАктивовИОбязательств,КоличествоОбъектовАналитики);
		КонецЦикла;
		
		
		Итоги = ТаблицаАктивовИОбязательств.НайтиСтроки(Новый Структура("ЭтоИтог", Истина));
		ОтрицательныеОбязательства = НайтиИтог(Итоги, "Обязательства") < 0;
		ОтрицательныеАктивы = НайтиИтог(Итоги, "Активы") < 0;
		
		Для Каждого Строка Из ТаблицаАктивовИОбязательств Цикл
			ОбработатьОтрицательныеАктивыИОбязательства(ВариантОтчета, Строка, ОтрицательныеОбязательства, ОтрицательныеАктивы);
...
Процедура  СформироватьСтроку(ВидАктивовИОбязательств, Показатель, Выборка, ОбластьСтрока, ПараметрыОтбора, КоличествоСубконто, ЭтоИтог = Ложь)
	...
	Если ВариантОтчета = 1 Тогда   // постоянные разницы
        СуммаКорректировки = Окр(мСтавкаНалогаНаПрибыль * Строка.СписаноНаННП, 2);
		
		Если СуммаКорректировки < 0 Тогда 
			Строка.Обязательства   = - СуммаКорректировки;
			Строка.Активы          = 0;
		Иначе
			 
			Строка.Активы        = СуммаКорректировки;
...
Процедура ОбработатьОтрицательныеАктивыИОбязательства(ВариантОтчета, Строка, ОтрицательныеОбязательства, ОтрицательныеАктивы)
	
	// Пересчет проводится только для постоянных разниц
	Если ВариантОтчета = 2 Тогда 
		Возврат;
	КонецЕсли;
...
Процедура ЦиклБезПодразделения(ВыборкаНачала,ВидАктивовИОбязательств,КоличествоОбъектовАналитики)
	
	Выборка1 = ВыборкаНачала.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока Выборка1.Следующий() Цикл
		Если КоличествоОбъектовАналитики >= 1 Тогда
			СформироватьСтроку(ВидАктивовИОбязательств, Выборка1.Субконто1, Выборка1, ОбластьСтрока1, Новый Структура("Субконто1", Выборка1.Субконто1), КоличествоОбъектовАналитики, КоличествоОбъектовАналитики = 1);
			Если КоличествоОбъектовАналитики = 1 Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Пока Выборка2.Следующий() Цикл
			Если КоличествоОбъектовАналитики >= 2 Тогда
				СформироватьСтроку(ВидАктивовИОбязательств, Выборка2.Субконто2, Выборка2, ОбластьСтрока2, Новый Структура("Субконто1,Субконто2", Выборка1.Субконто1, Выборка2.Субконто2), КоличествоОбъектовАналитики, КоличествоОбъектовАналитики = 2);
				Если КоличествоОбъектовАналитики = 2 Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			Пока Выборка3.Следующий() Цикл
				Если КоличествоОбъектовАналитики = 3 Тогда
					СформироватьСтроку(ВидАктивовИОбязательств, Выборка3.Субконто3, Выборка3, ОбластьСтрока3, Новый Структура("Субконто1,Субконто2, Субконто3", Выборка1.Субконто1, Выборка2.Субконто2, Выборка3.Субконто3), КоличествоОбъектовАналитики, КоличествоОбъектовАналитики = 3);
				КонецЕсли;
