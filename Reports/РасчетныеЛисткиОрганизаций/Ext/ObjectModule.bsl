...
Процедура НастроитьПостроительЗапросов(ПостроительОтчетов, ВидРасчетногоЛистка) Экспорт
	...
	НачислениеВозврат.Добавить(Перечисления.НДФЛРасчетыСБюджетомВидСтроки.ЗачетАвансовыхПлатежей);
	...
	ИначеЕсли ВидРасчетногоЛистка = "Рабочий" тогда
		...
		|						КОГДА Взаиморасчеты.ХарактерВыплаты = ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.ПоБеременностиИРодам)
		|							ТОГДА ""Пособие по беременности и родам""
	...
	Иначе
		...
		|			РегистрРасчета.ОсновныеНачисленияРаботниковОрганизаций КАК ОсновныеНачисленияРаботниковОрганизации
		|				ЛЕВОЕ СОЕДИНЕНИЕ СтрокиСФактическимПериодомДействия КАК СтрокиСФактическимПериодомДействия
		|				ПО ОсновныеНачисленияРаботниковОрганизации.Регистратор = СтрокиСФактическимПериодомДействия.Регистратор
		|					И ОсновныеНачисленияРаботниковОрганизации.НомерСтроки = СтрокиСФактическимПериодомДействия.НомерСтроки
		|		ГДЕ
		|			ОсновныеНачисленияРаботниковОрганизации.ПериодРегистрации МЕЖДУ &ПериодРегистрации И &КонецПериодаРегистрации
		|			И (ОсновныеНачисленияРаботниковОрганизации.Результат <> 0
		|					ИЛИ СтрокиСФактическимПериодомДействия.НомерСтроки ЕСТЬ НЕ NULL )
			...
			Для каждого СтрокаФизЛицо из СтрокаПериодаРегистрации.Строки Цикл
				...
				Для Сч = 0 по МаксКолвоСтрок Цикл
					...
					Если Сч <= КолВоСтрокВыплачено тогда
						...
						Если ВидРасчетногоЛистка = "Подробно" И ЗначениеЗаполнено(Выплачено.Строки[Сч].ДатаВедомости) Тогда
...
//
Процедура ЗадатьИменаОбластямРазбитьТабличныйДокументПоСтраницам(ТабличныйДокумент, ВыводитьПоСтраницам)
	...
	ЗадатьИмяОбластиРасчетногоЛистка(ТабличныйДокумент, ОбластьРЛ);
...
Процедура ЗадатьИмяОбластиРасчетногоЛистка(ТабличныйДокумент, ОбластьРЛ)
	
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			
		ПериодЛистка = Неопределено;
		Для НомерСтроки = ОбластьРЛ.Верх По ОбластьРЛ.Низ Цикл
			
			Для НомерКолонки = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
				
				Ячейка = ТабличныйДокумент.Область(НомерСтроки, НомерКолонки, НомерСтроки, НомерКолонки);
				Если ТипЗнч(Ячейка.Расшифровка) = Тип("Дата") Тогда
					ПериодЛистка = Формат(Ячейка.Расшифровка, "ДФ=ММгггг");
				ИначеЕсли ТипЗнч(Ячейка.Расшифровка) = Тип("СправочникСсылка.СотрудникиОрганизаций") Тогда
					
					ФизическоеЛицоСотрудника = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ячейка.Расшифровка, "Физлицо");
					Ид = ФизическоеЛицоСотрудника.УникальныйИдентификатор();
					ОбластьРЛ.Имя = "payslip_" + ПериодЛистка + "_0_" + СтрЗаменить(Ид, "-", "") + "_0";
