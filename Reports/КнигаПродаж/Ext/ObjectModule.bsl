...
Процедура СформироватьКнигуПродаж(ЭлементыФормы) Экспорт
	...
	УчетнаяПолитикаНУ = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(НачалоПериода, Организация);
...
Функция ПодготовитьКнигуКВыводуНаПечать(Периодичность = Неопределено, СписокСчетовФактур)
	...
	|	КОНЕЦ КАК НаАванс,
	|	ВЫБОР
	|		КОГДА НДСЗаписиКнигиПродажОбороты.ВидЦенности В (&ВидыЦенностей_СуммовыеРазницы)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НаСуммовуюРазницу,
	|	ВЫБОР
	|		КОГДА НДСЗаписиКнигиПродажОбороты.ВидЦенности В (&ВидыЦенностей_Аванс)
	|				ИЛИ НДСЗаписиКнигиПродажОбороты.ВидЦенности В (&ВидыЦенностей_СуммовыеРазницы)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаписиКнигиПродаж.Период,
	|	МАКСИМУМ(УчетнаяПолитикаОрганизаций.НДСНалоговыйПериод) КАК НДСНалоговыйПериод,
	|	МАКСИМУМ(УчетнаяПолитикаОрганизаций.МоментОпределенияНалоговойБазыНДС) КАК МоментОпределенияНалоговойБазыНДС
	...
	|		ПО (НАЧАЛОПЕРИОДА(ЗаписиКнигиПродаж.Период, МЕСЯЦ) >= УчетнаяПолитикаОрганизаций.Период)
	|ГДЕ
	...
	|		ПО ЗаписиКнигиПродаж.Период = УчетнаяПолитикаОрганизаций.Период
	|
	|УПОРЯДОЧИТЬ ПО
	|	НалоговыйПериод,
