...
//
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь, НачалоПодписи, ПоказыватьПодписи,
							ПредставлениеПериодаРегистрации) Экспорт
	...
	Отказ = Ложь;
	УчетнаяПолитикаРегл = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(ПериодРегистрации, Организация, Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	
	Если УчетнаяПолитикаРегл = Неопределено ИЛИ Не УчетнаяПолитикаРегл.ПоддержкаПБУ18 Тогда
		Возврат;
	КонецЕсли;
	...
	ЗаголовокОтчета.Параметры.НазваниеОрганизации = "" + СокрЛП(Организация.НаименованиеПолное);
	...
	Для каждого СтрокаВидАктиваОбязательства Из ТаблицаВидовАктивовИОбязательств Цикл
		...
		ТаблицаВидаАктивовИОбязательств = НалоговыйУчет.ОстаткиВременныхРазницПоВидуАктивовОбязательств(
			СтрокаВидАктиваОбязательства, Организация, НачалоПериода, КонецПериода);
		...
		ЕстьСчет = (ТаблицаВидаАктивовИОбязательств.Колонки.Найти("Счет") <> Неопределено);
		КолонкиГруппировок = "СчетОНАОНО," + ?(Не ЭтоОС И НЕ ЭтоНМА И ЕстьСчет, "Счет,", "") + "СтараяСумма";
		ТаблицаВидаАктивовИОбязательств.Свернуть(КолонкиГруппировок,"Сумма");
		...
		Для каждого Строка Из ТаблицаАктивовИОбязательств Цикл
			...
			СтрокаТаблицаВр.Аналитика =  ?(ЭтоОС, "01,03 - 02 ""Основные средства минус амортизация""", ?(ЭтоНМА08, "08.05 ""Приобретение нематериальных активов""", ?(ЭтоНМА, "04 - 05 ""НМА минус амортизация""", ?(НЕ ЕстьСчет, "", "" + Строка.Счет + " """ + Строка.Счет.Наименование + """"))));
		КОнецЦикла;
	КонецЦикла;
