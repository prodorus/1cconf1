...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь) Экспорт
 ДокументРезультат.Очистить();
	...
	СрезСведений = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(ДатаКон, Организация);
	...
	Запрос.УстановитьПараметр("Счет91",       Массив);
	
	Массив = Новый Массив;
	Массив.Добавить(ПланыСчетов.Налоговый.ВыбытиеОС);
	Массив.Добавить(ПланыСчетов.Налоговый.ВыбытиеМЦ);
	Массив.Добавить(ПланыСчетов.Налоговый.ПереданныеОбъектыНедвижимости);
	...
	Запрос.Текст =  
			...
			|	СУММА(ЕСТЬNULL(ДоходыИРасходы.СуммаОборотДт, 0)) КАК ОборотДт91,
			|	СУММА(ЕСТЬNULL(ДоходыИРасходы.СуммаОборотКт, 0)) КАК ОборотКт91,
			...
			|	
			|	ВЫБРАТЬ
			|		ДоходыИРасходыНМА.КорСубконто1,
			...
			|			И ПередачаНМА.ОтражатьВНалоговомУчете
			|		
			|		ОБЪЕДИНИТЬ ВСЕ
			|		
			|		ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|			И ВЫРАЗИТЬ(ПередачаОС.Регистратор КАК Документ.ПередачаОС).ПравоСобственностиПереходитПослеГосРегистрации) КАК ПереданныеАмортОбъекты
			|		ПО ДоходыИРасходы.ОбъектДоходовИРасхдов = ПереданныеАмортОбъекты.Объект
			|ГДЕ
			|	ПереданныеАмортОбъекты.Объект ЕСТЬ НЕ NULL 
			|
			|СГРУППИРОВАТЬ ПО
			|	ПереданныеАмортОбъекты.Объект,
			|	ПереданныеАмортОбъекты.Период";	   
 ...
 Для Каждого Выборка Из Результат Цикл    
	...
	ОбластьСтрока.Параметры.Выручка		            = Выборка.ОборотКт91;
	ОбластьСтрока.Параметры.ПервоначальнаяСтоимость = Выборка.Сумма01;
	ОбластьСтрока.Параметры.СуммаАморизации		    = Выборка.Сумма02;
	ОбластьСтрока.Параметры.РасходыПоРеализации		= Выборка.ОборотДт91 - (Выборка.Сумма01 - Выборка.Сумма02);
	ОбластьСтрока.Параметры.ОбщаяСуммаРасходов      = Выборка.ОборотДт91;
	Прибыль = Выборка.ОборотКт91 - Выборка.ОборотДт91;
	ОбластьСтрока.Параметры.СуммаУбытка		        = ?(Прибыль > 0, 0, -Прибыль);
	Убыток = Убыток + ?(Прибыль > 0, 0, -Прибыль);
	ДокументРезультат.Вывести(ОбластьСтрока);
 КонецЦикла;
	ОбластьПодвал.Параметры.ИтогоВыручка            = Результат.Итог("ОборотКт91");
	ОбластьПодвал.Параметры.ИтогоСуммаРасходов	    = Результат.Итог("ОборотДт91");
