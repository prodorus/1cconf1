...
Функция СобратьТекстЗапроса(Запрос) Экспорт 
	...
	Если мПоказатели.НайтиПоЗначению("СуммаПросроченногоДолга") <> Неопределено
		...
		|	ВзаиморасчетыПоДокументамРасчетов.ЧислоДолжников                        КАК ОбщееЧислоДолжников,
		|	СУММА(ВзаиморасчетыПоДокументамРасчетов.СуммаДолга)                     КАК СуммаДолга,
		|	ВЫБОР
		|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВзаиморасчетыПоДокументамРасчетов.Контрагент) > 0
		|		ТОГДА СУММА(ВзаиморасчетыПоДокументамРасчетов.ДнейПросрочки)/КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВзаиморасчетыПоДокументамРасчетов.Контрагент)
		|		ИНАЧЕ 0
		|		КОНЕЦ                                                               КАК ЧислоДнейПросрочки,
		|   КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВзаиморасчетыПоДокументамРасчетов.Контрагент)      КАК ЧислоДолжников
...
Процедура ВывестиСтроки(Таб, Макет, ТекущаяВыборка, ИндексТекущейГруппировки, ПоказателиПрошлойГруппировки)
	...
	Пока Выборка.Следующий() Цикл
		...
		Для каждого Строки Из мПоказатели Цикл
			...
			ИначеЕсли Найти(Строки.Значение, "Процент") > 0 ИЛИ Найти(Строки.Представление, "Средн") > 0 Тогда
