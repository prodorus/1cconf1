...
Процедура СформироватьМакет(ДокументРезультат, Режим, НачалоПериода, КонецПериода, СписокРаботников, ТаблицаРаботниковОрганизации, Ответственный, ГруппироватьПодразделения, ВозвращаемоеСообщениеОбОшибке)
	...
	Если Режим = "Табель" Тогда
		...
		|					ИЗ
		|						ВТТаблицаРаботниковОрганизации КАК ТаблицаРаботниковОрганизации
		|			
		|					ОБЪЕДИНИТЬ ВСЕ
		|			
		|					ВЫБРАТЬ
		...
		|					ИЗ
		|						ВТТаблицаРаботниковОрганизации КАК ТаблицаРаботниковОрганизации)) КАК ОтветственныеЛицаОрганизацийСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ФИОСрезПоследних.ФизЛицо,
		|	ФИОСрезПоследних.Фамилия,
		|	ФИОСрезПоследних.Имя,
		|	ФИОСрезПоследних.Отчество
		...
		|				ИЗ
		|					ВТОтветственныеЛица КАК Таблица)) КАК ФИОСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СотрудникиОрганизаций.Ссылка
		|ПОМЕСТИТЬ ВТСотрудникиОтветственного
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОтветственныеЛица КАК Руководитель
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОтветственныеЛица КАК РуководительКадровойСлужбыОрганизации
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТФИООтветственныхЛиц КАК ФИОКадровикаОрганизации
		...
		|			И (РуководительКадровойСлужбыОрганизации.ОтветственноеЛицо = ЗНАЧЕНИЕ(Перечисление.ОтветственныеЛицаОрганизаций.РуководительКадровойСлужбы))
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОтветственныеЛица КАК РуководительКадровойСлужбыПодразделения
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТФИООтветственныхЛиц КАК ФИОКадровикаПодразделения
		...
		|			И (РуководительКадровойСлужбыПодразделения.ОтветственноеЛицо = ЗНАЧЕНИЕ(Перечисление.ОтветственныеЛицаОрганизаций.РуководительКадровойСлужбы))
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			ВЫБОР
		...
		|					Сотрудник В
		|						(ВЫБРАТЬ
		|							РаботникиОрганизаций.Ссылка КАК Сотрудник
		|						ИЗ
		|							ВТСотрудникиОтветственного КАК РаботникиОрганизаций)) КАК РаботникиОрганизацийСрезПоследних
		|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&КонецПериода, Физлицо = &Ответственный) КАК ФИОФизЛицСрезПоследних
			...
			Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("ПериодРегистрации") Цикл
				...
				Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("Организация") Цикл
					...
					Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("ПодразделениеОрганизации") Цикл
						...
						Если Не ЗначениеЗаполнено(ОбластьДанныхПодвал.Параметры.ФИООтветственного) Тогда
						...
						КонецЕсли;
						Если Не ЗначениеЗаполнено(ОбластьДанныхПодвал.Параметры.ФИОРуководителя) Тогда
						...
						КонецЕсли;
						Если Не ЗначениеЗаполнено(ОбластьДанныхПодвал.Параметры.ФИОКадровика) Тогда
		...
		Иначе
			Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("ПериодРегистрации") Цикл
				...
				Пока ВыборкаПоРаботнику.СледующийПоЗначениюПоля("Организация") Цикл
					...
					Если ОтборПодразделения И ЗначениеЗаполнено(Подразделение) Тогда
						Если ТипЗнч(Подразделение) = Тип("СписокЗначений") И Подразделение.Количество() = 1 Тогда
							ОбластьДанныхШапка.Параметры.ИмяПодразделения = Строка(Подразделение[0]);
						Иначе
							ОбластьДанныхШапка.Параметры.ИмяПодразделения = Подразделение;
						КонецЕсли;
					КонецЕсли;
	...
	ИначеЕсли Режим = "Бланк" Тогда 
		...
		|					ГДЕ
		|						ИСТИНА
		|						И &УсловиеВЗапрос)) КАК РаботникиОрганизацийСрезПоследних
		...
		Если ДокументРезультат.ВысотаТаблицы = 0 Тогда
			...
			КонецЕсли;
			Если ОтборПодразделения и ТипЗнч(Подразделение) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
				ОбластьДанныхШапка.Параметры.ИмяПодразделения = Строка(Подразделение);
			КонецЕсли;
...
Функция СформироватьОтчет(Результат = Неопределено, ДанныеРасшифровки = Неопределено, ВыводВФормуОтчета = Истина, ВозвращаемоеСообщениеОбОшибке = Неопределено) Экспорт
	...
	Если НачалоПериода > РабочаяДата тогда
		...
		ОбработкаКомментариев.ПоказатьСообщения();
		Возврат Результат;
	ИначеЕсли КонецПериода > РабочаяДата тогда
		ОбработкаКомментариев = глЗначениеПеременной("глОбработкаСообщений");
		ОбработкаКомментариев.УдалитьСообщения();
		ОбработкаКомментариев.ДобавитьСообщение("Формирование табеля за неоконченный месяц", Перечисления.ВидыСообщений.Раздел);
		ОбработкаКомментариев.ДобавитьСообщение("При формировании табеля за будущий или текущий месяц, выводятся данные только до рабочей даты (" + Формат(РабочаяДата, "ДЛФ=ДД") + "). Изменить рабочую дату можно в меню ""Сервис/Параметры.""", Перечисления.ВидыСообщений.Информация);
