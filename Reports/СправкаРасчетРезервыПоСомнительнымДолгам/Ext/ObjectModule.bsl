...
Функция СформироватьТекстЗапроса()
	...
	|	РасчетРезервовПоСомнительнымДолгам.ВстречнаяЗадолженность КАК СуммаВстречногоДолга,
	|	РасчетРезервовПоСомнительнымДолгам.СуммаЗадолженности - РасчетРезервовПоСомнительнымДолгам.ВстречнаяЗадолженность КАК СуммаСомнительногоДолга,
	...
	|	РасчетРезервовПоСомнительнымДолгам.СуммаЗадолженности - РасчетРезервовПоСомнительнымДолгам.ВстречнаяЗадолженность - РасчетРезервовПоСомнительнымДолгам.НачисленоРанееБУ - РасчетРезервовПоСомнительнымДолгам.НачисленоБУ + РасчетРезервовПоСомнительнымДолгам.ВосстановленоБУ КАК Остаток2,
	|	РасчетРезервовПоСомнительнымДолгам.СуммаЗадолженности - РасчетРезервовПоСомнительнымДолгам.ВстречнаяЗадолженность - РасчетРезервовПоСомнительнымДолгам.НачисленоРанееБУ - РасчетРезервовПоСомнительнымДолгам.НачисленоБУ + РасчетРезервовПоСомнительнымДолгам.ВосстановленоБУ - (РасчетРезервовПоСомнительнымДолгам.СуммаЗадолженности - РасчетРезервовПоСомнительнымДолгам.ВстречнаяЗадолженность - РасчетРезервовПоСомнительнымДолгам.НачисленоРанееНУ - РасчетРезервовПоСомнительнымДолгам.НачисленоНУ + РасчетРезервовПоСомнительнымДолгам.ВосстановленоНУ) КАК Остаток2ПР,
	|	ВЫБОР
	...
	|ИТОГИ
	|	СУММА(СуммаДолга),
	|	СУММА(СуммаВстречногоДолга),
	|	СУММА(СуммаСомнительногоДолга),
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь, НачалоПодписи, ПоказыватьПодписи) Экспорт
	...
	ВыводитьВстречнуюЗадолженность = (ЗначениеЗаполнено(ДатаКон) И ДатаКон >= '2017-01-01');
	Если Не ВыводитьВстречнуюЗадолженность Тогда
		Макет.УдалитьОбласть(Макет.Область("ВстречнаяЗадолженность" + ?(ВариантОтчета = 2, "НУ", "")), ТипСмещенияТабличногоДокумента.ПоГоризонтали);
	КонецЕсли;	
	...
	Если ВариантОтчета = 2 Тогда
		...
		Если ДатаНач >= '2017-01-01' И КонецМесяца(ДатаНач) < КонецГода(ДатаНач) Тогда
			// Подменим дату документа в СтруктураШапкиДокумента на конец предыдущего года
			СтруктураПараметров.Дата = НачалоГода(ДатаНач) - 1;
			СуммаВыручкиПредыдущегоГода = НалоговыйУчет.ОпределитьНормуРасходовПоВыручке(СтруктураПараметров, 10);
			СуммаВыручки = Макс(СуммаВыручкиПредыдущегоГода, СуммаВыручки);
		КонецЕсли;		
	...
	Пока ВыборкаПериод.Следующий() Цикл
		...
		ОбластьШапка.Параметры.НомерКолонки4 = ?(ВыводитьВстречнуюЗадолженность, "4а", "4");
