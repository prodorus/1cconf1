...
Функция ПолучитьТаблицуЧисленностей(Организация, НачалоПериода, КонецПериода, РазворачиватьЧисленностьПоМесяцам = истина) Экспорт
	...
	|	СУММА(ВЫБОР
	|			КОГДА ПериодыРаботы.ВыборкаЕсть ЕСТЬ НЕ NULL 
	...
	|		ГДЕ
	|			ПериодыРаботников.Организация = &Организация
	|			И ПериодыРаботников.НеВключаетсяВСреднююЧисленность = 0
	|		
	|		ОБЪЕДИНИТЬ
	|		
	|		ВЫБРАТЬ
	...
	|		ИЗ
	|			ПериодыДоговорников КАК ПериодыДоговорников
	|		ГДЕ
	|			ПериодыДоговорников.Организация = &Организация
	|			И ПериодыДоговорников.НеВключаетсяВСреднююЧисленность = 0) КАК ПериодыРаботы
	...
	|ГДЕ
	|	МесяцыОтчета.Организация = &Организация
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ПолучитьЧисленностиНаДату(Организация, ДатаАктуальности) Экспорт
	
	ПустойРезультат = Новый ТаблицаЗначений;
	ПустойРезультат.Колонки.Добавить("СписочнаяЧисленность");
	ПустойРезультат.Колонки.Добавить("ЧисленностьЖенщин");
	ПустойРезультат.Колонки.Добавить("ЧисленностьИнвалидов");
	...
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("НачалоПериода", НачалоМесяца(ДатаАктуальности));
	Запрос.УстановитьПараметр("КонецПериода", КонецМесяца(ДатаАктуальности));
	СписокСостояний = Новый Массив;
	СписокСостояний.Добавить(Перечисления.СостоянияРаботникаОрганизации.ОтпускПоУходуЗаРебенком);
	СписокСостояний.Добавить(Перечисления.СостоянияРаботникаОрганизации.ОтпускПоБеременностиИРодам);
	СписокСостояний.Добавить(Перечисления.СостоянияРаботникаОрганизации.ОтпускУчебныйНеоплачиваемый);
	...
	|				ТОГДА ПериодыРаботы.ЗанимаемыхСтавок
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ЧисленностьИнвалидов
	|ИЗ
	|	ПериодыРаботников КАК ПериодыРаботы
	|		ЛЕВОЕ СОЕДИНЕНИЕ СведенияОбИнвалидностиФизлиц КАК СведенияОбИнвалидностиФизлиц
	|		ПО ПериодыРаботы.МесяцОтчета = СведенияОбИнвалидностиФизлиц.Период
	|			И ПериодыРаботы.Физлицо = СведенияОбИнвалидностиФизлиц.Физлицо
	|ГДЕ
	|	ПериодыРаботы.Организация = &Организация
	|	И ПериодыРаботы.НеВключаетсяВСреднююЧисленность = 0
	|	И ПериодыРаботы.Дата = &НачалоПериода";
	
	Запрос.УстановитьПараметр("НачалоПериода", НачалоДня(ДатаАктуальности));
	Запрос.УстановитьПараметр("Организация", Организация);
	#Если ТолстыйКлиентОбычноеПриложение тогда
	ФормированиеПечатныхФорм.ЗапомнитьПараметрыЗапроса(Запрос, МассивЗапросов);
	#КонецЕсли
