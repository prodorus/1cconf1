#Если Клиент Тогда
	...
	Функция СформироватьТекстЗапроса(ВидЗапроса, ВидУчета)
		...
		Если Не Найти(ВидЗапроса, "ОсновнаяДеятельность") = 0
			ИЛИ (ВидЗапроса = "НалогНаПрибыль" И ВидУчета = "НУ") Тогда
		...
		Если ВидЗапроса = "НалогНаПрибыль" И ВидУчета = "БУ" Тогда
	...
	ДатаКон  = КонецМесяца(ПериодРегистрации);
		...
		Если ВариантОтчета = 2 Тогда
			...
			СчетаРасходовЕНВД = ?(ЭтоУПП, ПланыСчетов.Хозрасчетный.ПрочиеРасходыОблагаемыеЕНВД,ПланыСчетов.Налоговый.ПрочиеРасходы);
			Запрос.УстановитьПараметр("ВидУчета", Перечисления.ВидыУчетаПоПБУ18.НУ);
		Иначе
	...
	Если Не ПрименяетсяУСН Тогда	
		...
		Для Каждого Выборка ИЗ Результат Цикл
			...
			КонецЕсли;
				...
				Иначе
					Если Выборка.КорСчет = ПланыСчетов.Хозрасчетный.ПостоянноеНалоговоеОбязательство Тогда
						Параметр = "Постоянные налоговые обязательства (+), активы (-)";
					Иначе
						Параметр = Выборка.КорСчет.Наименование;
					КонецЕсли;
					ОбластьСтрокаПБУ18.Параметры.Параметр =   Параметр;
		...
		Если ВариантОтчета = 2 Тогда
			Выборка = НалогНаПрибыль.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "ОБЩИЕ");
			
			Если Выборка.Количество() > 0 Тогда
				СтавкаФБ = 0;
				СтавкаРФ = 0;
				СтруктураПараметров = Новый Структура("Организация, Дата", Организация, КонецДня(ДатаКон));
				Ставка = НалоговыйУчет.ПолучитьСтавкуНалогаНаПрибыль(СтруктураПараметров);
				
				Выборка.Следующий();
				
				ДоходГод = Выборка.ДоходГод;
				РасходГод = Выборка.РасходГод;
				ДоходМесяц = Выборка.ДоходМесяц;
				РасходМесяц = Выборка.РасходМесяц;
				
				НалогГод = (ДоходГод - РасходГод) * Ставка;
				НалогМесяц = (ДоходМесяц - РасходМесяц) * Ставка;
