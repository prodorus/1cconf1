...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КоличествоИнструкций >= 8 Тогда
		ДатаСтрокой = СОКРЛП(ФайлИнструкций.ПолучитьСтроку(8));
		Если ПустаяСтрока(ДатаСтрокой) Тогда
			МесяцНачалаЭксплуатации = Дата(2015,1,1);
		Иначе
			МесяцНачалаЭксплуатации	= Дата(Сред(ДатаСтрокой,7,4),Сред(ДатаСтрокой,4,2),Лев(ДатаСтрокой,2));
		КонецЕсли;
	КонецЕсли;
	Если КоличествоИнструкций >= 9 Тогда
		ПереноситьНачисленияПредыдущейПрограммы	= ?(Число(ФайлИнструкций.ПолучитьСтроку(9))=1,Истина,Ложь);
	КонецЕсли;
...
Процедура ПодготовитьОтчетОбОшибкахСервер(ОтчетОбОшибках)
	...
	Макет = Обработки.ПереносДанныхВНовуюПрограмму.ПолучитьМакет("СписокОшибок");
