...
Перем мВестиУчетПлатежейПоГосконтрактам Экспорт;
Перем КорневойЭлементМанифеста;
Перем ИмяФайлаМанифеста;
Перем ДокументыКВыгрузке;
...
Процедура РассчитатьКурсИСуммуВзаиморасчетов(СтрокаПлатеж, мТекущаяВалютаВзаиморасчетов, ДатаКурса, ОбъектДокумента)
	...
	Если ОбъектДокумента.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеНалога
		И Не ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента.ВалютаВзаиморасчетов) Тогда
		СтруктураКурсаВзаиморасчетов = МодульВалютногоУчета.ПолучитьКурсВалюты(ОбъектДокумента.ВалютаДокумента, ДатаКурса);
	Иначе
...
Функция ПолучитьТекстЗапросаПоПлатежноеТребованиеВыставленное()
	...
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	"""" КАК СрокДействия,
	               |	ВЫБОР
				   |		КОГДА ПлатежноеТребованиеВыставленное.УсловиеОплаты = Значение(Перечисление.УсловияОплатыРасчетныхДокументов.САкцептом)
...
Функция Выгрузить(ДокументыНаЭкспорт, ВерсияФормата = "1.02") Экспорт
	...
	Попытка
		...
		ТекстОшибки = "Не удалось записать данные в файл. Возможно, отсутствует каталог." + ФайлВыгрузки;
		...
		Если мВестиУчетПлатежейПоГосконтрактам Тогда
			ТекстОшибки = "Не удалось выгрузить подтверждающие документы";
			ДополнитьВыгрузкуПодтверждающимиДокументами();
		КонецЕсли;
...
// Стыковка выгрузки подтверждаемых документов
Процедура ДополнитьВыгрузкуПодтверждающимиДокументами()
	Сообщить("Выполняется выгрузка подтверждающих документов");
	
	// Подготовка
	
	// Таблица "ДокументыКВыгрузке"
	ДокументыКВыгрузке = ПлатежныеДокументы.Выгрузить();
	
	НовыеИменаКолонок = Новый Структура("Документ, СуммаДокумента, ОрганизацияСчет",
		"Ссылка", "Сумма", "БанковскийСчет");
	Для Каждого НовоеИмяКолонки Из НовыеИменаКолонок Цикл
		ДокументыКВыгрузке.Колонки[НовоеИмяКолонки.Ключ].Имя = НовоеИмяКолонки.Значение;
	КонецЦикла;		
	
	ДокументыКВыгрузке.Колонки.Добавить("ЗаявкаНаРасходованиеДенежныхСредств");
	ЗаполнитьКолонкуЗаявка();
	
	// Таблица "БанковскиеСчета"
	БанковскиеСчета = СоздатьТаблицуБанковскихСчетов();
	СтрокаСчета = БанковскиеСчета.Добавить();
	ЗаполнитьСтрокуСчетаПоУмолчанию(СтрокаСчета, БанковскийСчет);
	
	// Выгрузка
	ВыгрузитьПодтверждающиеДокументы(БанковскиеСчета);
	
	Сообщить("Подтверждающие документы успешно выгружены");
КонецПроцедуры

Функция СоздатьТаблицуБанковскихСчетов()
	
	БанковскиеСчета = Новый ТаблицаЗначений();
	СтрокаИменКолонок = "Ссылка,Пометка,Выгружается,ПрямойОбмен,НастройкаОбмена,Банк,";
	...
	Для Каждого ИмяКолонки Из МассивИменКолонок Цикл
		БанковскиеСчета.Колонки.Добавить(ИмяКолонки);
	КонецЦикла;
...
Процедура ЗаполнитьСтрокуСчетаПоУмолчанию(СтрокаСчета, СсылкаНаСчет)
	
	СтрокаСчета.Ссылка 	        = СсылкаНаСчет;
	СтрокаСчета.Пометка         = Истина;
	СтрокаСчета.Выгружается     = Ложь;
	СтрокаСчета.ПрямойОбмен     = Ложь;
	СтрокаСчета.НастройкаОбмена = Новый Структура("НастройкаОбмена", "НастройкаОбмена");
	СтрокаСчета.Банк            = СсылкаНаСчет.Банк;
	
КонецПроцедуры

Процедура ЗаполнитьКолонкуЗаявка()
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ
	               |	ДокументыКВыгрузкеТЗ.Ссылка,
	               |	ДокументыКВыгрузкеТЗ.НомерСтроки
	               |ПОМЕСТИТЬ ДокументыКВыгрузкеВТ
	               |ИЗ
	               |	&ДокументыКВыгрузкеТЗ КАК ДокументыКВыгрузкеТЗ
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ПлатежноеПоручениеИсходящееРасшифровкаПлатежа.Ссылка,
	               |	МАКСИМУМ(ПлатежноеПоручениеИсходящееРасшифровкаПлатежа.ДокументПланированияПлатежа) КАК ДокументПланированияПлатежа
	               |ПОМЕСТИТЬ ДокументыПланированияПлатежаВТ
	               |ИЗ
	               |	Документ.ПлатежноеПоручениеИсходящее.РасшифровкаПлатежа КАК ПлатежноеПоручениеИсходящееРасшифровкаПлатежа
	               |ГДЕ
	               |	ПлатежноеПоручениеИсходящееРасшифровкаПлатежа.Ссылка.Дата МЕЖДУ &НачПериода И &КонПериода
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ПлатежноеПоручениеИсходящееРасшифровкаПлатежа.Ссылка
	               |
	               ...
	               |	МАКСИМУМ(ПлатежныйОрдерСписаниеДенежныхСредствРасшифровкаПлатежа.ДокументПланированияПлатежа)
	               |ИЗ
	               ...
	               |ГДЕ
	               |	ПлатежныйОрдерСписаниеДенежныхСредствРасшифровкаПлатежа.Ссылка.Дата МЕЖДУ &НачПериода И &КонПериода
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ПлатежныйОрдерСписаниеДенежныхСредствРасшифровкаПлатежа.Ссылка
	               |;
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ДокументыКВыгрузкеВТ.НомерСтроки,
	               |	ДокументыКВыгрузкеВТ.Ссылка,
	               |	ЕСТЬNULL(ДокументыПланированияПлатежаВТ.ДокументПланированияПлатежа, ЗНАЧЕНИЕ(Документ.ЗаявкаНаРасходованиеСредств.ПустаяСсылка)) КАК ЗаявкаНаРасходованиеДенежныхСредств
	               |ИЗ
	               |	ДокументыКВыгрузкеВТ КАК ДокументыКВыгрузкеВТ
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ДокументыПланированияПлатежаВТ КАК ДокументыПланированияПлатежаВТ
	               |		ПО ДокументыКВыгрузкеВТ.Ссылка = ДокументыПланированияПлатежаВТ.Ссылка";
	Запрос.УстановитьПараметр("ДокументыКВыгрузкеТЗ", ДокументыКВыгрузке);
	Запрос.УстановитьПараметр("НачПериода", НачалоДня(НачПериода));
	Запрос.УстановитьПараметр("КонПериода", КонецДня (КонПериода));
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	Пока Выборка.Следующий() Цикл
		ДокументыКВыгрузке[Выборка.НомерСтроки - 1].ЗаявкаНаРасходованиеДенежныхСредств = 
...
Процедура ВыгрузитьПодтверждающиеДокументы(БанковскиеСчета)
	...
	Для Каждого Счет из БанковскиеСчета Цикл
		...
		СчетаБанка = СоответствиеБанковИСчетов[Счет.Банк];
		Если СчетаБанка = Неопределено Тогда
			СчетаБанка = Новый Массив;
		КонецЕсли;
		СчетаБанка.Добавить(Счет);
		
		СоответствиеБанковИСчетов.Вставить(Счет.Банк, СчетаБанка);
	КонецЦикла;
...
Процедура ЗаписатьПодтверждающиеДокументы(Манифест, Банк)
	...
	НаименованиеБанка = СокрЛП(ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(НаименованиеБанка));
	
	КаталогВыгрузки = Константы.КаталогВыгрузкиПодтверждающихДокументов.Получить();
	КаталогВыгрузки = КаталогВыгрузки + РазделительКаталогов(КаталогВыгрузки);
	...
	Если ЗначениеЗаполнено(КаталогВыгрузки) Тогда
		...
		Для Каждого МанифестПорции Из МанифестыПоПорциям Цикл 
			...
			Архив = Новый ЗаписьZipФайла(КаталогВыгрузки + Банк.Код + " " + НаименованиеБанка + СуффиксПорции + ".zip",,,, УровеньСжатияZIP.Максимальный);
			Архив.Добавить(КаталогВременныхФайлов + ИмяФайлаМанифеста);
			...
			Для Каждого НайденныйКаталог Из НайденныеКаталоги Цикл
				...
				ФайлыКаталога = НайтиФайлы(ИсходныйКаталогФайлов, МаскаВсеФайлы());
				...
				Архив.Добавить(ИсходныйКаталогФайлов, РежимСохраненияПутейZIP.СохранятьОтносительныеПути);
...
Процедура ЗаписатьФайлыПодтверждающихДокументов(Манифест, КаталогВыгрузки, РезультатЗаписиДокументов)
	...
	Для Каждого ДокументКВыгрузке Из Манифест[КорневойЭлементМанифеста]["Doc"] Цикл 
		...
		Для Каждого ПодтверждающийДокумент Из ДокументКВыгрузке["embDoc"] Цикл
			...
			КаталогиКЗаписи.Свойство(ИмяУзлаИдентификаторовВыгрузки, МассивИдентификаторовФайлов);
			Если МассивИдентификаторовФайлов = Неопределено Тогда
				МассивИдентификаторовФайлов = Новый Массив;
			КонецЕсли;
			МассивИдентификаторовФайлов.Добавить(ПодтверждающийДокумент["DocID"]);
			
			КаталогиКЗаписи.Вставить(ИмяУзлаИдентификаторовВыгрузки, МассивИдентификаторовФайлов);
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого КаталогКЗаписи Из КаталогиКЗаписи Цикл 
		...
		Для Каждого ДокументКЗаписи Из КаталогКЗаписи.Значение Цикл 
			СсылкаНаФайл = Справочники.ХранилищеДополнительнойИнформации.ПолучитьСсылку(Новый УникальныйИдентификатор(ДокументКЗаписи));
			
			Файл = СсылкаНаФайл.ПолучитьОбъект().Хранилище.Получить(); // это двоичные данные файла
			Файл.Записать(КаталогЗаписиДокумента + СсылкаНаФайл.ИмяФайла);
			
			НовыйРазмер = ТаблицаРазмеров.Добавить();
			НовыйРазмер["Subfolder"] = ИмяПодкаталогаЗаписи;
			НовыйРазмер["DocId"] = ДокументКЗаписи;
			НовыйРазмер["СсылкаНаФайл"] = СсылкаНаФайл;
			НовыйРазмер["ПолноеИмяФайла"] = КаталогЗаписиДокумента + СсылкаНаФайл.ИмяФайла;
			НовыйРазмер["РазмерВБайтах"] = Файл.Размер();
			
			СуммарныйРазмерВБайтах = СуммарныйРазмерВБайтах + Файл.Размер();
		КонецЦикла;
	КонецЦикла;
...
Процедура ДобавитьИнформациюОПлатеже(МанифестБанка, ДокументыКВыгрузкеПоБанку)
	Для Каждого Документ Из ДокументыКВыгрузкеПоБанку Цикл
		...
		НомерБезЛидирующихНулей = ОбщегоНазначения.ПолучитьНомерНаПечать(ДокументСсылка);
		МанифестДокумента.Вставить("No", НомерБезЛидирующихНулей);
		МанифестДокумента.Вставить("Date", XMLДата(Документ.Дата));
		МанифестДокумента.Вставить("Summ", Документ.Сумма);
		...
		ЗаполнитьЗначенияСвойств(МанифестДокумента, 
			ИдентификаторИПредставлениеТипаПлатежа(Документ));
		
		МанифестДокумента.Вставить("IGK", Документ.БанковскийСчет.ГосударственныйКонтракт.Код);
		
		МанифестБанка[КорневойЭлементМанифеста]["Doc"].Добавить(МанифестДокумента);
		
		ДобавитьИнформациюОПодтверждающихДокументах(МанифестДокумента, Документ);
	КонецЦикла;
КонецПроцедуры
...
Процедура ДобавитьИнформациюОПодтверждающихДокументах(МанифестДокумента, Документ)
	...
	Для Каждого ПодтверждающийДокумент Из ПодтверждающиеДокументы Цикл 
		...
		МанифестПодтверждающегоДокумента.Вставить("DocId", Строка(ПодтверждающийДокумент.Файл.УникальныйИдентификатор()));
		МанифестПодтверждающегоДокумента.Вставить("No", ПодтверждающийДокумент.Номер);
		МанифестПодтверждающегоДокумента.Вставить("Date", XMLДата(ПодтверждающийДокумент.Дата));
		
		// Заполнение свойств IdDoc, DocName
		ЗаполнитьЗначенияСвойств(МанифестПодтверждающегоДокумента, 
			ИдентификаторИПредставлениеВидаДокумента(ПодтверждающийДокумент.ВидДокумента));
		
		МанифестПодтверждающегоДокумента.Вставить("FileName", СвойстваФайла(ПодтверждающийДокумент.Файл));
...
Функция ПорцииЗаписи(РезультатЗаписиДокументов, КаталогВременныхФайлов)
	...
	ПроверитьСоздатьКаталог(КаталогВременныхФайлов + "ПроверкаРазмера\");
	ИмяВременногоАрхива = КаталогВременныхФайлов + "ПроверкаРазмера\" + "ПроверочныйФайл.zip";
	...
	УдалитьФайлы(КаталогВременныхФайлов + "ПроверкаРазмера\");
	
	Возврат ПорцииЗаписи;
КонецФункции

Функция МанифестПоУмолчанию()
	Манифест = Новый Структура(КорневойЭлементМанифеста, 
		Новый Структура("DateTimeFile_Attribute, Author_Attribute, Doc", 
			XMLСтрока(Дата("00010101010101")), // Пустая дата
			Метаданные.Синоним, 
			Новый Массив));
...
Функция МанифестДокументаПоУмолчанию()
	Манифест = Новый Структура("No, Date, Summ, Subfolder, IdPayment, PaymentType, IGK, embDoc",,,,,,,, Новый Массив);
	
	Возврат Манифест;
КонецФункции

Функция МанифестПодтверждающегоДокументаПоУмолчанию()
	Манифест = Новый Структура("DocId, No, Date, IdDoc, DocName, FileName",,,,,, Новый Структура);
...
Процедура ЗаписатьЭлементыXML(ЗаписьXML, Элемент, ИмяЭлемента)
	...
	ИначеЕсли ТипЗнч(Элемент) = Тип("Массив") Тогда
		...
		КонецЦикла;
	ИначеЕсли Найти(ИмяЭлемента, "_Attribute") = 0 Тогда // Кроме атрибутов
		ЗаписьXML.ЗаписатьНачалоЭлемента(ИмяЭлемента);
...
Процедура ЗаписатьАтрибутыXML(ЗаписьXML, Элемент)
	Для Каждого ЭлементыЭлемента Из Элемент Цикл 
		Если Найти(ЭлементыЭлемента.Ключ, "_Attribute") > 0 Тогда
			ЗаписьXML.ЗаписатьАтрибут(СтрЗаменить(ЭлементыЭлемента.Ключ, "_Attribute", ""), XMLСтрока(ЭлементыЭлемента.Значение));
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция РазделительКаталогов(Каталог)
	Возврат ?(СтрокаЗаканчиваетсяНа(Каталог, "\"), "", "\");
КонецФункции

Функция СвойстваФайла(Файл)
	СвойстваФайла = МанифестФайлаПоУмолчанию();
	
	ИнфОФайле = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(Файл.ИмяФайла);
	ИнфОФайле.Расширение = Прав(ИнфОФайле.Расширение, СтрДлина(ИнфОФайле.Расширение)-1);
	ИнфОФайле.Вставить("Размер", Файл.Хранилище.Получить().Размер());
	
	СвойстваФайла.Вставить("FileName_Attribute", ИнфОФайле.ИмяБезРасширения);
	СвойстваФайла.Вставить("TypeFile_Attribute", ИнфОФайле.Расширение);
	СвойстваФайла.Вставить("SizeFile_Attribute", ИнфОФайле.Размер);
	
	Возврат СвойстваФайла;
КонецФункции

Функция XMLДата(Дата)
	Возврат Строка(Формат(Год(Дата),"ЧГ=0")) + "-" + Строка(Формат(Месяц(Дата), "ЧЦ=2; ЧВН=")) + "-" + Строка(Формат(День(Дата), "ЧЦ=2; ЧВН="));
КонецФункции
...
Функция ИдентификаторИПредставлениеТипаПлатежа(Документ)
	...
	Если ЗначениеЗаполнено(ЗаявкаНаРасходованиеДенежныхСредств) Тогда
		ИдентификаторИПредставлениеТипаПлатежа.Вставить("IdPayment", СокрЛП(ЗаявкаНаРасходованиеДенежныхСредств.ТипПлатежаФЗ275.Код));
		ИдентификаторИПредставлениеТипаПлатежа.Вставить("PaymentType", ЗаявкаНаРасходованиеДенежныхСредств.ТипПлатежаФЗ275.Наименование);
	КонецЕсли;
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента)
	...
	Если ВсеРеквизитыПоискаЕсть Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			Если ВыборкаНомер = НомерДляПоискаДок И (НЕ ЗначениеЗаполнено(Результат.НомерСчета) ИЛИ Результат.НомерСчета = СчетДляПоискаДок)
				И Результат.КонтрагентИНН = ?(Исходяший, СтрокаДокумента.ПолучательИНН, СтрокаДокумента.ПлательщикИНН) Тогда
