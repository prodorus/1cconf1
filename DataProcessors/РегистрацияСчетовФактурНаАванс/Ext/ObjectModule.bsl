...
Перем СписокТиповДокументовАванса;
...
Процедура ЗаполнитьСтрокиДокумента(ТаблицаРезультатов) Экспорт
	...
	// С 31 декабря 2005 г. регистрация авансов по реализации на экспорт не производится
	...
	Авансы = ТаблицаАвансовТекущая.Скопировать(, "ДокументОснование, Контрагент, ДокументАвансаДата, СтавкаНДС, ДоговорКонтрагента");
	Авансы.Свернуть("ДокументОснование, Контрагент, ДокументАвансаДата, СтавкаНДС, ДоговорКонтрагента");
	...
	ТаблицаАвансовТекущая.Индексы.Добавить("ДокументОснование, Контрагент, ДокументАвансаДата, СтавкаНДС");
	ТаблицаАвансов = ТаблицаАвансовТекущая.СкопироватьКолонки();
	
	ПоискПартии = Новый Структура("ДокументОснование, Контрагент, ДокументАвансаДата, СтавкаНДС, ДоговорКонтрагента");
				...
				Если СуммаАванса <> 0 Тогда
					...
					Если Строка.ДатаВыписки = '00010101' Тогда
						Если ТипЗнч(Строка.ДокументОснование) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
							Строка.ДатаВыписки = Строка.ДокументАвансаДата;
						Иначе	
	...
	Если ТаблицаАвансов <> Неопределено 
		...
		Для Каждого Выборка Из ТаблицаАвансов Цикл
			...
			РеализацияТоваровОблагаемыхНДСУПокупателя = УчетНДС.РеализацияТоваровОблагаемыхНДСУПокупателя(Выборка.ДокументАвансаДата, Выборка.ДоговорКонтрагента);
			...
			Если ЗначениеЗаполнено(ДокАванса) 
				...
				ТаблицаПоСтавкам.Колонки.Добавить("СчетНаОплату", 
					Новый ОписаниеТипов("ДокументСсылка.СчетНаОплатуПокупателю, ДокументСсылка.ЗаказПокупателя"));
				...
				Если ТипЗнч(ДокАванса) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
					...
					Если Выборка.СтавкаНДС <> NULL Тогда
						...
						Если СоответствиеСтавок[Выборка.СтавкаНДС] = Неопределено Тогда
							ТекСтавкаНДС = Выборка.СтавкаНДС;
						Иначе
							ТекСтавкаНДС = СоответствиеСтавок[Выборка.СтавкаНДС];
						КонецЕсли; 
				...
				Иначе
					...
					ЕстьСделка = ДокАвансаМетаданные.ТабличныеЧасти.РасшифровкаПлатежа.Реквизиты.Найти("Сделка") <> Неопределено;
					ЕстьДокументРасчетов = ДокАвансаМетаданные.ТабличныеЧасти.РасшифровкаПлатежа.Реквизиты.Найти("ДокументРасчетовСКонтрагентом") <> Неопределено;
					...
					Для Каждого СтрПлатежа Из ДокАванса.РасшифровкаПлатежа Цикл
						Если СтрПлатежа.ДоговорКонтрагента = Выборка.ДоговорКонтрагента Тогда
							...
							Если ЕстьДокументРасчетов
								И ЗначениеЗаполнено(СтрПлатежа.ДокументРасчетовСКонтрагентом)
								И СтрПлатежа.ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом Тогда
								Продолжить;
							КонецЕсли;
				...
				Для Каждого СтрокаПоСтавке Из ТаблицаПоСтавкам Цикл
					...
					СтавкаНДС					= УчетНДС.ОбщаяРасчетнаяСтавкаНДС(Выборка.ДокументАвансаДата);
					СтрокаТП.СтавкаНДС			= ?(РеализацияТоваровОблагаемыхНДСУПокупателя, СтавкаНДС, СтрокаПоСтавке.СтавкаНДС);
					...
					СтрокаТП.СуммаНДС = ?(РеализацияТоваровОблагаемыхНДСУПокупателя, 0, УчетНДС.РассчитатьСуммуНДС(СтрокаТП.Сумма, Истина, Истина, УчетНДС.ПолучитьСтавкуНДС(СтрокаТП.СтавкаНДС)));
			...
			Иначе
				...
				СтрокаТП.СчетФактура		= Выборка.СчетФактура;
				СтрокаТП.ПорядокРегистрацииСчетовФактурНаАванс	= Выборка.ПорядокРегистрацииСчетовФактурНаАванс;
				СтрокаТП.СуммаНДС = УчетНДС.РассчитатьСуммуНДС(СтрокаТП.Сумма, Истина, Истина, УчетНДС.ПолучитьСтавкуНДС(СтрокаТП.СтавкаНДС));
...
Процедура ПроверитьДатуВыпискиПоСтроке(СтрокаТаблицы)
	...
	ИначеЕсли СтрокаТаблицы.ПорядокРегистрацииСчетовФактурНаАванс = Перечисления.ПорядокРегистрацииСчетовФактурНаАванс.КромеЗачтенныхВТечениеПятиДней 
		...
		Или СтрокаТаблицы.Дата - СтрокаТаблицы.ДатаДокументаОснования > 4 * 86400) Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Строка №" + (СтрокаТаблицы.НомерСтроки) + ": счет-фактура должен быть выписан не позднее " + ?(КонецМесяца(СтрокаТаблицы.ДатаДокументаОснования) > КонецМесяца(КонецНалоговогоПериода), 
...
Процедура ОпределитьРанееВыписанныеСчетаФактуры(ТаблицаАвансов)
	...
	Если ТаблицаАвансов.Количество() > 0 Тогда
		...
		|	НАЧАЛОПЕРИОДА(СчетФактураВыданный.Ссылка.Дата, ДЕНЬ) КАК ДокументАвансаДата,
		...
		Если СФПоДокументам.Количество() > 0 Тогда
			...
			СтруктураОтбора = Новый Структура("ДокументОснование, Контрагент, ДокументАвансаДата, ДоговорКонтрагента");
			СтруктураОтбораБезДаты = Новый Структура("ДокументОснование, Контрагент, ДоговорКонтрагента");
			СтруктураОтбораСводно = Новый Структура("ДокументОснование, ДокументАвансаДата");
			...
			Для Каждого СФ Из СФПоДокументам Цикл
				Если СФ.ВыписыватьСчетаФактурыСводно Тогда
					...
					Для Каждого СтрокаОтбора Из СтрокиОтбора Цикл
						...
						СтрокаОтбора.ДатаВыписки = СФ.ДокументАвансаДата;
					КонецЦикла;
				Иначе
			...
			СтруктураОтбора = Новый Структура("ДокументОснование, Контрагент, ДатаВыписки, ДоговорКонтрагента");
			СтруктураОтбора.ДатаВыписки = '00010101';
				...
				Если СчФ = Неопределено Тогда
					...
					Если УчетНДС.ПолучитьВерсиюПостановления(СчФ.Дата) = 2 Тогда
						...
						ИначеЕсли ЗначениеЗаполнено(СчФ.ДоговорКонтрагента)
							И УчетНДС.РеализацияТоваровОблагаемыхНДСУПокупателя(СчФ.Дата, СчФ.ДоговорКонтрагента) Тогда
					...
					ДокументОснование = СтрокаСФ.ДокументОснование;
					
					СчФ.ДокументыОснования.Очистить();
					СчФ.ДокументыОснования.Добавить().ДокументОснование = ДокументОснование;
					...
					Если ТипОснования = Тип("ДокументСсылка.ПлатежноеПоручениеВходящее")
						...
						Если ТипОснования = Тип("ДокументСсылка.ПлатежноеПоручениеВходящее") 
							...
							И УчетНДС.ВедетсяУчетНДСПоФЗ56(СчФ.Дата) Тогда
							СчФ.ИдентификаторГосКонтракта = ДокументОснование.СчетОрганизации.ГосударственныйКонтракт.Код;
						КонецЕсли;
					ИначеЕсли ТипОснования = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") 
						...
						СчФ.НомерПлатежноРасчетногоДокумента = ОбщегоНазначения.ПолучитьНомерНаПечать(ДокументОснование);
						СчФ.ДатаПлатежноРасчетногоДокумента =  ДокументОснование.Дата;
...
Для каждого ТипДокументаАванса Из Массив_ТипыДокументовАванса Цикл
	...
	ИначеЕсли ОписаниеТипов_ДокументыРасчетовСПокупателями.СодержитТип(ТипДокументаАванса) Тогда
		СписокТиповДокументовАванса.Добавить(Метаданные.НайтиПоТипу(ТипДокументаАванса).Имя);
	КонецЕсли; 
КонецЦикла;
