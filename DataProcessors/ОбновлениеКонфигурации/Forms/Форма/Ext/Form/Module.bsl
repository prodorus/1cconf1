...
КонецФункции	
////////////////////////////////////////////////////////////////////////////////
// ВСПОМОГАТЕЛЬНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ (КЛИЕНТ)
...
Процедура СохранитьНастройкиОбновленияКонфигурации()
	...
	Если ПараметрыРаботыКлиента.НастройкиОбновления.ЕстьДоступДляОбновления 
		...
		АдминистрированияИБ = СоединенияИБКлиентСервер.НовыеПараметрыАдминистрированияИБ();
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено) 
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ОбновленияНеОбнаружено Тогда
		...
		Если СтроковыеФункцииКлиентСервер.СравнитьВерсии(СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ВерсияКонфигурации,
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.ВыборРежимаОбновленияСервер Тогда
		...
		Если Объект.КодЗадачиПланировщика = 0 И НЕ ДатаВремяОбновленияУстановлена Тогда
			Объект.ДатаВремяОбновления		= ВернутьДату(ОбновлениеКонфигурацииКлиентСервер.ДобавитьДни(
				НачалоДня(ТекущаяДата()), 1), Объект.ДатаВремяОбновления);
...
Функция ПроверитьДопустимуюДатуОбновления(ДатаВремя, ВыдаватьСообщения = Истина)
	...
	Если ДатаВремя < Сейчас Тогда
		Если ВыдаватьСообщения Тогда
			Предупреждение(НСтр("ru = 'Обновление конфигурации может быть запланировано только на будущую дату и время.'"));
		КонецЕсли;	
		Возврат Ложь;
	КонецЕсли;
	Если ДатаВремя > ДобавитьМесяц(Сейчас, 1) Тогда
		Если ВыдаватьСообщения Тогда
			Предупреждение(НСтр("ru = 'Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.'"));
		КонецЕсли;	
		Возврат Ложь;
	КонецЕсли;
		...
		Если Файл.Значение <> Неопределено И Файл.Значение.Получен <> Истина Тогда	// может быть также Неопределено
			Если Объект.ИсточникОбновления = 0 Тогда
				...
				Если Не РезультатПроверкиЛегальности.ЗначениеРезультата Тогда
					...
					ТекстОшибки = "Не удалось подтвердить легальность получения обновления через Интернет по причине:
					|%1";
					ТекстОшибки = ОбновлениеКонфигурацииКлиент.ПодставитьПараметрыВСтроку(ТекстОшибки, РезультатПроверкиЛегальности.ТекстОшибки);
					
					Объект.ТехническаяИнформацияОбОшибке = ?(ПустаяСтрока(Объект.ТехническаяИнформацияОбОшибке), ТекстОшибки, 
					Объект.ТехническаяИнформацияОбОшибке + "; " + ТекстОшибки);	
...
Функция ПроверитьОбновлениеИТС(ВыдаватьСообщения = Истина)
	...
	Если СтроковыеФункцииКлиентСервер.СравнитьВерсии(ТекущаяВерсияКонфигурации, ПоследняяВерсияКонфигурации) >= 0 Тогда  
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ПараметрыПроверкиЛегальности()
	...
	ИдентификаторИнформационнойБазы = ОбщегоНазначения.ИдентификаторИнформационнойБазы();
