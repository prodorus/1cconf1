...
Процедура ИзменитьТипЛогистическойЕдиницыПоТипуУпаковки(ЗаполнениеПоПереданномуШтрихкоду = Ложь)
	Если ТипШтрихкодаСтрокой = "Code128Формат2" Тогда
		...
		Для Каждого ЭлементСписка Из ПолныйСписокВыбора Цикл
			НовыйЭлементСписка = Элементы.ГруппаРеквизитыCode128Формат2ТипЛогистическойЕдиницы.СписокВыбора.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
		КонецЦикла;
...
Процедура ПолучитьВес(Команда)
	...
	ОписаниеОповещения = ОписаниеОповещенияЕГАИС("ПолучитьВесЗавершение", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ПечатьЭтикетокШтрихкода(Команда)
	...
	ШтрихкодыУпаковокКлиентПереопределяемый.ПечатьШтрихкодовУпаковок(ОбъектыПечати, ЭтаФорма);
...
Процедура Подключаемый_ВыполнитьКоманду(Команда)
	ПодключаемыеКомандыКлиент.ВыполнитьКоманду(ЭтаФорма, Команда, Элементы.СписокДокументыЗакупки);
КонецПроцедуры

&НаСервере
Процедура Подключаемый_ВыполнитьКомандуНаСервере(Контекст, Результат)
	ПодключаемыеКоманды.ВыполнитьКоманду(ЭтаФорма, Контекст, Элементы.СписокДокументыЗакупки, Результат);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ОбновитьКоманды()
	ПодключаемыеКомандыКлиентСервер.ОбновитьКоманды(ЭтаФорма, Элементы.СписокДокументыЗакупки);
КонецПроцедуры
...
Процедура ГенерацияШтрихкода()
	...
	Если ПревышенаМаксимальнаяДлинаШтрихкода Тогда
		...
		Информация = ФорматированнаяСтрока(МассивСтрок);
	Иначе
...
Процедура НайтиИПрочитатьПредыдущиеНастройкиШтрихкодов(НастройкиЗаполнения, ПромежуточныеРезультаты)
	...
	Если ПоследнееЗначениеШтрихкода = Неопределено Тогда
		...
		Иначе
			...
			"Таблица.ТипШтрихкод В (ЗНАЧЕНИЕ(Перечисление.ТипыШтрихкодов.SSCC),
...
Функция ПолучитьИдентификаторыОрганизации(ТипШтрихкодаСтрокой, Знач ВключаяДату = Неопределено)
	...
	Возврат ИнтеграцияЕГАИСПереопределяемый.ИдентификаторыОрганизации(ВключаяДату, ТолькоКодФСРАР);
...
Функция ПроверкаКлиент()
	...
	ИначеЕсли ТипШтрихкодаСтрокой = "Code128Формат1" Тогда
		...
		КонецЕсли;
		//Если НЕ ЗначениеЗаполнено(НомерПлощадкиМаркировки) Тогда
		//	ТекстСообщения = НСтр("ru = 'Не заполнено поле ""Номер площадки маркировки""'");
		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"НомерПлощадкиМаркировки",,Отказ);
		//КонецЕсли;
