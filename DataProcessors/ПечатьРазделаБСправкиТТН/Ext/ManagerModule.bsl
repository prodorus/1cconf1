...
Функция ПечатьТТНСправкаРазделБ(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	|		И ТТНИсходящаяЕГАИС.Грузоотправитель.Контрагент <> Неопределено
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		ТТНИсходящаяЕГАИС.Ссылка В(&МассивДокументов)
	|		И ТТНИсходящаяЕГАИС.Грузополучатель.Контрагент <> Неопределено) КАК ВложенныйЗапрос";
	...
	Запрос.УстановитьПараметр("ПустаяОрганизация"       , Неопределено);
