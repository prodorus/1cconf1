...
Процедура ЗаполнитьДанныеСправок(НовыйДок, НомерПервойСправки)
	...
	Если СправкиПоНДФЛ.ВыводитьФорму2НДФЛ2019Года(НовыйДок.НалоговыйПериод, НовыйДок.Дата) Тогда
		НовыйДок.мЗаписиОДоходах = РегистрыСведений.СправкиНДФЛДляИФНССведенияОДоходах.СоздатьНаборЗаписей();
		НовыйДок.мЗаписиОДоходах.Загрузить(СтруктураДанных.Доходы);
		НовыйДок.мЗаписиОВычетах = РегистрыСведений.СправкиНДФЛДляИФНССведенияОВычетах.СоздатьНаборЗаписей();
		НовыйДок.мЗаписиОВычетах.Загрузить(СтруктураДанных.Вычеты);
	Иначе 
...
Процедура ЗаполнитьТаблицуРеестров() Экспорт 
	...
	Пока Выборка.Следующий() Цикл
		...
		Документы2НДФЛОбъект = Выборка.Ссылка.ПолучитьОбъект();
		Если Выборка.Порядок = 1 Тогда
			СведенияОДоходаИВычетах = СправкиПоНДФЛПолныеПрава.СведенияОДоходахИВычетахДокумента(Выборка.Ссылка);
			Документы2НДФЛОбъект.мЗаписиОДоходах = РегистрыСведений.СправкиНДФЛДляИФНССведенияОДоходах.СоздатьНаборЗаписей();
			Документы2НДФЛОбъект.мЗаписиОДоходах.Загрузить(СведенияОДоходаИВычетах.СведенияОДоходах);
			Документы2НДФЛОбъект.мЗаписиОВычетах = РегистрыСведений.СправкиНДФЛДляИФНССведенияОВычетах.СоздатьНаборЗаписей();
			Документы2НДФЛОбъект.мЗаписиОВычетах.Загрузить(СведенияОДоходаИВычетах.СведенияОВычетах);
		КонецЕсли;
		СоответствиеСсылокИОбъектов.Вставить(Выборка.Ссылка, Документы2НДФЛОбъект);
	КонецЦикла;
	...
	Пока Выборка.СледующийПоЗначениюПоля(ИмяКодаТерритории) Цикл
		...
		Пока Выборка.СледующийПоЗначениюПоля("Порядок") Цикл
			Пока Выборка.Следующий() Цикл
				...
				Если Не СправкиПоНДФЛ.ВыводитьФорму2НДФЛ2019Года(СтруктураРеквизитов.НалоговыйПериод, СтруктураРеквизитов.Дата)
					И НовыйДок.СотрудникиОрганизации.Количество() = 3000 Тогда
					ЗаполнитьДанныеСправок(НовыйДок, НомерПервойСправки);
					НомерПервойСправки = НомерПервойСправки + НовыйДок.СотрудникиОрганизации.Количество();
					НовыйДок = СоздатьРеестр2НДФЛ(СтруктураРеквизитов);
					ЗаполнитьЗначенияСвойств(НовыйДок,Выборка,ИмяКодаТерритории);
