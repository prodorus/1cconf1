...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	СобытияФормЕГАИСКлиент.ВнешнееСобытиеПолученыШтрихкоды(ОповещениеПриЗавершении, ЭтаФорма, Источник, Событие, Данные, ПараметрыСканированияАкцизныхМарок);
...
&НаКлиенте
Процедура ДекорацияОтсканируйтеУпаковкуАлкогольнойПродукцииОбработкаНавигационнойСсылки(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	НавигационнаяСсылкаФорматированнойСтроки = "ОтметитьЧтоАлкогольнаяПродукцияНайденаВнеУпаковки";
...
Функция ТекстОтложите()
	...
	Если СтатусПроверки = ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияАлкогольнойПродукции.Отложена") Тогда
		...
		Возврат СтрШаблон(НСтр("ru = '%1 уже отложена и помечена стикером № ""%2"".'"), 
			              ТекстУпаковка, 
			              Формат(СледующийСтикерОтложено - 1, "ЧДЦ=; ЧГ=0"));
...
Процедура СформироватьИнформациюОРезультатахПоиска()
	...
	СтрокаШтрихкод = ФорматированнаяСтрока("""" + ПредставлениеШтрихкода + """", Новый Шрифт(,,Истина));
	...
	Иначе
		...
		Префикс  = НСтр("ru = 'Штрихкод'");
		Постфикс = НСтр("ru = 'в данных документа не найден.'");
		
	КонецЕсли;
	
	МассивСтрок = Новый Массив();
	МассивСтрок.Добавить(Префикс);
	МассивСтрок.Добавить(СтрокаШтрихкод);
	МассивСтрок.Добавить(Постфикс);
	
	ИнформацияОРезультатахПоиска = Новый ФиксированныйМассив(МассивСтрок);
	
	ИнтеграцияЕГАИСУТ.СформироватьФорматированнуюСтроку(ЭтаФорма,
		ИнформацияОРезультатахПоиска,
		"ГруппаИнформацияОРезультатахПоиска",
		"");
...
Процедура Отметить(Команда)
	...
	СтандартнаяОбработка = Ложь;
	ДекорацияОтсканируйтеУпаковкуОбработкаНавигационнойСсылки(Неопределено, "ОтметитьЧтоУпаковкаБезУпаковки", СтандартнаяОбработка);
