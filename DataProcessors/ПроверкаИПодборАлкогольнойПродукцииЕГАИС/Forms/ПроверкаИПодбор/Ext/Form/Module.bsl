...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если РежимПроверки <> Неопределено Тогда
		СформироватьПредставлениеРежимаПроверки();
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не ВыполняетсяЗакрытие Тогда
		...
		Если Модифицированность Тогда
			...
			ОписаниеОповещенияПослеВопросаПриЗакрытииФормы = ОписаниеОповещенияЕГАИС("ПослеВопросаПриЗакрытииФормы", ЭтаФорма);
			
			ПоказатьВопросЕГАИС(ОписаниеОповещенияПослеВопросаПриЗакрытииФормы, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
...
&НаКлиенте
Процедура ИнформацияТребуетсяПеремаркировкаОбработкаНавигационнойСсылки(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	НавигационнаяСсылкаФорматированнойСтроки = ИнтеграцияЕГАИСУТКлиентСервер.НавигационнаяСсылкаСтроки(Элемент, ИнформацияТребуетсяПеремаркировка);
...
&НаКлиенте
Процедура ПредставлениеРежимаПроверкиОбработкаНавигационнойСсылки(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	НавигационнаяСсылкаФорматированнойСтроки = ИнтеграцияЕГАИСУТКлиентСервер.НавигационнаяСсылкаСтроки(Элемент, ПредставлениеРежимаПроверки);
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ИзменитьРежимПроверки" Тогда
		...
		ОповещениеОИзмененииРежимаПроверки = ОписаниеОповещенияЕГАИС("ПослеИзмененияРежимаПроверки", ЭтаФорма);
		
		ОткрытьФормуЕГАИС("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ИзменениеРежимаПроверки", ПараметрыОткрытия, ЭтаФорма, 
...
Процедура ДеревоМаркированнойПродукцииСтатусПроверкиПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ОбновитьВыводимоеПредставлениеПроверкиСодержимого(ЭтаФорма, ТекущиеДанные);
...
Процедура ДеревоМаркированнойПродукцииПриАктивизацииСтроки(Элемент)
	...
	ОбновитьВыводимоеПредставлениеПроверкиСодержимого(ЭтаФорма, Элементы.ДеревоМаркированнойПродукции.ТекущиеДанные);
...
Процедура АлкогольнаяПродукцияКОпределениюСправок2ПередУдалением(Элемент, Отказ)
	...
	Если ТекущиеДанные.Маркируемая
		...
		ПоказатьПредупреждениеЕГАИС(, ТекстПредупреждения);
		Отказ = Истина;
...
Процедура ПроверкаЗавершена(Команда)
	...
	Если КоличествоНепроверенных > 0
		...
		ОписаниеОповещенияПослеОтветаНаВопрос = ОписаниеОповещенияЕГАИС("ОтветНаВопросПриЗавершенииПроверки", ЭтаФорма);
		
		ОткрытьФормуЕГАИС("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ВопросПередСохранениемРезультатовПроверки",
...
Процедура ПоискНечитаемойАкцизнойМарки(Команда)
	...
	ОповещениеОЗакрытииПоискаНечитаемойАкцизнойМарки = ОписаниеОповещенияЕГАИС("ПоискНечитаемойАкцизнойМаркиПослеЗакрытия", ЭтаФорма);
	
	ОткрытьФормуЕГАИС("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ПоискНесчитываемойАкцизнойМарки",,ЭтаФорма,,,,
...
Процедура ДобавитьМаркированнуюУпаковку(Команда)
	...
	ОповещениеПослеГенерацииШтрихкодаУпаковки = ОписаниеОповещенияЕГАИС("ПослеГенерацииШтрихкодаДляНовойУпаковки", ЭтаФорма);
	
	ОткрытьФормуЕГАИС("Обработка.ГенерацияШтрихкодовУпаковок.Форма.Форма", ПараметрыОткрытияФормы, ЭтаФорма,,,,
...
Процедура МаркироватьУпаковку(Команда)
	...
	Если СтрокаСУпаковкойДляПеремаркировки = Неопределено Тогда
		...
		ПоказатьПредупреждениеЕГАИС( , НСтр("ru = 'Спозиционируйтесь на упаковке, которую хотите перемаркировать.'"));
		Возврат;
	...
	ОповещениеПослеПеремаркировкиУпаковки = ОписаниеОповещенияЕГАИС("ПослеПеремаркировкиУпаковки",
	                                                                 ЭтаФорма, 
	                                                                 ДополнительныеПараметры);
	
	ОткрытьФормуЕГАИС("Обработка.ГенерацияШтрихкодовУпаковок.Форма.Форма", ДанныеУпаковки, ЭтаФорма,,,,
...
Процедура ПоискНечитаемойАкцизнойМаркиПослеЗакрытия(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		АкцизныеМаркиЕГАИСКлиент.ОбработатьДанныеШтрихкода(ОписаниеОповещенияЕГАИС("ПоискПоШтрихкодуЗавершение", ЭтаФорма),
		                                                   ЭтаФорма, ДанныеШтрихкода, ПараметрыСканированияАкцизныхМарок());
...
Процедура ПоискПоШтрихкодуВыполнить(Команда)
	...
	ШтрихкодированиеНоменклатурыЕГАИСКлиентПереопределяемый.ПоказатьВводШтрихкода(
		ОписаниеОповещенияЕГАИС("РучнойВводШтрихкодаЗавершение", ЭтаФорма));
...
Процедура НачатьПроверкуЗаново(Команда)
	...
	ОписаниеОповещенияПослеОтвета = ОписаниеОповещенияЕГАИС("ПослеВопросаОНачалеПроверкиЗаново", ЭтаФорма);
	...
	ПоказатьВопросЕГАИС(ОписаниеОповещенияПослеОтвета, ТекстВопроса,РежимДиалогаВопрос.ДаНет);
...
Функция ПустаяТаблицаХешСумм()
	...
	ТаблицаХешСумм.Колонки.Добавить("ХешСумма", ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(50));
	ТаблицаХешСумм.Колонки.Добавить("ЗначениеШтрихкода", ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(200));
	ТаблицаХешСумм.Колонки.Добавить("ИдентификаторВДереве", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15, 0, ДопустимыйЗнак.Неотрицательный));
...
Процедура ПроверитьКорректностьШтрихкодаПриДобавлении(Штрихкод, Отказ)
	...
	Если ПустаяСтрока(Штрихкод) Тогда
		ПоказатьПредупреждениеЕГАИС(, НСтр("ru = 'Упаковке не может быть назначен пустой Штрихкод.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ИдентификаторСтроки <> Неопределено Тогда
		...
		ПоказатьПредупреждениеЕГАИС(,ТекстСообщения); 
		Отказ = Истина;
...
Процедура ПеремаркироватьУпаковку(Штрихкод, ИдентификаторСтрокиУпаковки)
	...
	ЗаменитьШтрихКодВСпискахУпаковок(ЭтаФорма,СтрокаСУпаковкой.ЗначениеШтрихкода, Штрихкод);
...
Процедура ОтобразитьИнформациюОНеобходимостиПеремаркировки()
	...
	Если КоличествоУпаковокКоторыеНеобходимоПеремаркировать > 0 Тогда
		...
		СтрокаТребуется = ФорматированнаяСтрока(ТекстТребуется,, ЦветаСтиля.ЦветТекстаТребуетВниманияГИСМ);
		...
		СтрокаОтбор = ФорматированнаяСтрока(ТескстОтбор,, ЦветаСтиля.ЦветГиперссылкиГИСМ,, "ИзменитьОтборТребуетсяПеремаркировка");
		
		МассивСтрок = Новый Массив();
		МассивСтрок.Добавить(СтрокаТребуется);
		МассивСтрок.Добавить(СтрокаОтбор);
		
		ИнформацияТребуетсяПеремаркировка = Новый ФиксированныйМассив(МассивСтрок);
		
		ИнтеграцияЕГАИСУТ.СформироватьФорматированнуюСтроку(ЭтаФорма,
			ИнформацияТребуетсяПеремаркировка,
			"ГруппаИнформацияТребуетсяПеремаркировка",
			"ИнформацияТребуетсяПеремаркировкаОбработкаНавигационнойСсылки");
...
Функция ВосстановитьСохраненныеРезультатыПроверки(ДанныеПроверки)
	...
	Если ЗначениеЗаполнено(ШтрихкодТекущейПроверяемойУпаковки) Тогда
		...
		Если ИдентификаторТекущейПроверяемойУпаковки <> Неопределено Тогда
			...
			ОбновитьВыводимоеПредставлениеПроверкиСодержимого(ЭтаФорма, ДанныеТекущейСтроки);
		Иначе
...
Процедура ЗавершитьПроверку(СтатусПроверки)
	...
	Если Результат.ЕстьОшибки Тогда
		...
		ПоказатьПредупреждениеЕГАИС(, Результат.ТекстОшибки);
		Возврат;
...
Функция РезультатСканированияАлкогольнойПродукции(СтатусПроверки, УникальныйИдентификаторФормыВладельца)
	...
	ВозвращаемоеДерево.Колонки.Добавить("ЗначениеШтрихкода",       ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(200));
	...
	ВозвращаемоеДерево.Колонки.Добавить("КоличествоАкцизныхМарок", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15));
...
Процедура ОбработатьСканированиеИмеющегосяВДеревеШтрихкода(ДанныеШтрихкода, ТекущаяСтрокаДерева, ИдентификаторНайденнойСтроки)
	...
	ОповещениеПослеВыбораДействия = ОписаниеОповещенияЕГАИС("ВыборДействияПоРезультатамВводаШтрихкодаПриЗавершении", ЭтаФорма, ДополнительныеПараметры);
	
	ОткрытьФормуЕГАИС("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ВыборДействияПоРезультатамВводаШтрихкода", ПараметрыОткрытияФормы, 
...
Процедура ОбработатьНеНайденныйВДеревеШтрихКод(ДанныеШтрихкода, ТекущаяСтрокаДерева)
	...
	Если РежимПодбораСуществующихУпаковок
		...
		Если ПараметрыСканированияАкцизныхМарок.ДоступныеСтатусы.Найти(ПредопределенноеЗначение("Перечисление.СтатусыАкцизныхМарок.ПустаяСсылка")) = Неопределено Тогда
			...
			ОткрытьФормуЕГАИС("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ИнформацияОНевозможностиДобавленияОтсканированного", ПараметрыОткрытияФормы,
	...
	ОповещениеПослеВыбораДействия = ОписаниеОповещенияЕГАИС("ВыборДействияПоРезультатамВводаШтрихкодаПриЗавершении", ЭтаФорма);
...
Функция ПредставлениеРежимаПроверки(РежимПроверки)
	...
	Если Не РежимПросмотра Тогда
		СтрокаДействие = ФорматированнаяСтрока(СтрокаДействие,, ЦветаСтиля.ЦветГиперссылкиГИСМ,, "ИзменитьРежимПроверки");
	КонецЕсли;
	
	Строки = Новый Массив();
	Строки.Добавить(ТекстВступление);
	Строки.Добавить(СтрокаДействие);
	
	Возврат Новый ФиксированныйМассив(Строки);
...
&НаСервере
Процедура СформироватьПредставлениеРежимаПроверки()
	
	ПредставлениеРежимаПроверки = ПредставлениеРежимаПроверки(РежимПроверки);
	ИнтеграцияЕГАИСУТ.СформироватьФорматированнуюСтроку(ЭтаФорма,
		ПредставлениеРежимаПроверки,
		"ГруппаПредставлениеРежимаПроверки",
		"ПредставлениеРежимаПроверкиОбработкаНавигационнойСсылки");
...
Процедура ДобавитьНовуюАлкогольнуюПродукциюВДерево(ДанныеШтрихкода, ДобавлятьВУпаковку = Неопределено)
	...
	ИзменитьДанныеПулаАлкогольнойПродукции(ЭтаФорма, ДанныеШтрихкода.АлкогольнаяПродукция, ДанныеШтрихкода.Справка2, Ложь);
...
Процедура ДобавитьДанныеСуществующейУпаковкиВДеревоМаркированнойПродукции(ШтрихкодУпаковки, СоответствиеШтрихкодовСтрокДерева)
	...
	ЭтоУпаковка = ИнтеграцияЕГАИСКлиентСервер.ЭтоУпаковка(ШтрихкодУпаковки.ТипУпаковки);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаИзмененийАлкогольнойПродукции Цикл
		...
		Если СтрокаТаблицы.Изменение = 1 Тогда
			...
			Если РежимПодбораСуществующихУпаковок Тогда
				...
				ОпределениеСправок2ПриДобавленииСтроки(ЭтаФорма, СтрокаТаблицы);
				
			Иначе
				
				ИзменитьДанныеПулаАлкогольнойПродукции(ЭтаФорма, 
