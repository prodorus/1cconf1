...
Функция Подключаемый_СтраницаМастераНачалоНастройкиОбменаССервисом_ПриПереходеДалее(Отказ)
		...
		Если Не Отказ Тогда
			//Отказ = Истина;
			//ИзменитьПорядковыйНомерПерехода(0);
		КонецЕсли;
...
&НаСервере
Процедура ПолучитьОписаниеОграниченийВыгрузкиДанных(НастройкаОтборовНаУзле)
	
	ОписаниеНастроекВыгрузкиДанных = ОбменДаннымиСервер.ОписаниеОграниченийПередачиДанных(Объект.ИмяПланаОбмена, НастройкаОтборовНаУзле, ВерсияКонфигурацииКорреспондента);
...
Процедура ТаблицаПереходовРасширеннойНастройкиОбменаССервисом()
	...
	// Задание параметров подключения; Проверка подключения
	ТаблицаПереходовНоваяСтрока(1,  "СтраницаМастераНачалоНастройкиОбменаССервисом", "", "СтраницаНавигацииНачало","СтраницаМастераНачалоНастройкиОбменаССервисом_ПриПереходеДалее");
