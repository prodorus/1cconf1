	...
	Пока ЕстьДокументы Цикл
		...
		Если ЕстьДокументы Тогда
			Для Каждого СтрокаДокумента Из ВыборкаДокументов Цикл
				...
				ИначеЕсли СтрокаДокумента.ДанныеДокументовОплаты Тогда
					
					УчетНДСФормированиеДвижений.СформироватьДвиженияНДСНомераДокументовОплаты(СтрокаДокумента.Ссылка, Неопределено);
...
Функция ПолучитьТаблицуДокументов(ДатаНачала, СсылкаНач, ДатаКонца, БылПоследнийДокумент, МассивОрганизаций)
	...
	ТекстЗапроса = 
		...
		|	ЛОЖЬ КАК ДанныеДокументовОплаты
		|ИЗ
	...
	ЗапросПоДаннымДокументовОплаты = "
	|ВЫБРАТЬ
	|	Источник.Ссылка,
	|	Источник.Ссылка.Дата,
	|	Источник.Ссылка.МоментВремени,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	ИСТИНА
	|ИЗ
	|	Документ.СчетФактураВыданный.ДатаНомерДокументовОплаты КАК Источник
	|ГДЕ
	|	Источник.ДатаПлатежноРасчетногоДокумента <> ДАТАВРЕМЯ(1, 1, 1)
	|	И Источник.Ссылка.Дата >= ДАТАВРЕМЯ(2014, 10, 1)	
	|	И Источник.Ссылка.Дата >= &ДатаПоследнего
	|	И Источник.Ссылка.Дата <= &ДатаКон
	|	И Источник.Ссылка.Организация В(&МассивОрганизаций)
	|	И Источник.Ссылка.МоментВремени > &МоментНач
	...
	ТекстЗапроса = ТекстНачалоПакета + 
		...
		|	Источник.Дата,
		|	Источник.МоментВремени,
		|	Источник.ДанныеДокументовОплаты КАК ДанныеДокументовОплаты,
