...
Функция ПечатьПеречисленияЗарплаты(СсылкаНаДокумент) Экспорт
	...
	Если ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее") Тогда
		...
		|	ПлатежноеПоручениеИсходящее.Контрагент КАК Банк,
		...
		ЗапросПоДокументу.УстановитьПараметр("СсылкаНаДокумент", СсылкаНаДокумент);
		
		ПеречислениеЗаработнойПлаты = ЗапросПоДокументу.Выполнить().Выгрузить();
		Если ПеречислениеЗаработнойПлаты.Количество() > 0 Тогда
			ЗаполнитьЗначенияСвойств(Платеж, ПеречислениеЗаработнойПлаты[0]);
			Ведомости = ПеречислениеЗаработнойПлаты[0].Ведомости.ВыгрузитьКолонку("Ссылка");
		КонецЕсли	
		
	ИначеЕсли ТипЗнч(СсылкаНаДокумент) = Тип("ДокументСсылка.ЗарплатаКВыплатеОрганизаций") Тогда
...
Функция ПолучитьИмяИАдресноеСокращение(Знач СтрокаЭлемента, АдресноеСокращение) Экспорт
	...
	// Есть сокращения, состоящие из двух слов
	Если ДвусловныеИсключения = Неопределено Тогда 
		ДвусловныеИсключения = Новый Массив;
		АдресныеИсключения = Справочники.АдресныеСокращения.Выбрать();
		Пока АдресныеИсключения.Следующий() Цикл
			Сокращение = СокрЛП(АдресныеИсключения.Сокращение);
			Если Найти(Сокращение, " ") > 0 Тогда
				ДвусловныеИсключения.Добавить(Сокращение);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Если СтрЧислоВхождений(Буфер, " ") > 1 Тогда
		ЧастиСтроки = ОбщегоНазначенияЗК.РазложитьСтрокуВМассивПодстрок(Буфер, " ");
		КоличествоЧастей = ЧастиСтроки.Количество();
		Сокращение = ЧастиСтроки[КоличествоЧастей -2] + " " + ЧастиСтроки[КоличествоЧастей -1];
		Если ДвусловныеИсключения.Найти(Сокращение) = Неопределено Тогда 
			Сокращение = ЧастиСтроки[КоличествоЧастей -1];
		КонецЕсли;
		ПозицияПослПробела = СтрДлина(Буфер) - СтрДлина(Сокращение);
	КонецЕсли;
...
Функция ЗадатьПараметрыСеанса(вхДатаФормирования, вхНомерДоговора, вхОрганизация, вхРасчетныйСчетОрганизации, ИДДокумента = "", вхДатаДоговора = Неопределено) Экспорт
	...
	Попытка
		Если вхДатаДоговора = Неопределено Тогда
			ДатаДоговора = Дата(1,1,1);
		Иначе
			ДатаДоговора = вхДатаДоговора;
		КонецЕсли;
	Исключение
		БезОшибок = Ложь;
	КонецПопытки;
...
Функция ПолучитьАдресСтрокой(Объект, ВидАдреса) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если СокрП(УправлениеКонтактнойИнформацией.ПолучитьПредставлениеАдреса(Выборка)) <> Выборка.Представление Тогда
			СтрокаАдреса = "";
		КонецЕсли;
...
Функция МиграционнаяКартаСтрокой(СтрокаЗаявки)
	
	СтрокаМиграционнойКарты = ""
							+ СтрокаЗаявки.НомерМиграционнойКарты + Символы.ПС 
							+ ПолучитьДатуXML(СтрокаЗаявки.ДатаНачалаПребыванияМиграционнойКарты) + Символы.ПС 
							+ ПолучитьДатуXML(СтрокаЗаявки.ДатаОкончанияПребыванияМиграционнойКарты);
	
	Возврат СтрокаМиграционнойКарты;
	
КонецФункции

Функция МиграционныйДокументСтрокой(СтрокаЗаявки)
	
	СтрокаМиграционныйДокумент = ""
							+ СтрокаЗаявки.КодМиграционногоДокумента + Символы.ПС 
							+ СтрокаЗаявки.НомерМиграционногоДокумента + Символы.ПС 
							+ ПолучитьДатуXML(СтрокаЗаявки.ДатаНачалаПребыванияМиграционногоДокумента) + Символы.ПС 
							+ ПолучитьДатуXML(СтрокаЗаявки.ДатаОкончанияПребыванияМиграционногоДокумента);
	
	Возврат СтрокаМиграционныйДокумент;
	
КонецФункции

// УРОВЕНЬ ПРЕДСТАВЛЕНИЯ
Функция ДобавитьОткрытиеСчета(Организация, ФизЛицо, ВидВклада, ДатаВыплатыЗаработнойПлаты, СтрокаЗаявки) Экспорт
	...
	Иначе
		СтрокаТЧ.АдресМестаРаботы = ПолучитьАдресСтрокой(Организация.ИндивидуальныйПредприниматель, "ЮрАдресФизЛица");
	КонецЕсли;	
	...
	СтрокаТЧ.МиграционнаяКарта = МиграционнаяКартаСтрокой(СтрокаЗаявки);
	СтрокаТЧ.МиграционныйДокумент = МиграционныйДокументСтрокой(СтрокаЗаявки);
	ОсновнойСотрудникФизлица = ПроцедурыУправленияПерсоналом.ОсновнойСотрудникФизлица(ФизЛицо, Организация);
	Если НЕ ОсновнойСотрудникФизлица = Неопределено Тогда
		СтрокаТЧ.ТабельныйНомер = ОсновнойСотрудникФизлица.Код;
		СтрокаТЧ.ДатаОформления = ОсновнойСотрудникФизлица.ДатаПриемаНаРаботу;
	КонецЕсли;
	СтрокаТЧ.ПрогнозируемыйМесячныйДоход = СтрокаЗаявки.ПрогнозируемыйМесячныйДоход;
	СтрокаТЧ.ДатаВыплатыЗаработнойПлаты = ДатаВыплатыЗаработнойПлаты;
	
КонецФункции

Функция ДобавитьЗакрытиеСчета(Организация, ФизЛицо, СтрокаЗаявки) Экспорт
	
	СтрокаТЧ = ЗакрытиеСчетов.Добавить();
	СтрокаТЧ.Сотрудник = ФизЛицо;
	СтрокаТЧ.НомерЛицевогоСчета = СтрокаЗаявки.НомерЛицевогоСчета;
	СтрокаТЧ.ДатаЗакрытия = СтрокаЗаявки.ДатаЗакрытия;
...
Функция ЭкспортироватьДанные(НомерОтправки, Отделение, ИмяФайла = Неопределено) Экспорт
	...
	КаталогНаДиске = Новый Файл(КаталогЭкспортаДанных);
	Если НЕ КаталогНаДиске.Существует() Тогда
		Сообщить("Каталог экспорта не существует.");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ЗакрытиеСчетов.Количество() > 0 Тогда
		...
		Ветка = ДобавитьЭлемент(СчетаПК, "СписокУвольнений");
		ДобавитьЗакрытиеСчетаСотрудника(Ветка, ЗакрытиеСчетов);
		КоличествоЗаписей = КоличествоЗаписей + ЗакрытиеСчетов.Количество();
		СуммаИтого = 0;
		Операция = "u";
	...
	Если ЗачислениеЗарплаты.Количество() > 0 Тогда
		...
		ДобавитьЭлемент(СчетаПК, "ВидЗачисления", "01");
	...
	ДобавитьКонтрольныеСуммы(СчетаПК, КоличествоЗаписей, СуммаИтого, Операция);
...
Функция СоздатьСчетаПК(ДеревоЭкспорта) Экспорт
	...
	Если ФорматФайлаОбмена = ФорматФайла34 Тогда
		ДобавитьАтрибут(ОсновнойУзел, "xmlns:xs",	"http://www.w3.org/2001/XMLSchema");
		ДобавитьАтрибут(ОсновнойУзел, "xmlns:xsi",	"http://www.w3.org/2001/XMLSchema-instance");
		ДобавитьАтрибут(ОсновнойУзел, "xmlns",		"http://v8.1c.ru/edi/edi_stnd/109");
		ДобавитьАтрибут(ОсновнойУзел, "xsi:type",	"СчетПК");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДатаДоговора) Тогда
		ДобавитьАтрибут(ОсновнойУзел, "ДатаДоговора", ПолучитьДатуXML(ДатаДоговора));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(РасчетныйСчетОрганизации) Тогда
...
Функция ДобавитьОткрытиеСчетаСотрудника(Ветка, СтруктураОткрытиеСчетов) Экспорт
	...
	|	ОткрытиеСчетов.АдресМестаРаботы,
	...
	|	ОткрытиеСчетов.МиграционнаяКарта,
	|	ОткрытиеСчетов.МиграционныйДокумент,
	|	ОткрытиеСчетов.ТабельныйНомер,
	|	ОткрытиеСчетов.ДатаОформления,
	|	ОткрытиеСчетов.ПрогнозируемыйМесячныйДоход,
	|	ОткрытиеСчетов.ДатаВыплатыЗаработнойПлаты
	...
	|	ВТОткрытиеСчетов.АдресМестаРаботы,
	...
	|	ВТОткрытиеСчетов.МиграционнаяКарта,
	|	ВТОткрытиеСчетов.МиграционныйДокумент,
	|	ВТОткрытиеСчетов.ТабельныйНомер,
	|	ВТОткрытиеСчетов.ДатаОформления,
	|	ВТОткрытиеСчетов.ПрогнозируемыйМесячныйДоход,
	|	ВТОткрытиеСчетов.ДатаВыплатыЗаработнойПлаты,
	|	ФизическиеЛица.ДатаРождения,
	|	ФизическиеЛица.ИНН,
	...
	|	ВТОткрытиеСчетов.Резидент,
	|	ВТОткрытиеСчетов.Гражданство,
	|	ВТОткрытиеСчетов.Должность
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		// АдресМестаРаботы
		Если НЕ ПустаяСтрока(СтрЗаменить(Выборка.АдресМестаРаботы, Символы.ПС, "")) Тогда
			АдресМестаРаботы = Выборка.АдресМестаРаботы;
			ДобавитьАдресВДеревоЭкспорта(УзелОС, "АдресМестаРаботы", СтрПолучитьСтроку(АдресМестаРаботы,1), "Российская Федерация",,"643",
			СтрПолучитьСтроку(АдресМестаРаботы,2),
			СтрПолучитьСтроку(АдресМестаРаботы,3),
			СтрПолучитьСтроку(АдресМестаРаботы,4),
			СтрПолучитьСтроку(АдресМестаРаботы,5),
			СтрПолучитьСтроку(АдресМестаРаботы,6),
			СтрПолучитьСтроку(АдресМестаРаботы,7),
			СтрПолучитьСтроку(АдресМестаРаботы,8),
			СтрПолучитьСтроку(АдресМестаРаботы,9),
			СтрПолучитьСтроку(АдресМестаРаботы,10),
			СтрПолучитьСтроку(АдресМестаРаботы,11),
			СтрПолучитьСтроку(АдресМестаРаботы,12),
			СтрПолучитьСтроку(АдресМестаРаботы,13),
			СтрПолучитьСтроку(АдресМестаРаботы,14));
		КонецЕсли;
		
		Если (ФорматФайлаобмена = ФорматФайла3 Или ФорматФайлаобмена = ФорматФайла32 Или ФорматФайлаобмена = ФорматФайла33 Или ФорматФайлаобмена = ФорматФайла34) И ЗначениеЗаполнено(Выборка.Должность) Тогда
		...
		Если НЕ ПустаяСтрока(СтрЗаменить(Выборка.АдресПрописки, Символы.ПС, "")) Тогда
			...
			ДобавитьАдресВДеревоЭкспорта(УзелОС, "АдресПрописки", СтрПолучитьСтроку(АдресПрописки, 1), "Российская Федерация",,"643",
		...
		Если НЕ ПустаяСтрока(Выборка.РабочийТелефон) Тогда
			Если ФорматФайлаобмена = ФорматФайла34 Тогда
				ДобавитьЭлемент(УзелОС, "РабочийТелефон", Лев(СокрЛП(Выборка.РабочийТелефон),40));
			Иначе
		...
		Если НЕ ПустаяСтрока(Выборка.ДомашнийТелефон) Тогда
			Если ФорматФайлаобмена = ФорматФайла34 Тогда
				ДобавитьЭлемент(УзелОС, "ДомашнийТелефон", Лев(СокрЛП(Выборка.ДомашнийТелефон), 40));
			Иначе
		...
		Если ФорматФайлаобмена = ФорматФайла3 Или ФорматФайлаобмена = ФорматФайла32 Или ФорматФайлаобмена = ФорматФайла33 Или ФорматФайлаобмена = ФорматФайла34 Тогда
			...
			Иначе
				Если ФорматФайлаобмена = ФорматФайла34 Тогда
					ДобавитьЭлемент(УзелОС, "Гражданство", "РОССИЯ");
				КонецЕсли;
			КонецЕсли;
			
			// Категория населения
			Если ЗначениеЗаполнено(Выборка.КатегорияНаселения) Тогда
			...
			Если ЗначениеЗаполнено(Выборка.КонтрольнаяИнформация) Тогда
				ДобавитьЭлемент(УзелОС, "КонтрольнаяИнформация", Выборка.КонтрольнаяИнформация);
			КонецЕсли;
			
		КонецЕсли;
		
		Если ФорматФайлаобмена = ФорматФайла32 Или ФорматФайлаобмена = ФорматФайла33 Или ФорматФайлаобмена = ФорматФайла33 Или ФорматФайлаобмена = ФорматФайла34 Тогда
			
			МиграционнаяКарта = Выборка.МиграционнаяКарта;
			МиграционныйДокумент = Выборка.МиграционныйДокумент;
			ДобавитьНерезидентВДеревоЭкспорта(УзелОС, "Нерезидент",
				СтрПолучитьСтроку(МиграционнаяКарта, 1),
				СтрПолучитьСтроку(МиграционнаяКарта, 2),
				СтрПолучитьСтроку(МиграционнаяКарта, 3),
				СтрПолучитьСтроку(МиграционныйДокумент, 1),
				СтрПолучитьСтроку(МиграционныйДокумент, 2),
				СтрПолучитьСтроку(МиграционныйДокумент, 3),
				СтрПолучитьСтроку(МиграционныйДокумент, 4));
			
			// Табельный номер
			Если ЗначениеЗаполнено(Выборка.ТабельныйНомер) Тогда
				ДобавитьЭлемент(УзелОС, "ТабельныйНомер", Выборка.ТабельныйНомер);
			КонецЕсли;
			
			// Дата оформления
			Если ЗначениеЗаполнено(Выборка.ДатаОформления) Тогда
				ДобавитьЭлемент(УзелОС, "ДатаОформления", ПолучитьДатуXML(Выборка.ДатаОформления));
			КонецЕсли;
			
			// Сумма заработной платы
			Если ЗначениеЗаполнено(Выборка.ПрогнозируемыйМесячныйДоход) Тогда
				ДобавитьЭлемент(УзелОС, "СуммаЗаработнойПлаты", Выборка.ПрогнозируемыйМесячныйДоход);
			КонецЕсли;
			
			// Дата выплаты
			Если ЗначениеЗаполнено(Выборка.ДатаВыплатыЗаработнойПлаты) Тогда
				ДатаВыплатыЗаработнойПлаты = НачалоГода(?(ЗначениеЗаполнено(Выборка.ДатаОформления),Выборка.ДатаОформления, ТекущаяДата())) + (Выборка.ДатаВыплатыЗаработнойПлаты - 1) * 86400;
				ДобавитьЭлемент(УзелОС, "ДатаВыплаты",  ПолучитьДатуXML(ДатаВыплатыЗаработнойПлаты));
			КонецЕсли;
			
			// Адрес информирования
			Если НЕ ПустаяСтрока(СтрЗаменить(Выборка.АдресПроживания, Символы.ПС, "")) Тогда
				АдресИнформирования = Выборка.АдресПроживания;
			Иначе
				АдресИнформирования = Выборка.АдресПрописки;
			КонецЕсли;
			Если НЕ ПустаяСтрока(СтрЗаменить(АдресИнформирования, Символы.ПС, "")) Тогда
				ДобавитьАдресВДеревоЭкспорта(УзелОС, "АдресИнформирования", СтрПолучитьСтроку(АдресИнформирования, 1), "Российская Федерация",,,
				СтрПолучитьСтроку(АдресИнформирования, 2),
				СтрПолучитьСтроку(АдресИнформирования, 3),
				СтрПолучитьСтроку(АдресИнформирования, 4),
				СтрПолучитьСтроку(АдресИнформирования, 5),
				СтрПолучитьСтроку(АдресИнформирования, 6),
				СтрПолучитьСтроку(АдресИнформирования, 7),
				СтрПолучитьСтроку(АдресИнформирования, 8),
				СтрПолучитьСтроку(АдресИнформирования, 9),
				СтрПолучитьСтроку(АдресИнформирования, 10),
				СтрПолучитьСтроку(АдресИнформирования, 11),
				СтрПолучитьСтроку(АдресИнформирования, 12),
				СтрПолучитьСтроку(АдресИнформирования, 13),
				СтрПолучитьСтроку(АдресИнформирования, 14));
			КонецЕсли;
...
Функция ДобавитьЗакрытиеСчетаСотрудника(Ветка, СтруктураЗакрытиеСчетов) Экспорт
	
	Для Каждого СтрокаЗакрытие Из СтруктураЗакрытиеСчетов Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаЗакрытие.Сотрудник) Тогда 
			ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки("Формирование данных на закрытие счетов", "Не указано физ лицо для закрытия счета");
			Возврат Неопределено;
		ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаЗакрытие.НомерЛицевогоСчета) Тогда 
			ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки("Формирование данных на закрытие счетов", "Не указан номер лицевого счета для закрытия");
			Возврат Неопределено;
		ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаЗакрытие.ДатаЗакрытия) Тогда 
			ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки("Формирование данных на закрытие счетов", "Не указана дата закрытия номера лицевого счета");
			Возврат Неопределено;
		КонецЕсли;
	КонецЦикла;
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ТаблицаЗакрытиеСчетов", СтруктураЗакрытиеСчетов);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗакрытиеСчетов.Сотрудник КАК ФизЛицо,
	|	ЗакрытиеСчетов.НомерСтроки,
	|	ЗакрытиеСчетов.НомерЛицевогоСчета,
	|	ЗакрытиеСчетов.ДатаЗакрытия
	|ПОМЕСТИТЬ ВТЗакрытиеСчетов
	|ИЗ
	|	&ТаблицаЗакрытиеСчетов КАК ЗакрытиеСчетов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ФизическиеЛица.Наименование,
	|	ЗакрытиеСчетов.ФизЛицо,
	|	ЗакрытиеСчетов.НомерСтроки КАК НомерСтроки,
	|	ЗакрытиеСчетов.НомерЛицевогоСчета КАК НомерЛицевогоСчета,
	|	ЗакрытиеСчетов.ДатаЗакрытия КАК ДатаЗакрытия,
	|	ЕСТЬNULL(ФИОФизЛицСрезПоследних.Фамилия, ФизическиеЛица.Наименование) КАК Фамилия,
	|	ЕСТЬNULL(ФИОФизЛицСрезПоследних.Имя, """") КАК Имя,
	|	ЕСТЬNULL(ФИОФизЛицСрезПоследних.Отчество, """") КАК Отчество
	|ИЗ
	|	ВТЗакрытиеСчетов КАК ЗакрытиеСчетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица КАК ФизическиеЛица
	|		ПО ЗакрытиеСчетов.ФизЛицо = ФизическиеЛица.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(
	|				,
	|				ФизЛицо В
	|					(ВЫБРАТЬ
	|						ЗакрытиеСчетов.ФизЛицо
	|					ИЗ
	|						ВТЗакрытиеСчетов КАК ЗакрытиеСчетов)) КАК ФИОФизЛицСрезПоследних
	|		ПО ЗакрытиеСчетов.ФизЛицо = ФИОФизЛицСрезПоследних.ФизЛицо
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	...
	Пока Выборка.Следующий() Цикл
		...
		// НомерЛицевогоСчета
		ДобавитьЭлемент(УзелОС, "НомерСчета", Лев(СокрЛП(Выборка.НомерЛицевогоСчета), 20));
		
		// ДатаЗакрытия
		ДобавитьЭлемент(УзелОС, "ДатаУвольнения", "" + ПолучитьДатуXML(Выборка.ДатаЗакрытия));
...
Функция ДобавитьКонтрольныеСуммы(ВеткаДерева, КоличествоЗаписей, СуммаИтого, Операция = "") Экспорт
	
	УзелКС = ДобавитьЭлемент(ВеткаДерева, "КонтрольныеСуммы");
	
	ДобавитьЭлемент(УзелКС, "КоличествоЗаписей", КоличествоЗаписей);
	
	ДобавлятьЭлементСуммаИтого = Истина;
	Если СуммаИтого = 0 И ФорматФайлаобмена = ФорматФайла34 И Операция = "o" Тогда
		ДобавлятьЭлементСуммаИтого = Ложь;
	КонецЕсли;
	Если ДобавлятьЭлементСуммаИтого Тогда
		ДобавитьЭлемент(УзелКС, "СуммаИтого", Формат(СуммаИтого, "ЧЦ=15; ЧДЦ=2; ЧРД=.; ЧРГ=; ЧН=0; ЧГ=0"));
	КонецЕсли;
...
Функция ДобавитьСотрудникаВДеревоЭкспорта(лВетка, лНпп, лФамилия, лИмя, лОтчество, лОтделениеБанка, лФилиалОтделенияБанка) Экспорт
	...
	Если НЕ ((ФорматФайлаобмена = ФорматФайла3 Или ФорматФайлаобмена = ФорматФайла32 Или ФорматФайлаобмена = ФорматФайла33 Или ФорматФайлаобмена = ФорматФайла34) И лОтчество = "") Тогда
...
Процедура ДобавитьМестоРожденияВДеревоЭкспорта(ВеткаДерева, СтрокаМестоРождения)
	СтруктураМестоРождения = РегламентированнаяОтчетность.РазложитьМестоРождения(СтрокаМестоРождения);
	Если ПустаяСтрока(СтруктураМестоРождения.Страна) 
		И ПустаяСтрока(СтруктураМестоРождения.Область) 
		И ПустаяСтрока(СтруктураМестоРождения.Район)
		И ПустаяСтрока(СтруктураМестоРождения.НаселенныйПункт) Тогда
		Возврат;
	КонецЕсли;
...
КонецПроцедуры //ДобавитьМестоРожденияВДеревоЭкспорта()
...
Функция ДобавитьУдЛичностиВДеревоЭкспорта(лВетка, лИмя, лВидДокумента, лСерия, лНомер, лДатаВыдачи, лКемВыдан, лКодПодразделения = "") Экспорт
	...
	ИначеЕсли ФорматФайлаобмена = ФорматФайла34 Тогда
		ДобавитьЭлемент(ВеткаУдЛичности, "ВидДокумента", ?(лВидДокумента = "", " ", ЛЕВ(лВидДокумента.Наименование, 75)));
		Если Не ПустаяСтрока(лСерия) Тогда
			ДобавитьЭлемент(ВеткаУдЛичности, "Серия", лСерия);
		КонецЕсли;
		ДобавитьЭлемент(ВеткаУдЛичности, "Номер", ?(лНомер = "", " ", лНомер));
		ДобавитьЭлемент(ВеткаУдЛичности, "ДатаВыдачи", ?(ЗначениеЗаполнено(лДатаВыдачи), ПолучитьДатуXML(лДатаВыдачи), ПолучитьДатуXML(Дата(1900,1,1))));
		ДобавитьЭлемент(ВеткаУдЛичности, "КемВыдан", ?(ПустаяСтрока(лКемВыдан), " ", Лев(лКемВыдан, 250)));
		Если Не ПустаяСтрока(лКодПодразделения) Тогда
	...
	ИначеЕсли ФорматФайлаОбмена = ФорматФайла3 Или ФорматФайлаобмена = ФорматФайла32 Или ФорматФайлаобмена = ФорматФайла33 Тогда
		ДобавитьЭлемент(ВеткаУдЛичности, "ВидДокумента", ?(лВидДокумента = "", "", ЛЕВ(лВидДокумента.Наименование, 75)));
		ДобавитьЭлемент(ВеткаУдЛичности, "Серия", лСерия);
		ДобавитьЭлемент(ВеткаУдЛичности, "Номер", лНомер);
		ДобавитьЭлемент(ВеткаУдЛичности, "ДатаВыдачи", ПолучитьДатуXML(лДатаВыдачи));
...
Функция ДобавитьНерезидентВДеревоЭкспорта(лВетка, лИмя,
											лМиграционнаяКартаНомер,
											лМиграционнаяКартаДатаНачалаПребывания,
											лМиграционнаяКартаДатаОкончанияПребывания,
											лМиграционныйДокументКодДокумента,
											лМиграционныйДокументНомерДокумента,
											лМиграционныйДокументДатаНачалаПребывания,
											лМиграционныйДокументДатаОкончанияПребывания) Экспорт
	
	Если ЗначениеЗаполнено(лМиграционнаяКартаНомер) Или ЗначениеЗаполнено(лМиграционныйДокументНомерДокумента) Тогда
		ВеткаНерезидент = ДобавитьЭлемент(лВетка, лИмя);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(лМиграционнаяКартаНомер) Тогда
		ВеткаМиграционнаяКарта = ДобавитьЭлемент(ВеткаНерезидент, "МиграционнаяКарта");
		ДобавитьЭлемент(ВеткаМиграционнаяКарта, "Номер", лМиграционнаяКартаНомер);
		ДобавитьЭлемент(ВеткаМиграционнаяКарта, "ДатаНачалаПребывания", лМиграционнаяКартаДатаНачалаПребывания);
		ДобавитьЭлемент(ВеткаМиграционнаяКарта, "ДатаОкончанияПребывания", лМиграционнаяКартаДатаОкончанияПребывания);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(лМиграционныйДокументНомерДокумента) Тогда
		ВеткаМиграционныйДокумент = ДобавитьЭлемент(ВеткаНерезидент, "МиграционныйДокумент");
		ДобавитьЭлемент(ВеткаМиграционныйДокумент, "КодДокумента", лМиграционныйДокументКодДокумента);
		ДобавитьЭлемент(ВеткаМиграционныйДокумент, "НомерДокумента", лМиграционныйДокументНомерДокумента);
		ДобавитьЭлемент(ВеткаМиграционныйДокумент, "ДатаНачалаПребывания", лМиграционныйДокументДатаНачалаПребывания);
		ДобавитьЭлемент(ВеткаМиграционныйДокумент, "ДатаОкончанияПребывания", лМиграционныйДокументДатаОкончанияПребывания);
	КонецЕсли;
...
Функция СоздатьНовыйПотокXMLСчетаПК() Экспорт
	...
	Если ПустаяСтрока(НаименованиеОрганизации) Тогда
		ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Не указано наименование организации!"));
		Возврат Неопределено;
	КонецЕсли;
	
	Если ПустаяСтрока(ИНН) Тогда
		ПроцедурыУправленияПерсоналом.СообщитьОбОшибкеОбработки(СтадияОбработки, ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Не указан ИНН организации!"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция ЗаписатьТесктНаДиск(ТекстДляЗаписи, ФайлДляЗаписи) Экспорт
	...
	ПотокЗаписи = Новый ЗаписьТекста(ФайлДляЗаписи, ?(Кодировка = 2, "windows-1251", КодировкаТекста.UTF8));
...
//	ШаблонЭлементаВерхнегоУровня.Вставить("РасчетныйСчетОрганизации", Неопределено);
	...
	Для Каждого СтрокаСчетаПК из СчетаПК.Строки Цикл
		...
		Если Лев(СтрокаСчетаПК.Имя, 1) = "x" Тогда
			Продолжить;
		КонецЕсли;
...
ФорматФайла33 = Перечисления.ФорматФайлаОбменаПоЗарплатномуПроекту.Версия33;
ФорматФайла34 = Перечисления.ФорматФайлаОбменаПоЗарплатномуПроекту.Версия34;