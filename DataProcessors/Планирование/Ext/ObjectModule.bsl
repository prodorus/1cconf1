...
Процедура УстановитьПериод() Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&ДатаНач, " + СтрокаТекущаяПериодичность + ", 1), " + СтрокаТекущаяПериодичность + ") КАК ДатаНач,
	|	КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(&ДатаКон, " + СтрокаТекущаяПериодичность + ", 1), " + СтрокаТекущаяПериодичность + ") КАК ДатаКон");
