...
Функция ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюИсходящиеСообщения() Экспорт
...
Функция ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюВходящиеСообщения() Экспорт
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюВходящиеСообщения(Параметры) Экспорт
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА УдалитьПротоколОбменаЕГАИС.ВидДокумента В (&Квитанции)
	|				И ТаблицаСообщениеОснованиеКвитанции.Ссылка ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ОтметитьВыполнениеОбработки Тогда
				Продолжить;
		ИначеЕсли Не ЗначениеЗаполнено(ОрганизацияЕГАИС) Тогда
			// Без организации ЕГАИС записать данные в протокол обмена нельзя.
			Продолжить;
		КонецЕсли;
