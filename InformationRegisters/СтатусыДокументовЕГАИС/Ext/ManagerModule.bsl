...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(МассивСсылок) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.Проведен ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|ИЗ
	|	Документ.АктПостановкиНаБалансЕГАИС КАК ДанныеДокумента
	|ГДЕ
	...
	|	ДанныеДокумента.УдалитьСтатусОбработки
	|ИЗ
	|	Документ.АктСписанияЕГАИС КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивСсылок)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДокументовЕГАИС.ВозвратИзРегистра2),
	|	ДанныеДокумента.УдалитьСтатусОбработки
	|ИЗ
	|	Документ.ВозвратИзРегистра2ЕГАИС КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивСсылок)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ДанныеДокумента.УдалитьСтатусОбработки
	|ИЗ
	|	Документ.ОстаткиЕГАИС КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивСсылок)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДокументовЕГАИС.ПередачаВРегистр2),
	|	ДанныеДокумента.УдалитьСтатусОбработки
	|ИЗ
	|	Документ.ПередачаВРегистр2ЕГАИС КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивСсылок)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДокументовЕГАИС.ТТН),
	|	ДанныеДокумента.УдалитьСтатусОбработки
	|ИЗ
	|	Документ.ТТНВходящаяЕГАИС КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивСсылок)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ДанныеДокумента.УдалитьСтатусОбработки
	|ИЗ
	|	Документ.ТТНИсходящаяЕГАИС КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивСсылок)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДокументовЕГАИС.ЧекККМ),
	|	ДанныеДокумента.УдалитьСтатусОбработки
	|ИЗ
	|	Документ.ЧекЕГАИС КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивСсылок)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ДанныеДокумента.УдалитьСтатусОбработки
	|ИЗ
	|	Документ.ЧекЕГАИСВозврат КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивСсылок)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ТТНВходящаяЕГАИС") Тогда
			ИменаРеквизитов = "СтатусПроверкиИПодбора";
