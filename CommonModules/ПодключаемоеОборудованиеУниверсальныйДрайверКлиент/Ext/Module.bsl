			...
			ПараметрыФискализации = МенеджерОборудованияВызовСервера.ПодготовитьДанныеФискализацииЧека(ВходныеПараметры, ПараметрыПодключения);
		 	...
		 	Попытка
				...
				Если Результат Тогда  
					...
					ВыходныеПараметры.Очистить();
					ВыходныеПараметры.Добавить(ПараметрыФискализации.НомерСменыККТ//НомерСмены
						);
					ВыходныеПараметры.Добавить(ПараметрыФискализации.НомерЧекаККТ//НомерЧека
						);
					ВыходныеПараметры.Добавить(2); // 2 - Открыта - Состояние смены  
					ВыходныеПараметры.Добавить(ПараметрыФискализации.ДатаВремяЧека//ДатаСеанса
						);
					ВыходныеПараметры.Добавить(ПараметрыФискализации.ФискальныйПризнак//ФискальныйПризнак
						);
					ВыходныеПараметры.Добавить(ПараметрыФискализации.АдресСайтаПроверки//АдресСайтаПроверки
						);
					ВыходныеПараметры.Добавить(ПараметрыФискализации.РегистрационныйНомерККТ//РегистрационныйНомерККТ
						);
					ВыходныеПараметры.Добавить(ПараметрыФискализации.ДанныеЧекаXML//ДанныеЧека
						);
...
//	Если Не ПараметрыПодключения.Свойство("ТипОборудования") Тогда
//		ВерсияККТ = ?(ПараметрыРегистрации.Свойство("ВерсияФФДККТ"), ПараметрыРегистрации.ВерсияФФДККТ, "1.0");
//		Если ВерсияККТ = "1.1" Тогда
//			ВерсияФФД = 2;
//		ИначеЕсли ВерсияККТ = "1.0.5" Тогда
...
//			ВерсияФФД = 0;
//		КонецЕсли;
//		ДанныеЧека = МенеджерОборудованияВызовСервера.ПолучитьXMLПакетДляЧекаКоррекции_ТО(ОбщиеПараметры, ВыходныеПараметры, НовыйФормат, ВидовОплаты, ВерсияФФД);
//	Иначе
