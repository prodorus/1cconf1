...
Процедура ПодготовитьСообщенияКПередачеПодтверждениеПроведения(Ответ, Контекст) Экспорт
	...
	ДанныеОНепроведенныхДокументах = ОбщегоНазначения.ПровестиДокументы(Контекст.НепроведенныеДокументы, ТипДокументов);
	...
	Если НепроведенныеДокументы.Количество() > 0 Тогда
		...
		Обработчик = ОписаниеОповещенияЕГАИС("ПодготовитьСообщенияКПередачеПослеПодтверждениеПродолжения", ИнтеграцияЕГАИССлужебныйКлиент, Контекст);
		ПоказатьВопросЕГАИС(Обработчик, ТекстДиалога, КнопкиДиалога);
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьРезультатВыполненияОбменаЕГАИС(ДополнительныеПараметры) Экспорт
	...
	ОткрытьФормуЕГАИС("ОбщаяФорма.РезультатВыполненияОбменаЕГАИС", ДополнительныеПараметры);
...
Процедура ПослеЗавершенияОбмена(Изменения, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		КоличествоИзменений = Изменения.Количество();
		Если КоличествоИзменений Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Изменено объектов: %1'"), КоличествоИзменений)
			+Символы.ПС
			+"Изменения доступны в протоколе обмена ЕГАИС";
		Иначе
			ТекстСообщения = НСтр("ru = 'Изменений не произведено'");
		КонецЕсли;
	...
	Если ДополнительныеПараметры.ОповещениеПриЗавершении <> Неопределено Тогда
		ВыполнитьОбработкуОповещенияЕГАИС(ДополнительныеПараметры.ОповещениеПриЗавершении, Изменения);
	КонецЕсли;
...
Процедура ОбработатьОчередьПередачиДанных(ОповещениеПриЗавершении, ДанныеДляВыполненияОбменаНаКлиенте) Экспорт
	...
	Иначе
		...
		Если ОповещениеПриЗавершении <> Неопределено Тогда
			ВыполнитьОбработкуОповещенияЕГАИС(ОповещениеПриЗавершении, ДанныеДляВыполненияОбменаНаКлиенте.Изменения);
		КонецЕсли;
...
Процедура ПослеОбработкиОчередиПередачиДанных(Изменения, Контекст) Экспорт
	...
	ОповещениеПриЗавершенииПолученияСпискаВходящихДокументов = ОписаниеОповещенияЕГАИС(
...
Процедура ОбработатьОчереднуюПорциюДанных() Экспорт
	ВыполнитьОбработкуОповещенияЕГАИС(ПараметрыПриложения["ЕГАИС.ДанныеСеансаОбмена"].ОбработчикОповещения);
КонецПроцедуры
...
Процедура ПередатьСообщения(СообщенияXML, Контекст)
	...
	ПараметрыПриложения.Вставить("ЕГАИС.ДанныеСеансаОбмена", 
		СохраняемыеДанныеОбмена(
			СообщенияXML,
			Контекст,
			ОписаниеОповещенияЕГАИС("ПередатьСообщение", ИнтеграцияЕГАИССлужебныйКлиент)));
...
Процедура ПередатьСообщение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеОбменаЕГАИС.СообщенийXML > ДанныеОбменаЕГАИС.НомерКОбработке Тогда
		...
		ОтправитьHTTPЗапрос(
			ОписаниеОповещенияЕГАИС("ПослеПередачиСообщения", ИнтеграцияЕГАИССлужебныйКлиент),
...
Процедура ПослеПередачиСообщений()
	...
	Если ОповещениеПриЗавершении <> Неопределено Тогда
		ВыполнитьОбработкуОповещенияЕГАИС(ОповещениеПриЗавершении, ПараметрОповещения);
	КонецЕсли;
...
Процедура ПолучитьСпискиВходящихСообщений(ОрганизацииЕГАИС, Контекст) Экспорт
	...
	ПараметрыПриложения.Вставить("ЕГАИС.ДанныеСеансаОбмена", 
		СохраняемыеДанныеОбмена(
			ОрганизацииЕГАИС,
			Контекст,
			ОписаниеОповещенияЕГАИС("ПолучитьСписок", ИнтеграцияЕГАИССлужебныйКлиент)));
	ПодключитьОбработчикОжидания("ОбработчикОжиданияОбменаОчереднойПорциейДанныхЕГАИС", 0.1, Истина);
...
Процедура ПолучитьСписок(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеОбменаЕГАИС.СообщенийXML > ДанныеОбменаЕГАИС.НомерКОбработке Тогда
		...
		Если НастройкаОбменаНаКлиенте.ЗагружатьВходящиеДокументы Тогда
			...
			ОтправитьHTTPЗапрос(
				ОписаниеОповещенияЕГАИС("ПослеПолученияСписка", ИнтеграцияЕГАИССлужебныйКлиент),
				НастройкаОбменаНаКлиенте,
				ПараметрыHTTPЗапроса);
...
Процедура ПолучитьСообщения(ДанныеДокументовКПолучению, Контекст)
	...
	ПараметрыПриложения.Вставить("ЕГАИС.ДанныеСеансаОбмена", 
		СохраняемыеДанныеОбмена(
			...
			Контекст,
			ОписаниеОповещенияЕГАИС("ПолучитьСообщение", ИнтеграцияЕГАИССлужебныйКлиент)));
	ПодключитьОбработчикОжидания("ОбработчикОжиданияОбменаОчереднойПорциейДанныхЕГАИС", 0.1, Истина);
...
Процедура ПолучитьСообщение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеОбменаЕГАИС.СообщенийXML > ДанныеОбменаЕГАИС.НомерКОбработке Тогда
		...
		ОтправитьHTTPЗапрос(
			ОписаниеОповещенияЕГАИС("ПослеПолученияСообщения", ИнтеграцияЕГАИССлужебныйКлиент),
			НастройкаОбменаНаКлиенте,
			ПараметрыHTTPЗапроса);
	Иначе
...
Процедура ПослеПолученияСообщений()
	...
	Если Результат.Количество() = 0 Тогда
		...
		Если Контекст.ОповещениеПриЗавершении <> Неопределено Тогда
			ВыполнитьОбработкуОповещенияЕГАИС(Контекст.ОповещениеПриЗавершении, Результат);
		КонецЕсли;
...
Процедура ПодтвердитьЗагруженныеСообщения(ДанныеДокументовКУдалению, Контекст)
	...
	ПараметрыПриложения.Вставить("ЕГАИС.ДанныеСеансаОбмена", 
		СохраняемыеДанныеОбмена(
			...
			Контекст,
			ОписаниеОповещенияЕГАИС("ПодтвердитьСообщение", ИнтеграцияЕГАИССлужебныйКлиент)));
	ПодключитьОбработчикОжидания("ОбработчикОжиданияОбменаОчереднойПорциейДанныхЕГАИС", 0.1, Истина);
...
Процедура ПодтвердитьСообщение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеОбменаЕГАИС.СообщенийXML > ДанныеОбменаЕГАИС.НомерКОбработке Тогда
		...
		ОтправитьHTTPЗапрос(
			ОписаниеОповещенияЕГАИС("ПослеПодтвержденияСообщения", ИнтеграцияЕГАИССлужебныйКлиент),
			НастройкаОбменаНаКлиенте,
			ПараметрыHTTPЗапроса);
...
Процедура ПослеПодтвержденияСообщений()
	...
	Если ОповещениеПриЗавершении <> Неопределено Тогда
		ВыполнитьОбработкуОповещенияЕГАИС(ОповещениеПриЗавершении, Изменения);
	КонецЕсли;
...
Процедура ОтправитьHTTPЗапрос(ОповещениеПриЗавершении, НастройкаОбменаНаКлиенте, ПараметрыHTTPЗапроса)
		...
		РезультатОтправкиHTTPЗапроса = ИнтеграцияЕГАИСКлиентСервер.ОтправитьHTTPЗапрос(
			НастройкаОбменаНаКлиенте,
			ПараметрыHTTPЗапроса);
		ВыполнитьОбработкуОповещенияЕГАИС(ОповещениеПриЗавершении, РезультатОтправкиHTTPЗапроса);
...
Функция ОбменУжеЗапущен(Контекст, ВыводитьСообщения = Истина)
	...
	Если Контекст.ВОсновнойФорме = Ложь Тогда
		Контекст.Форма = ОткрытьФормуЕГАИС("ОбщаяФорма.ДлительнаяОперация",
