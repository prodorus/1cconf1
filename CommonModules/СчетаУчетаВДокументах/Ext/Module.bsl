...
Процедура УстановитьСчетаУчета(ЗаполняемыйОбъект, ТаблицаРеквизитов, СтруктураСчетовУчета, ХозяйственнаяСитуация, ДанныеСклада = Неопределено, РозничныйСклад=Неопределено, ЗаполнятьБУ=Истина, ЗаполнятьНУ=Истина, Перезаполнять=Ложь, ЗаполнятьКлоныКолонок=Истина)
	...
	КолонкиЗаполняемыйОбъект = ?(ТипЗнч(ЗаполняемыйОбъект) = Тип("СтрокаТаблицыЗначений"), ЗаполняемыйОбъект.Владелец().Колонки, Неопределено);
	
	Для Каждого ДанныеРеквизита Из СтрокиРеквизитов Цикл
		
		Реквизит = ДанныеРеквизита.Реквизит;
		
		Если КолонкиЗаполняемыйОбъект <> Неопределено И КолонкиЗаполняемыйОбъект.Найти(Реквизит) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьСубконтоЗатрат(ДанныеСтроки, ТаблицаРеквизитов)
	...
	РеквизитыСубконтоЗатрат = РеквизитыСубконтоЗатрат();
	ВидСубконтоСтатьиЗатрат    = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат; // Для планов счетов Хозрасчетный и Налоговый используется общий план видов характеристик
	
	Для Каждого СтрокаТаблицыРеквизитов Из ТаблицаРеквизитов Цикл
		
		НомерСубконто = Неопределено;
		ИмяСубконто   = СтрокаТаблицыРеквизитов.Реквизит;
		
		Если НЕ РеквизитыСубконтоЗатрат.Свойство(ИмяСубконто, НомерСубконто) Тогда
			// Этот реквизит здесь не обрабатываем
			Продолжить;
		КонецЕсли;
		
		Если НЕ ПроверитьНеобходимостьУстановкиСубконто(
					ДанныеСтроки[СтрокаТаблицыРеквизитов.СчетЗатрат], 
					НомерСубконто, 
					ВидСубконтоСтатьиЗатрат) Тогда
			// Нет необходимости устанавливать данное субконто
			Продолжить;
		КонецЕсли;
		...
		УстановитьСубконто(
			ИмяСубконто,
			, // ИмяРеквизитаШапки
			СтрокаТаблицыРеквизитов.СтатьяЗатратТабличнойЧасти,
...
//
Процедура УстановитьСубконтоПодразделение(ДанныеСтроки, ТаблицаРеквизитов, СтруктураШапкиДокумента)
	...
	Для Каждого СтрокаТаблицыРеквизитов Из ТаблицаРеквизитов Цикл
		...
		Если НЕ ПроверитьНеобходимостьУстановкиСубконто(
					ДанныеСтроки[СтрокаТаблицыРеквизитов.СчетЗатрат], 
					НомерСубконто, 
					ВидСубконтоПодразделения) Тогда
			// Нет необходимости устанавливать данное субконто
			Продолжить;
		КонецЕсли;
		...
		УстановитьСубконто(
			ИмяСубконто,
			СтрокаТаблицыРеквизитов.ПодразделениеШапки,
			СтрокаТаблицыРеквизитов.ПодразделениеТабличнойЧасти,
			СтруктураШапкиДокумента,
			ДанныеСтроки);
...
// 
Функция ПроверитьНеобходимостьУстановкиСубконто(Счет, НомерСубконто, ВидСубконто)
	...
	Если ВидыСубконтоНаСчете[НомерСубконто - 1].ВидСубконто <> ВидСубконто Тогда
...
Процедура УстановитьСубконто(
	...
	Если НЕ ЗначениеЗаполнено(ИмяРеквизитаШапки) 
		 И НЕ (ЗначениеЗаполнено(ИмяРеквизитаТабличнойЧасти)) Тогда
	...
	Если ЗначениеЗаполнено(ИмяРеквизитаШапки)
		 И ЗначениеЗаполнено(СтруктураШапкиДокумента) Тогда
		ЗначениеСубконто = СтруктураШапкиДокумента[ИмяРеквизитаШапки]
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ИмяРеквизитаТабличнойЧасти)
		 И ЗначениеЗаполнено(ДанныеСтроки) Тогда
		ЗначениеСубконто = ДанныеСтроки[ИмяРеквизитаТабличнойЧасти];
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ЗначениеСубконто) 
		И ДанныеСтроки[ИмяСубконто] <> ЗначениеСубконто Тогда
...
Процедура ЗаполнитьСчетаУчетаТабличнойЧасти(ИмяТабличнойЧасти, ДанныеТабличнойЧасти, Объект, ЗаполнятьБУ=Истина, ЗаполнятьНУ=Истина, ЗаполнятьСчетаЗатрат=Истина, ЗаполнятьСчетаНоменклатуры=Истина, ЗаполнятьВЛюбомРежиме=Ложь) Экспорт
	...
	Для Каждого ДанныеСтроки Из СтрокиТабличнойЧасти Цикл
		...
		Если ЗаполнятьСчетаНоменклатуры Тогда
			...
			Для Каждого ДанныеСклада Из ТаблицаСкладов Цикл
				...
				УстановитьСчетаУчета(ДанныеСтроки, ТаблицаРеквизитов, СтруктураСчетовУчета, ХозяйственнаяСитуация, ДанныеСклада, Розничный, ЗаполнятьБУ, ЗаполнятьНУ, Истина, Ложь);
...
Процедура ЗаполнитьПриПроведенииСчетаУчетаТабличнойЧасти(ИмяТабличнойЧасти, ДанныеТабличнойЧасти, СтруктураШапкиДокумента) Экспорт
	...
	Если НЕ СтруктураШапкиДокумента.Свойство("ОтражатьВБухгалтерскомУчете") ИЛИ НЕ СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		...
		Если ТаблицаРеквизитов.Найти("ПринадлежностьНоменклатуры", "Реквизит") = Неопределено
				И НЕ СтруктураШапкиДокумента.Свойство("КорректироватьБУиНУ") Тогда
			Возврат;
		КонецЕсли;
	...
	Для Каждого ДанныеСтроки Из ДанныеТабличнойЧасти Цикл
		...
		// Субконото подразделение
		УстановитьСубконтоПодразделение(ДанныеСтроки, ТаблицаРеквизитов, СтруктураШапкиДокумента);
...
Функция ОпределитьХозяйственнуюСитуациюПоДокументу(ИмяТабличнойЧасти, ИмяДокумента, СтруктураШапкиДокумента)
	...
	ИначеЕсли ИмяДокумента = "КорректировкаПоступления" Тогда
