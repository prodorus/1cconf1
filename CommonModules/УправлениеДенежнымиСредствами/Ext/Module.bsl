...
// Возвращает таблицу, для проведения документа по регистру "ДвиженияДСпоГосконтрактам"
//
Функция ПолучитьТаблицуДвиженияДСпоГосконтрактам(Документ) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Поступление.Ссылка.СчетОрганизации КАК БанковскийСчетОрганизации,
	|	Поступление.Ссылка.СчетКонтрагента КАК БанковскийСчетКонтрагента,
	|	Поступление.Ссылка.Контрагент КАК Контрагент,
	|	ЕСТЬNULL(КонтрактыИсполнителей.Ссылка, ЗНАЧЕНИЕ(Справочник.КонтрактыИсполнителей.ПустаяСсылка)) КАК КонтрактИсполнителя,
	...
	|	ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.ПустаяСсылка) КАК ТипПлатежа,
	|	Поступление.СуммаПлатежа КАК Доход,
	|	0 КАК Расход
	|ИЗ
	|	Документ.ПлатежноеПоручениеВходящее.РасшифровкаПлатежа КАК Поступление
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КонтрактыИсполнителей КАК КонтрактыИсполнителей
	|		ПО Поступление.ДоговорКонтрагента = КонтрактыИсполнителей.Договор
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КонтрактыСЗаказчиками КАК КонтрактыСЗаказчиками
	|		ПО Поступление.ДоговорКонтрагента = КонтрактыСЗаказчиками.Договор
	|ГДЕ
	|	Поступление.Ссылка = &Документ
	|	И (НЕ КонтрактыИсполнителей.Ссылка ЕСТЬ NULL 
	|			ИЛИ НЕ КонтрактыСЗаказчиками.Ссылка ЕСТЬ NULL )
	|	И Поступление.ДоговорКонтрагента <> &ПустойДоговор
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Списание.Ссылка.СчетОрганизации,
	|	Списание.Ссылка.СчетКонтрагента,
	|	Списание.Ссылка.Контрагент,
	|	ЕСТЬNULL(КонтрактыИсполнителей.Ссылка, ЗНАЧЕНИЕ(Справочник.КонтрактыИсполнителей.ПустаяСсылка)),
	|	ЕСТЬNULL(КонтрактыСЗаказчиками.Ссылка, ЗНАЧЕНИЕ(Справочник.КонтрактыСЗаказчиками.ПустаяСсылка)),
	|	Списание.ДокументПланированияПлатежа.СтатьяКалькуляции,
	|	Списание.ДокументПланированияПлатежа.ТипПлатежаФЗ275,
	|	0,
	|	Списание.СуммаПлатежа
	|ИЗ
	|	Документ.ПлатежноеПоручениеИсходящее.РасшифровкаПлатежа КАК Списание
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КонтрактыИсполнителей КАК КонтрактыИсполнителей
	|		ПО Списание.ДоговорКонтрагента = КонтрактыИсполнителей.Договор
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КонтрактыСЗаказчиками КАК КонтрактыСЗаказчиками
	|		ПО Списание.ДоговорКонтрагента = КонтрактыСЗаказчиками.Договор
	|ГДЕ
	|	Списание.Ссылка = &Документ
	|	И (НЕ КонтрактыИсполнителей.Ссылка ЕСТЬ NULL 
	|			ИЛИ НЕ КонтрактыСЗаказчиками.Ссылка ЕСТЬ NULL )
	|	И Списание.ДоговорКонтрагента <> &ПустойДоговор";
		...
		Если ТаблицаКонтроляПроцентаОплаты.Количество() > 0 Тогда
			...
			Для Каждого Платеж Из ТаблицаКонтроляПроцентаОплаты Цикл
				...
				Если Выборка.НайтиСледующий(СтруктураПоиска) Тогда
					...
					СуммаНеобходимойПредоплаты = Окр(СуммаЗаказа * ПроцентПредоплаты / 100, 2);
					
					Если СуммаОплаты < СуммаНеобходимойПредоплаты Тогда
						...
						ОбщегоНазначения.СообщитьОбОшибке("Недостаточна предоплата по заказу " + Платеж.Сделка + 
							...
							""" требуется предоплата в размере " + ПроцентПредоплаты + "% на сумму " + СуммаНеобходимойПредоплаты + 
							" " + Валюта + " ; Поступила предоплата  в размере " + Окр(ПроцентСовершеннойПредоплаты, 2, 1) + 
							"% на сумму " +	СуммаОплаты + " " + Валюта, Отказ, Заголовок);
					КонецЕсли;
	...
	Если Оплачено И ОтраженоВОперУчете Тогда  // Проводим по фактическим взаиморасчетам
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПКО.РасчетыПоКредитамИЗаймамСРаботниками Тогда
			...
			Иначе 
				...
				СуммаРасчетов = ДвиженияПоКонтрагентам[0].СуммаУпр;
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПКО.ВозвратДенежныхСредствРаботником Тогда
			...
			Иначе
				...
				СтрокаДвижений.Период      = Дата;
				СтрокаДвижений.ФизЛицо     = Контрагент;
				СтрокаДвижений.СуммаУпр    = - ДвиженияПоКонтрагентам[0].СуммаУпр;
