...
Функция ПредметСтрокой(СсылкаНаПредмет) Экспорт
	...
	ОбщегоНазначенияЗКПереопределяемый.УстановитьПредставлениеПредмета(СсылкаНаПредмет, Результат); 
...
//  отдельным Фамилии,Имени и Отчеству
//
Функция ИнициалыФизЛицаФамилия(Объект = "", Фамилия = " ", Имя = " ", Отчество = " ", НаДату = Неопределено) Экспорт
...
Функция ФамилияИмяОтчествоФизЛица(Объект, НаДату = Неопределено)
	...
	Если ТипОбъекта <> Тип("Неопределено") Тогда
		...
		ИначеЕсли СписокТиповСсылок.СодержитТип(ТипОбъекта) Или СписокТиповОбъектов.СодержитТип(ТипОбъекта) Тогда
			...
			ФИО = РегламентированнаяОтчетность.ПолучитьФИОФизЛица(Объект, НаДату);
			
			Если ЗначениеЗаполнено(ФИО.Фамилия + ФИО.Имя + ФИО.Отчество) Тогда
				Возврат ФИО;
			КонецЕсли;
	...
	Результат.Вставить("Фамилия",	?(КоличествоПодстрок > 0,ФИО[ИндексПервогоЭлемента],""));
	Результат.Вставить("Имя",		?(КоличествоПодстрок > 1,ФИО[ИндексПервогоЭлемента + 1],""));
	Результат.Вставить("Отчество",	?(КоличествоПодстрок > 2,ФИО[ИндексПервогоЭлемента + 2],""));
...
Процедура ДобавитьПереименование(Итог, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	...
	Попытка
		Результат = СтроковыеФункцииКлиентСервер.СравнитьВерсии(ВерсияИБ, ВерсияБиблиотеки);
	Исключение
...
Функция ОписаниеСвойствОбъекта(ОбъектМетаданных, Свойства) Экспорт
	...
	МассивСвойств = РегламентированнаяОтчетность.СтрРазделить82(Свойства, ",");
