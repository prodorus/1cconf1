...
Процедура ОткрытьФайлы(ОбъектФайла, ВыделенныеСтроки = Неопределено, ВопросОПерезаписи = Истина) Экспорт
	...
	Иначе
		...
		Для каждого СсылкаФайл из ВыделенныеСтроки Цикл
			...
			Если ТипЗнч(СсылкаФайл) = Тип("СтрокаТаблицыЗначений") Тогда
				ДанныеФайла = СсылкаФайл.Данные;
			Иначе
				ДанныеФайла = СсылкаФайл.Хранилище;
			КонецЕсли;
			
			ОткрытьФайлИзХранилища(СсылкаФайл.ИмяФайла, ДанныеФайла, СпособПерезаписи, ВопросОПерезаписи);
...
Процедура ОткрытьФормуСпискаФайловИИзображений(СтруктураДляСпискаИзображений, СтруктураДляСпискаДополнительныхФайлов, ОбязательныеОтборы, ФормаВладелец, ИмяСправочника = "ХранилищеДополнительнойИнформации") Экспорт
	...
	КонецЕсли; 
	Если СтруктураДляСпискаДополнительныхФайлов.Свойство("РасширениеФормыФайламиРезюме") Тогда
		Если СтруктураДляСпискаДополнительныхФайлов.РасширениеФормыФайламиРезюме Тогда
