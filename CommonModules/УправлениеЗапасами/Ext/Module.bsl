...
Процедура ПолучитьДеревоОстатковНоменклатуры(ДеревоОстатков, Номенклатура, ОтображатьСклады, 
	...
	Если ДатаАктуальности = Неопределено Тогда
		ДатаАктуальности = Дата(3000,1,1);
	КонецЕсли;
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|				ИЗ
		|					РегистрНакопления.ЗаказыПоставщикам.Остатки(&ДатаАктуальности, Номенклатура = &Номенклатура) КАК ЗаказыПоставщикамОстатки
		|				ЛЕВОЕ СОЕДИНЕНИЕ
		|					РегистрНакопления.РазмещениеЗаказовПокупателей.Остатки(&ДатаАктуальности, Номенклатура = &Номенклатура) КАК РазмещениеЗаказовПокупателейОстатки
		|				ПО
...
Процедура ПолучитьДеревоОстатковНоменклатурыСвободныеОстатки(ДеревоОстатков, Номенклатура, ОтображатьСклады, 
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|			, 0                                                        КАК ТоварыКПередачеСоСкладовОстатки
		|		ИЗ
		|			РегистрНакопления.СвободныеОстатки.Остатки(&ДатаАктуальности, Номенклатура = &Номенклатура" + ОграничениеПоСкладу + ") КАК СвободныеОстатки
        |
		|		ОБЪЕДИНИТЬ ВСЕ
		|		
		|		ВЫБРАТЬ
		...
		Для Каждого СтрокаКомплект Из НайденныеКомплекты Цикл
			...
			Для Каждого СтрокаКомплектующая Из НайденныеКомплектующие Цикл
				...
				Для Сч = 1 По ТаблицаРезультат.Колонки.Количество() Цикл
					...
					ИначеЕсли ИмяКолонки = "СчетУчетаБУКомплекта" И ЕстьСчетУчетаБУКомплекта Тогда
						
						НоваяСтрока.СчетУчетаБУКомплекта = СтрокаКомплект.СчетУчетаБУ;
						
					ИначеЕсли ИмяКолонки = "СчетУчетаНУКомплекта" И ЕстьСчетУчетаНУКомплекта Тогда
						
						НоваяСтрока.СчетУчетаНУКомплекта = СтрокаКомплект.СчетУчетаНУ;
