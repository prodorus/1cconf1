...
Функция СформироватьТекстЗапросаВременнаяТаблицаПродажиРеглРасширеннаяАналитика()
	...
	|				ИЛИ УчетПродажИСебестоимости.Регистратор ССЫЛКА Документ.РеализацияУслугПоПереработке
...
Функция СформироватьТекстЗапросаБазаВручнуюРасширеннаяАналитика()
	...
	|
	|	//ДляУпрУчета ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетУчета,
	|	//ДляБухУчета КлючиАналитикаВидаУчета.СчетУчета,
	|	//ДляНалУчета КлючиАналитикаВидаУчета.СчетУчетаНУ КАК СчетУчета,
	|
	...
	|	//ДляБухУчета 0 КАК БазаОстатокНЗПНУ,
	|	
	|	//ДляНалУчета БазаРаспределенияЗатрат.База КАК БазаНУ,
	|	//ДляНалУчета БазаРаспределенияЗатрат.БазаПриход КАК БазаПриходНУ,
	|	//ДляНалУчета БазаРаспределенияЗатрат.БазаОстатокНЗП КАК БазаОстатокНЗПНУ,
	|		
	...
	|	
	|	//ДляРеглУчета ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|	//ДляРеглУчета 	РегистрСведений.АналитикаВидаУчета КАК КлючиАналитикаВидаУчета
	|	//ДляРеглУчета ПО
	|	//ДляРеглУчета 	БазаРаспределенияЗатрат.АналитикаВидаУчета = КлючиАналитикаВидаУчета.Ссылка
	|	
	|ГДЕ
...
Функция СформироватьВременнуюТаблицуБазаРаспределенияЗатрат(
	...
	Для Каждого ВидОтраженияВУчете Из СтруктураПараметров.МассивВидовУчета Цикл
		...
		// Поместим имя временной таблицы в структуру
		Если ТипЗнч(СтруктураСформированныхТаблицБазы) = Тип("Структура") Тогда
			СтруктураСформированныхТаблицБазы.Вставить(ИмяВременнойТаблицы);
		КонецЕсли;
...
Процедура РассчитатьБазуРаспределения(
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Для каждого Элемент Из СтруктураСформированныхТаблицБазы Цикл
		Запрос.Текст = "УНИЧТОЖИТЬ " + Элемент.Ключ;
		Запрос.Выполнить();
	КонецЦикла;
