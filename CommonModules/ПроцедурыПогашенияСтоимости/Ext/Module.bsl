...
Функция СформироватьТекстЗапросаПоПартиямМатериаловВЭксплуатацииРасширеннаяАналитика()
	...
	|	
	|	//ДляРеглУчета СУММА(УчетЗатрат.СтоимостьНУОстаток) КАК ПогашеннаяСтоимостьНУ,
	|	//ДляРеглУчета СУММА(УчетЗатрат.ПостояннаяРазницаОстаток) КАК ПогашеннаяСтоимостьПР,
	...
	|	//ДляРеглУчета (УчетЗатрат.КоличествоНУНачальныйОстаток + УчетЗатрат.КоличествоНУПриход) КАК КоличествоНУОстаток,
	|	//ДляРеглУчета (УчетЗатрат.СтоимостьНУНачальныйОстаток + УчетЗатрат.СтоимостьНУПриход) КАК СтоимостьНУОстаток,
	|	//ДляРеглУчета ЕСТЬNULL(ТаблицаПогашеннойСтоимости.ПогашеннаяСтоимостьНУ, 0) КАК ПогашеннаяСтоимостьНУОстаток,
	|	
	|	//ДляРеглУчета (УчетЗатрат.КоличествоНУНачальныйОстаток + УчетЗатрат.КоличествоНУПриход) КАК КоличествоПРОстаток,
	|	//ДляРеглУчета (УчетЗатрат.ПостояннаяРазницаНачальныйОстаток + УчетЗатрат.ПостояннаяРазницаПриход) КАК СтоимостьПРОстаток,
	|	//ДляРеглУчета ЕСТЬNULL(ТаблицаПогашеннойСтоимости.ПогашеннаяСтоимостьПР, 0) КАК ПогашеннаяСтоимостьПРОстаток,
	...
	|	//ДляРегУчетаБезНП И (УчетЗатрат.КоличествоНУНачальныйОстаток + УчетЗатрат.КоличествоНУПриход) > 0
...
Функция СформироватьТекстЗапросаПоПартиямМатериаловВЭксплуатации()
	...
	|	//ДляНалУчета ПартииМатериалов.ПогашеннаяСтоимостьОстаток КАК ПогашеннаяСтоимостьОстаток
	|
	|ИЗ
...
Функция СформироватьТаблицуОстатковМатериаловВЭксплуатации(
	...
	Если НЕ СтруктураШапкиДокумента.ОрганизацияПрименяетУСН Тогда
		ТекстЗапросаСКомментариями = СтрЗаменить(ТекстЗапросаСКомментариями, "ДляРегУчетаБезНП", "ДляРеглУчета");
	КонецЕсли;
...
Процедура РассчитатьСуммуСписанияИзЭксплуатации(
	...
	ПогашатьСтоимостьВНУКакВБУ = СтруктураШапкиДокумента.Свойство("ПогашатьСтоимостьВНУКакВБУ")
				И СтруктураШапкиДокумента.ПогашатьСтоимостьВНУКакВБУ
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаТаблицы Из МассивСтрок Цикл
			...
			СтоимостьНУ = 0;
			ПогашеннаяСтоимостьНУ = 0;
			КорректировкаСтоимостиСписанияНУ = 0;
			
			ВременнаяРазница = 0;
			ПогашеннаяСтоимостьВР = 0;
			КорректировкаСтоимостиСписанияВР = 0;
			
			ПостояннаяРазница = 0;
			ПогашеннаяСтоимостьПР = 0;
			КорректировкаСтоимостиСписанияПР = 0;
			
			Если СтруктураШапкиДокумента.ИспользоватьРасширеннуюАналитику Тогда
				
				Стоимость = Выборка.Стоимость;
				Если ПогашатьСтоимостьВНУКакВБУ Тогда
					СтоимостьНУ = Выборка.СтоимостьНУ;
					ПостояннаяРазница = Выборка.СтоимостьПР;
					ВременнаяРазница = Стоимость - СтоимостьНУ - ПостояннаяРазница;
				КонецЕсли;
				Если КоличествоОстаток <= СписатьКоличество Тогда
					...
					Если ПогашатьСтоимостьВНУКакВБУ Тогда
						ПогашеннаяСтоимостьНУ = СтрокаТаблицы.ПогашеннаяСтоимостьНУОстаток;
						ПогашеннаяСтоимостьПР = СтрокаТаблицы.ПогашеннаяСтоимостьПРОстаток;
						ПогашеннаяСтоимостьВР = ПогашеннаяСтоимость - ПогашеннаяСтоимостьНУ - ПогашеннаяСтоимостьПР;
					КонецЕсли;
				Иначе
					...
					Если ПогашатьСтоимостьВНУКакВБУ Тогда
						ПогашеннаяСтоимостьНУ = Окр(СписатьКоличество * СтрокаТаблицы.ПогашеннаяСтоимостьНУОстаток / КоличествоОстаток, 2, 1);
				...
				Если ПогашатьСтоимостьВНУКакВБУ Тогда
					КорректировкаСтоимостиСписанияНУ = Окр(СписатьКоличество * СтрокаТаблицы.СтоимостьНУОстаток / КоличествоОстаток, 2, 1) - СтоимостьНУ;
					КорректировкаСтоимостиСписанияПР = Окр(СписатьКоличество * СтрокаТаблицы.СтоимостьПРОстаток / КоличествоОстаток, 2, 1) - ПостояннаяРазница;
					КорректировкаСтоимостиСписанияВР = КорректировкаСтоимостиСписания - КорректировкаСтоимостиСписанияНУ - КорректировкаСтоимостиСписанияПР;
				КонецЕсли;
			...
			СтрокаТаблицы.СтоимостьОстаток = СтрокаТаблицы.СтоимостьОстаток - Стоимость;
			СтрокаТаблицы.ПогашеннаяСтоимостьОстаток = СтрокаТаблицы.ПогашеннаяСтоимостьОстаток - ПогашеннаяСтоимость;
			Если СтруктураШапкиДокумента.ИспользоватьРасширеннуюАналитику И ПогашатьСтоимостьВНУКакВБУ Тогда
				
				СтрокаТаблицы.СтоимостьНУОстаток = СтрокаТаблицы.СтоимостьНУОстаток - СтоимостьНУ;
				СтрокаТаблицы.ПогашеннаяСтоимостьНУОстаток = СтрокаТаблицы.ПогашеннаяСтоимостьНУОстаток - ПогашеннаяСтоимостьНУ;
				
				СтрокаТаблицы.СтоимостьНУОстаток = СтрокаТаблицы.СтоимостьНУОстаток - СтоимостьНУ;
				СтрокаТаблицы.ПогашеннаяСтоимостьНУОстаток = СтрокаТаблицы.ПогашеннаяСтоимостьНУОстаток - ПогашеннаяСтоимостьНУ;
				
				СтрокаТаблицы.СтоимостьПРОстаток = СтрокаТаблицы.СтоимостьПРОстаток - ПостояннаяРазница;
				СтрокаТаблицы.ПогашеннаяСтоимостьПРОстаток = СтрокаТаблицы.ПогашеннаяСтоимостьПРОстаток - ПогашеннаяСтоимостьПР;
				
				СтрокаТаблицы.ВременнаяРазницаОстаток = СтрокаТаблицы.СтоимостьОстаток - СтрокаТаблицы.СтоимостьНУОстаток
					- СтрокаТаблицы.СтоимостьПРОстаток;
				СтрокаТаблицы.ПогашеннаяСтоимостьПРОстаток = СтрокаТаблицы.ПогашеннаяСтоимостьОстаток - СтрокаТаблицы.ПогашеннаяСтоимостьНУОстаток
					- СтрокаТаблицы.ПогашеннаяСтоимостьПРОстаток;
			...
			Если СтруктураШапкиДокумента.ИспользоватьРасширеннуюАналитику Тогда
				...
				Если ПогашатьСтоимостьВНУКакВБУ Тогда
					...
					СтруктураСуммНУ = 
						Новый Структура(
										"СтоимостьНУ,
										|ПогашеннаяСтоимостьНУ,
										|КорректировкаСтоимостиСписанияНУ,
										
										|ВременнаяРазница,
										|ПогашеннаяСтоимостьВР,
										|КорректировкаСтоимостиСписанияВР,
										
										|ПостояннаяРазница,
										|ПогашеннаяСтоимостьПР,
										|КорректировкаСтоимостиСписанияПР",
					...
					Если ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВРегламентированномУчете Тогда
						СформироватьДвиженияПоРегиструБухгалтерииПогашениеСтоимости(
							...
							0, //Количество,
							ПогашеннаяСтоимость,
							ПогашеннаяСтоимостьНУ, // СтоимостьНал,
							ПогашеннаяСтоимостьПР, // ПостояннаяРазница,
							ПогашеннаяСтоимостьВР, // ВременнаяРазница,
							Выборка.КодОперации,
							ТаблицаОшибок,
							СтруктураНаборыЗаписей
						);
	...
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаПоМатериалам Цикл
		...
		Иначе
			...
			Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
				...
				НоваяСтрока.КоличествоНал = СтрокаТабличнойЧасти.Количество;
				Если СтруктураШапкиДокумента.ТипСтоимости = Перечисления.ВидыНормативнойСтоимостиПроизводства.Фиксированная Тогда
					Если СтруктураШапкиДокумента.Свойство("ПогашатьСтоимостьВНУКакВБУ")
						И СтруктураШапкиДокумента.ПогашатьСтоимостьВНУКакВБУ Тогда
						НоваяСтрока.СтоимостьНал = СтрокаТабличнойЧасти.СтоимостьРегл;
					Иначе
...
Процедура СформироватьДвиженияПоРегиструПартииМатериаловВЭксплуатации(
	...
	Если ВидОтраженияВУчете <> Перечисления.ВидыОтраженияВУчете.ОтражатьВНалоговомУчете
		ИЛИ (СтруктураШапкиДокумента.Свойство("ПогашатьСтоимостьВНУКакВБУ") И СтруктураШапкиДокумента.ПогашатьСтоимостьВНУКакВБУ) Тогда
...
Функция СформироватьЗапросПоОстаткамМатериаловВЭксплуатацииРасширеннаяАналитика(
	...
	|   //ДляРеглУчета 			0
	|	//ДляРеглУчета 		КОНЕЦ
	|	//ДляРеглУчета 		) КАК СтоимостьНУНачальныйОстаток,
	|   //ДляРеглУчета	СУММА(УчетЗатрат.СтоимостьНУКонечныйОстаток) КАК СтоимостьНУКонечныйОстаток,
	|	//ДляРеглУчета	СУММА(
	|	//ДляРеглУчета		ВЫБОР КОГДА РегистрАналитикаВидаУчета.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчета.ПогашеннаяСтоимость) ТОГДА
	|	//ДляРеглУчета			- УчетЗатрат.СтоимостьНУНачальныйОстаток
	|	//ДляРеглУчета		ИНАЧЕ
	|	//ДляРеглУчета			0
	|	//ДляРеглУчета		КОНЕЦ
	|	//ДляРеглУчета		) КАК ПогашеннаяСтоимостьНУНачальныйОстаток,
	|	//ДляРеглУчета	0 КАК ПогашеннаяСтоимостьНУКонечныйОстаток,
	|
	|	//ДляРеглУчета СУММА(
	|	//ДляРеглУчета 		ВЫБОР КОГДА РегистрАналитикаВидаУчета.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчета.МатериалыВЭксплуатации) ТОГДА
	...
	|	//ДляРеглУчета			0
	|	//ДляРеглУчета		КОНЕЦ
	|	//ДляРеглУчета		) КАК ПогашеннаяСтоимостьПРНачальныйОстаток,
...
Функция СформироватьЗапросПоОстаткамМатериаловВЭксплуатации(
	...
	|	//ДляНалУчета ПартииМатериалов.ПогашеннаяСтоимостьНачальныйОстаток КАК ПогашеннаяСтоимостьНачальныйОстаток,
	|
	...
	|	//ДляНалУчета ПартииМатериалов.ПогашеннаяСтоимостьВРПриход = 0 И ПартииМатериалов.ПогашеннаяСтоимостьПриход = 0 И ПартииМатериалов.ПогашеннаяСтоимостьПРПриход = 0 // если погашение стоимости не производилось в текущем месяце
	|
	|УПОРЯДОЧИТЬ ПО
	|	//ДляРеглУчета ПартииМатериалов.Организация,
...
Процедура РассчитатьПогашеннуюСтоимость(
	...
	Пока Выборка.Следующий() Цикл
		...
		Тогда
			...
			Если СтруктураШапкиДокумента.ИспользоватьРасширеннуюАналитику И ПогашатьСтоимостьВНУКакВБУ Тогда
				СтоимостьНУОстаток = Выборка.СтоимостьНУНачальныйОстаток - Выборка.ПогашеннаяСтоимостьНУНачальныйОстаток;
			ИначеЕсли ПогашатьСтоимостьВНУКакВБУ Тогда // Партионный учет
				СтоимостьОстаток = Выборка.СтоимостьНачальныйОстаток - Выборка.ПогашеннаяСтоимостьНачальныйОстаток;
			КонецЕсли;
		Иначе
		...
		ИначеЕсли ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВРегламентированномУчете Тогда
			...
			Если ПогашатьСтоимостьВНУКакВБУ Тогда
				ПогашеннаяСтоимостьНУ = Выборка.СтоимостьНУНачальныйОстаток / Выборка.СрокПолезногоИспользования;
			КонецЕсли;
		...
		Тогда
			...
			Если СтруктураШапкиДокумента.ИспользоватьРасширеннуюАналитику Тогда
				ПогашеннаяСтоимостьНУ = Макс(Мин(ПогашеннаяСтоимостьНУ, СтоимостьНУОстаток), 0);
				ПогашеннаяСтоимостьНУ = Окр(ПогашеннаяСтоимостьНУ, 2, 1);
			КонецЕсли;
		...
		ИначеЕсли ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВРегламентированномУчете Тогда
			...
			Если ПогашатьСтоимостьВНУКакВБУ Тогда
				НоваяСтрока.СтоимостьНал = ПогашеннаяСтоимостьНУ;
				НоваяСтрока.ВременнаяРазница = ПогашеннаяСтоимость - ПогашеннаяСтоимостьНУ - ПогашеннаяПостояннаяРазница;
			КонецЕсли;
		КонецЕсли;
		
		Если СтруктураШапкиДокумента.ИспользоватьРасширеннуюАналитику
			И ПогашатьСтоимостьВНУКакВБУ 
			И ВидОтраженияВУчете = Перечисления.ВидыОтраженияВУчете.ОтражатьВРегламентированномУчете Тогда
			
			СтруктураСуммНУ = Новый Структура(
									"ПогашеннаяСтоимостьНУ,
									|ПогашеннаяСтоимостьПР,
									|ПогашеннаяСтоимостьВР",
									ПогашеннаяСтоимостьНУ,
...
Функция СформироватьЗапросПоСписаннымПартиямРасширеннаяАналитика(
	...
	|		ВЫБОР КОГДА ПартииМатериалов.ОтражатьВНалоговомУчете
	|			И (РегистрКорАналитикаУчетаПартий.НазначениеИспользования.СпособПогашенияСтоимости = ЗНАЧЕНИЕ(Перечисление.СпособыПогашенияСтоимости.ПогашатьСтоимостьПриПередачеВЭксплуатацию)
	//БЛОКИРОВКА УСЛОВИЯ, ВНОСЯЩЕГО РАЗЛИЧИЯ В ПРИНЦИП ФОРМИРОВАНИЯ СуммаНал и СуммаБух при ПогашатьСтоимостьВНУКакВБУ = Истина
	|				ИЛИ (Не ЕСТЬNULL(СпособыОтраженияРасходов.НеУчитываемыеВНУ, Ложь) И НЕ &ПогашатьСтоимостьВНУКакВБУ))
	|		ТОГДА
	...
	Запрос.УстановитьПараметр("ПогашатьСтоимостьВНУКакВБУ",
		?(СтруктураШапкиДокумента.Свойство("ПогашатьСтоимостьВНУКакВБУ"),СтруктураШапкиДокумента.ПогашатьСтоимостьВНУКакВБУ,Ложь));
...
Функция СформироватьЗапросПоСписаннымПартиям(
	...
	|
	|	//ДляНалУчета СУММА(
	//ДОБАВЛЯЕМ УСЛОВИЕ КАК ДЛЯ БУ, НО ТОЛЬКО ПРИ УСТНОВЛЕННОМ ПогашатьСтоимостьВНУКакВБУ = ИСТИНА
	|	//ДляНалУчета 	ВЫБОР КОГДА ПартииМатериалов.ОтражатьВНалоговомУчете И (ПартииМатериалов.НазначениеИспользования.СпособПогашенияСтоимости = ЗНАЧЕНИЕ(Перечисление.СпособыПогашенияСтоимости.ПогашатьСтоимостьПриПередачеВЭксплуатацию) ИЛИ НЕ &ПогашатьСтоимостьВНУКакВБУ) ТОГДА
	...
	|
	|	СУММА(
	|	ВЫБОР КОГДА ПартииМатериалов.ОтражатьВНалоговомУчете И (ПартииМатериалов.НазначениеИспользования.СпособПогашенияСтоимости = ЗНАЧЕНИЕ(Перечисление.СпособыПогашенияСтоимости.ПогашатьСтоимостьПриПередачеВЭксплуатацию) ИЛИ НЕ &ПогашатьСтоимостьВНУКакВБУ) ТОГДА
	|			ПартииМатериалов.Количество
	|		ИНАЧЕ
	|			0
	|		КОНЕЦ) КАК КоличествоНал,
...
Процедура ПогашениеСтоимостиПриПередачеВЭксплуатацию(
	...
	ПогашатьСтоимостьВНУКакВБУ = ПроцедурыПогашенияСтоимости.ПогашатьСтоимостьВНУКакВБУ(СтруктураПараметров.Организация,СтруктураПараметров.Период);
...
Функция ПогашатьСтоимостьВНУКакВБУ(Организация,Период) Экспорт

	УчетнаяПолитика = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(Период,Организация);
	Если УчетнаяПолитика <> Неопределено И ЗначениеЗаполнено(УчетнаяПолитика.СпособПогашенияСтоимостиСпецодеждыНУ) Тогда
		Возврат УчетнаяПолитика.СпособПогашенияСтоимостиСпецодеждыНУ = Перечисления.СпособыПогашенияСтоимостиНУ.АналогичноСпособуПринятомуДляБухгалтерскогоУчета;
	Иначе
		Возврат Ложь;
	КонецЕсли;
