...
Функция ПолучитьМассивРодительскихЭлементов(Ссылка, МассивРодительскихЭлементов = Неопределено) Экспорт
	...
	Если Не ИмяРеквизитаРодителя = "Родитель" или Ссылка.Метаданные().Иерархический Тогда
		...
		Пока ЗначениеЗаполнено(ТекущийРодитель) И ТекущийРодитель <> Ссылка Цикл
...
Функция ПолучитьМассивДочернихЭлементов(Родитель, МассивЭлементов = Неопределено, ТолькоГруппы = Ложь, Корень = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если МассивЭлементов.Найти(Выборка.Ссылка) <> Неопределено ИЛИ Выборка.Ссылка = Корень Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ДобавлятьРекурсивно Тогда
			ПолучитьМассивДочернихЭлементов(Выборка.Ссылка, МассивЭлементов, ТолькоГруппы, Корень);
		КонецЕсли;
...
Процедура ПроверкаСуществующихЗаписейРегистраСведений(НаборЗаписей, СтруктураПараметров, Отказ)
	...
	// Формирование текста условия отбора для проверки только требуемых записей.
	...
	Для каждого ЭлементОтбора Из НаборЗаписей.Отбор Цикл
		...
		УсловиеОтбора = УсловиеОтбора +
			"ТекущийРегистр." + ЭлементОтбора.Имя + " = &" + ЭлементОтбора.Имя;
	...
	УсловиеПроверки = УсловиеПроверкиСуществующихЗаписейРегистра(Запрос, СтруктураПараметров);
	
	Если УсловиеОтбора <> "" Тогда
		УсловиеПроверки = УсловиеОтбора +
		"
		|И (" + УсловиеПроверки + ")";
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Условие", УсловиеПроверки);
...
Функция УсловиеПроверкиСуществующихЗаписейРегистра(Запрос, СтруктураПараметров)
	...
	Если СтруктураПараметров.ЕстьОрганизация Тогда
		...
		Для Каждого КлючИЗначение ИЗ СтруктураПараметров.СоответствиеГраницЗапрета Цикл
			Номер = Формат(Индекс, "ЧГ=0");
			Условие = Условие + ?(Условие = "", "", "
			|	ИЛИ ") + "  ТекущийРегистр." + ИмяПоляПериода + " <= &ДатаЗапрета" + Номер + "
			|		И ТекущийРегистр.Организация = &Организация" + Номер;
			Запрос.УстановитьПараметр("Организация" + Номер, КлючИЗначение.Ключ);
			ГраницаПериода = КлючИЗначение.Значение;
			Запрос.УстановитьПараметр("ДатаЗапрета" + Номер, ?(ГраницаПериода = Неопределено, NULL, ГраницаПериода));
			Индекс = Индекс + 1;
		КонецЦикла;
	Иначе
		Условие = "	ТекущийРегистр." + ИмяПоляПериода + " <= &ДатаЗапрета";
		ГраницаПериода = СтруктураПараметров.СоответствиеГраницЗапрета[Справочники.Организации.ПустаяСсылка()];
...
Процедура ОпределитьДоступностьВозможностьИзмененияДопДанныхПоДатеЗапрета(СсылкаНаОбъект, ФормаОбъекта, 
	ИмяКоманднойПанели = "", ИменаКнопок = "") Экспорт
	
	ОбъектОтбора = СсылкаНаОбъект.ПолучитьОбъект();
	Если Лев(Строка(ТипЗнч(ОбъектОтбора)),8) = "Документ" Тогда
		Если НастройкаПравДоступа.ДокументВЗакрытомПериоде(ОбъектОтбора) Тогда
			ФормаОбъекта.ТолькоПросмотр = Истина;
			Если ИмяКоманднойПанели <> "" И ИменаКнопок <> "" Тогда
				мИменаКнопок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИменаКнопок,,Истина);
				Для Каждого ИмяКнопки Из мИменаКнопок Цикл
					ФормаОбъекта.ЭлементыФормы[ИмяКоманднойПанели].Кнопки[СокрЛП(ИмяКнопки)].Доступность = Ложь;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли
	КонецЕсли;

КонецПроцедуры // ОпределитьДоступностьВозможностьИзмененияДопДанныхПоДатеЗапрета

Функция ПолучитьОбъектыДоступаДокумента(ИмяДокумента)
	
	Результат = Новый Соответствие;
	
	Если ИмяДокумента = "АвансовыйПлатежИностранцаПоНДФЛ" Тогда
	...
	ИначеЕсли ИмяДокумента = "АктПроверкиСтраховыхВзносов" Тогда
		Результат.Вставить("ФизическиеЛица.ФизическоеЛицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
	...
	ИначеЕсли ИмяДокумента = "ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ" Тогда
		Результат.Вставить("РаботникиОрганизации.ФизЛицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
	...
	ИначеЕсли ИмяДокумента = "ПачкаДокументовСПВ_2" Тогда
		Результат.Вставить("РаботникиОрганизации.ФизЛицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
 		Результат.Вставить("ЗаписиОСтаже.ФизЛицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
	...
	ИначеЕсли ИмяДокумента = "РеестрДанныхЭЛНЗаполняемыхРаботодателем" Тогда
		Результат.Вставить("ДанныеЭЛН.ФизЛицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
	...
	ИначеЕсли ИмяДокумента = "СведенияОЗастрахованныхЛицахСЗВ_М" Тогда
		Результат.Вставить("РаботникиОрганизации.ФизЛицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
	...
	ИначеЕсли ИмяДокумента = "СведенияОСтраховомСтажеЗастрахованныхЛицСЗВ_СТАЖ" Тогда
		Результат.Вставить("РаботникиОрганизации.ФизЛицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
		Результат.Вставить("ЗаписиОСтаже.ФизЛицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
	...
	ИначеЕсли ИмяДокумента = "СправкаПоНДФЛВНалоговыйОрганС2015" Тогда
		Результат.Вставить("СотрудникиОрганизации.Сотрудник", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
		
	ИначеЕсли ИмяДокумента = "СправкаПоНДФЛСотруднику" Тогда
		Результат.Вставить("Сотрудник", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
		
	ИначеЕсли ИмяДокумента = "СправкаПоНДФЛСотрудникуС2015" Тогда
		Результат.Вставить("Сотрудник", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
	...
	ИначеЕсли ИмяДокумента = "ПачкаРазделов6РасчетаРСВ_1" Тогда
		Результат.Вставить("РаботникиОрганизации.Физлицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
		Результат.Вставить("СведенияОЗаработке.Физлицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
		Результат.Вставить("СведенияОЗаработкеНаВредныхИТяжелыхРаботах.Физлицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
		Результат.Вставить("ЗаписиОСтаже.Физлицо", Перечисления.ВидыОбъектовДоступа.ФизическиеЛица);
