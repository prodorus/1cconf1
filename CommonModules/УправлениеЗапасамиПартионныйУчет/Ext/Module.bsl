		...
		Если ТаблицаПартийБУ <> Неопределено И ТаблицаПартийБУ.Количество() > 0 Тогда
			...
			Для Каждого Партия Из ТаблицаПартийБУ Цикл
				...
				Если (НЕ БУ_Тек) Тогда
					...
					Если ОтразитьВУчетеМПЗ Тогда
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиБУ, Партия, СтруктураШапкиДокумента.ДокументРеализацииДата, "БУ", Ложь, Истина, Ложь, ПараметрыРасчетовСКомитентом);
					КонецЕсли;
					...
					Если ОтразитьВУчетеМПЗ Тогда
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиБУ, Партия, СтруктураШапкиДокумента.Дата, "БУ", Ложь, Ложь, Истина, ПараметрыРасчетовСКомитентом);
					КонецЕсли;
					Если Партия.Комиссионный Тогда
				...
				Если (БУ_Тек) Тогда
					...
					Если ОтразитьВУчетеМПЗ Тогда						
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиБУ, Партия, СтруктураШапкиДокумента.Дата, "БУ", ПрошлыйПериод, Ложь, Ложь, ПараметрыРасчетовСКомитентом);
					КонецЕсли;
					Если Партия.Комиссионный Тогда
		...
		Если ТаблицаПартийНУ <> Неопределено И ТаблицаПартийНУ.Количество() > 0 И СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			Для Каждого Партия Из ТаблицаПартийНУ Цикл
				...
				Если (НЕ НУ_Тек) Тогда
					...
					Если ОтразитьВУчетеМПЗ Тогда
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.ДокументРеализацииДата, "НУ", Ложь, Истина, Ложь, ПараметрыРасчетовСКомитентом);
					КонецЕсли;
					...
					//Возврат корректировки со счета К
					Если ОтразитьВУчетеМПЗ Тогда
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.Дата, "НУ", Ложь, Ложь, Истина, ПараметрыРасчетовСКомитентом);
						Если БУ_Тек Тогда
							Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.Дата, "ПР", ПрошлыйПериод, Ложь, Ложь, ПараметрыРасчетовСКомитентом);
						КонецЕсли;
					КонецЕсли;
					Если НЕ Партия.Комиссионный Тогда
				...
				Если (НУ_Тек) Тогда
					//Коррктировка текущего периода
					Если ОтразитьВУчетеМПЗ Тогда
						Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.Дата, "НУ", ПрошлыйПериод, Ложь, Ложь, ПараметрыРасчетовСКомитентом);
						Если НЕ БУ_Тек Тогда
							Документы.КорректировкаРеализации.СформироватьПроводкиПоТовару(СтруктураШапкиДокумента, ПроводкиНУ, Партия, СтруктураШапкиДокумента.Дата, "ПР", ПрошлыйПериод, Ложь, Ложь, ПараметрыРасчетовСКомитентом);
						КонецЕсли;
					КонецЕсли;
...
Процедура СписаниеНаПостоянныеЗатратыНал(СтрокаДокумента, СтруктураПараметров, Строка)
	...
	Если СтрокаДокумента.СтатьяЗатрат <> Неопределено
		И СтрокаДокумента.СтатьяЗатрат.ВидРасходовНУ = Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения Тогда
...
Процедура ДвижениеПартийТоваров(ОбрабатываемыйДокумент, ТаблицаСписания = Неопределено,
								...
								ФормироватьПроводки = Неопределено, СписыватьПартииНДС = Неопределено, УчетнаяПолитика = Неопределено, Отказ = Неопределено) Экспорт
...
Процедура ПодготовитьТаблицыДокументаВозвратИзЭксплуатации(СтруктураШапкиДокумента, СтруктТаблицДокумента);
	...
	Для Каждого ЭлементСтруктуры Из СтруктТаблицДокумента Цикл
		...
		Если НЕ (СтруктураШапкиДокумента.Свойство("ПогашатьСтоимостьВНУКакВБУ")
	...
	ИначеЕсли ТипДокумента =  Тип("ДокументСсылка.ВозвратМатериаловИзЭксплуатации") Тогда
		Если СтрокаТаблицы.ТипСтоимости = Перечисления.ВидыНормативнойСтоимостиПроизводства.Фиксированная Тогда
			...
			Если СтруктураШапкиДокумента.Свойство("ПогашатьСтоимостьВНУКакВБУ")
					...
					И СтрокаТаблицы.НазначениеИспользования.СпособПогашенияСтоимости = Перечисления.СпособыПогашенияСтоимости.Линейный Тогда
				СтруктураСуммы.Вставить("СтоимостьНУ", СтрокаТаблицы.Стоимость);
			Иначе
