...
Функция ПолучитьСтруктуруПечатныхФорм(СотрудникОбъект) Экспорт
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, ЭтотОбъект) Экспорт
	...
	Если ИмяМакета = "РегламентированныйОтпуск" Тогда
		ТабДокумент = ОстаткиОтпусков.СправкаПоОтпускамСотрудника(ЭтотОбъект.Ссылка, ОбщегоНазначенияЗК.ПолучитьРабочуюДату());
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, "Справка по отпуску сотрудника");
...
Процедура ЗаполнитьРеквизитыПоШтатномуРасписанию(Форма) Экспорт
	...
	Если ДанныеШР.Количество() > 0 Тогда
		Если Не ЗначениеЗаполнено(Форма.ВалютаТарифнойСтавки) Тогда
			Форма.ВалютаТарифнойСтавки = ДанныеШР[0].ВалютаТарифнойСтавки;
		КонецЕсли;
		Если ЗначениеЗаполнено(ДанныеШР[0].ГрафикРаботы) Тогда
			Форма.ГрафикРаботы = ДанныеШР[0].ГрафикРаботы;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Форма.ТарифнаяСтавка) и ДанныеШР[0].МаксимальнаяТарифнаяСтавка = ДанныеШР[0].МинимальнаяТарифнаяСтавка Тогда
			Форма.ТарифнаяСтавка = ДанныеШР[0].МаксимальнаяТарифнаяСтавка;
		КонецЕсли;
	КонецЕсли;
	Если НадбавкиПоШР.Количество() > 0 и Форма.ПерсональныеНадбавки.Количество() = 0 Тогда
		Для каждого НадбавкаПоШР Из НадбавкиПоШР Цикл
			ЗаполнитьЗначенияСвойств(Форма.ПерсональныеНадбавки.Добавить(), НадбавкаПоШР);
		КонецЦикла;
	КонецЕсли;
...
Процедура ФормаСпискаПриПолученииДанныхДополнительно(Форма, Элемент, ОформленияСтрок) Экспорт
	
	СотрудникиОрганизацийДополнительный.ФормаСпискаПриПолученииДанныхДополнительно(Форма, Элемент, ОформленияСтрок);
	
КонецПроцедуры

Процедура ФормаСпискаПриАктивизацииСтрокиДополнительно(Форма, Элемент, ДанныеСтроки) Экспорт
	
	СотрудникиОрганизацийДополнительный.ФормаСпискаПриАктивизацииСтрокиДополнительно(Форма, Элемент, ДанныеСтроки);
	
КонецПроцедуры

Процедура ФормаСпискаПередЗакрытиемДополнительно(Форма, Отказ, СтандартнаяОбработка) Экспорт
	
	СотрудникиОрганизацийДополнительный.ФормаСпискаПередЗакрытиемДополнительно(Форма, Отказ, СтандартнаяОбработка);
	
КонецПроцедуры

Процедура ФормаСпискаВыполнитьДополнительныеДействия(Форма, Кнопка) Экспорт
...
Процедура СформироватьПодменюВвестиДокумент(МассивКнопокПодменю, СтруктураСоответствияДействий, ДанныеСотрудника, Форма) Экспорт
	...
	Если ДанныеСотрудника <> Неопределено Тогда
		...
		Иначе
			...
			ДатаУвольненияИзКомпании	= Дата(1,1,1);
		КонецЕсли;
	Иначе
	...
	Если Уволен И УволенИзКомпании Тогда
		Для Каждого КнопкаПодменю Из МассивКнопокПодменю Цикл
			КнопкаПодменю.Доступность = Ложь;
		КонецЦикла;
		Возврат;
	...
	Если ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.ТрудовойДоговор Тогда
		...
		Если Не Принят И Не Уволен Тогда
		...
		Если ИспользоватьУправленческийУчетЗарплаты Тогда
			...
			Если Не ПринятВКомпанию И Не УволенИзКомпании Тогда
			...
			Если ПринятВКомпанию И Не УволенИзКомпании Тогда
	...
	ИначеЕсли ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.Авторский
		...
		ДобавитьКнопкуВПодменю(МассивКнопокПодменю, "ДоговорНаВыполнениеРаботСФизЛицом", ТипКнопкиКоманднойПанели.Действие, "Договор на выполнение работ", СтруктураСоответствияДействий.ДействиеВводНаОснованииДоговорНаВыполнениеРаботСФизЛицом);
...
Процедура УстановитьОтборПоРежимуОтображенияСписка(ЭтаФорма, РежимОтображенияСписка) Экспорт
	...
	Иначе
		...
		Если РежимОтображенияСписка = "ПоСтруктуреКомпании" Тогда
			...
			Отборы.ТекущееПодразделениеКомпании.ВидСравнения		= ВидСравнения.ВСписке;
			Отборы.ТекущееПодразделениеКомпании.Значение			= СписокПодразделений;
		КонецЕсли;
...
Процедура ОбработкаПолученияДанныхВыбораДополнительно(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Для Каждого ИмяОтбора Из Отборы Цикл
		Если Параметры.Отбор.Свойство(ИмяОтбора) И НЕ ЗначениеЗаполнено(Параметры.Отбор[ИмяОтбора]) Тогда
			Параметры.Отбор.Удалить(ИмяОтбора);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры // ОбработкаПолученияДанныхВыбораДополнительно
