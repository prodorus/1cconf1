...
Процедура ДобавитьЗадачиНамеченныеСобытияПоПерсоналу(ВремяФормированияЗадач, МассивОрганизаций, ЕстьЗадачиВозвратНаРаботу, ЕстьЗадачиКадровоеПеремещение, ЕстьЗадачиОценкаИспытательногоСрока, ЕстьЗадачиУвольнение)
	...
	ДатаНачала		= ОбщегоНазначенияЗК.ДобавитьИнтервал(ВремяФормированияЗадач,Перечисления.Периодичность.Месяц, -1);
	ДатаОкончания	= ОбщегоНазначенияЗК.ДобавитьИнтервал(ВремяФормированияЗадач,Перечисления.Периодичность.Неделя, 1);
...
Процедура ДобавитьЗадачиРасчетЗарплаты(АктуальныйПериодРегистрации)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ПустаяСтрока(Выборка.НаименованиеОрганизации) Тогда
			НаименованиеОрганизации = " по " + ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("организации ") + СокрЛП(Выборка.НаименованиеОрганизации);
		КонецЕсли;
