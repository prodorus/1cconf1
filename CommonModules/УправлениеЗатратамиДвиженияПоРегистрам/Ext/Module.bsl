...
Функция СформироватьТекстЗапросаПоТаблицеЗатрат()
	...
	|				И Не &НДСВключенВСтоимость И Не &УчетАгентскогоНДС
	|			ТОГДА
...
Функция СформироватьЗапросПоТаблицеЗатрат(
	...
	Если СтруктураШапкиДокумента.Свойство("УчетАгентскогоНДС") Тогда
		Запрос.УстановитьПараметр("УчетАгентскогоНДС", СтруктураШапкиДокумента.УчетАгентскогоНДС);
	Иначе
		Запрос.УстановитьПараметр("УчетАгентскогоНДС", Ложь);	
	КонецЕсли;
...
Функция ДополнитьСтруктуруШапкиДокументаНастройкиУчетаЗатрат(
	...
	Если ТипЗнч(СтруктураШапкиДокумента.Ссылка) <> Тип("ДокументСсылка.ПередачаМатериаловВэксплуатацию") Тогда
