...
Процедура ФормаНастройкиУзловПриСозданииНаСервере(Форма, Отказ) Экспорт
	...
	Если Форма.Параметры.Свойство("ПолучитьЗначенияПоУмолчанию") Тогда
		
		МассивИмя = РегламентированнаяОтчетность.СтрРазделить82(Форма.ИмяФормы, ".");
		ИмяПланаОбмена = МассивИмя[1];
		НастройкаОтборовНаУзле                   = НастройкаОтборовНаУзле(ИмяПланаОбмена, Форма.ВерсияКорреспондента);
		НастройкаОтборовНаУзлеБазыКорреспондента = НастройкаОтборовНаУзлеБазыКорреспондента(ИмяПланаОбмена, Форма.ВерсияКорреспондента, МассивИмя[3]);
	...
	Если Не Форма.Параметры.Свойство("ПроверкаЗаполнения") И Не Форма.Параметры.Свойство("ПолучитьЗначенияПоУмолчанию") Тогда
...
//
// Пример:
//  Если СтроковыеФункцииКлиентСервер.СравнитьВерсии(ОбменДаннымиСервер.ВерсияКорреспондента(Корреспондент), "2.1.5.1") >= 0 Тогда ... КонецЕсли;
//
...
Процедура ВыполнитьСинхронизациюПриОтсутствииОбновленияИнформационнойБазы(
	...
	Если ЗагрузитьСообщениеОбменаДанными() Тогда
		...
		Исключение
			Если КонфигурацияИзменена() Тогда
				Если НЕ ПараметрыСеанса.РежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском.Свойство(
...
Процедура ПриДобавленииПараметровРаботыКлиентскойЛогикиСтандартныхПодсистемПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить("ПроверитьНеобходимостьОбновленияКонфигурацииПодчиненногоУзла",
		...
		И ПользователиСервер.РолиДоступны("ВыполнениеСинхронизацииДанных"));
...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазыПоВнешнемуСоединению(Отказ, УзелИнформационнойБазы,
	...
	ОбменСБСП20 = СтроковыеФункцииКлиентСервер.СравнитьВерсии("2.1.1.10", ВерсияБСППоВнешнемуСоединению) > 0;
...
Процедура ПослеЧтенияСообщенияОбмена(Знач Получатель, Знач СообщениеОбмена, Знач СообщениеПрочитано, СтандартнаяОбработка, Знач УдалитьСообщение = Истина)
	...
	Если ЭтоПодчиненныйУзелРИБ()
		...
		Если НЕ СообщениеПрочитано
		   И ПараметрыСеанса.РежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском.Свойство("СообщениеПолученоИзКэша") Тогда
...
Процедура ПроверитьВозможностьВыполненияОбменов() Экспорт
	...
	Если Не ПользователиСервер.РолиДоступны("ВыполнениеСинхронизацииДанных, НастройкаСинхронизацииДанных") Тогда
...
Процедура ПроверитьВозможностьАдминистрированияОбменов() Экспорт
	...
	Если Не ПользователиСервер.РолиДоступны("НастройкаСинхронизацииДанных") Тогда
...
Функция ПолучитьWSПроксиПоПараметрамПодключения(
	...
	Попытка
		...
		Исключение
			...
			СообщениеПользователю   = КраткоеПредставлениеОшибки(Информация);
...
Функция ОбработкаДляЗагрузкиДанных(Отказ, Знач УзелИнформационнойБазы, Знач ИмяФайлаСообщенияОбмена) Экспорт
	...
	Если ЭтоПланОбменаXDTO Тогда
			ОбработкаОбменаДанными = Обработки.КонвертацияОбъектовXDTO.Создать();
	Иначе
...
Процедура УстановитьОбщиеПараметрыДляОбработкиОбменаДанными(ОбработкаОбменаДанными, СтруктураНастроекОбмена, ОбменСБСП20 = Ложь)
	...
	Если ОбменДаннымиПовтИсп.ЭтоПланОбменаXDTO(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
		Возврат ;
	КонецЕсли;
	...
	Если Не ОбменСБСП20 И НЕ ОбменДаннымиПовтИсп.ЭтоПланОбменаXDTO(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
...
Процедура ЗарегистрироватьОшибкуПроведенияДокумента(Объект, УзелОбмена, СообщениеОбОшибке) Экспорт
	...
	СтрокаСообщения = НСтр("ru = 'Не удалось провести документ %1, полученный из другой информационной базы, по причине %2.
...
Процедура ЗарегистрироватьОшибкуЗаписиОбъекта(Объект, УзелОбмена, СообщениеОбОшибке) Экспорт
	...
	СтрокаСообщения = НСтр("ru = 'Не удалось записать объект %1, полученный из другой информационной базы, по причине %2.
		|Возможно не заполнены все реквизиты, обязательные к заполнению.'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Строка(Объект), СообщениеОбОшибке);
...
Процедура ИнтерактивноеИзменениеВыгрузкиСохранитьНастройки(ДополнениеВыгрузки, Знач ПредставлениеНастройки) Экспорт
	...
	ОбработкаДополнения.КомпоновщикОтбораВсехДокументов.Инициализировать(
		Новый ИсточникДоступныхНастроекКомпоновкиДанных(Данные.СхемаКомпоновки));
	ОбработкаДополнения.КомпоновщикОтбораВсехДокументов.ЗагрузитьНастройки(Данные.Настройки);
