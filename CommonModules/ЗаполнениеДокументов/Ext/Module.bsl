...
Процедура ВыполнитьВосстановление(ДатаКонцаОбработки,
	...
	Пока ЕстьДокументы Цикл
		...
		Если ДокументСсылка.Проведен Тогда
			...
			ИспользуетсяРасширеннаяАналитикаУчета = УправлениеЗапасами.ИспользуетсяРасширеннаяАналитикаУчета(ДокументМоментВремени.Дата);
				...
				Если ТаблицаСписания.Количество() <> 0 Тогда				
					...
					УправлениеЗапасамиПартионныйУчет.ДвижениеПартийТоваров (ДокументСсылка,
										   ...
										   СтруктураНаборовЗаписей,
									  	   ФормироватьПроводки,
									  	   Бух,
										   УчетнаяПолитика, ОшибкаТекущегоДокумента);
			...
			Если НЕ ИспользуетсяРасширеннаяАналитикаУчета Тогда
				УправлениеПроизводствомДвиженияПоРегистрам.ПровестиДокументПоПроизводственнымРегистрам(ДокументСсылка, Упр, Бух, Бух, ОшибкаТекущегоДокумента);
			КонецЕсли;
