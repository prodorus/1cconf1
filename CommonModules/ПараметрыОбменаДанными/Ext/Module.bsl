...
Функция ПолучитьТаблицуВозможныхОбменов() Экспорт
	...
	СтрокаТаблицы.ПрефиксПоУмолчанию = ПолучитьПрефиксИзИмениКонфигурации(); // Префикс по умолчанию
...
Функция ПолучитьТаблицуВозможныхОбменовСРознинойТорговлей() Экспорт
	...
	СтрокаТаблицы.ИмяПланаОбмена            = "ОбменУправлениеПредприятиемРозничнаяТорговля";	// определяет имя плана обмена откуда брать данные
	СтрокаТаблицы.НаименованиеКонфигурации  = "Розница, редакция 1.0";
	СтрокаТаблицы.ПредставлениеКонфигурации = "Розница, редакция 1.0"; 		   					// представление для пользователя
...
Функция ПолучитьПрефиксИзИмениКонфигурации()
	
	ИмяКонфигурации = Метаданные.Имя;
	Префикс = "";
	
	Для НомерСимвола = 1 По СтрДлина(ИмяКонфигурации) Цикл
		Буква = Сред(ИмяКонфигурации, НомерСимвола, 1);
		Если ВРег(Буква) = Буква Тогда
			Префикс = Префикс + Буква;
		КонецЕсли;
	КонецЦикла;
