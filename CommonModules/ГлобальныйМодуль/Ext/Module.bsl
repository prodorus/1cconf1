...
Функция СтрШаблон(Знач СтрокаПодстановки,
	...
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаПодстановки,
...
Функция СтрокаЗаканчиваетсяНа(Строка, Подстрока) Экспорт
	
	Возврат (Подстрока = Прав(Строка, СтрДлина(Подстрока)));
	
КонецФункции

Функция МаскаВсеФайлы() Экспорт
