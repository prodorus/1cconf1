...
Процедура ЗаполнитьОбъектРедактированияАдресаПоСтруктуре(Объект, СтруктураАдреса) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(ТипДома) = Тип("Строка") И ВРег(ТипДома) = "ВЛАДЕНИЕ" ИЛИ ТипДома = Перечисления.ТипыДомов.Владение Тогда
		
			Объект.ТипДома = Перечисления.ТипыДомов.Владение;
		...
		Если ТипЗнч(ТипКорпуса) = Тип("Строка") И ВРег(ТипКорпуса) = "СТРОЕНИЕ" ИЛИ ТипКорпуса = Перечисления.ТипыКорпусов.Строение Тогда
			
			Объект.ТипКорпуса = Перечисления.ТипыКорпусов.Строение;
			
		ИначеЕсли ТипЗнч(ТипКорпуса) = Тип("Строка") И ВРег(ТипКорпуса) = "ЛИТЕРА" ИЛИ ТипКорпуса = Перечисления.ТипыКорпусов.Литера Тогда
			
			Объект.ТипКорпуса = Перечисления.ТипыКорпусов.Литера;
		...
		Если ТипЗнч(ТипКвартиры) = Тип("Строка") И ВРег(ТипКвартиры) = "ОФ." ИЛИ ТипКвартиры = Перечисления.ТипыКвартир.Офис Тогда
			
			Объект.ТипКвартиры = Перечисления.ТипыКвартир.Офис;
			
		ИначеЕсли ТипЗнч(ТипКвартиры) = Тип("Строка") И СокрЛП(ВРег(ТипКвартиры)) = "ПОМЕЩЕНИЕ" ИЛИ ТипКвартиры = Перечисления.ТипыКвартир.Помещение Тогда
			
			Объект.ТипКвартиры = Перечисления.ТипыКвартир.Помещение;
			
		ИначеЕсли ТипЗнч(ТипКвартиры) = Тип("Строка") И СокрЛП(ВРег(ТипКвартиры)) = "КОМНАТА" ИЛИ ТипКвартиры = Перечисления.ТипыКвартир.Комната Тогда
			
			Объект.ТипКвартиры = Перечисления.ТипыКвартир.Комната;
...
Функция ПолучитьПредставлениеАдресаПоСтруктуре(СтруктураАдреса) Экспорт
	...
	Если Не АдресЗаПределамиРФ Тогда
		...
		КонечныйСимвол = "";
		Если СтруктураАдреса.Свойство("ТипКвартиры") И ЗначениеЗаполнено(СтруктураАдреса.ТипКвартиры) Тогда
			Если СтруктураАдреса.ТипКвартиры = Перечисления.ТипыКвартир.Помещение Тогда
				КонечныйСимвол = " ";
			КонецЕсли;
			ПредставлениеКвартиры = ПредставлениеКвартиры + СтруктураАдреса.ТипКвартиры + КонечныйСимвол;
		Иначе
			ПредставлениеКвартиры = ПредставлениеКвартиры + "кв." + КонечныйСимвол;
		КонецЕсли;
		
		ДополнитьПредставлениеАдреса(ТекПредставление, СокрЛП(СтруктураАдреса.Квартира), ПредставлениеКвартиры);
...
//
Процедура УстановитьПризнакЗаписиПоУмолчанию(НаборЗаписей, ТекущаяЗапись) Экспорт
	
	Если НЕ ТекущаяЗапись.ЗначениеПоУмолчанию Тогда
		...
		Если Не ЕстьЗаписьПоУмолчанию Тогда
...
Функция НайтиЗаписьКонтактнойИнформации(НаборЗаписей, ПараметрыПоиска) Экспорт
	
	НайденныСтроки = НайтиЗаписиКонтактнойИнформации(НаборЗаписей, ПараметрыПоиска);
	
	Возврат ?(НайденныСтроки.Количество() > 0, НайденныСтроки[0], Неопределено);
	
КонецФункции // НайтиЗаписьКонтактнойИнформации

Функция НайтиЗаписиКонтактнойИнформации(НаборЗаписей, ПараметрыПоиска) Экспорт
	...
	Для Каждого СтрокаНабора Из НаборЗаписей Цикл
		...
		Для Каждого КлючИЗначение Из ПараметрыПоиска Цикл
			СтрокаПодходит = СтрокаПодходит И СтрокаНабора[КлючИЗначение.Ключ] = КлючИЗначение.Значение;
		КонецЦикла;
		Если СтрокаПодходит Тогда
			НайденныеСтроки.Добавить(СтрокаНабора);
		КонецЕсли;
	КонецЦикла;
...
Функция АдресЭлектроннойПочтыПоУмолчанию() Экспорт
	...
	Возврат ВидАдресаЭППоУмолчанию;
	
КонецФункции // АдресЭлектроннойПочтыПоУмолчанию
...
//  Ложь   - если не входит
//
Функция НомерДомаВИнтервале(НомерДома, Знач Интервал) Экспорт
...
Функция ПолучитьИндекс(Знач НазваниеРегиона, Знач НазваниеРайона, Знач НазваниеГорода, Знач НазваниеНаселенногоПункта, Знач НазваниеУлицы, Знач НомерДома, Знач НомерКорпуса) Экспорт
	...
	Если Город.Код > 0 И ЗначениеЗаполнено(НомерДома) И НЕ ЗначениеЗаполнено(Индекс) Тогда
