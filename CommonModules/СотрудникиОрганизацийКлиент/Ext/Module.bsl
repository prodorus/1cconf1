...
Процедура СформироватьСписокОбособленныхПодразделений(ЭтаФорма) Экспорт
	...
	ЭтаФорма.мСписокОбособленныхПодразделений = ОбщегоНазначенияЗК.ПолучитьСписокОбособленныхПодразделенийОрганизации(Организация);
...
Процедура ОбработатьОтображениеОбособленногоПодразделения(ИзменятьДанные = Истина, ЭтаФорма) Экспорт
	...
	Если ЭтаФорма.ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.ТрудовойДоговор Тогда
		...
		Если ЭлементыФормы.Найти("ОбособленноеПодразделение1") <> Неопределено Тогда
...
Процедура ФормаЭлементаОбновитьПредставлениеЭлемента(ИмяОбновляемогоЭлемента, ЭтаФорма, Параметр = Неопределено) Экспорт
	...
	Если ИмяОбновляемогоЭлемента = "Гражданство" Тогда
		...
		Если мНаборЗаписейГражданство.Количество() > 0 Тогда
			Гражданство = "" + мНаборЗаписейГражданство[0].Страна + ", " + ФизическиеЛицаКлиент.ОписаниеВидаЗастрахованногоЛица(мНаборЗаписейГражданство[0].ВидЗастрахованногоЛица);
			ЭтаФорма.Модифицированность = Истина;
		Иначе
			// в регистре сведений нет записей, покажем значение по умолчанию
			Гражданство = "РОССИЯ, имеет право на пенсионное, медицинское и социальное страхование";
		КонецЕсли;
	...
	ИначеЕсли ИмяОбновляемогоЭлемента = "СтатусНалогоплательщика" Тогда
		...
		СтатусНалогоплательщика = ЭтаФорма.СтатусНалогоплательщика;
		
		Если Не ЭтаФорма.мДоступнаРольРасчетчика И Не ЭтаФорма.мДоступнаРольКадровика Тогда
			СтатусНалогоплательщика = "<данные не доступны>";
			Возврат;
		КонецЕсли;
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			ИначеЕсли  Выборка.Статус = Перечисления.СтатусыНалогоплательщиковПоНДФЛ.ЧленЭкипажаСуднаПодФлагомРФ Тогда
				СтатусНалогоплательщика = "Физическое лицо является членом экипажа судна, плавающего под Государственным флагом РФ. При исчислении НДФЛ с оплаты труда будет применяться ставка 13%";
			ИначеЕсли  Выборка.Статус = Перечисления.СтатусыНалогоплательщиковПоНДФЛ.Беженец Тогда
				СтатусНалогоплательщика = "Физическое лицо является беженцем (либо получено временное убежище). При исчислении НДФЛ с оплаты труда будет применяться ставка 13%";
			ИначеЕсли Выборка.Статус = Перечисления.СтатусыНалогоплательщиковПоНДФЛ.ГражданинСтраныЕАЭС Тогда
				СтатусНалогоплательщика = "Физическое лицо является гражданином Беларуси, Казахстана, Армении или Киргизии. При исчислении НДФЛ с оплаты труда будет применяться ставка 13%";
			ИначеЕсли Выборка.Статус = Перечисления.СтатусыНалогоплательщиковПоНДФЛ.НерезидентРаботающийНаОснованииПатента Тогда
				СтатусНалогоплательщика = "Иностранный гражданин осуществляет трудовую деятельность по найму на основании патента. При исчислении НДФЛ с оплаты труда будет применяться ставка 13%";
			КонецЕсли;
		Иначе
			СтатусНалогоплательщика = "Резидент. При исчислении НДФЛ с оплаты труда будет применяться ставка 13%";
		КонецЕсли;
		
		ЭтаФорма.СтатусНалогоплательщика = СтатусНалогоплательщика;
	...
	ИначеЕсли ИмяОбновляемогоЭлемента = "СтандартныеВычеты" Тогда	
		...
		Запрос.УстановитьПараметр("парамФизлицо", Ссылка.Физлицо);
		Запрос.УстановитьПараметр("парамДатаСреза", НачалоМесяца(ОбщегоНазначенияЗК.ПолучитьРабочуюДату()));
		...
		Если Выборка.Следующий() Тогда
			...
			|	ВЫБОР
			|		КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			|				И НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения < &парамДатаСреза
			...
			|	НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета.Код КАК КодВычетаКод
			|ИЗ
			...
			Иначе
				СтандартныеВычеты = СтандартныеВычеты + "Вычеты на детей:";
				Выборка = РезультатЗапроса.Выбрать();
		        Пока Выборка.Следующий() Цикл
					СтандартныеВычеты = СтандартныеВычеты + " с кодом " + СправкиПоНДФЛПовтИсп.СвойстваВычета(Выборка.КодВычета).ТекущийКод + " на " + Выборка.КоличествоДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.КоличествоДетей)
				КонецЦикла;
	...
	ИначеЕсли ИмяОбновляемогоЭлемента = "ИмущественныеВычеты" Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			+ "Вычет: " + СправкиПоНДФЛПовтИсп.СвойстваВычета(Выборка.КодВычета).ТекущийКод + ", размер: " + Выборка.РазмерВычета + ". ";
		КонецЦикла;	
...
Процедура ЗаполнитьНаборыПоФизлицу(ЭтаФорма) Экспорт
	...
	ОбщегоНазначенияЗК.ПолучитьСвязанныеДанныеФизлица(Параметр, ЭтаФорма.мНаборЗаписейГражданство);
	...
	КонецЕсли;
	ОбщегоНазначенияЗК.ПолучитьСвязанныеДанныеФизлица(Параметр, ЭтаФорма.мНаборЗаписейИнвалидность);
	Если ЭтаФорма.мНаборЗаписейИнвалидность.Количество() > 0 Тогда
	...
	Если ЭтаФорма.мНаборЗаписейПринятыеПоДолгосрочнымДоговорам.Количество() > 0 Тогда
...
Процедура ФормаЭлементаИННПриИзменении(Элемент, ЭтаФорма) Экспорт   
	...
	Если ЭлементыФормы.Найти("ИНН1") <> Неопределено Тогда
		ЭлементыФормы.ИНН1.ЦветТекстаПоля 	= ЭлементыФормы.ИНН.ЦветТекстаПоля;
		ЭлементыФормы.ИНН1.Подсказка 		= ЭлементыФормы.ИНН.Подсказка;
	КонецЕсли;
...
Процедура ФормаЭлементаСтраховойНомерПФРПриИзменении(Элемент, ЭтаФорма) Экспорт   
	...
	Если ЭлементыФормы.Найти("СтраховойНомерПФР1") <> Неопределено Тогда
		ЭлементыФормы.СтраховойНомерПФР1.ЦветТекстаПоля 	= ЭлементыФормы.СтраховойНомерПФР.ЦветТекстаПоля;
		ЭлементыФормы.СтраховойНомерПФР1.Подсказка 			= ЭлементыФормы.СтраховойНомерПФР.Подсказка;
	КонецЕсли;
...
Процедура ОбработкаРеквизитовТрудовогоДоговора(ЭтаФорма) Экспорт
	...
	Если Не ЗначениеЗаполнено(ОбособленноеПодразделение)
		или Не Организация = ОбщегоНазначенияЗК.ГоловнаяОрганизация(ОбособленноеПодразделение) Тогда
		ЭтаФорма.ОбособленноеПодразделение = Организация;
	КонецЕсли;
	...
	Если Не ПодразделениеОрганизации.Пустая() 
		...
		ЭтаФорма.ПодразделениеОрганизации = Справочники.ПодразделенияОрганизаций.ПустаяСсылка();
		Предупреждение("Выбранное подразделение не соответствует " + ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("организации") + "!");
	КонецЕсли;
	...
	Если НадоПроверятьВидЗанятости Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Иначе 
				...
				ТекстВопроса = "В " + ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("организации") + " уже есть сотрудник """ + Выборка.ФизлицоНаименование + """ с видом занятости """ + Выборка.ВидЗанятости + """,
...
Процедура ПодразделениеДолжностьНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка, ТипЭлемента, ЭтаФорма) Экспорт
	...
	СписокВыбора = ПроцедурыУправленияПерсоналом.ВыбратьИзШтатногоРасписанияОрганизации(ТипЭлемента, Организация, ?(ТипЭлемента = "Должность", ПодразделениеОрганизации, Должность), ДатаАктуальности);
...
Процедура ПроверитьИПодобратьОрганизациюПередОткрытиемФормы(ЭтаФорма) Экспорт
	...
	Если Не ЗначениеЗаполнено(Организация)  Тогда
		...
		ЭтаФорма.Организация = ОбщегоНазначенияЗК.ГоловнаяОрганизация(ОбособленноеПодразделение);
...
Процедура ОбработатьПоказЭУЯвляетсяСтудентом(ЭтаФорма) Экспорт
...
Процедура ФормаЭлементаПриОткрытии(ЭтаФорма) Экспорт
	...
	Иначе
		...
		СотрудникиОрганизацийПереопределяемый.СформироватьПодменюВвестиДокумент(ЭтаФорма.мМассивКнопокПодменю, ЭтаФорма.мСтруктураСоответствияДействий, ЭтаФорма.ЭтотОбъект, ЭтаФорма);
...
Процедура ФормаЭлементаПриЗаписи(Отказ, ЭтаФорма)Экспорт
	...
	Если мНаборЗаписейПринятыеПоДолгосрочнымДоговорам.Модифицированность() Тогда
		...
		Если НадоЗаписатьНаборЗаписей Тогда
			Для каждого СтрокаНабора Из мНаборЗаписейПринятыеПоДолгосрочнымДоговорам Цикл
				СтрокаНабора.Физлицо = Ссылка.Физлицо;
			КонецЦикла;                      
...
Процедура ФормаЭлементаСтатусНалогоплательщикаНачалоВыбора(Элемент, СтандартнаяОбработка, ЭтаФорма) Экспорт
...
Функция ПолучитьТекстМакетаВстроеннойСправки(ЭтаФорма)
	...
	Если ЭтаФорма.мЭтоНовый Тогда
		...
		|		<DIV><B>Если сотрудник ранее уже работал " + ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("в организации") + "...</B><BR>
...
Процедура ОткрытьФормуНовогоДокументаНаОснованииДанныхСотрудника(ИмяДокумента, Сотрудники) Экспорт
	
	Если ТипЗнч(Сотрудники) = Тип("Массив") И Сотрудники.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(Сотрудники) = Тип("СправочникСсылка.СотрудникиОрганизаций") Тогда
	...
	Иначе
		...
		СотрудникВШапке = Метаданные.Документы[ИмяДокумента].Реквизиты.Найти("Сотрудник") <> Неопределено;
		
		Если НЕ СотрудникВШапке Тогда
			ДокументОбъект = Документы[ИмяДокумента].СоздатьДокумент();
		КонецЕсли;
		
		Для Каждого СотрудникСсылка Из Сотрудники Цикл
			Если СотрудникВШапке Тогда
				ДокументОбъект = Документы[ИмяДокумента].СоздатьДокумент();
			КонецЕсли;
			ДокументОбъект.Заполнить(СотрудникСсылка);
			Если СотрудникВШапке Тогда
				ФормаДокумента = ДокументОбъект.ПолучитьФорму();
				ФормаДокумента.Открыть();
			КонецЕсли;
...
КонецПроцедуры // СоздатьНовыйДокументНаОснованииДанныхСотрудника
