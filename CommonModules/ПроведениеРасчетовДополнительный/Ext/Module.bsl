...
КонецФункции // ПолучитьДатуВступленияВСилуИзмененийПоСоциальнымПособиям2006()
...
Функция ПолучитьДатуВступленияВСилуИзмененийПоОтпускам2008() Экспорт
	...
	Возврат	'20080106' // Постановление опубликовано в Российской газете от 29 декабря 2007 г. №922, вступает в силу по истечении 7 календарных дней - с 6 января 2008 года. 

КонецФункции // ПолучитьДатуВступленияВСилуИзмененийПоОтпускам2008()
...
// 
Процедура УточнитьУчетНалогаПоЦеннымБумагам(Движения, Отказ) Экспорт
	
	Если Отказ Или Движения.Найти("НДФЛСведенияОДоходах") = Неопределено Или Движения.НДФЛСведенияОДоходах.Количество() = 0 Тогда
		Возврат
	КонецЕсли;
	
	Регистры = Новый Массив;
	Регистры.Добавить("НДФЛПредоставленныеСтандартныеВычетыФизЛиц");
	Регистры.Добавить("НДФЛРасчетыСБюджетом");
	
	Движения.НДФЛСведенияОДоходах.Записать();
	
	ДвиженияДокумента = Движения.НДФЛСведенияОДоходах.Выгрузить();
	...
	Иначе
		...
		|	ФизлицаСОднотипнымиДоходами.ПодразделениеОрганизации КАК ПодразделениеОрганизации,
		|	СведенияОДоходахНДФЛ.МесяцНалоговогоПериода
		|ИЗ
		...
		СтруктураПоиска = Новый Структура("ФизЛицо, ПодразделениеОрганизации, МесяцНалоговогоПериода");
...
Функция ПрогнозНДФЛ(ПолнОблагаемыйДоходЗаГод, СтрокаПрогноза, ДопВсегоИсчисленный)
    Если СтрокаПрогноза.Резидент Тогда
        ...
        Возврат Окр((ПолнОблагаемыйДоходЗаГод - ПримененныеВычетыЛичные 
					- ПримененныеВычетыДетские - ПримененныеВычетыДетскиеДвойные - ПримененныеВычетыДетскиеДвойныеВторые 
					- ПримененныеВычетыНаТретьегоРебенка - ПримененныеВычетыНаТретьегоРебенкаДвойные - ПримененныеВычетыНаТретьегоРебенкаДвойныеВторые 
					- ПримененныеВычетыНаВторогоРебенка - ПримененныеВычетыНаВторогоРебенкаДвойные - ПримененныеВычетыНаВторогоРебенкаДвойныеВторые 
					- ПримененныеВычетыДетскиеИнвалидов - ПримененныеВычетыДетскиеДвойныеИнвалидов - ПримененныеВычетыДетскиеДвойныеВторыеИнвалидов - ПримененныеВычеты318 - ПримененныеВычеты312 - ПримененныеВычеты311) * Ставка) - (СтрокаПрогноза.ИсчисленныйНалогДоТекПериода + ДопВсегоИсчисленный);
    Иначе
        Ставка = 0.30;
...
Процедура ДополнитьМассивАвтоматическиРассчитываемыхПоказателей(АвтоРассчитываемыеПоказатели) Экспорт
	...
	УправлениеПоказателямиСхемМотивации.ДополнитьМассивАвтоматическиРассчитываемыхПоказателей(АвтоРассчитываемыеПоказатели);
...
Функция ПериодыРасчетаБазыВсехДругихНачислений(ДокументСсылка, ПериодРегистрации)
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.НачислениеОтпускаРаботникамОрганизаций") Тогда
		...
		|ГДЕ
		|	НачислениеОтпускаРаботникамОрганизаций.Ссылка = &Регистратор
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ОсновныеНачисленияРаботниковОрганизаций.ДатаНачалаОсновногоОтпуска КАК ДатаНачала,
		|	ОсновныеНачисленияРаботниковОрганизаций.ДатаОкончанияОсновногоОтпуска КАК ДатаОкончания,
		|	ОсновныеНачисленияРаботниковОрганизаций.Ссылка КАК Регистратор,
		|	ОсновныеНачисленияРаботниковОрганизаций.ПериодРегистрации
		|ПОМЕСТИТЬ ТекущийОтпуск
		|ИЗ
		|	ВТТаблицаДокумента КАК ОсновныеНачисленияРаботниковОрганизаций
		|ГДЕ
		|	ОсновныеНачисленияРаботниковОрганизаций.ОсновнойОтпуск
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ВЫБОР
		|		КОГДА ДополнительныеНачисленияРаботниковОрганизаций.Дата < ДополнительныеНачисленияРаботниковОрганизаций.ПериодРегистрации
		|			ТОГДА ДополнительныеНачисленияРаботниковОрганизаций.ПериодРегистрации
		|		КОГДА ДополнительныеНачисленияРаботниковОрганизаций.Дата > КОНЕЦПЕРИОДА(ДополнительныеНачисленияРаботниковОрганизаций.ПериодРегистрации, МЕСЯЦ)
		|			ТОГДА КОНЕЦПЕРИОДА(ДополнительныеНачисленияРаботниковОрганизаций.ПериодРегистрации, МЕСЯЦ)
		|		ИНАЧЕ ДополнительныеНачисленияРаботниковОрганизаций.Дата
		|	КОНЕЦ,
		|	ДополнительныеНачисленияРаботниковОрганизаций.Дата,
		|	ДополнительныеНачисленияРаботниковОрганизаций.Ссылка,
		|	ДополнительныеНачисленияРаботниковОрганизаций.ПериодРегистрации
		|ИЗ
		|	ВТТаблицаДокумента КАК ДополнительныеНачисленияРаботниковОрганизаций
		|ГДЕ
		|	ДополнительныеНачисленияРаботниковОрганизаций.КомпенсацияОтпуска
		|	И НЕ ДополнительныеНачисленияРаботниковОрганизаций.ОсновнойОтпуск
		|	И НЕ ДополнительныеНачисленияРаботниковОрганизаций.ДополнительныйОтпуск
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ОсновныеНачисленияРаботниковОрганизаций.ДатаНачалаДополнительногоОтпуска,
		|	ОсновныеНачисленияРаботниковОрганизаций.ДатаОкончанияДополнительногоОтпуска,
		|	ОсновныеНачисленияРаботниковОрганизаций.Ссылка,
		|	ОсновныеНачисленияРаботниковОрганизаций.ПериодРегистрации
		|ИЗ
		|	ВТТаблицаДокумента КАК ОсновныеНачисленияРаботниковОрганизаций
		|ГДЕ
		|	ОсновныеНачисленияРаботниковОрганизаций.ДополнительныйОтпуск
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	ТекущийОтпуск КАК ПериодыТекущегоОтпуска
		|
		|СГРУППИРОВАТЬ ПО
		|	ПериодыТекущегоОтпуска.Сотрудник,
		|	ПериодыТекущегоОтпуска.ПериодРегистрации,
...
КонецФункции // ПериодыРасчетаБазыВсехДругихНачислений
