...
Процедура ДополнитьМассивУзловДляВыгрузкиСправочника(Элемент, МассивУзловДляПередачи, 
	...
	ИначеЕсли (ЕстьСправочникНаправленияСписанияВыпущеннойПродукции И ТипЭлемента = Тип("СправочникОбъект.НаправленияСписанияВыпущеннойПродукции"))
		...
		ИЛИ ТипЭлемента = Тип("СправочникСсылка.ЗаявкиКандидатов")
		ИЛИ ТипЭлемента = Тип("СправочникОбъект.КонтрактыИсполнителей")
		ИЛИ ТипЭлемента = Тип("СправочникСсылка.КонтрактыИсполнителей") Тогда		
	...
	ИначеЕсли ТипЭлемента = Тип("СправочникОбъект.ПодразделенияОрганизаций")
		...
		ИЛИ ТипЭлемента = Тип("СправочникСсылка.РегистрацияВИФНС")
		ИЛИ ТипЭлемента = Тип("СправочникОбъект.ДоверенностиНалогоплательщика")
		ИЛИ ТипЭлемента = Тип("СправочникСсылка.ДоверенностиНалогоплательщика")
		ИЛИ ТипЭлемента = Тип("СправочникОбъект.КонтрактыСЗаказчиками")
		ИЛИ ТипЭлемента = Тип("СправочникСсылка.КонтрактыСЗаказчиками") Тогда
	...
	ИначеЕсли ТипЭлемента = Тип("СправочникОбъект.ЭтапыКонтрактов")
		ИЛИ ТипЭлемента = Тип("СправочникСсылка.ЭтапыКонтрактов") Тогда
		
		СтруктураПараметров = Новый Структура("Владелец", Справочники.КонтрактыИсполнителей.ПустаяСсылка());
		ЗаполнитьСтруктуруПараметровПоЭлементу(Элемент, "Справочники", ТипЭлемента, СтруктураПараметров);
		
		СсылкаНаОрганизацию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметров.Владелец, "Организация");
		
		ДополнитьМассивУзламиПоОрганизации(МассивУзловДляПередачи, СсылкаНаОрганизацию, СоответствиеУзловИОрганизаций);
...
Процедура ДополнитьМассивУзловДляВыгрузкиДокумента(Элемент, МассивУзловДляПередачи, 
	...
	Иначе	
		...
		Если Метаданные.ПланыОбмена.ОбменУППУПП.Состав.Содержит(МетаданныеДокумента) Тогда
...
Функция ОпределениеУзловДляВыгрузкиНабораЗаписейРегистраСведений(Элемент, МетаданныеРегистра, 
	...
	ИначеЕсли ЕстьРегистрСостоянияСогласованияЗаявок И ТипЭлемента = Тип("РегистрСведенийНаборЗаписей.СостоянияСогласованияЗаявок") Тогда
...
Процедура ПередЗаписьюНабораЗаписейДляОбменаПоОрганизации(Источник, Отказ, Замещение, ИмяБазовогоТипа, ТипЗначенияИсточника = Неопределено)
	...
	Если Замещение Тогда
		...
		Для Каждого ЗначениеОтбора Из Источник.Отбор Цикл
			...
			Если НЕ ЗначениеОтбора.Использование Тогда
				Продолжить;
			КонецЕсли;
