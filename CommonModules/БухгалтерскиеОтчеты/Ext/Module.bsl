...
// Параметры:
//	ФормаОтчета
//  ПоказыватьПодписи - булево. Отображать пометку или нет
Процедура УправлениеПометкамиКнопокПодписейКоманднойПанели(ФормаОтчета, Знач ПоказыватьПодписи) Экспорт
	
	ФормаОтчета.ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Подписи.Пометка = ПоказыватьПодписи;
	ФормаОтчета.ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Подменю.Кнопки.Подписи.Пометка = ПоказыватьПодписи;
...
Процедура ДобавитьТекстПроЕдиницуИзмерения(ТекстПараметр) Экспорт
	
	ТекстПараметр = "" + ТекстПараметр + Символы.ПС + "Единица измерения: рубль (код по ОКЕИ 383)";
		...
		Для Каждого Строка Из СтрокиОтбора Цикл
			...
			Попытка
				...
				Иначе
					...
					Если НЕ ЗначениеЗаполнено(ЭлементОтбора.ВидСравнения) Тогда
...
Процедура СформироватьОтчетОборотноСальдовойВедомости(ОбъектОтчета, ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0,
	...
	Знач ВидУчета = Неопределено, Знач Валюта = Неопределено, Знач ОтражениеВНУ = Неопределено, 
	Знач Сценарий = Неопределено, Знач ВВалютеСценария = Неопределено, ПоказыватьПодвал = Истина, ПоказыватьПодписи = Ложь) Экспорт
	...
	Если ОбъектОтчета.ИмяРегистраБухгалтерии = "Хозрасчетный" Тогда
		// Вывод подписей отчета
		БухгалтерскиеОтчеты.СформироватьИВывестиПодписиОтчета(ОбъектОтчета, ДокументРезультат, ВысотаЗаголовка, ПоказыватьПодписи);
	КонецЕсли;
...
Процедура НастроитьОтборыДанныхПопараметрам(Параметры, Корреспонденции, Валюта, ОтборПоВалюте) Экспорт
	...
	Если ЗначениеЗаполнено(Видучета) Тогда
		...
		Для Каждого Стр Из Корреспонденции Цикл
			ОтборСубконтоНовый = ОтборВидаУчетаДтКт.Скопировать();
			Для Каждого ОтборСубконтостр ИЗ стр.Субконто Цикл
				ОтборСубконтоНовыйстр = ОтборСубконтоНовый.Добавить();
				ЗаполнитьЗначенияСвойств(ОтборСубконтоНовыйстр, ОтборСубконтостр);
			КонецЦикла;
			стр.Субконто = ОтборСубконтоНовый;
		КонецЦикла;
	КонецЕсли;
