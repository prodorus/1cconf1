...
Процедура УстановитьПодменюВыбораВидаОперации(ЭлементМеню, ОбъектЗаполнения, ОбъектОбработкиВыбораОперации) Экспорт
	...
	Для Каждого ЭлементЗаполнения Из ОбъектЗаполнения Цикл
		...
		Если НРег(Лев(ЭлементЗаполнения.Имя, 7)) = НРег("Удалить")
		 ИЛИ МассивНедоступныхВидовОпераций.Найти(ЭлементЗаполнения) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ПунктМеню = ЭлементМеню.Кнопки.Добавить(
			ЭлементЗаполнения.Имя,
...
//
Функция ПроверитьМодифицированность(Объект) Экспорт
	
	Если Метаданные.Документы.Содержит(Объект.Метаданные()) Тогда 
		Возврат ПроверитьМодифицированностьДокумента(Объект);
	Иначе
		Возврат ПроверитьМодифицированностьОбъекта(Объект);
	КонецЕсли;
...
//
Функция ПроверитьМодифицированностьВФорме(Объект, Форма, ОписаниеДействия = "печати") Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		Ответ = ЗадатьВопрос("Документ изменен. Для " + ОписаниеДействия + " его необходимо записать" + СтрПроведен + ".
...
Процедура ПоказатьСтруктуруПодчиненностиДокумента(ДокументСсылка, СтруктураПоФЗ275 = Ложь) Экспорт
	
	Если Не СтруктураПоФЗ275 Тогда
	...
	Иначе
		ОбщегоНазначения275КлиентСервер.ПоказатьСтруктуруПодчиненности(ДокументСсылка);
	КонецЕсли;
...
Процедура ПроверитьОГРНОформитьПолеВвода(ОГРН, ЮрФизЛицо, ПолеВвода) Экспорт
	...
	Если ЗначениеЗаполнено(ОГРН) и Не ОбщегоНазначенияЗКПереопределяемый.ОГРНСоответствуетТребованиям(ОГРН, ЮрФизЛицо, ТекстСообщенияОшибки) Тогда
		ПолеВвода.ЦветТекстаПоля = ЦветаСтиля.ЦветОсобогоТекста;
...
Процедура ПроверитьКППОрганизацииОформитьПолеВвода(КПП, ЮрФизЛицо, ПолеВвода) Экспорт
	...
	ТекстСообщенияОшибки = "";
	Если ЗначениеЗаполнено(КПП) и Не ОбщегоНазначенияЗКПереопределяемый.КППОрганизацииСоответствуетТребованиям(КПП, ТекстСообщенияОшибки) Тогда
		ПолеВвода.ЦветТекстаПоля = ЦветаСтиля.ЦветОсобогоТекста;
		ПолеВвода.Подсказка = ТекстСообщенияОшибки;
	Иначе
		ПолеВвода.ЦветТекстаПоля = ЦветаСтиля.ЦветТекстаПоля;
...
Функция ВернутьШестнадцатиричноеПредставлениеЦвета(ВыбранныйЦвет) Экспорт
	...
	Красный	= ОбщегоНазначенияЗК.ДесятичноеВШестнадцатиричное(ВыбранныйЦвет.Красный);
	Зеленый	= ОбщегоНазначенияЗК.ДесятичноеВШестнадцатиричное(ВыбранныйЦвет.Зеленый);
	Синий	= ОбщегоНазначенияЗК.ДесятичноеВШестнадцатиричное(ВыбранныйЦвет.Синий);
...
Процедура ВывестиОкноСПредупреждениемОбИзменении(НазваниеФормы, КлючФормы) Экспорт 
	...
	Если КлючФормы = "УведомитьОбИзмененияхСроковПринятияКВычетуНДС" Тогда
		ОписаниеПредупреждения = НСтр("ru = 'В соответствии с ФЗ №382 от 29.11.2014 с 1 января 2015 года, если счет-фактура получен до срока представления декларации, НДС по принятым к учету в отчетном периоде ценностям можно принять к вычету.'");
	КонецЕсли;
	
	Если КлючФормы = "УведомитьОНеобходимостиПерепроведения" Тогда
		ОписаниеПредупреждения = НСтр("ru = 'Для установки корректного кода вида операций в книгах покупок и продаж, журнале учета полученных и выставленных счетов-фактур необходимо до формирования отчетности по НДС выполнить перепроведение документов, начиная с 01.01.2015.'");
	КонецЕсли;
