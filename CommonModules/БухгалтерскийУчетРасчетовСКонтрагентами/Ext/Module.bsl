...
Функция ОпределениеНаправленияДвиженияДляДокументаПриобретенияРеализации(ВидДокумента,Знач ВидОперации = неопределено)Экспорт
	...
	Если ВидДокумента = "ОтчетКомитентуОПродажах" или типЗнч(ВидДокумента) = Тип("ДокументСсылка.ОтчетКомитентуОПродажах") Тогда
	...
	Если ВидДокумента = "ОтчетКомиссионераОПродажах" или типЗнч(ВидДокумента) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
		...
		ВидДействийДокумента.Вставить("РасчетыВозврат",ВидОперацииРасчет);
	ИначеЕсли ВидДокумента = "ОтчетКомитентуОПродажах" или типЗнч(ВидДокумента) = Тип("ДокументСсылка.ОтчетКомитентуОПродажах") Тогда
		// Вид операции моет быть задан, а может быть и не задан
		ВидДействийДокумента.Вставить("РасчетыВозврат",ВидОперацииРасчет);	
	...
	ИначеЕсли ЗначениеЗаполнено(ВидОперации) тогда
		...
		РасчетВозвратПоВидуОпераций.Вставить(Перечисления.ВидыОперацийРеализацияТоваров.АктВыполненныхРабот, ВидОперацииРасчет);
		РасчетВозвратПоВидуОпераций.Вставить(Перечисления.ВидыОперацийРеализацияТоваров.АктНаПередачуПрав, ВидОперацииРасчет);
...
Функция ПодготовитьТаблицуДляРегистраРасчетовПоПриобретению(СтруктураПараметровВзаиморасчетов, СтруктураШапкиДокумента) Экспорт
	...
	Для Каждого Движение Из ТаблицаДвижений Цикл
		...
		Для каждого ВидСубконто из КоррСчет.ВидыСубконто Цикл
			...
			Если Движение.ТекущаяТаблица = "Услуги" И ЗначениеЗаполнено(Движение.СтатьяЗатрат) И ЗначениеЗаполнено(Движение["Субконто" + НомерСубконто])
			...
			Иначе
				...
				ИначеЕсли ДопустимыеТипыСубконто.СодержитТип(Тип("СправочникСсылка.ОбъектыСтроительства"))
					И ТаблицаДвижений.Колонки.Найти("ОбъектСтроительства") <> Неопределено Тогда
					ЗначениеСубконто = Движение.ОбъектСтроительства;
				КонецЕсли;
			КонецЕсли; 
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			Для каждого ВидСубконто из СчетУчетаНУ.ВидыСубконто Цикл
				...
				Если Движение.ТекущаяТаблица = "Услуги" И ЗначениеЗаполнено(Движение.СтатьяЗатрат) И ЗначениеЗаполнено(Движение["СубконтоНУ" + НомерСубконто + "Базовое"])
					И НЕ УправлениеЗатратами.ПроверитьСтатьюЗатратНаПроизводственныеРасходы(Движение.СтатьяЗатрат) Тогда
					// Это услуги непроизводственного характера. При формировании проводок использовалась аналитика из ТЧ.  
	...
	Иначе
		ТаблицаДвижений.Колонки.Удалить(ТаблицаДвижений.Колонки.ОбъектСтроительства);
	КонецЕсли;	
...
//
Процедура БухгалтерскийУчетРасчетыСКонтрагентами_Оплата(СтруктураПараметров, СтруктураШапкиДокумента, СтруктураДвижений, Отказ, Заголовок, ПринадлежностьПоследовательностям = Неопределено, ВызванРегламентнойПроцедурой = ложь, УдаляемыеВидыРегОпераций = Неопределено, КэшВидовСубконтоПоСчетам = Неопределено, ВосстановлениеПоследовательностиРасчсетов = Ложь) Экспорт
	
	Если ТипЗнч(СтруктураПараметров) = Тип("Булево")
		ИЛИ НЕ СтруктураПараметров.Свойство("Таблица")
		ИЛИ (СтруктураПараметров.Свойство("Таблица") И СтруктураПараметров.Таблица.Количество()=0) тогда
	...
	Если Не ОпределятьАвансыПриПроведенииДокумента тогда
		...
		Для каждого ТекущийПлатеж из ТаблицаПлатежей Цикл
			...
			Для каждого ВидСубконтоКоррСчета из ВидыСубконтоКоррСчета цикл
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + СуфиксКоррСчета], Проводка["Субконто" + СуфиксКоррСчета], ВидСубконтоКоррСчета.НомерСтроки, ТекущийПлатеж["КоррСубконто" + ВидСубконтоКоррСчета.НомерСтроки]);
			КонецЦикла;
		...
		Если СтрокаПлатежа.ВестиПоДокументамРасчетовСКонтрагентом Тогда
			...
			Если НетОстатков Тогда
				...
				Если СтрокаПлатежа.ЗаписьОтражаетсяВРегистре Тогда
					...
					Если ВосстановлениеПоследовательностиРасчсетов Тогда
						
						ТекстСообщения = "Указана сумма взаиморасчетов, превышающая непогашенный остаток по документу расчетов с контрагентом!
						| Договор: " + СтрокаПлатежа.ДоговорКонтрагента + 
						?(НЕ ЗначениеЗаполнено(СтрокаПлатежа.Сделка),", сделка не указана",", сделка: " + СтрокаПлатежа.Сделка)+ 
						", документ расчетов: " + СтрокаПлатежа.ДокументРасчетовСКонтрагентом + "; 
						| Остаток " + 0 + " " + СтрокаПлатежа.ВалютаВзаиморасчетов + "; "
						+ " указана сумма " + СтрокаПлатежа.СуммаВзаиморасчетов + " " + СтрокаПлатежа.ВалютаВзаиморасчетов + "; "
						+ " превышение " + СтрокаПлатежа.СуммаВзаиморасчетов + " " + СтрокаПлатежа.ВалютаВзаиморасчетов;
					...
					Если ТаблицаПлатежей.Колонки.Найти("ДатаОплаты") <> Неопределено Тогда
						СтрокаОстатка.ДатаОплаты = СтрокаПлатежа.ДатаОплаты;
					ИначеЕсли СтруктураПараметров.Свойство("Дата") 
						И ЗначениеЗаполнено(СтруктураПараметров.Дата) Тогда
						СтрокаОстатка.ДатаОплаты = СтруктураПараметров.Дата;
					Иначе 
						//ситуация маловероятная, но возможная, 
						//т.е. поле Дата либо нет, 
						//либо она не определена
						СтрокаОстатка.ДатаОплаты = Дата(1,1,1);
					КонецЕсли;
			...
			Если СтрокаОстатка.СуммаВзаиморасчетов < СтрокаПлатежа.СуммаВзаиморасчетов Тогда
				СтрокаОстатка.СуммаВзаиморасчетов = СтрокаПлатежа.СуммаВзаиморасчетов;
			КонецЕсли;
		...
		Для каждого СтрокаОстатка Из СтрокиОстатка Цикл
			...
			Если СтрокаПлатежа.РасчетыВУсловныхЕдиницах И ТекущаяСуммоваяРазница <> 0 Тогда
				...
				Если НЕ ЭтоВозврат Тогда
					...
					//С 01.01.2015 года новый порядок отражения суммовых разниц. 
					//Суммовые разницы могут возникать только по задолженностям возникшим до 01.01.2015
					
					Если НЕ ТекДокументРасчетов = Неопределено и ТекДокументРасчетов.Дата < '20150101' Тогда
	...
	ИначеЕсли НаправлениеРасчетовСКонтрагентом = Перечисления.ВидыРасчетовСКонтрагентами.ПоПриобретению Тогда 
		УчетНДСФормированиеДвижений.СформироватьДвиженияНалоговыйАгентПоОплате_КурсовыеРазницы(СтруктураШапкиДокумента, ТаблицаОплатыДляНДС, ПроводкиНДС, Отказ)
	КонецЕсли;
	
	Если ПроводкиНДС <> Неопределено 
		И ПроводкиНДС.Количество() <> 0 Тогда 
		Для Каждого ПроводкаНДС Из ПроводкиНДС Цикл
			Проводка = ПроводкиБУ.Добавить();
			ЗаполнитьЗначенияСвойств(Проводка, ПроводкаНДС);
			Проводка.ВидРегламентнойОперации  = ВидРегламентнойОперации;
			Для К = 1 По 3 Цикл
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, К, ПроводкаНДС["СубконтоДт" + К], Истина);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, К, ПроводкаНДС["СубконтоКт" + К], Истина);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;		
	...
	Для каждого СтрокаПлатежа из ТаблицаПлатежей Цикл
		...
		ФормироватьПроводкиПоСчетамВАЛ = Не ОценкаДоходовРасходовПоКурсуАвансов И РасчетыВВалюте2008
			И НЕ СтруктураШапкиДокумента.ОрганизацияПрименяетУСН
			И НЕ СтруктураШапкиДокумента.ВидДокумента = "КорректировкаДолга";
		...
		Если СтрокаПлатежа.ДолгРег > 0 Тогда 
			...
			ВидыСубконтоКоррСчета =  БухгалтерскийУчет.ОпределитьВидСубконтоПоСчету(Проводка["Счет" + СуфиксКоррСчета], КэшВидовСубконтоПоСчетам);
			Для каждого ВидСубконтоКоррСчета из ВидыСубконтоКоррСчета цикл
				БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + СуфиксКоррСчета], Проводка["Субконто" + СуфиксКоррСчета], ВидСубконтоКоррСчета.НомерСтроки, СтрокаПлатежа["КоррСубконто" + ВидСубконтоКоррСчета.НомерСтроки]);
			КонецЦикла;
...
Процедура РаспределениеСуммовойРазницы(СуммоваяРазницаПоДокументам, СтруктураПараметров, СтруктураШапкиДокумента, СтруктураДвижений, Отказ, Заголовок, УдаляемыеВидыРегОпераций = Неопределено, КэшВидовСубконтоПоСчетам = Неопределено)
	...
	Для каждого Строка Из ТаблицаРаспределенияСР Цикл 
		...
		Если Строка.Период >= '20111001' Тогда
	...
	Для Каждого ТекКорректировка из ТаблицаРаспределенияСР Цикл
		...
		Если ТекКорректировка.Период < '20111001' 
			...
			Иначе
				...
				УчетнаяПолитикаНУ = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиРегл(ТекКорректировка.Период, Организация);
	...
	Если ПрименяетсяУчетПРВКурсовыхРазницахПоРасчетамВУЕ 
        ...
        Если НЕ (МассивНДС_НУ = Неопределено ИЛИ МассивНДС_НУ.Количество() = 0) Тогда
            Для Каждого СтрокаНДС Из МассивНДС_НУ Цикл
				Если СтрокаНДС.ДатаДокумента < '20111001' Тогда
...
Процедура ДвижениеДенегПрочийРасход(Объект, РасчетыВВалюте, СчетКт, СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	Для каждого ТекущийПлатеж из РеестрПлатежей Цикл
		...
		СчетАванса = ?(ПрочиеРасчеты, ТекущийПлатеж.СчетОплаты, ТекущийПлатеж.СчетАванса);
		...
		Если НужноЗаплатить > 0 Тогда // Оплата от покупателя
			...
			Проводка.Активность  = Истина;
			Проводка.Организация = Организация;
			Проводка.Содержание  = ?(ПрочиеРасчеты, "Прочие расчеты", ВидОперации);
			...
			Если СтруктураШапкиДокумента.Свойство("ОбособленноеПодразделение") Тогда
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт,"Подразделения",СтруктураШапкиДокумента.ОбособленноеПодразделение);
			КонецЕсли;
		...
		Если СуммаАванса <> 0 тогда //Зачесть аванс
			...
			Проводка.Активность  = Истина;
			Проводка.Организация = Организация;
			Проводка.Содержание  = ?(ПрочиеРасчеты, "Прочие расчеты", "Аванс");
...
Процедура ДвижениеДенегПрочийПриход(Объект, РасчетыВВалюте, СчетДт, СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	Для каждого ТекущийПлатеж из РеестрПлатежей Цикл
		...
		Если НужноЗаплатить > 0 Тогда // Оплата от покупателя
			...
			Если СтруктураШапкиДокумента.Свойство("ОбособленноеПодразделение") Тогда
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,Проводка.СубконтоДт,"Подразделения",СтруктураШапкиДокумента.ОбособленноеПодразделение);
			КонецЕсли;
...
//                 <продолжение описания параметра>
//
Функция РасчетыВУсловныхЕдиницахПриобретениеРеализация(СтруктураПараметров,ВалютаРегламентированногоУчета,РежимПроведения=Неопределено,ПроводкиБУ,Объект = Null,Отказ=Ложь,УдаляемыеВидыРегОпераций=Неопределено, ПолучатьПроводкиПоСсылке = Ложь) экспорт
	...
	Если ТаблицаРегистрации.Колонки.Найти("НДСВал") = Неопределено Тогда
		...
		Если не ТаблицаРегистрации.Итог("СуммаНДС") = 0 Тогда
			Для каждого СтрокаТаблицы Из ТаблицаРегистрации Цикл
				...
				Иначе
					СтрокаТаблицы.НДСВал = Окр(УчетНДС.РассчитатьСуммуНДС(СтрокаТаблицы.СуммаВзаиморасчетов,
													   Истина, Истина,
	...
	Если ВызванРегламентнойПроцедурой тогда
		...
		ДвиженияПоРегиструВзаиморасчетов = ОбщегоНазначения.ПолучитьНаборЗаписейПоСсылке(Ссылка, РегистрыНакопления["ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов"], Истина);
	Иначе
		...
		ДвиженияПоРегиструВзаиморасчетов = ОбщегоНазначения.ПолучитьДвижение(Объект, "ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов");
	КонецЕсли;
	...
	ТаблицаДвиженийПоРегиструВзаиморасчетов = Новый ТаблицаЗначений();
	
	Если НЕ ДвиженияПоРегиструВзаиморасчетов = Неопределено Тогда
		ТаблицаДвиженийПоРегиструВзаиморасчетов = ДвиженияПоРегиструВзаиморасчетов.Выгрузить();
	КонецЕсли;
	...
	Иначе
		...
		Если ИспользоватьОтборПоДокументам Тогда
			ДокументыРасчетов = ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ТаблицаРегистрации.ВыгрузитьКолонку("ДокументРасчетов"),Истина);
	...
	Для каждого СтрокаТаблицыСделок Из ТаблицаСделокДокумента Цикл
		...
		Если ЕстьОстаткиАвансовПоСделке ИЛИ ВыделятьАвансыПриКорректировке Тогда
			...
			Для каждого ОстатокАванса Из ОбщиеОстаткиАвансов Цикл
				...
				КурсОстаткаАванса              = ?(ОстатокАвансаВал = 0, 0, ОстатокАвансаРег / ОстатокАвансаВал); // не округляется до 4-х знаков, т.к. могут быть валюты с кратностью <> 1
				Если ВыделятьАвансыПриКорректировке Тогда
		...
		Если СуммаСделкиВал <> 0 ИЛИ СуммаСделкиРег <> 0 Тогда 
			...
			Если СуммаСделкиВал = 0 И НЕ (СуммаСделкиРег < -0.05 ИЛИ СуммаСделкиРег > 0.05) Тогда
				
				// случай погрешности округления, а не оставшейся задолженности. 
				// добавим ее на последнюю строчку ДвиженияПоРегиструУЕ и в сумму ПроводкиБУ по зачету аванса
				Если ДвиженияПоРегиструУЕ.Количество() > 0 Тогда
					Движение = ДвиженияПоРегиструУЕ[ДвиженияПоРегиструУЕ.Количество()-1];
					Движение.СуммаРег 	= Движение.СуммаРег + СуммаСделкиРег; 
					
					Если ТаблицаДвиженийПоРегиструВзаиморасчетов.Количество() > 0 Тогда
						ТаблицаДвиженийПоРегиструВзаиморасчетов[ТаблицаДвиженийПоРегиструВзаиморасчетов.Количество()-1].СуммаРегл = Движение.СуммаРег;
						ДвиженияПоРегиструВзаиморасчетов.Загрузить(ТаблицаДвиженийПоРегиструВзаиморасчетов);
						ДвиженияПоРегиструВзаиморасчетов.Записать();
					КонецЕсли;
					
					Если ЭтоВозврат И Продажи Тогда
						СуммаДокументаРег   = СуммаДокументаРег - СуммаСделкиРег;  
					Иначе
						СуммаАвансаРег 		= СуммаАвансаРег - СуммаСделкиРег;
					КонецЕсли;
				КонецЕсли;
		...
		Иначе
			...
			Если Метаданные.РегистрыНакопления.Найти("БракВПроизводствеБухгалтерскийУчет") 			<> Неопределено
				...
				Для каждого Строка из ТаблицаРаспределенияСР Цикл
					...
					ХарактерЗатрат = УправлениеЗатратами.ПолучитьХарактерЗатратПоСчетуЗатрат(Строка["КоррСчет"], СтатьяЗатрат);
					...
					ИначеЕсли ЕстьБракБУ И ХарактерЗатрат = Перечисления.ХарактерЗатрат.БракВПроизводстве Тогда
	...
	Иначе
		...
		СтрокаКРаспределнию = ТаблицаРаспределенияСР.Добавить();
		СтрокаКРаспределнию.Регистратор = Ссылка;
		СтрокаКРаспределнию.СуммаСНДС = ?(ТипЗнч(Ссылка) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя"), СуммоваяРазница, -СуммоваяРазница);
		
		Если УчетНДС.РасчетНДСвРубляхПоСтавкеДокумента(Ссылка.Дата) И Ссылка.Дата >= Дата('20151001') Тогда
			
			СоответствиеКолонокТаблицы = Новый Структура("СуммаСНДС, СтавкаНДС, СуммаНДС",
			"РублеваяСумма", "СтавкаНДС", "СуммаНДС");
			
			СтрокаКРаспределнию.СуммаНДС = 
			УчетНДСФормированиеДвижений.ОпределитьСуммуНДСКорректировкиПоСтрокамДокумента(-СуммоваяРазница, ТаблицаРегистрации, СоответствиеКолонокТаблицы);
		...
		Для каждого Строка из ТаблицаРаспределенияСР Цикл
			...
			// Вычислим общий НДС от исходной суммы + суммовая разница и сравним его с полученным НДС суммовой разницы
			// если есть погрешность в копейку - откорректируем её
			Если НЕ (Строка.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС ИЛИ Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС0) Тогда
				ОбщаяСуммаБазисаИСуммовойРазницыСНДС = Строка.РублеваяСумма + Строка.СуммаБезНДС + Строка.СуммаНДС;
				Базис_НДС = ТаблицаРегистрации.Итог("СуммаНДС");
				
				НДС = УчетНДС.РассчитатьСуммуНДС(ОбщаяСуммаБазисаИСуммовойРазницыСНДС, Истина, Истина, УчетНДС.ПолучитьСтавкуНДС(Строка.СтавкаНДС));
				Дельта = Окр(НДС - Базис_НДС - Строка.СуммаНДС, 2, 1);
				
				Если Макс(Дельта, -Дельта) = 0.01 Тогда
					Строка.СуммаНДС = Строка.СуммаНДС + Дельта;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;//Подготовка таблицы распределения
		...
		Если не НДСВключенВСтоимость тогда
			...
			// НДС только в части собственных товаров (без учета комиссионных)
			ПоНДС = ОбщегоНазначения.СформироватьКорректирующиеЗаписи(ТаблицыРеализацииСобственныхИКомиссионых.ТаблицаРаспределенияСР_Собственный,
																	  ТаблицыРеализацииСобственныхИКомиссионых.ТаблицаПроводокБУ_Собственный,
																	  Распределение,СтруктураОтбора,ИсключитьБазовый,Дополнение);
			БухгалтерскийУчет.ДополнитьНаборЗаписейРегистраЗаписямиТаблицы(ПроводкиБУ, ПоНДС);
		КонецЕсли;
			...
			Если Метаданные.РегистрыНакопления.Найти("БракВПроизводствеБухгалтерскийУчет") 			<> Неопределено
				...
				Для каждого Строка из ТаблицаРаспределенияСР Цикл
					...
					Если СтруктураПараметров.ОтражатьВНалоговомУчете тогда
						...
						ХарактерЗатрат = УправлениеЗатратами.ПолучитьХарактерЗатратПоСчетуЗатрат(СчетЗатратНУ, СтатьяЗатрат, "Налоговый");
						Если ХарактерЗатрат = Перечисления.ХарактерЗатрат.ПроизводственныеРасходы Тогда
							...
							КонецЕсли;
						ИначеЕсли ЕстьБракНУ И ХарактерЗатрат = Перечисления.ХарактерЗатрат.БракВПроизводстве Тогда
			 			    Если СтатьяЗатрат.ВидРасходовНУ = Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения Тогда
			 			...
			 			ИначеЕсли ХарактерЗатрат = Перечисления.ХарактерЗатрат.ИздержкиОбращения
							  ...
							  ИЛИ ХарактерЗатрат = Перечисления.ХарактерЗатрат.ОбщехозяйственныеРасходы Тогда
						    Если СтатьяЗатрат.ВидРасходовНУ = Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения Тогда
...
//
Функция РазделитьРеализациюСобственногоИКомиссионногоТовара(ТаблицаРаспределенияСР, ТаблицаПроводокБУ, СтруктураОтбора) 
	...
	Для Каждого СтрокаТаблицыРаспределения Из ТаблицаРаспределенияСР Цикл
		СтрокаДельты 				= ТаблицаДельтаРаспределения.Добавить();
		СтрокаДельты.СчетДт 		= СтрокаТаблицыРаспределения[СтруктураОтбора.СчетДт];
		Если СтруктураОтбора.Свойство("СубконтоДт1") Тогда
			СтрокаДельты.СубконтоДт1 	= СтрокаТаблицыРаспределения[СтруктураОтбора.СубконтоДт1];
		КонецЕсли;
		Если СтруктураОтбора.Свойство("СубконтоДт2") Тогда
			СтрокаДельты.СубконтоДт2 	= СтрокаТаблицыРаспределения[СтруктураОтбора.СубконтоДт2];
		КонецЕсли;
		Если СтруктураОтбора.Свойство("СубконтоДт3") Тогда
			СтрокаДельты.СубконтоДт3 	= СтрокаТаблицыРаспределения[СтруктураОтбора.СубконтоДт3];
		КонецЕсли;
		
		СтрокаДельты.СчетКт 		= СтрокаТаблицыРаспределения[СтруктураОтбора.СчетКт];
		Если СтруктураОтбора.Свойство("СубконтоКт1") Тогда
			СтрокаДельты.СубконтоКт1 	= СтрокаТаблицыРаспределения[СтруктураОтбора.СубконтоКт1];
		КонецЕсли;
		Если СтруктураОтбора.Свойство("СубконтоКт2") Тогда
			СтрокаДельты.СубконтоКт2 	= СтрокаТаблицыРаспределения[СтруктураОтбора.СубконтоКт2];
		КонецЕсли;
		Если СтруктураОтбора.Свойство("СубконтоКт3") Тогда
			СтрокаДельты.СубконтоКт3 	= СтрокаТаблицыРаспределения[СтруктураОтбора.СубконтоКт3];
		КонецЕсли;
		
		СтрокаДельты.ВалютнаяСумма	= СтрокаТаблицыРаспределения.ВалютнаяСумма;
		СтрокаДельты.ПризнакСтрокаТаблицыРаспределения = 1;
	КонецЦикла;
	
	// Данные из регистра бухгалтерии берем с минусом.
	Для Каждого СтрокаПроводкиРаспределения Из ТаблицаПроводокБУ Цикл
	
		Если НЕ ЗначениеЗаполнено(СтрокаПроводкиРаспределения.ВалютнаяСуммаДт) Тогда
			Продолжить;
		КонецЕсли;
		
		// Уже имеющиеся корректировки не учитываем, чтобы не задвоить.
		Если СтрокаПроводкиРаспределения.ВидРегламентнойОперации = Перечисления.ВидыРегламентныхОпераций.КорректировкиПриВалютныхРасчетах Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаДельты 				= ТаблицаДельтаРаспределения.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаДельты, СтрокаПроводкиРаспределения);
		СтрокаДельты.ВалютнаяСумма	= - СтрокаПроводкиРаспределения.ВалютнаяСуммаДт;
	...
	Пока ИндексСтрокиРаспределения >= 0 Цикл
		СтрокаТаблицыРаспределения = ТаблицаДельтаРаспределения[ИндексСтрокиРаспределения];
		
		Если СтрокаТаблицыРаспределения.ВалютнаяСумма = 0 Тогда
	...
	Если ТаблицаДельтаРаспределения.Количество() = 0 Тогда
	...
	ИначеЕсли ТаблицаДельтаРаспределения.Количество() = КоличествоСтрокДоУдаления Тогда
	
		// Не совпала ни одна строка из регистра с проводками, значит 
		// весь товар является комиссионным.
		СтруктураРезультат.Вставить("ТаблицаРаспределенияСР_Комиссионный", ТаблицаРаспределенияСР.Скопировать());
		СтруктураРезультат.Вставить("ТаблицаПроводокБУ_Комиссионный", ТаблицаПроводокБУ.Скопировать());
	
	Иначе
	
		// По текущему документу была реализация как собственного, так и комиссионного товара,
		// необходимо разделение.
	
		СтруктураПоискаПроводок = Новый Структура("СчетДт, СубконтоДт1, СубконтоДт2, СубконтоДт3,
										  		  |СчетКт, СубконтоКт1, СубконтоКт2, СубконтоКт3");
		
		СтруктураПоискаРаспределения = Новый Структура();
										  
		// По умолчанию сначала все считаем реализацией собственного товара, далее в цикле выделим из него 
		// комиссионный товар.
		СтруктураРезультат.ТаблицаРаспределенияСР_Собственный 	= ТаблицаРаспределенияСР.Скопировать();
		СтруктураРезультат.ТаблицаПроводокБУ_Собственный 		= ТаблицаПроводокБУ.Скопировать();
		
		Для Каждого СтрокаДельты Из ТаблицаДельтаРаспределения Цикл
			
			Если СтрокаДельты.ПризнакСтрокаТаблицыРаспределения = 0 Тогда
				// Надо исключить строки из проводок по реализации собственных товаров
				// и включить в проводки по реализации комиссионных товаров.
				ЗаполнитьЗначенияСвойств(СтруктураПоискаПроводок, СтрокаДельты);
			
				НайденныеСтроки 	= СтруктураРезультат.ТаблицаПроводокБУ_Собственный.НайтиСтроки(СтруктураПоискаПроводок);
			
				Для Инд = 0 По НайденныеСтроки.Количество() - 1 Цикл
					НоваяСтрока = СтруктураРезультат.ТаблицаПроводокБУ_Комиссионный.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденныеСтроки[Инд]);
					СтруктураРезультат.ТаблицаПроводокБУ_Собственный.Удалить(НайденныеСтроки[Инд]);
				КонецЦикла;
				
			Иначе
			
				// Надо исключить из таблицы распределения по собственному товару 
				// и включить в таблицу по комиссионному товару.
				Для Каждого КлючИЗначение Из СтруктураОтбора Цикл
					Если ТаблицаДельтаРаспределения.Колонки.Найти(КлючИЗначение.Ключ) <> Неопределено Тогда
						СтруктураПоискаРаспределения.Вставить(КлючИЗначение.Значение, СтрокаДельты[КлючИЗначение.Ключ]);
					КонецЕсли;
				КонецЦикла;
			
				НайденныеСтроки = СтруктураРезультат.ТаблицаРаспределенияСР_Собственный.НайтиСтроки(СтруктураПоискаРаспределения);
				Для Инд = 0 По НайденныеСтроки.Количество() - 1 Цикл
					НоваяСтрока = СтруктураРезультат.ТаблицаРаспределенияСР_Комиссионный.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденныеСтроки[Инд]);
					СтруктураРезультат.ТаблицаРаспределенияСР_Собственный.Удалить(НайденныеСтроки[Инд]);
				КонецЦикла;
...
КонецФункции // РазделитьРеализациюСобственногоИКомиссионногоТовара()
