...
Функция ЧекЕГАИСXML(ДокументСсылка, РезультатыЗапроса, МенеджерВременныхТаблиц) Экспорт
	...
	АдресТорговогоОбъекта = ИнтеграцияЕГАИСПереопределяемый.АдресТорговогоОбъекта(?(ЗначениеЗаполнено(Шапка.ДокументОснование),
		Шапка.ДокументОснование.Организация,
		Шапка.ОрганизацияЕГАИС.Контрагент));
	...
	Если Не ЗначениеЗаполнено(АдресТорговогоОбъекта) Тогда
		ИнтеграцияЕГАИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеXML,
			СтрШаблон(НСтр("ru = 'Не заполнен адрес организации: %1'"), Шапка.ДокументОснование.Организация));
		СообщенияXML.Добавить(СообщениеXML);
		Возврат СообщенияXML;
	КонецЕсли;
