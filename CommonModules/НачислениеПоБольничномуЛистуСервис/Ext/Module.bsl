...
Процедура ПрочитатьРазмерыПособия(Объект, ДополнительныеДанные, ТолькоДополнительныеДанные, ГоловнаяОрганизация = Неопределено) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ДатаСтажа) Тогда
		...
		Иначе 
			...
			ИначеЕсли Объект.УсловияИсчисленияКод2 = "47" Тогда
				Объект.УсловияИсчисленияКод2 = ""
			ИначеЕсли Объект.УсловияИсчисленияКод3 = "47" Тогда
...
Функция ОбработатьСобытиеОтСканераШтрихкода(ОписаниеСобытия, ОписаниеОшибки = "") Экспорт
	...
	Если СписокПараметров[0] = "01" Тогда
		...
		Если СписокПараметров[33] = "1" Тогда
			СтруктураРеквизитов.Вставить("ПоставленаНаУчетВРанниеСрокиБеременности", Перечисления.ПоставленаНаУчетВРанниеСрокиБеременности.Поставлена);
		ИначеЕсли ЗначениеЗаполнено(СписокПараметров[33]) Тогда
			СтруктураРеквизитов.Вставить("ПоставленаНаУчетВРанниеСрокиБеременности", Перечисления.ПоставленаНаУчетВРанниеСрокиБеременности.НеПоставлена);
		ИначеЕсли СписокПараметров[16] = "05" Тогда 
			СтруктураРеквизитов.Вставить("ПоставленаНаУчетВРанниеСрокиБеременности", Перечисления.ПоставленаНаУчетВРанниеСрокиБеременности.НеИзвестно);
		КонецЕсли;
