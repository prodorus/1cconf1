...
Функция РеквизитыПервичногоДокумента(Документ, ТЧ = "", Строка = 0, УказыватьВид = Ложь) Экспорт
	...
	Если Документ.Дата >= Дата(2018, 1, 1) Тогда
		РеквизитыПервичногоДокумента = ДатаНаПечать + ", № " + НомерНаПечать;
	Иначе
...
Процедура НачислениеЗП(СтруктураШапкиДокумента, ДвиженияРегистров, КУДиР) //
	...
	Если ТаблицаЗачетаНалогов.Итог("Сумма") > 0 Тогда
		...
		ОплатаПоставщикуУСН(СтруктураШапкиДокумента, ТаблицаЗачетаНалогов, ДвиженияРегистров, КУДиР_Оплаты);
	КонецЕсли;
...
Процедура СписаниеРБПУСН(СтруктураШапкиДокумента, ДвиженияРегистров, КУДиР)
	...
	//Если переданы виды анализируемых РБП - добавим отбор по соответствующим элементам расходов
	Если СтруктураШапкиДокумента.Свойство("ВидыРБП", ВидыРБП) Тогда			
		Если ЗначениеЗаполнено(ВидыРБП) И ТипЗнч(ВидыРБП) = Тип("Массив") Тогда
			
			Запрос = Новый Запрос;
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	РасходыБудущихПериодов.Ссылка КАК ЭлементРБП
			|ИЗ
			|	Справочник.РасходыБудущихПериодов КАК РасходыБудущихПериодов
			|ГДЕ
			|	РасходыБудущихПериодов.ВидРБП В(&ВидыРБП)"
			;
			...
			Фильтр = ТаблицаФильтров.Добавить();
			Фильтр.ИмяПоля			= "ЭлементРасхода";
			Фильтр.ЗначениеПоля	    = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ЭлементРБП");
			Фильтр.Условие			= "В";
