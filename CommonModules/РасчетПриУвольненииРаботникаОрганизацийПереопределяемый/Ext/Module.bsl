...
//		
Процедура СформироватьДоходыПоКодамНДФЛ(Начисления, ВыборкаПоШапкеДокумента, НаборЗаписей) 
	...
	Запрос.УстановитьПараметр("парамСотрудник", ВыборкаПоШапкеДокумента.Сотрудник);
	Запрос.УстановитьПараметр("парамПодразделениеОрганизации", ВыборкаПоШапкеДокумента.ПодразделениеОрганизации);
	...
	Запрос.УстановитьПараметр("парамДатаВыплатыДохода", ?(НачалоМесяца(ВыборкаПоШапкеДокумента.ПериодРегистрации) = НачалоМесяца(ВыборкаПоШапкеДокумента.ДатаУвольнения), ВыборкаПоШапкеДокумента.ДатаУвольнения,КонецМесяца(ВыборкаПоШапкеДокумента.ПериодРегистрации)));
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Начисления.НомерСтроки КАК НомерСтроки,
	|	&парамСотрудник КАК Сотрудник,
	|	Начисления.ВидРасчета,
	|	Начисления.Результат,
	|	&парамПериодРегистрации КАК ДатаНачала,
	|	&парамПериодРегистрации КАК ДатаОкончания,
	|	&парамПодразделениеОрганизации КАК ПодразделениеОрганизации,
	|	&парамПерерассчитываемыйДокумент КАК СторнируемыйДокумент,
	|	Начисления.Сторно
	...
	|	Начисления.ВидРасчета,
	|	ВЫРАЗИТЬ(Начисления.ВидРасчета КАК ПланВидовРасчета.ДополнительныеНачисленияОрганизаций).КодДоходаНДФЛ КАК КодДохода,
	...
	|ГДЕ
	|	Начисления.ВидРасчета ССЫЛКА ПланВидовРасчета.ДополнительныеНачисленияОрганизаций
	|	И ВЫРАЗИТЬ(Начисления.ВидРасчета КАК ПланВидовРасчета.ДополнительныеНачисленияОрганизаций).КодДоходаНДФЛ <> ЗНАЧЕНИЕ(Справочник.ДоходыНДФЛ.ПустаяСсылка)";
	Запрос.Выполнить();
...
Процедура ВыполнитьПрочиеДвиженияДокумента(ДокументОбъект, ВыборкаПоШапкеДокумента, Отказ, Заголовок, ВыборкаПоНачислениям = Неопределено) Экспорт 
	...
	Если НЕ Отказ Тогда
		...
		СформироватьДоходыПоКодамНДФЛ(ДокументОбъект.Начисления, ВыборкаПоШапкеДокумента, ДокументОбъект.Движения.НДФЛСведенияОДоходах);
