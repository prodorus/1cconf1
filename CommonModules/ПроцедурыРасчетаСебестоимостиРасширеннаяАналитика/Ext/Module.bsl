...
Процедура ВыполнитьДействияДокумента(СтруктураШапкиДокумента, ТаблицаОшибок) Экспорт
	...
	ИспользуетсяНУ = Истина; // Признак необходимости НУ
	Если ЕстьРасчетФактическойСебестоимости И (НЕ ПростоеРаспределение ИЛИ ИспользуетсяНУ)Тогда
	...
	Если НЕ ПроцедурыПогашенияСтоимости.ПогашатьСтоимостьВНУКакВБУ(СтруктураШапкиДокумента.Организация,СтруктураШапкиДокумента.Дата) Тогда
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
			ПереквалификацияРазницПриПередачеВЭксплуатацию(СтруктураШапкиДокумента);
		КонецЕсли;	
	КонецЕсли;
	
	Если ЕстьРасчетБазыРаспределения И НЕ ПростоеРаспределение Тогда
...
Процедура РасчетФактическойСебестоимостиВыпуска(
	...
	Если НЕ СтруктураШапкиДокумента.ПростоеРаспределение Тогда
...
// Процедура формирования временной таблицы "АналитикаВидаУчетаПрямыеРасходыНУ".
//
// Параметры:
//  СтруктураШапкиДокумента – Структура - Реквизиты документа "Расчет себестоимости"
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц
//
Процедура СформироватьВременнуюТаблицуАналитикаВидаУчетаПрямыеРасходыНУ(СтруктураШапкиДокумента, МенеджерВременныхТаблиц)
	
	ТаблицаСчетаУчетаПрямыхРасходовНУ = Новый ТаблицаЗначений;
	ТаблицаСчетаУчетаПрямыхРасходовНУ.Колонки.Добавить("СчетУчетаНУ", Новый ОписаниеТипов("ПланСчетовСсылка.Налоговый"));
	
	СчетаУчетаНУ = Новый Массив;
	СчетаУчетаНУ.Добавить(ПланыСчетов.Налоговый.ПрямыеРасходыОсновногоПроизводства);
	СчетаУчетаНУ.Добавить(ПланыСчетов.Налоговый.ПрямыеРасходыВспомогательныхПроизводств);
	СчетаУчетаНУ.Добавить(ПланыСчетов.Налоговый.ПрямыеОбщепроизводственныеРасходы);
	СчетаУчетаНУ.Добавить(ПланыСчетов.Налоговый.ПрямыеРасходыПоВыявленномуБраку);
	СчетаУчетаНУ.Добавить(ПланыСчетов.Налоговый.ПрямыеРасходыОбслуживающихПроизводств);
	
	Если НЕ СтруктураШапкиДокумента.ДиректКостинг Тогда
		СчетаУчетаНУ.Добавить(ПланыСчетов.Налоговый.ПрямыеОбщехозяйственныеРасходы);
	КонецЕсли;
	
	Для каждого СчетУчетаНУ Из СчетаУчетаНУ Цикл
		НоваяСтрока = ТаблицаСчетаУчетаПрямыхРасходовНУ.Добавить();
		НоваяСтрока.СчетУчетаНУ = СчетУчетаНУ;
	КонецЦикла;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТаблицаСчетаУчетаНУ.СчетУчетаНУ КАК СчетУчетаНУ
	|	
	|ПОМЕСТИТЬ ТаблицаСчетаУчетаПрямыхРасходовНУ
	|ИЗ
	|	&ТаблицаСчетаУчетаПрямыхРасходовНУ КАК ТаблицаСчетаУчетаНУ
	|	
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетУчетаНУ
	|;
	...
	|	
	|ПОМЕСТИТЬ АналитикаВидаУчетаПрямыеРасходыНУ
	|
	|ИЗ
	|	РегистрСведений.АналитикаВидаУчета КАК РегистрАналитикаВидаУчета
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ТаблицаСчетаУчетаПрямыхРасходовНУ КАК ТаблицаСчетаУчетаПрямыхРасходовНУ
	|	ПО
	|		РегистрАналитикаВидаУчета.СчетУчетаНУ = ТаблицаСчетаУчетаПрямыхРасходовНУ.СчетУчетаНУ
	|	
	|ГДЕ
	|	РегистрАналитикаВидаУчета.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчета.Затраты)
	...
	|
	|///////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ТаблицаСчетаУчетаПрямыхРасходовНУ";
	
	Запрос.УстановитьПараметр("Организация",                       СтруктураШапкиДокумента.Организация);
	Запрос.УстановитьПараметр("ТаблицаСчетаУчетаПрямыхРасходовНУ", ТаблицаСчетаУчетаПрямыхРасходовНУ);
	...
	РасширеннаяАналитикаУчета.ПоказатьВременнуюТаблицу(
		СтруктураШапкиДокумента,
		МенеджерВременныхТаблиц,
		"АналитикаВидаУчетаПрямыеРасходыНУ"
	);
...
Процедура СформироватьВременнуюТаблицуУчетЗатрат(
	...
	Если НЕ СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		СформироватьВременнуюТаблицуАналитикаВидаУчетаПрямыеРасходыНУ(СтруктураШапкиДокумента, МенеджерВременныхТаблиц);
	КонецЕсли;
	...
	|	//ДляРеглУчета ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ОбщепроизводственныеРасходыНеОблагаемыеЕНВД) КАК СчетУчета,
	...
	|ВЫБРАТЬ
	|	&ДетализацияПоФиксированнойСтатьеЗатрат КАК ДетализацияПоФиксированнойСтатьеЗатрат,
	|	//ДляУпрУчета ЗНАЧЕНИЕ(Перечисление.ХарактерЗатрат.ОбщехозяйственныеРасходы),
	|	//ДляРеглУчета ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ОбщехозяйственныеРасходыНеОблагаемыеЕНВД) КАК СчетУчета,
	...
	|	//ДляРеглУчета ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.БракВПроизводствеНеОблагаемоеЕНВД) КАК СчетУчета,
	...
	|	КОНЕЦ КАК СерияЗатраты,
	|
	|	//ДляРеглУчета 	ВЫБОР КОГДА (НормируемыеВидыРасходовНУ.ВидРасходовНУ ЕСТЬ NULL ИЛИ НЕ АналитикаВидаУчетаПрямыеРасходыНУ.Ссылка ЕСТЬ NULL) ТОГДА
	...
	|	//ДляРеглУчета 	НормируемыеВидыРасходовНУ КАК НормируемыеВидыРасходовНУ
	|	//ДляРеглУчета ПО
	|	//ДляРеглУчета 	РегистрАналитикаУчетаЗатрат.СтатьяЗатрат.ВидРасходовНУ = НормируемыеВидыРасходовНУ.ВидРасходовНУ
	|	
	|	//ДляРеглУчета ЛЕВОЕ СОЕДИНЕНИЕ
	|	//ДляРеглУчета 	АналитикаВидаУчетаПрямыеРасходыНУ КАК АналитикаВидаУчетаПрямыеРасходыНУ
	|	//ДляРеглУчета ПО
	|	//ДляРеглУчета 	Затраты.АналитикаВидаУчета = АналитикаВидаУчетаПрямыеРасходыНУ.Ссылка
	|	
	|ГДЕ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////////////
	|//ДляРеглУчета УНИЧТОЖИТЬ НормируемыеВидыРасходовНУ;
	|
	|
	|////////////////////////////////////////////////////////////////////////////////////////
	|//ДляРеглУчета УНИЧТОЖИТЬ АналитикаВидаУчетаПрямыеРасходыНУ";
...
Процедура ФормированиеПроводокПоРаспределениюЗатрат(
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КодОперации = Перечисления.КодыОперацийЗатраты.ЗакрытиеКосвенныхРасходовНУ
			 ИЛИ Выборка.КодОперации = Перечисления.КодыОперацийЗатраты.ЗакрытиеКосвенныхРасходовНУФикс Тогда
			...
			Если СтруктураШапкиДокумента.ДиректКостинг
			   ...
			   И СтруктураШапкиДокумента.УчетнаяПолитика.ДиректКостингСпособСписания = Перечисления.ДиректКостингСпособыСписания.НаУправленческиеРасходы Тогда
		...
		ИначеЕсли Выборка.КодОперации = Перечисления.КодыОперацийЗатраты.СписаниеРасходовНаПродажу Тогда
			...
			// Если 
			//	- не используется простое распределение
			//	- применяется директ-костинг
			//	- расходы отражаются в БУ и НУ по разному (в БУ - на 90.02, в НУ - на 90.08)
			// то необходимые проводки по списанию общехозяйственных расходов со счета 26.02 уже сформированы.
			Если НЕ СтруктураШапкиДокумента.ПростоеРаспределение
			   И СтруктураШапкиДокумента.ДиректКостинг
			   ...
			   И Выборка.СчетУчетаНУ = ПланыСчетов.Налоговый.КосвенныеОбщехозяйственныеРасходы
			Тогда
...
Процедура ФормированиеПроводокПоВыручкеДляНУ(СтруктураШапкиДокумента)
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Корректировка <> 0 И ТекТребуетсяКорректировкаПОНУ Тогда
...
//
Функция ПолучитьДанныеПоВыпускУслугРеглУчет(Запрос, ДатаДокумента, Организация)
...
//   Результат запроса с данными об услугах и параметрах их учета.
//
Функция ПолучитьДанныеПоВыпускУслугУпрУчет(Запрос, ДатаДокумента)
...
Процедура СформироватьДвиженияПоВыпускуУслуг(СтруктураШапкиДокумента)
	...
	Иначе
		Результат = ПолучитьДанныеПоВыпускУслугРеглУчет(Запрос, СтруктураШапкиДокумента.Период, СтруктураШапкиДокумента.Организация);
	КонецЕсли;	
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе
				...
				СтруктраАналитикиВидаУчета.Вставить("СчетУчетаНУ",				Выборка.СчетУчетаЗатратНУ);
...
Функция ПолучитьСпособРаспределения(ЗначенияСвойствРаспределения, СпособыРаспределения) Экспорт
	
	Результат = Неопределено;
	
	Для Каждого СпособРаспределения Из СпособыРаспределения Цикл
		
		Если (СпособРаспределения.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ПустаяСсылка()
				ИЛИ СпособРаспределения.ХарактерЗатрат = ЗначенияСвойствРаспределения.ХарактерЗатрат)
			И (СпособРаспределения.ВидПодразделения = Перечисления.ВидыПодразделений.ПустаяСсылка()
				ИЛИ СпособРаспределения.ВидПодразделения = ЗначенияСвойствРаспределения.ВидПодразделения)
			И (СпособРаспределения.Подразделение = Справочники.Подразделения.ПустаяСсылка()
				ИЛИ СпособРаспределения.Подразделение = ЗначенияСвойствРаспределения.Подразделение)
			И (СпособРаспределения.СтатьяЗатрат = Справочники.СтатьиЗатрат.ПустаяСсылка()
				ИЛИ СпособРаспределения.СтатьяЗатрат = ЗначенияСвойствРаспределения.СтатьяЗатрат) Тогда
				
			Если ЗначениеЗаполнено(СпособРаспределения.СпособРаспределения) Тогда
				Результат = СпособРаспределения.СпособРаспределения;
			КонецЕсли;
...
//
Функция ПолучитьНастройкуРаспределения(НастройкаЗакрытияМесяца) Экспорт
...
//
Процедура ЗаполнитьСпособыРаспределенияЗатратУпр(СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт

	Если НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента.НастройкаЗакрытияМесяца) Тогда
		Возврат;
	КонецЕсли; 
	
	НастройкаРаспределения = ПолучитьНастройкуРаспределения(СтруктураШапкиДокумента.НастройкаЗакрытияМесяца);
	Если НастройкаРаспределения.Количество() = 0 Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(Выборка.СтатьяЗатрат) 
		 ИЛИ НЕ ЗначениеЗаполнено(Выборка.Подразделение) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Выборка.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ОбщехозяйственныеРасходы Тогда
		...
		ИначеЕсли Выборка.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ОбщепроизводственныеРасходы Тогда
			ХарактерРаспределенияЗатрат = Перечисления.ХарактерРаспределенияЗатрат.УчитыватьПодразделение;
		ИначеЕсли Выборка.ХарактерЗатрат = Перечисления.ХарактерЗатрат.ПроизводственныеРасходы Тогда
			ХарактерРаспределенияЗатрат = Перечисления.ХарактерРаспределенияЗатрат.ПроизводственныеРасходы;
		Иначе
		...
		Иначе
			НоваяСтрока.СпособРаспределенияЗатрат = СпособРаспределения;
		КонецЕсли;
...
КонецПроцедуры // ЗаполнитьСпособыРаспределенияЗатратУпр

// Процедура заполняет регистр "СпособыРаспределенияСтатейЗатратОрганизаций"
//
Процедура ЗаполнитьСпособыРаспределенияЗатратРегл(СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	|	УчетЗатратОстатки.КоличествоОстаток <> 0
	|	ИЛИ УчетЗатратОстатки.СтоимостьОстаток <> 0
	|	ИЛИ УчетЗатратОстатки.КоличествоНУОстаток <> 0
	|	ИЛИ УчетЗатратОстатки.СтоимостьНУОстаток <> 0
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = "";
		Если Выборка.КоличествоПодразделений = 0 Тогда
			ТекстСообщения = "C подразделением организации """ + Выборка.ПодразделениеОрганизации + """ не связано ни одно подразделение.";
		ИначеЕсли Выборка.КоличествоПодразделений > 1 Тогда
			ТекстСообщения = "С подразделением организации """ + Выборка.ПодразделениеОрганизации + """ связано более чем одно подразделение.";
		КонецЕсли;
	...
	Если СписокПропущенныхНастроек.Количество() <> 0 Тогда
		...
		Для Каждого ТекстСообщения Из СписокПропущенныхНастроек Цикл
			ОбщегоНазначения.Сообщение(ТекстСообщения.Представление, СтатусСообщения.Информация, Заголовок, ТекстСообщения.Значение, СтрокаРодитель);
		КонецЦикла;
...
КонецПроцедуры // ЗаполнитьСпособыРаспределенияЗатратРегл

// Возвращает массив видов нормируемых расходов НУ
//
Функция ПолучитьВидыНормируемыхРасходовНУ() Экспорт
	...
	Возврат НормируемыеВидыРасходовНУ;
...
//	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц
//
Процедура СформироватьВременнуюТаблицуНормируемыеВидыРасходовНУ(МенеджерВременныхТаблиц) Экспорт
	...
	НормируемыеВидыРасходовНУ = ПолучитьВидыНормируемыхРасходовНУ();
	
	Для Каждого ВидРасходовНУ Из НормируемыеВидыРасходовНУ Цикл
		НоваяСтрока = ТаблицаНормируемыхВидовРасходовНУ.Добавить();
...
//	МенеджерВременныхТаблиц - МенеджерВременныхТаблиц
//
Процедура СформироватьВременнуюТаблицуРаспределяемыхХарактеровЗатрат(СтруктураШапкиДокумента, МенеджерВременныхТаблиц)
	...
	Для Каждого ХарактерЗатрат Из ХарактерыЗатрат Цикл
		НоваяСтрока = ТаблицаРаспределяемыхХарактеровЗатрат.Добавить();
		НоваяСтрока.ХарактерЗатрат = ХарактерЗатрат;
	КонецЦикла;
...
//
Функция ПолучитьСчетаУчетаКосвенныхРасходовНУ(УчетнаяПолитика) Экспорт
	...
	Если НЕ УчетнаяПолитика.ДиректКостинг 
		ИЛИ УчетнаяПолитика.ДиректКостингСпособСписания = Перечисления.ДиректКостингСпособыСписания.НаСебестоимостьПродаж Тогда
		СчетаУчетаНУ.Добавить(ПланыСчетов.Налоговый.КосвенныеОбщехозяйственныеРасходы);
	КонецЕсли;
	
	Возврат СчетаУчетаНУ;
...
//
Процедура СформироватьВременнуюТаблицуСчетовКосвенныхРасходовНУ(
	СтруктураШапкиДокумента, 
	МенеджерВременныхТаблиц)
	...
	Для каждого СчетУчетаНУ из СчетаУчетаНУ Цикл
		НоваяСтрока = ТаблицаСчетовКосвенныхРасходовНУ.Добавить();
		НоваяСтрока.СчетУчетаНУ = СчетУчетаНУ;
	КонецЦикла;
...
Процедура ПереквалификацияНепринимаемыхРасходов(СтруктураШапкиДокумента, СтруктураНаборыЗаписей)
	...
	Пока Выборка.Следующий() Цикл
		...
		РасширеннаяАналитикаУчета.СформироватьДвиженияПоРегиструУчетЗатрат(
			...
			Неопределено,  // КорАналитикаУчетаВыпуска
			ВидДвиженияНакопления.Расход,
			Перечисления.КодыОперацийПартииМатериаловВЭксплуатации.СписаниеИзЭксплуатацииФикс, // Корректировать эти движения не следует
			0,
			?(СтруктураШапкиДокумента.ПоддержкаПБУ18, Выборка.СуммаНУ, 0),
			0,
			Выборка.СуммаНУ, // Сумма НУ
			0,             // Сумма ПР
			СтруктураНаборыЗаписей.УчетЗатрат);
		...
		Если СтруктураШапкиДокумента.ПоддержкаПБУ18 Тогда
			...
			РасширеннаяАналитикаУчета.СформироватьДвиженияПоРегиструУчетЗатрат(
				...
				0,
				0,              // Сумма НУ
				Выборка.СуммаНУ,  // Сумма ПР
				СтруктураНаборыЗаписей.УчетЗатрат);
		КонецЕсли; // СтруктураШапкиДокумента.ПоддержкаПБУ18
...
КонецПроцедуры //ПереквалификацияНепринимаемыхРасходов()
...
//
Процедура ВыполнитьСписаниеКосвенныхРасходовНУПоРасчетнойСтоимости(СтруктураШапкиДокумента, СтруктураНаборыЗаписей)
	...
	Пока Выборка.Следующий() Цикл
		...
		КорАналитикаВидаУчета = Выборка.КорАналитикаВидаУчета; // Т.е. аналитика учета прямых, на которую надо отнести суммы по БУ
...
КонецПроцедуры // ВыполнитьСписаниеКосвенныхРасходовНУПоРасчетнойСтоимости()
...
//	СтруктураНаборыЗаписей - Структура - Структура наборов записей регистров.
//
Процедура ВыполнитьСписаниеКосвенныхРасходовНУПоФиксированнойСтоимости(СтруктураШапкиДокумента, СтруктураНаборыЗаписей)
	...
	Пока Выборка.Следующий() Цикл
		...
		РасширеннаяАналитикаУчета.СформироватьДвиженияПоРегиструУчетЗатрат(
			...
			ВидДвиженияНакопления.Расход,
			Перечисления.КодыОперацийЗатраты.ЗакрытиеКосвенныхРасходовНУФикс,
			0, // Количество
			0, // Стоимость
			0, // КоличествоНУ
			Выборка.СтоимостьНУ,
...
КонецПроцедуры // ВыполнитьСписаниеКосвенныхРасходовНУПоФиксированнойСтоимости()
