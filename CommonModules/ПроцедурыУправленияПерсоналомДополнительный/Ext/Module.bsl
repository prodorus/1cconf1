...
Функция ПодготовитьДанныеПоУправленческимОтпускам(
	...
	Иначе
		...
		|	NULL КАК Физлицо,
		|	NULL КАК ДнейЗапланированногоОтпуска
		|ПОМЕСТИТЬ ВТЗапланированныеОтпуска";
...
Процедура ОбновитьОстаткиОтпусков() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ОстаткиОтпусковОбъект.Наименование		= Строка(ОстаткиОтпусковОбъект.Физлицо) + ", " + Формат(ОстаткиОтпусковОбъект.ДатаАктуальности, "ДФ=dd.MM.yyyy");
		КонецЕсли;
...
Функция ВернутьОписаниеОтпуска(ВыборкаПоОстаткам) Экспорт
	
	Комментарий	= Строка(ВыборкаПоОстаткам.ДнейОтпуска) + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",ВыборкаПоОстаткам.ДнейОтпуска);
	
	РасшифровкаКомментария = "";
	
	Если ВыборкаПоОстаткам.ОтпускВСчетЕжегодногоБезТекущегоГода > 0 Тогда
		РасшифровкаКомментария = ", " + ВыборкаПоОстаткам.ОтпускВСчетЕжегодногоБезТекущегоГода + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",ВыборкаПоОстаткам.ОтпускВСчетЕжегодногоБезТекущегоГода) + " за счет отпусков прошлых лет";
	КонецЕсли;
	...
	Если ВыборкаПоОстаткам.ОтпускВСчетДополнительного > 0 Тогда
		...
		КонецЕсли;
		РасшифровкаКомментария = РасшифровкаКомментария + Строка(ВыборкаПоОстаткам.ОтпускВСчетДополнительного) + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",ВыборкаПоОстаткам.ОтпускВСчетДополнительного) + " доп. отпуска";
	КонецЕсли;
	...
	Если ВыборкаПоОстаткам.ОтпускАвансом > 0 Тогда
		...
		Иначе
			РасшифровкаКомментария = РасшифровкаКомментария + " и ";
		КонецЕсли;
		РасшифровкаКомментария = РасшифровкаКомментария + Строка(ВыборкаПоОстаткам.ОтпускАвансом) + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",ВыборкаПоОстаткам.ОтпускАвансом) + " в счет будущего периода";
	КонецЕсли;
	...
	Если ВыборкаПоОстаткам.Мероприятие <> NULL Тогда
		Если Не ПустаяСтрока(Комментарий) Тогда
			Комментарий = Комментарий + ". ";
		КонецЕсли;
