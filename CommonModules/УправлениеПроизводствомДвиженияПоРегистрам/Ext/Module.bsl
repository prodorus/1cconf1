...
Функция СформироватьТекстЗапросаДляДокументаКомплектацияНоменклатуры(ВидКомплектации)
	...
	Иначе
		...
		|
		|	Док.ДоляСтоимости,
		|	0 КАК Количество, // Количество получим в результате распределения пропорционально доле стоимости
		|	0 КАК Сумма,
		|	0 КАК СуммаРегл,
		|	0 КАК СуммаНал
		|
...
Процедура СформироватьДвиженияПоРаспределениюМатериалов(
	...
	Если ТипДокумента = Тип("ДокументСсылка.КомплектацияНоменклатуры")
	   И СтруктураШапкиДокумента.ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда
	...
	Иначе
		// Количество распределения задано явно
	...
	Для каждого Строка из ТаблицаРаспределения Цикл
		...
		Если РаспределятьПоДолеСтоимости Тогда
			Строка.Количество = РаспределениеКоличества[ТаблицаРаспределения.Индекс(Строка)];
		КонецЕсли;
		...
		ИначеЕсли Строка.ВидВыпуска = Перечисления.ВидыВыпуска.Выпуск Тогда
		...
		ИначеЕсли Строка.ВидВыпуска = Перечисления.ВидыВыпуска.Наработка Тогда
...
Процедура ПровестиДокументПоПроизводственнымРегистрам(ДокументСсылка, Упр, Бух, Нал, ОшибкаТекущегоДокумента = Ложь) Экспорт
	...
	Если ((ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену"))
	  ...
	  И (Бух ИЛИ Упр) Тогда
		...
		ИначеЕсли (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КомплектацияНоменклатуры"))
		  И ОшибкаТекущегоДокумента Тогда
		...
		СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН = СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН И Нал;
...
Процедура ПолучитьАналитикуНаправленияВыпуска(
	...
	ИначеЕсли КодОперации = Перечисления.КодыОперацийНезавершенноеПроизводство.ОтрицательныеЗатраты 
			ИЛИ КодОперации = Перечисления.КодыОперацийЗатратыНаВыпускПродукции.ОтрицательныеЗатраты Тогда
