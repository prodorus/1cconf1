...
Функция ПолучитьПредставлениеНастройкиРасчета(Документ) Экспорт
	...
	// Штатная работа
		...
		Если ЗначениеЗаполнено(Документ.ДнейНеВыплаты) Тогда
			ТекстЗадержка = "с компенсацией за задержку на" +" "+ Документ.ДнейНеВыплаты +" "+ ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней", Документ.ДнейНеВыплаты)
		Иначе
			ТекстЗадержка = ""
		КонецЕсли;
...
Функция ИмяСтраницыНастройкиРасчета(ХарактерВыплаты) Экспорт
	...
	ИначеЕсли 	ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.ПоБольничнымЛистам	
		...
		ИЛИ		ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Задолженность Тогда
...
// Рассчитывает "усредненную" 1/300 или 1/150 часть ключевой ставки ЦБ
//
// Параметры
//  нет
//
...
Функция ОпределитьПроцентКомпенсации(ДатаДокумента, ДнейНеВыплаты) Экспорт
	...
	ДатаПовышенияПроцентаКомпенсации = '20161003'; // Федеральный закон от 03.07.2016 № 272-ФЗ
	ПервыйДеньЗадержки = ДатаДокумента - 86400 * (ДнейНеВыплаты - 1);
	Если ДатаДокумента < ДатаПовышенияПроцентаКомпенсации Или ПервыйДеньЗадержки >= ДатаПовышенияПроцентаКомпенсации Тогда
		...
		Если ПервыйДеньЗадержки >= ДатаПовышенияПроцентаКомпенсации Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "300", "150")
		КонецЕсли;
	...
	Иначе
		...
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(СУММА(ДанныеОПроцентах.ПроцентКомпенсации), 0) КАК ПроцентКомпенсации
		|ИЗ
		|	(ВЫБРАТЬ
		|		(РАЗНОСТЬДАТ(ПериодыИзмененияСтавки.ДатаC, ПериодыИзмененияСтавки.ДатаПо, ДЕНЬ) + 1) * ПериодыИзмененияСтавки.Размер / 300 / 100 КАК ПроцентКомпенсации
		|	ИЗ
		|		(ВЫБРАТЬ
		|			ВЫБОР
		|				КОГДА СтавкаРефинансированияЦБ.Период >= &ПервыйДеньЗадержки
		...
		|			КОНЕЦ КАК ДатаC,
		|			ЕСТЬNULL(ВЫБОР
		|					КОГДА МИНИМУМ(СледующаяДата.Период) > &ДеньПредшествующийПовышениюКомпенсации
		|						ТОГДА &ДеньПредшествующийПовышениюКомпенсации
		|					ИНАЧЕ МИНИМУМ(ДОБАВИТЬКДАТЕ(СледующаяДата.Период, ДЕНЬ, -1))
		|				КОНЕЦ, &ДеньПредшествующийПовышениюКомпенсации) КАК ДатаПо,
		|			СтавкаРефинансированияЦБ.Размер КАК Размер
		|		ИЗ
		|			РегистрСведений.СтавкаРефинансированияЦБ КАК СтавкаРефинансированияЦБ
		|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкаРефинансированияЦБ КАК СледующаяДата
		|				ПО СтавкаРефинансированияЦБ.Период < СледующаяДата.Период
		|		ГДЕ
		|			СтавкаРефинансированияЦБ.Период <= &ДеньПредшествующийПовышениюКомпенсации
		|		
		|		СГРУППИРОВАТЬ ПО
		|			СтавкаРефинансированияЦБ.Размер,
		|			СтавкаРефинансированияЦБ.Период
		...
		|						ТОГДА &ДеньПредшествующийПовышениюКомпенсации
		|					ИНАЧЕ МИНИМУМ(ДОБАВИТЬКДАТЕ(СледующаяДата.Период, ДЕНЬ, -1))
		|				КОНЕЦ, &ДеньПредшествующийПовышениюКомпенсации) >= &ПервыйДеньЗадержки) КАК ПериодыИзмененияСтавки
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		(РАЗНОСТЬДАТ(ПериодыИзмененияСтавки.ДатаC, ПериодыИзмененияСтавки.ДатаПо, ДЕНЬ) + 1) * ПериодыИзмененияСтавки.Размер / 150 / 100
		|	ИЗ
		|		(ВЫБРАТЬ
		|			ВЫБОР
		|				КОГДА СтавкаРефинансированияЦБ.Период >= &ДатаПовышенияКомпенсации
		|					ТОГДА СтавкаРефинансированияЦБ.Период
		|				ИНАЧЕ &ДатаПовышенияКомпенсации
		|			КОНЕЦ КАК ДатаC,
		|			ЕСТЬNULL(ВЫБОР
		|					КОГДА МИНИМУМ(СледующаяДата.Период) > &ДеньВыплаты
		|						ТОГДА &ДеньВыплаты
		...
		|						ТОГДА &ДеньВыплаты
		|					ИНАЧЕ МИНИМУМ(ДОБАВИТЬКДАТЕ(СледующаяДата.Период, ДЕНЬ, -1))
		|				КОНЕЦ, &ДеньВыплаты) >= &ДатаПовышенияКомпенсации) КАК ПериодыИзмененияСтавки) КАК ДанныеОПроцентах";
	...
	Запрос.УстановитьПараметр("ДатаПовышенияКомпенсации", ДатаПовышенияПроцентаКомпенсации);
	Запрос.УстановитьПараметр("ДеньПредшествующийПовышениюКомпенсации", ДатаПовышенияПроцентаКомпенсации - 86400);
	Запрос.Текст = ТекстЗапроса;
	
	Возврат Запрос.Выполнить().Выгрузить()[0].ПроцентКомпенсации

КонецФункции // ОпределитьПроцентКомпенсации()
...
Функция ПолучитьФизлицаСотрудниковПодразделенияНаДату(Организация, ПодразделениеОрганизации = Неопределено, Дата, ФизЛица = Неопределено)
	...
	|	ПодразделенияФизлиц.Период,
	|	ПодразделенияФизлиц.Работает
	|ПОМЕСТИТЬ ВТПринятыеНаСледующийДень
	|ИЗ
	|	ВТПодразделенияФизлицШтат КАК ПодразделенияФизлиц
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПодразделенияФизлицШтат КАК НовыеПодразделения
	|		ПО ПодразделенияФизлиц.Физлицо = НовыеПодразделения.Физлицо
	|			И ПодразделенияФизлиц.Период = НовыеПодразделения.Период
	|			И ПодразделенияФизлиц.Работает <> НовыеПодразделения.Работает
	|ГДЕ
	|	НЕ ПодразделенияФизлиц.Работает
	|	И НовыеПодразделения.Физлицо ЕСТЬ НЕ NULL 
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ФизЛицо
	|;
	...
	|		ПО ПодразделенияФизлиц.Физлицо = ДействующиеПериодыФизЛиц.Физлицо
	|			И ПодразделенияФизлиц.Период = ДействующиеПериодыФизЛиц.Период
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПринятыеНаСледующийДень КАК СтарыеПодразделенияПринятыхНаСледующийДень
	|		ПО ПодразделенияФизлиц.Период = СтарыеПодразделенияПринятыхНаСледующийДень.Период
	|			И ПодразделенияФизлиц.Физлицо = СтарыеПодразделенияПринятыхНаСледующийДень.ФизЛицо
	|			И ПодразделенияФизлиц.Подразделение = СтарыеПодразделенияПринятыхНаСледующийДень.Подразделение
	|			И ПодразделенияФизлиц.Работает = СтарыеПодразделенияПринятыхНаСледующийДень.Работает
	|ГДЕ
	|	СтарыеПодразделенияПринятыхНаСледующийДень.ФизЛицо ЕСТЬ NULL 
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ФизЛицо
	|;
...
Процедура ОтобразитьЭУКомпенсацииЗаЗадержкуЗарплаты(ПоказыватьКомпенсацию, ЭтаФорма, ДействиеКнопки) Экспорт
	...
	Если ПоказыватьКомпенсацию Тогда
		Если Кнопки.Найти("ЗарегистрироватьДоходыПоКомпенсации") = Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		Кнопка = Кнопки.Найти("РазделительЗарегистрироватьДоходыПоКомпенсации");
		Если Кнопка <> Неопределено Тогда
			Кнопки.Удалить(Кнопка);
		КонецЕсли;
		Кнопка = Кнопки.Найти("ЗарегистрироватьДоходыПоКомпенсации");
		Если Кнопка <> Неопределено Тогда
			Кнопки.Удалить(Кнопка);
		КонецЕсли;
	КонецЕсли;
