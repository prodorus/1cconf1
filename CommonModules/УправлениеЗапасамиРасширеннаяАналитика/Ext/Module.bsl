...
Процедура СформироватьДополнительныеДвиженияПоРегистрамУчетаЗатрат(Источник,СтруктураНаборовЗаписейПоВидамУчета,ТаблицаДляФормированияДвиженийПоРегистру)  
	...
	Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.ПередачаМатериаловВЭксплуатацию") Тогда
		...
		СтруктураПараметров.Вставить("ПогашатьСтоимостьВНУКакВБУ",
			?(Источник.мСтруктураШапкиДокумента.Свойство("ПогашатьСтоимостьВНУКакВБУ"),Источник.мСтруктураШапкиДокумента.ПогашатьСтоимостьВНУКакВБУ,Ложь));
...
Процедура СформироватьДвиженияПоРеализацииПринятогоНаКомиссиюТовара(СтруктураШапкиДокумента, СтруктураТабличныхЧастей, Ссылка, Движения, ЕстьБезналичныеРасчеты)
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		ТаблицаДляОбработки = Неопределено;
		Если Не СтруктураТабличныхЧастей.Свойство("ТаблицаПоТоварамДляКомиссии", ТаблицаДляОбработки) Тогда
			ТаблицаДляОбработки = СтруктураТабличныхЧастей.ТаблицаПоТоварам;
		КонецЕсли;
		...
		Для Каждого ТекущаяСтрока Из ТаблицаДляОбработки Цикл
			...
			Если ТекущаяСтрока.ПринадлежностьНоменклатуры = Перечисления.ПринадлежностьНоменклатуры.Принятый Тогда
				...
				ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда 
					Если ТипЗнч(ТекущаяСтрока.ДокументПартии) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
						Покупатель 		= Ссылка.Контрагент;
						ДатаРеализации  = ТекущаяСтрока.ДокументРеализацииДата;
					ИначеЕсли ТипЗнч(ТекущаяСтрока.ДокументПартии) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
						Покупатель 		= Справочники.Контрагенты.ПустаяСсылка();
						ДатаРеализации  = ТекущаяСтрока.ДокументРеализацииДата;
					Иначе
						Покупатель 		= Ссылка.Контрагент;
				...
				Иначе
					...
					Если Не СтруктураШапкиДокумента.Свойство("ДокументРеализацииДата", ДатаРеализации) Тогда
						ДатаРеализации = Ссылка.Дата;
					КонецЕсли;
				КонецЕсли;
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		// Проводки формируются в модуле объекта
		Возврат;
	КонецЕсли;
