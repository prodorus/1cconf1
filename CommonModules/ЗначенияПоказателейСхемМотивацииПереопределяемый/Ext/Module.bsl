...
Функция Автозаполнение(Организация, ПериодДействия, ТаблицаЗапроса = Неопределено, Сотрудники = Неопределено, Показатели = Неопределено, ОдинВидРасчета = Ложь, ИзДокумента = Ложь) Экспорт
	...
	Иначе
		...
		Если ИзДокумента Тогда
			...
			|	ЗначенияПоказателейСхемМотивации.ПериодДействия КАК ПериодДействия,
			|	ВЫБОР
			|		КОГДА ЗначенияПоказателейСхемМотивации." + ЗаполнениеДокументовЗК.ВалютаПоказателяИмяПоляЗапроса() + " = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
...
Процедура ПриСозданииФормыСписка(Форма) Экспорт
...
Процедура РегистрСведенийСписокПередНачаломИзменения(Элемент, Отказ, Форма) Экспорт
	
	УправлениеПоказателямиСхемМотивации.СписокЕжемесячныхПоказателейПередНачаломИзменения(Элемент, Отказ, Форма);
	
КонецПроцедуры

Процедура РегистрСведенийСписокПередУдалением(Элемент, Отказ, Форма) Экспорт
