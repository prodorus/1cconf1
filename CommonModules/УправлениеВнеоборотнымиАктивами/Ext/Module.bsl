...
// Вызывается при заполнении элемента универсальной обработкой загрузки
// классификаторов.
//
Процедура ПриЗагрузкеОКОФ(Источник, Отказ) Экспорт
	
	Перем РежимЗагрузки;
	
	ДополнительныеСвойстваИсточника = Источник.ДополнительныеСвойства;
	
	Если ДополнительныеСвойстваИсточника.Свойство("ЗагрузкаКлассификаторов", РежимЗагрузки)
	   И РежимЗагрузки Тогда
		...
		Иначе
			...
			Основание = ДополнительныеСвойстваИсточника.ОбъектXDTO;
			
			//xdtoАмортизационныеГруппы = Основание.АмортизационныеГруппы; не работает ввиду ошибки платформы
			Попытка
		...
		НаборЗаписей.Отбор.ОКОФ.Установить(Источник.Ссылка);
...
// Процедура заполняет амортизационную группу
// 
Процедура ПолучитьАмортизационныеГруппы(ОКОФ, мАмортизационныеГруппы) Экспорт
	мРодителиОКОФ = Новый Массив;
	Если ЗначениеЗаполнено(ОКОФ) Тогда
		ВыбратьРодителейОКОФ(ОКОФ, мРодителиОКОФ);
		КодОКОФ = мРодителиОКОФ;
		ЗапросОКОФ = Новый Запрос;
		ЗапросОКОФ.УстановитьПараметр("КодОКОФ", КодОКОФ);
		ЗапросОКОФ.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	АмортизационныеГруппыОКОФ.ОКОФ,
		|	АмортизационныеГруппыОКОФ.АмортизационнаяГруппа,
		|	АмортизационныеГруппыОКОФ.Примечание,
		|	АмортизационныеГруппыОКОФ.АмортизационнаяГруппа.Порядок КАК АмортизационнаяГруппаПорядок
		|ИЗ
		|	РегистрСведений.АмортизационныеГруппыОКОФ КАК АмортизационныеГруппыОКОФ
		|ГДЕ
		|	АмортизационныеГруппыОКОФ.ОКОФ В(&КодОКОФ)
		|
		|УПОРЯДОЧИТЬ ПО
		|	АмортизационнаяГруппаПорядок" ;
		РезультатЗапроса = ЗапросОКОФ.Выполнить();
		Нашли=Ложь;
		Выборка = РезультатЗапроса.Выгрузить();
		Для Каждого ЭлементОКОФ Из мРодителиОКОФ Цикл
			Для Каждого ЭлементВыборки Из Выборка Цикл
				Если  ЭлементВыборки.ОКОФ = ЭлементОКОФ Тогда
					ВыбраннаяГруппа = ЭлементВыборки.АмортизационнаяГруппа;
					Если мАмортизационныеГруппы.Найти(ВыбраннаяГруппа,"Группа")= Неопределено Тогда
						НоваяСтрока = мАмортизационныеГруппы.Добавить();
						НоваяСтрока.Группа=ВыбраннаяГруппа;
						НоваяСтрока.Примечание = ЭлементВыборки.Примечание;
					КонецЕсли;
					Нашли = Истина;
				КонецЕсли;
			КонецЦикла;
...
// Процедура составляет список родителей по коду ОКОФ
//
Процедура ВыбратьРодителейОКОФ(ЭлементОКОФ, мРодителиОКОФ)
	мРодителиОКОФ.Очистить();
	Если Не ЭлементОКОФ.Пустая() Тогда
		мРодителиОКОФ.Добавить(ЭлементОКОФ);
		РодительОКОФ = ЭлементОКОФ.Родитель;
		Пока Не РодительОКОФ.Пустая() Цикл
			мРодителиОКОФ.Добавить(РодительОКОФ);
			РодительОКОФ = РодительОКОФ.Родитель;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Функция РасчетАмортизацииНалогРегл(Объект,ДатаРасчета,Организация,ОсновноеСредство = Неопределено, ВыдаватьСообщения = Истина) Экспорт
	...
	|	((ГОД(&ДатаПереходаКЛинейномуМетоду) - ГОД(СостоянияОСОрганизаций.ДатаВводаВЭксплуатацию)) * 12 + (МЕСЯЦ(&ДатаПереходаКЛинейномуМетоду) - МЕСЯЦ(СостоянияОСОрганизаций.ДатаВводаВЭксплуатацию))) КАК ФактическийСрокПриПереходеКЛинейномуМетоду,
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		СрокИспользованияДляВычисления = Макс(1, ?(ВыборкаПоОС.ПереходКЛинейномуМетоду И ВыборкаПоОС.ФактическийСрокПриПереходеКЛинейномуМетоду > 0,
...
Функция РасчетАмортизацииНМА_НИОКР_НалогРегл(Объект,ДатаРасчета,Организация,НематериальныйАктив = Неопределено, ВыдаватьСообщения = Истина) Экспорт
	...
	|	((ГОД(&ДатаПереходаКЛинейномуМетоду) - ГОД(СостоянияНМАОрганизаций.ДатаВводаВЭксплуатацию)) * 12 + (МЕСЯЦ(&ДатаПереходаКЛинейномуМетоду) - МЕСЯЦ(СостоянияНМАОрганизаций.ДатаВводаВЭксплуатацию))) КАК ФактическийСрокПриПереходеКЛинейномуМетоду,
	...
	Для каждого ВыборкаПоНМА из ТаблицаПоНМА Цикл
		...
		СрокИспользованияДляВычисления = Макс(1, ?(ВыборкаПоНМА.ПереходКЛинейномуМетоду И ВыборкаПоНМА.ФактическийСрокПриПереходеКЛинейномуМетоду > 0,
