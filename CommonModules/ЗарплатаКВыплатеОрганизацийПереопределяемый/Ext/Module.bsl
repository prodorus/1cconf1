...
Процедура ПровестиРасчет(Документ, Знач ХарактерВыплатыОтложенного = Неопределено) Экспорт
	...
	|			ИЛИ НАЧАЛОПЕРИОДА(ЗарплатаЗаМесяц.ПериодВзаиморасчетов, ДЕНЬ) = &ПериодРегистрации)
...
Процедура ПроверитьФизлицаДляАвтозаполнения(ДокументОбъект, Физлица, Отказ, Заголовок) Экспорт
	...
	Если ДокументОбъект.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезБанк Тогда
		...
		Если ФизЛица.Количество() = 0 Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Не найдены лицевые счета сотрудников в банке " +"«"+ СокрЛП(ДокументОбъект.Банк) +"»"+ "!", Отказ, Заголовок);
		КонецЕсли;
...
Функция ХарактерВыплатыПоДокументу(ДокументСсылка) Экспорт
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.НДФЛПерерасчет")
		...
		ИЛИ ТипДокумента = Тип("ДокументСсылка.ПереносДанных") Тогда
...
Функция ДополнительныеХарактерыВыплатыСНачисленнымНДФЛ() Экспорт

	ДополнительныеХарактеры = Новый Массив;
	ДополнительныеХарактеры.Добавить(Перечисления.ХарактерВыплатыЗарплаты.Премии);
	Возврат ДополнительныеХарактеры
...
Функция ПолучитьСтруктуруПечатныхФорм(ДокументОбъект) Экспорт
	...
	мУпрощенныйУчетВзаиморасчетов = ПроцедурыУправленияПерсоналом.ЗначениеУчетнойПолитикиПоПерсоналуОрганизации(глЗначениеПеременной("глУчетнаяПолитикаПоПерсоналуОрганизации"), ДокументОбъект.Организация, "УпрощенныйУчетВзаиморасчетов");	
	
	СтруктураПечатныхФорм = Новый Структура;
	
	Если НЕ ЗначениеЗаполнено(ДокументОбъект.СпособВыплаты) ИЛИ ДокументОбъект.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезКассу Тогда
		
		СтруктураПечатныхФорм.Вставить("Т53", "Форма Т-53");
		СтруктураПечатныхФорм.Вставить("Т49", "Форма Т-49");
		СтруктураПечатныхФорм.Вставить("КО2", "РКО");
		СтруктураПечатныхФорм.Вставить("Реестр", "Реестр депонированных сумм");
		
		Если мУпрощенныйУчетВзаиморасчетов Тогда
			СтруктураПечатныхФорм.Вставить("КарточкаДепонента","Карточка депонента");
		КонецЕсли;	
		
	КонецЕсли;	
	
	Если НЕ ЗначениеЗаполнено(ДокументОбъект.СпособВыплаты) ИЛИ  ДокументОбъект.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезБанк Тогда
		
		СтруктураПечатныхФорм.Вставить("ПереченьСокр", "Список плательщиков в банк (ФИО сокращенно)");
		СтруктураПечатныхФорм.Вставить("ПереченьПолностью", "Список плательщиков в банк (ФИО полностью)");
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, ДокументОбъект) Экспорт
	...
	Если ИмяМакета = "Т53" тогда
		...
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ДокументОбъект, "Платежная ведомость Т-53 "));
	...
	ИначеЕсли ИмяМакета = "Т49" тогда
		...
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ДокументОбъект,"Платежная ведомость Т-49 "));
	...
	ИначеЕсли ИмяМакета = "КО2" тогда
		...
		Если ТабДокумент.ВысотаТаблицы > 0 Тогда
			Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ДокументОбъект,"Расходные кассовые ордера "));
		КонецЕсли;
	...
	ИначеЕсли ИмяМакета = "Реестр" тогда
		...
		Если ТабДокумент.ВысотаТаблицы > 0 Тогда
			Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ДокументОбъект,"Реестр депонированных сумм "));
		КонецЕсли;
	...
	ИначеЕсли ИмяМакета = "ПереченьСокр" тогда
		...
		Если ТабДокумент.ВысотаТаблицы > 0 Тогда
			Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ДокументОбъект,"Список плательщиков в банк "));
		КонецЕсли;
	...
	ИначеЕсли ИмяМакета = "КарточкаДепонента" тогда
		
		ТабДокумент = Документы.ДепонированиеОрганизаций.ПечатьКарточкиДепонента(ДокументОбъект, ДепонируемыеСуммыДляПечати(ДокументОбъект)); 	
		Если ТабДокумент.ВысотаТаблицы > 0 Тогда
			Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ДокументОбъект,"Список плательщиков в банк "));
		КонецЕсли;
...
Функция ДепонируемыеСуммыДляПечати(ДокументОбъект)

	ДепонируемыеСуммы = ДокументОбъект.Зарплата.Выгрузить(Новый Структура("ВыплаченностьЗарплаты", Перечисления.ВыплаченностьЗарплаты.Задепонировано), "Физлицо, Сумма");
	ДепонируемыеСуммы.Колонки.Добавить("Ведомость", Новый ОписаниеТипов("ДокументСсылка.ЗарплатаКВыплатеОрганизаций"));
	
	ДепонируемыеСуммы.ЗаполнитьЗначения(ДокументОбъект.Ссылка, "Ведомость");
	
	Возврат ДепонируемыеСуммы;
...
Процедура ВыполнитьДополнительныеПроверкиШапкиДокумента(ШапкаДокумента, Отказ, Заголовок) Экспорт 
	...
	Если ШапкаДокумента.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезБанк И НЕ ЗначениеЗаполнено(ШапкаДокумента.Банк)Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Не указан банк!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ВыполнитьДополнительныеПроверкиСтрокиДокумента(ШапкаДокумента, СтрокаЗарплаты, УпрощенныйУчетВзаиморасчетов, Отказ, Заголовок = "", СтрокаНачалаСообщенияОбОшибке = "") Экспорт 
	...
	Если УпрощенныйУчетВзаиморасчетов Тогда
		Если ШапкаДокумента.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезКассу И (НЕ ЗначениеЗаполнено(СтрокаЗарплаты.ВыплаченностьЗарплаты) ИЛИ СтрокаЗарплаты.ВыплаченностьЗарплаты = Перечисления.ВыплаченностьЗарплаты.НеВыплачено) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не проставлена отметка о выплате или депонировании!", Отказ, Заголовок);
		КонецЕсли;
	КонецЕсли;
...
Процедура ФормаСпискаСоздатьКомандыВыплаты(Форма, Действие) Экспорт
	
	ТипКнопки	= ТипКнопкиКоманднойПанели.Подменю;
	Текст		= "На основании";
	Пояснение	= "Ввести на основании";
	
	Кнопки = Новый Массив;
	Кнопки.Добавить(Форма.ЭлементыФормы.ДействияФормы.Кнопки.Выплатить);
	Кнопки.Добавить(Форма.ЭлементыФормы.ДействияФормы.Кнопки.ПодменюДействия.Кнопки.Выплатить);

	Для Каждого Кнопка Из Кнопки Цикл
		Кнопка.ТипКнопки = ТипКнопки;
		Кнопка.Текст	 = Текст;
		Кнопка.Подсказка = Текст;
...
Процедура ФормаСпискаОбновитьКомандыВыплаты(Форма, Действие) Экспорт
	
	ТекущиеДанные = Форма.ЭлементыФормы.ДокументСписок.ТекущиеДанные;
	
	СписокДокументовВводимыхНаОсновании = СписокДокументовВводимыхНаОсновании(ТекущиеДанные);
	
	РаботаСДиалогамиЗК.ЗаполнитьКнопкиКоманднойПанелиСпискомКоманд(
		Форма.ЭлементыФормы.ДействияФормы.Кнопки.Выплатить.Кнопки,
		СписокДокументовВводимыхНаОсновании,
		Действие);
	РаботаСДиалогамиЗК.ЗаполнитьКнопкиКоманднойПанелиСпискомКоманд(
		Форма.ЭлементыФормы.ДействияФормы.Кнопки.ПодменюДействия.Кнопки.Выплатить.Кнопки,
		СписокДокументовВводимыхНаОсновании,
		Действие);
		
		
	КнопкиМенюПерейти 	= Форма.ЭлементыФормы.ДействияФормы.Кнопки.ПодменюПерейти.Кнопки;
	ИмяКоманды = Метаданные.Обработки.ВыплатаЗарплатыРасходнымиОрдерами.Имя;
	
	Если ТекущиеДанные <> Неопределено Тогда
		РазрешенаВыплатаРасходнымиОрдерами =
			ТекущиеДанные.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезКассу
			И НЕ ПроцедурыУправленияПерсоналом.ЗначениеУчетнойПолитикиПоПерсоналуОрганизации(глЗначениеПеременной("глУчетнаяПолитикаПоПерсоналуОрганизации"), ТекущиеДанные.Организация, "УпрощенныйУчетВзаиморасчетов")
	Иначе		
		РазрешенаВыплатаРасходнымиОрдерами = Ложь
	КонецЕсли;
		
	Если  РазрешенаВыплатаРасходнымиОрдерами Тогда
		Если КнопкиМенюПерейти.Найти(ИмяКоманды) = Неопределено Тогда
			НоваяКнопка = КнопкиМенюПерейти.Вставить(0, ИмяКоманды, ТипКнопкиКоманднойПанели.Действие, "Выплата зарплаты расходными ордерами", Действие);
...
Процедура ФормаСпискаОбработатьКомандыВыплаты(Форма, Кнопка) Экспорт
	...
	ИмяКоманды = Кнопка.Имя;
	
	Если ИмяКоманды = Метаданные.Обработки.ВыплатаЗарплатыРасходнымиОрдерами.Имя Тогда
	...
	Иначе
		...
		ОбработкаВводаНаОсновании(ИмяКоманды, ТекущиеДанные, Форма)
		...
		Если НЕ УпрощенныйУчетВзаиморасчетов Тогда
			...
			Если ТекущиеДанные.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезКассу Тогда
				ДокументыВводимыеНаОсновании.Добавить(
					Метаданные.Документы.РасходныйКассовыйОрдер.Имя, 
					Метаданные.Документы.РасходныйКассовыйОрдер.Синоним,,
					БиблиотекаКартинок.Документ);
				ДокументыВводимыеНаОсновании.Добавить(
					Метаданные.Документы.ДепонированиеОрганизаций.Имя, 
					Метаданные.Документы.ДепонированиеОрганизаций.Синоним,,
					БиблиотекаКартинок.Документ);
			ИначеЕсли ТекущиеДанные.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезБанк Тогда
				ДокументыВводимыеНаОсновании.Добавить(
					Метаданные.Документы.ПлатежноеПоручениеИсходящее.Имя, 
					Метаданные.Документы.ПлатежноеПоручениеИсходящее.Синоним,,
					БиблиотекаКартинок.Документ);
			КонецЕсли;
...
Процедура ОбработкаВводаНаОсновании(ИмяМетаданныхДокумента, ТекущиеДанные, Форма) Экспорт
	...
	Если ПравоДоступа("Добавление", МетаданныеДокумента) Тогда
		...
		Исключение
			...
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ОбщегоНазначенияЗК.ПолучитьПричинуОшибки(ИнформацияОбОшибке()).Описание, Отказ, "Создание документа «" + МетаданныеДокумента.Синоним + "»");
