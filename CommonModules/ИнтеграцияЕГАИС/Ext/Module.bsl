...
Функция ДобавитьЗаписьВПротоколОбмена(ТекстСообщенияXML, Реквизиты, ПроверятьХешБезСсылки = Ложь) Экспорт
	...
	ХешСуммаBase64 = Base64Строка(ХешированиеДанныхЕГАИС("SHA256", ТекстСообщенияXML));
...
Функция ПредставлениеСтатусаЕГАИС(СтатусЕГАИС, ВходящиеДальнейшееДействие, ДопустимыеДействия) Экспорт
	...
	Если ТипЗнч(СтатусЕГАИС) <> Тип("Строка") Тогда
		ОписаниеСтатуса = ФорматированнаяСтрока(Строка(СтатусЕГАИС));
		...
		Если СтатусыЦветТекстаТребуетВниманияЕГАИС.Найти(СтатусЕГАИС) <> Неопределено Тогда
			ОписаниеСтатуса.ЦветТекста = ЦветаСтиля.ЦветТекстаТребуетВниманияГИСМ;
		КонецЕсли;
		
	Иначе
		ОписаниеСтатуса = ФорматированнаяСтрока(СтатусЕГАИС);
	КонецЕсли;
	...
	ОписаниеЗапятой = ФорматированнаяСтрока(",");
	ОписаниеИли     = ФорматированнаяСтрока(НСтр("ru = 'или'"));
	
	СтрокиДальнейшееДействие.Добавить(ОписаниеСтатуса);
	...
	Для Каждого ДальнейшееДействие Из ДальнейшиеДействия Цикл
		...
		Если ЗначениеЗаполнено(ТекстГиперссылки) Тогда
			...
			Если СтрокиДальнейшееДействие.Количество() = 1 Тогда
				//СтрокиДальнейшееДействие.Добавить(ОписаниеЗапятой);
				СтрокиДальнейшееДействие[0].Заголовок = СтрокиДальнейшееДействие[0].Заголовок + ",";
			ИначеЕсли СтрокиДальнейшееДействие.Количество() > 2 Тогда
				СтрокиДальнейшееДействие.Добавить(ОписаниеИли);
			КонецЕсли;
			
			ОписаниеДальнейшегоДействия = ФорматированнаяСтрока(НРег(Строка(ДальнейшееДействие)),
				Новый Шрифт(,,Истина), ЦветаСтиля.ЦветГиперссылкиГИСМ,, ТекстГиперссылки);
			
			СтрокиДальнейшееДействие.Добавить(ОписаниеДальнейшегоДействия);
			
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат Новый ФиксированныйМассив(СтрокиДальнейшееДействие);
...
Функция ДанныеДокументаЕГАИС(МетаданныеДокумента, ДокументыПоОснованию, СтатусыОформления) Экспорт
	...
	Если МассивДокументов.Количество() > 0 Тогда
		...
		ФорматированныеСтроки.Добавить(ФорматированнаяСтрока(ТекстНадписи,, Цвет,,ИмяКоманды));
	...
	ВозвращаемоеЗначение = Новый Структура;
	ВозвращаемоеЗначение.Вставить("Представление",       Новый ФиксированныйМассив(ФорматированныеСтроки));
...
Функция ВыгрузитьРезультатыЗапроса(Запрос, ТекстыЗапроса, ОбходРезультата = Неопределено, ДобавитьРазделитель = Ложь)
	...
	Для Каждого ТекстЗапроса из ТекстыЗапроса Цикл
		...
		Если Не ПустаяСтрока(ИмяТаблицы) Тогда
			...
			Если Таблицы.Свойство(ИмяТаблицы) Тогда
				ОбъединитьТаблицыРезультатовЗапроса(Результат[Индекс].Выгрузить(ОбходРезультата), Таблицы[ИмяТаблицы]);
			Иначе
...
Функция ЗагрузитьТТНВходящуюЕГАИС(ДанныеДокументаТТН, ДанныеДокументаУведомлениеОРегистрации, ОрганизацияЕГАИС, ДанныеДокументаИсторияПоСправкам2) Экспорт
	...
	Если РезультатДобавленияЗаписиТТН.НовоеСообщение Тогда
		...
		Для Каждого ЭлементДанных Из ДанныеДокументаУведомлениеОРегистрации.Объект.Content.Position Цикл
			...
			Если ДанныеДокументаУведомлениеОРегистрации.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V3 Тогда
	...
	Если РезультатДобавленияЗаписи.НовоеСообщение Тогда
		...
		Для Каждого СтрокаКвитанцииXDTO Из ДанныеДокумента.Объект.Content.Position Цикл
			...
			Для Каждого ЭлементДанных Из СписокСправок2XDTO Цикл
				...
				Если ДанныеДокумента.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V3 Тогда
...
Функция ЗагрузитьАлкогольнуюПродукцию(СписокАлкогольнойПродукцииXDTO)
	...
	|					И ТаблицаАлкогольнойПродукции.ТипПродукции <> ЗНАЧЕНИЕ(Перечисление.ТипыПродукцииЕГАИС.ПустаяСсылка)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
...
Функция СформироватьУникальныйИдентификатор(ИсходнаяСтрока, ИмяСправочника)
	...
	ХешСумма = Base64Строка(ХешированиеДанныхЕГАИС("SHA256", Префиксы[ИмяСправочника] + ИсходнаяСтрока));
...
Функция ПодготовитьДанныеДляПодбораСправок2(Товары)
	...
	ТаблицаДляЗаполнения.Колонки.Добавить("Номенклатура",         Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаДляЗаполнения.Колонки.Добавить("Характеристика",       Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	ТаблицаДляЗаполнения.Колонки.Добавить("Серия",                Новый ОписаниеТипов("СправочникСсылка.СерииНоменклатуры"));
	...
	ТаблицаДляСписания.Колонки.Добавить("Номенклатура",         Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаДляСписания.Колонки.Добавить("Характеристика",       Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	ТаблицаДляСписания.Колонки.Добавить("Серия",                Новый ОписаниеТипов("СправочникСсылка.СерииНоменклатуры"));
...
Функция ЗаполнитьСправку2ВТабличнойЧасти(Товары, АлкогольнаяПродукция, Справка2, ОстатокПоСправке, СтруктураПересчетаСуммы, КоэффициентыПересчетаВЕдиницыЕГАИС, ТаблицаСоответствия, ТаблицаСоответствияБезАлкоПродукции = Неопределено)
	...
	Отбор2.Вставить("Серия",                Справочники.СерииНоменклатуры.ПустаяСсылка());
