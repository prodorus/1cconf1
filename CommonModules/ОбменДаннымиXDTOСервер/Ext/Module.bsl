...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Для Каждого ВерсияФормата Из ОбменДаннымиПереопределяемый.ПолучитьВерсииУниверсальногоФорматаОбмена() Цикл
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока НомерИтерации = 0 Цикл
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Если ОбменДаннымиПереопределяемый.ПолучитьВерсииУниверсальногоФорматаОбмена().Найти(ФорматОбмена.Версия) = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ДоступныеВерсииПолучателя  = ОбменДаннымиПереопределяемый.ПолучитьВерсииУниверсальногоФорматаОбмена();
...
Функция МинимальнаяВерсияФорматаОбмена(Знач УзелИнформационнойБазы)
	...
	ВерсииФормата = ОбменДаннымиПереопределяемый.ПолучитьВерсииУниверсальногоФорматаОбмена();
