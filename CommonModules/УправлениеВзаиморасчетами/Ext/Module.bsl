...
Функция ПровестиРасчетРезервовПоСомнительнымДолгам(СтруктураПараметров, РежимФормированияПроводок, НормаРасходовПоВыручке, ИспользуетсяДляВводаОстатков) Экспорт
	...
	|	ВЫБОР
	|		КОГДА &ФормироватьБУ
	|			ТОГДА ЕСТЬNULL(ТаблицаВсехРасчетов.ДолгБУ, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ДолгБу,
	|	ВЫБОР
	|		КОГДА &ФормироватьНУ
	|			ТОГДА ЕСТЬNULL(ТаблицаВсехРасчетов.ДолгНУ, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ДолгНУ,
	|	ВЫБОР
	...
	ТаблицаДолгов.Колонки.Добавить("СрокДолга", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(4, 0, ДопустимыйЗнак.Неотрицательный)));
	ТаблицаДолгов.Колонки.Добавить("ВстречнаяЗадолженностьБУ", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(12, 2)));
	ТаблицаДолгов.Колонки.Добавить("ВстречнаяЗадолженностьНУ", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(12, 2)));
	...
	Если ДатаКонцаМесяца > '2017-01-01' И ТаблицаДолгов.Итог("СрокДолга") > 0 Тогда
		ТаблицаДолгов.Колонки.Добавить("ДатаПризнанияЗадолженности", Новый ОписаниеТипов("Дата"));
		ДобавитьДатуДокументаКРезультатуЗапроса(ТаблицаДолгов, "ДокументДолга", "ДатаПризнанияЗадолженности");
		ДанныеВстречнойЗадолженности = ДанныеВстречнойЗадолженности(СтруктураПараметров.Организация, ДатаКонцаМесяцаГраница, МенеджерВременныхТаблиц);
		РаспределитьВстречнуюЗадолженностьНаСомнительныеДолгиКонтрагентов(ТаблицаДолгов, ДанныеВстречнойЗадолженности);
	КонецЕсли;
	...
	|	ТаблицаДолгов.ВстречнаяЗадолженностьБУ КАК ВстречныйДолгБУ,
	|	ТаблицаДолгов.ВстречнаяЗадолженностьНУ КАК ВстречныйДолгНУ,
	...
	Запрос.Текст =  "ВЫБРАТЬ
					...
					|	ТаблицаДолгов.ДолгБУ - ТаблицаДолгов.ВстречныйДолгБУ КАК НачисленоБУ,
					|	ТаблицаДолгов.ДолгНУ - ТаблицаДолгов.ВстречныйДолгНУ КАК НачисленоНУ,
	                |	ВЫБОР
	                |		КОГДА &ФормироватьБУ
	                ...
	                |	ВЫБОР
	                |		КОГДА &ФормироватьБУ
	                |			ТОГДА ТаблицаДолгов.ВстречныйДолгБУ
	                |		ИНАЧЕ ТаблицаДолгов.ВстречныйДолгНУ
	                |	КОНЕЦ КАК ВстречнаяЗадолженность,
	                ...
	                |	ТаблицаСомнительныхДолгов.ВстречнаяЗадолженность,
	                ...
	                |	ТаблицаСомнительныхДолговИОстатковРезервов.ВстречнаяЗадолженность,
	                ...
	                |	СУММА(Таблица.ВстречнаяЗадолженность) КАК ВстречнаяЗадолженность,
...
//
// Данные выбираются из регистра бухгалтерии Хозрасчетный, т.к. 
// 		- в части приобретения данные в нем совпадают с регистром накопления РасчетыПоПриобретениюВУсловныхЕдиницахОрганизации
//		- в нем также есть данные по прочим расчетам (кредиты, займы и прочее)
//
// Параметры:
//  ОтчетнаяДата              - Граница - момент времени, на который начисляется резерв. Конец отчетного месяца (включительно)
//  МенеджерВременныхТаблиц	  - МенеджерВременныхТаблиц - содержит временные таблицы, используемые для отбора данных.
// 
// Возвращаемое значение:
//  ВыборкаИзРезультатаЗапроса
//
Функция ДанныеВстречнойЗадолженности(Организация, ОтчетнаяДата, МенеджерВременныхТаблиц)
	...
	|						Таблица.Контрагент
	|					ИЗ
	|						Таблица КАК Таблица)) КАК Задолженность
	|ГДЕ
...
Процедура РаспределитьВстречнуюЗадолженностьНаСомнительныеДолгиКонтрагентов(ТаблицаДолгов, ДанныеВстречнойЗадолженности)
	
	ТаблицаСомнительныхДолговКонтрагента = ТаблицаДолгов.СкопироватьКолонки(
		"ДатаПризнанияЗадолженности,
		|ДолгБУ,
		|ДолгНУ");
	ТаблицаСомнительныхДолговКонтрагента.Колонки.Добавить("СтрокаРасчета");
	
	ТаблицаДолгов.Индексы.Добавить("Контрагент");
	...
	Пока ДанныеВстречнойЗадолженности.Следующий() Цикл
		...
		ЗаполнитьТаблицуСомнительныхДолговКонтрагента(ТаблицаДолгов, ТаблицаСомнительныхДолговКонтрагента, Отбор);
		
		// Распределение встречной задолженности в БУ
		РаспределяемаяСумма = ДанныеВстречнойЗадолженности.Задолженность;
		Если ТаблицаСомнительныхДолговКонтрагента.Итог("ДолгБУ") > РаспределяемаяСумма Тогда
			...
			Для каждого СомнительныйДолг Из ТаблицаСомнительныхДолговКонтрагента Цикл
				...
				СомнительныйДолг.СтрокаРасчета.ВстречнаяЗадолженностьБУ = Мин(СомнительныйДолг.ДолгБУ, РаспределяемаяСумма);
				РаспределяемаяСумма = РаспределяемаяСумма - СомнительныйДолг.СтрокаРасчета.ВстречнаяЗадолженностьБУ;
		...
		Иначе	
			...
			Для каждого СомнительныйДолг Из ТаблицаСомнительныхДолговКонтрагента Цикл
				СомнительныйДолг.СтрокаРасчета.ВстречнаяЗадолженностьБУ = СомнительныйДолг.ДолгБУ;
			КонецЦикла; 
		...
		РаспределяемаяСумма = ДанныеВстречнойЗадолженности.Задолженность;
		Если ТаблицаСомнительныхДолговКонтрагента.Итог("ДолгНУ") > РаспределяемаяСумма Тогда
			...
			Для каждого СомнительныйДолг Из ТаблицаСомнительныхДолговКонтрагента Цикл
				...
				СомнительныйДолг.СтрокаРасчета.ВстречнаяЗадолженностьНУ = Мин(СомнительныйДолг.ДолгНУ, РаспределяемаяСумма);
		...
		Иначе	
			...
			Для каждого СомнительныйДолг Из ТаблицаСомнительныхДолговКонтрагента Цикл
				СомнительныйДолг.СтрокаРасчета.ВстречнаяЗадолженностьНУ = СомнительныйДолг.ДолгНУ;
			КонецЦикла; 
...
Процедура ЗаполнитьТаблицуСомнительныхДолговКонтрагента(ТаблицаДолгов, ТаблицаСомнительныхДолговКонтрагента, Отбор)
	
	ТаблицаСомнительныхДолговКонтрагента.Очистить();
	Долги = ТаблицаДолгов.НайтиСтроки(Отбор);
	
	Для каждого СтрокаДолга Из Долги Цикл
		
		Если СтрокаДолга.СрокДолга = 0 Тогда
			Продолжить;
		КонецЕсли;	
