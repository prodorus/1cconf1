...
Функция РеквизитыЮрЛицПоНаименованию(Знач Наименование, Знач КодРегиона = "", Знач Адрес = "") Экспорт
	...
	Если Прокси <> Неопределено Тогда
		...
		Попытка
			...
			СписокXDTO = ?(Ответ.CorporationSearchResult = Неопределено, Неопределено, 
...
Процедура ПреобразоватьКонтактнуюИнформацию(СтрокаНабора, КонтактнаяИнформация) Экспорт
	...
	ИначеЕсли СтрокаНабора.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		СтруктураПолейКИ.Свойство("ТипДома", Буфер);
		СтрокаНабора.ТипДома = ?(ТипыДомов.Найти(НРег(Буфер)) = Неопределено, Неопределено, Перечисления.ТипыДомов[Буфер]);
		
		СтруктураПолейКИ.Свойство("ТипКорпуса", Буфер);
		СтрокаНабора.ТипКорпуса = ?(ТипыКорпусов.Найти(НРег(Буфер)) = Неопределено, Неопределено, Перечисления.ТипыКорпусов[Буфер]);
		
		СтруктураПолейКИ.Свойство("ТипКвартиры", Буфер);
		Если Буфер = "кв." Тогда
			СтрокаНабора.ТипКвартиры = Перечисления.ТипыКвартир.Квартира;
		ИначеЕсли Буфер = "оф." Тогда
			СтрокаНабора.ТипКвартиры = Перечисления.ТипыКвартир.Офис;
		КонецЕсли;
		СтрокаНабора.ТипКвартиры = ?(ТипыКвартир.Найти(НРег(Буфер)) = Неопределено, Неопределено, Перечисления.ТипыКвартир[Буфер]);
...
Процедура ЗаменитьПолныеНаименованияАдресныхОбъектовСокращенными(Объект, АдресныеСокращения = Неопределено)
	...
	Если АдресныеСокращения = Неопределено Тогда
		АдресныеСокращения = УправлениеКонтактнойИнформацией.СокращенияАдресныхОбъектов(Новый Массив);
	КонецЕсли;
...
Процедура ПроверитьДополнительныеАдресныеЭлементы(Объект, Фабрика)
	...
	Для каждого ТипОбъектовАдресации Из УправлениеКонтактнойИнформацией.ТипыОбъектовАдресацииАдресаРФ() Цикл
...
Функция СформироватьПредставлениеТелефона(КодСтраны, КодГорода, НомерТелефона, Добавочный, Комментарий) Экспорт
	...
	НТ = УправлениеКонтактнойИнформацией.ПривестиНомерТелефонаКШаблону(НомерТелефона);
...
// УправлениеКонтактнойИнформациейКлиентСерверПовтИсп {
...
// ВспомогательныеПроцедурыИФункции для преобразования контактной информации из нового в старый формат {
