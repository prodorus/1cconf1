...
Функция ПечатьТ53(ДокументОбъект) Экспорт
	...
	ВыборкаРаботники = СформироватьЗапросДляПечати(ДокументОбъект, "ПоТабличнойЧастиДокумента").Выбрать();
...
Функция ПечатьТ49(ДокументОбъект) Экспорт
	...
	Если ВыборкаДляШапки.Следующий() Тогда
		...
		Если ВыборкаДляШапки.Сумма <> 0 Тогда
			ОбластьМакетаШапка.Параметры.СуммаВсегоПрописью = ОбщегоНазначенияЗК.СформироватьСуммуПрописью(ВыборкаДляШапки.Сумма, ВалютаРегламентированногоУчета);
		...
		Если ВыборкаДляШапки.ОплаченоПолностью Тогда
			Если ВыборкаДляШапки.ВсегоДепонировано > 0 Тогда
				ОбластьМакетаПодвал.Параметры.ВсегоДепонировано = ОбщегоНазначенияЗК.СформироватьСуммуПрописью(ВыборкаДляШапки.ВсегоДепонировано, ВалютаРегламентированногоУчета);
			Иначе 
				ОбластьМакетаПодвал.Параметры.ВсегоДепонировано = "---";
			КонецЕсли;
			Если ВыборкаДляШапки.ВсегоВыплачено > 0 Тогда
				ОбластьМакетаПодвал.Параметры.ВсегоВыплачено = ОбщегоНазначенияЗК.СформироватьСуммуПрописью(ВыборкаДляШапки.ВсегоВыплачено, ВалютаРегламентированногоУчета);
			Иначе 
				ОбластьМакетаПодвал.Параметры.ВсегоВыплачено = "---";
			КонецЕсли;
			ВывестиДанныеРКО = ВыборкаДляШапки.КоличествоРКО = 1;
	...
	Пока ВыборкаПоФЛ.СледующийПоЗначениюПоля("ФизЛицо") Цикл
		...
		Если ВыборкаПоФЛ.ФИО = Null Тогда
			ОбластьСтрока.Параметры.ФИО	= ОбщегоНазначенияЗК.ФамилияИнициалыФизЛица(ВыборкаПоФЛ.ФизЛицоНаименование);
		КонецЕсли;
	...
	Пока ВыборкаРаботники.Следующий() Цикл
		...
		Если ВыборкаРаботники.ВыводитьРКО Тогда
			...
			Если ЗначениеЗаполнено(ВыборкаРаботники.Фамилия) Или ЗначениеЗаполнено(ВыборкаРаботники.Имя) Или ЗначениеЗаполнено(ВыборкаРаботники.Отчество) Тогда
				СообщениеОбОшибке = "";
				Если Не УниверсальныеМеханизмы.ПросклонятьФамилиюИмяОтчество(КомпонентаСклоненияФИО, ВыборкаРаботники.Фамилия, ВыборкаРаботники.Имя, ВыборкаРаботники.Отчество, 3, ВыборкаРаботники.Пол, ТекстВыдать, СообщениеОбОшибке) Тогда
					ОбщегоНазначенияЗК.СообщитьОбОшибке(СообщениеОбОшибке);
				КонецЕсли;
			Иначе
				УниверсальныеМеханизмы.Просклонять(КомпонентаСклоненияФИО, ВыборкаРаботники.ФИО, 3, ВыборкаРаботники.Пол, ТекстВыдать);
			КонецЕсли;
			
			ОбластьМакета.Параметры.ФИОПолучателя = ТекстВыдать;
			ОбластьМакета.Параметры.СуммаПрописью = ОбщегоНазначенияЗК.СформироватьСуммуПрописью(ВыборкаРаботники.Сумма, Валюта);
...
Функция СформироватьЗапросДляПечатиРКО(ДокументОбъект)
	...
	|	ЗарплатаКВыплатеОрганизацийЗарплата.Физлицо.Пол КАК Пол,
	|	ВЫБОР
...
//  Результат запроса
//
Функция СформироватьЗапросДляПечати(ДокументОбъект, Режим)
...
Функция СформироватьЗапросДляПечатиРеестраДепонентов(ДокументОбъект)
	...
	Запрос.УстановитьПараметр("Организация",  ОбщегоНазначенияЗК.ГоловнаяОрганизация(ДокументОбъект.Организация));
...
Функция ПолучитьДанныеДокумента(Ссылка) Экспорт
	...
	|								ИНАЧЕ ЗарплатаКВыплатеОрганизаций.Организация.НаименованиеПолное
	|							КОНЕЦ
	|					ИНАЧЕ ЗарплатаКВыплатеОрганизаций.Организация.ОсновнойБанковскийСчет.ТекстКорреспондента
	|				КОНЕЦ
	|		ИНАЧЕ """"
	...
	|								ИНАЧЕ ЗарплатаКВыплатеОрганизаций.Банк.НаименованиеПолное
	|							КОНЕЦ
	|					ИНАЧЕ ЗарплатаКВыплатеОрганизаций.Банк.ОсновнойБанковскийСчет.ТекстКорреспондента
	|				КОНЕЦ
	|		ИНАЧЕ """"
