...
Функция ОтправитьHTTPЗапрос(ТранспортныйМодуль, Параметры) Экспорт
	...
	HTTPЗапрос = Новый HTTPЗапрос(Параметры.АдресЗапроса);
	...
	Если Параметры.ТипЗапроса = "POST" Тогда
		...
		//Удалим BOM. Адаптация.
		ВременнаяСтрока = Base64Строка(HTTPЗапрос.ПолучитьТелоКакДвоичныеДанные());
		ВременнаяСтрока = Сред(ВременнаяСтрока, 5);
		HTTPЗапрос.УстановитьТелоИзДвоичныхДанных(Base64Значение(ВременнаяСтрока));
	КонецЕсли;
