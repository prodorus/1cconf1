...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента) Экспорт
	...
	Результат = НСтр("ru = 'Выгружается справочная информация.'");
	Результат = Результат + Символы.ПС + НСтр("ru = 'Источник кадровых сведений: '");
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = 'Создать обмен с конфигурацией ""Документооборот КОРП, ред. 1.4""'");
...
Функция КраткаяИнформацияПоОбмену() Экспорт
	...
	ПоясняющийТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Позволяет синхронизировать данные между конфигурациями ""%1"" и ""1С:Документооборот КОРП, ред. 1.4"".
