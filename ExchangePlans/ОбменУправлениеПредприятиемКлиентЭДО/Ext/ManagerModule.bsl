...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента) Экспорт
	...
	Если НастройкаОтборовНаУзле.ИспользоватьОтборПоУчастникамЭДО Тогда
		...
		ОграничениеОтборПоУчастникамЭДО = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отбор по участникам ЭДО:
...
Функция ОписаниеЗначенийПоУмолчанию(ЗначенияПоУмолчаниюНаУзле, ВерсияКорреспондента) Экспорт
	...
	Результат = НСтр("ru = 'В документах поступления:
		|[ОписаниеСклад]
		|[ОписаниеЦена]'");
		
	Результат = СтрЗаменить(Результат, "[ОписаниеСклад]", ОписаниеСклад);
	Результат = СтрЗаменить(Результат, "[ОписаниеЦена]", ОписаниеЦена);
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = 'Создать обмен с конфигурацией ""Клиент ЭДО, ред. 2.5""'");
...
Функция КраткаяИнформацияПоОбмену() Экспорт
	...
	ПоясняющийТекст = НСтр("ru = 'Позволяет синхронизировать данные между конфигурациями ""Управление производственным предприятием, ред. 1.3"" и ""Клиент ЭДО, ред. 2.5"". 
	|Из приложения ""Управление производственным предприятием"" в приложение ""Клиент ЭДО"" переносятся справочники и все необходимые документы
...
Функция ПодробнаяИнформацияПоОбмену() Экспорт
	...
	Возврат "ПланОбмена.ОбменУправлениеПредприятиемКлиентЭДО.Форма.ПодробнаяИнформация";
